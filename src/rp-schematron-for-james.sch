<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:sqf="http://www.schematron-quickfix.com/validator/process" xmlns:meca="http://manuscriptexchange.org" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:ali="http://www.niso.org/schemas/ali/1.0/" xmlns:file="java.io.File" xmlns:java="http://www.java.com/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" queryBinding="xslt2">
                ...
                <xsl:when test="matches($author-name,'^[\p{Lu}\.]+\s[\p{L}\p{P}\s’]+$')">
                    <string-name xmlns="">
                        <xsl:analyze-string select="$author-name" regex="{'^([\p{Lu}\s\.]+)\s+([\p{L}\p{P}\s’]+)$'}">
                            <xsl:matching-substring>
                                <given-names><xsl:value-of select="regex-group(1)"/></given-names>
                                <xsl:text> </xsl:text>
                                <surname><xsl:value-of select="regex-group(2)"/></surname>
                            </xsl:matching-substring>
                        </xsl:analyze-string>
                    </string-name>
                </xsl:when>
                    ...
                    <string-name xmlns="">
                        <xsl:analyze-string select="$author-name" regex="{'^([\p{L}\p{P}\s’]+)\s+([\p{Lu}\s\.]+)$'}">
                            <xsl:matching-substring>
                                ...
                                <xsl:text> </xsl:text>
                                <given-names><xsl:value-of select="regex-group(2)"/></given-names>
                            </xsl:matching-substring>
                        </xsl:analyze-string>
                    </string-name>
                </xsl:when>
  ...
  <sqf:fixes>
    ...
    <report test="matches(.,'\p{Zs}$')" role="error" sqf:fix="replace-normalize-space" id="given-names-test-9">[given-names-test-9] given-names ends with a space, which cannot be correct - '<value-of select="."/>'.</report>
      ...
      <report test="$country-count lt 1" role="warning" sqf:fix="add-ror-country" id="aff-no-country">[aff-no-country] Affiliation does not contain a country element: <value-of select="."/></report>
      ...
      <report test="count(descendant::institution-id) gt 1" role="error" sqf:fix="pick-aff-ror-1 pick-aff-ror-2 pick-aff-ror-3" id="aff-multiple-ids">[aff-multiple-ids] Affiliation contains more than one institution-id element: <value-of select="string-join(descendant::institution-id,'; ')"/> in <value-of select="."/></report>
      ...
      <report test="ancestor::contrib-group[@content-type='section'] and not(parent::contrib)" role="error" id="editor-aff-placement">[editor-aff-placement] Editor aff elements should be placed as a direct child of the editor contrib element. This one is a child of <value-of select="parent::*/name()"/>.</report>
      ...
      <assert test="$valid-country" role="warning" id="gen-country-test">[gen-country-test] affiliation contains a country which is not in the list of standardised countriy names - <value-of select="."/>. Is that OK? For a list of allowed countries, refer to https://github.com/elifesciences/eLife-JATS-schematron/blob/master/src/countries.xml.</assert>
      
      <report test="not(@country = $valid-country/@country)" role="warning" id="gen-country-iso-3166-test">[gen-country-iso-3166-test] country does not have a 2 letter ISO 3166-1 @country value. Should it be @country='<value-of select="$valid-country/@country"/>'?.</report>
      ...
      <report test="(country or ancestor::contrib[@contrib-type='author' and not(ancestor::sub-article)]) and exists($matching-ror) and not(contains(country[1],$matching-ror/*:country[1]))" role="warning" id="aff-ror-country">[aff-ror-country] Affiliation has a ROR id, but its country is not the same one as in the ROR data. Is that OK? ROR has '<value-of select="$matching-ror/*:country"/>', but the affiliation country is <value-of select="country[1]"/>.</report>
  ...
  <pattern id="test-editor-contrib-group-pattern"><rule context="article/front/article-meta/contrib-group[@content-type='section']" id="test-editor-contrib-group">
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hjuk3-contrib-test-2" test="(count(xref[@ref-type='aff']) + count(aff) = 0)" role="warning" id="contrib-test-2">[contrib-test-2] The <value-of select="if (role) then role[1] else 'editor contrib'"/> doesn't have an affiliation - <value-of select="$name"/> - is this correct? Please check eJP or ask Editorial for the correct affiliation to be added in eJP.</report>
      
      <report test="$name=''" role="error" id="editor-check-1">[editor-check-1] The <value-of select="if (role) then role[1] else 'editor contrib'"/> doesn't have a name element, which must be incorrect. Please check eJP or ask Editorial for the correct affiliation to be added in eJP.</report>
    ...
    <pattern id="preprint-ref-checks-pattern"><rule context="mixed-citation[@publication-type='preprint']" id="preprint-ref-checks">
       ...
       <report test="lpage" role="error" id="preprint-ref-lpage">[preprint-ref-lpage] This preprint reference (<value-of select="if (ancestor::ref/@id) then concat('id ',ancestor::ref/@id) else 'no id'"/>) has an lpage element - <value-of select="lpage"/>. That information is either tagged incorrectly, or the publication-type is wrong.</report>
       
       <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#err-elem-cit-journal-6-7" test="count(fpage) gt 1 or count(lpage) gt 1 or count(elocation-id) gt 1 or count(comment) gt 1" role="error" id="err-elem-cit-preprint-6-7">[err-elem-cit-preprint-6-7] The following elements may not occur more than once in a &lt;mixed-citation&gt;: &lt;fpage&gt;, &lt;lpage&gt;, &lt;elocation-id&gt;, and &lt;comment&gt;. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(fpage)"/> &lt;fpage&gt;, <value-of select="count(lpage)"/> &lt;lpage&gt;, <value-of select="count(elocation-id)"/> &lt;elocation-id&gt;, and <value-of select="count(comment)"/> &lt;comment&gt; elements.</report>
    ...
    <pattern id="book-ref-checks-pattern"><rule context="mixed-citation[@publication-type='book']" id="book-ref-checks">
      ...
      <report see="https://elifeproduction.slab.com/posts/book-references-x4trb0n2#hgboy-err-elem-cit-book-36-6" test="count(lpage) &gt; 1 or count(fpage) &gt; 1" role="error" id="err-elem-cit-book-36-6">[err-elem-cit-book-36-6] At most one &lt;lpage&gt; and one &lt;fpage&gt; are allowed. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(lpage)"/> &lt;lpage&gt; elements and <value-of select="count(fpage)"/> &lt;fpage&gt; elements.</report>
        ...
        <report test="matches(.,'[“”&quot;]')" role="warning" sqf:fix="delete-quote-characters" id="book-source-4">[book-source-4] Book reference (<value-of select="if (ancestor::ref/@id) then concat('id ',ancestor::ref/@id) else 'no id'"/>) has a source that contains speech quotes - <value-of select="."/>. Is that correct?</report>
      </rule></pattern>
  ...
  <pattern id="confproc-ref-checks-pattern"><rule context="mixed-citation[@publication-type='confproc']" id="confproc-ref-checks">
    ...
    <pattern id="ref-pub-id-checks-pattern"><rule context="ref//pub-id" id="ref-pub-id-checks">
        ...
        <report test="(@pub-id-type='pmcid') and not(matches(.,'^PMC[0-9]{5,15}$'))" role="error" id="ref-pmcid-conformance">[ref-pmcid-conformance] pub-id is tagged as a pmcid, but it is not a valid PMCID ('PMC' followed by 5+ digits) - <value-of select="."/>. The id must be either incorrect or have the wrong pub-id-type.</report>
        
        <report test="(@pub-id-type='arxiv') and not(matches(.,'^(\d{2}(0[1-9]|1[0-2])\.\d{4,5}|\d{2}(0[1-9]|1[0-2])\d{3})$'))" role="error" id="ref-arxiv-conformance">[ref-arxiv-conformance] pub-id is tagged as an arxiv id, but it is not a valid arxiv id (a number in the format yymm.nnnnn or yymmnnn) - <value-of select="."/>. The id must be either incorrect or have the wrong pub-id-type.</report>
  ...
  <pattern id="ref-source-checks-pattern"><rule context="ref//source" id="ref-source-checks">
    ...
    <pattern id="mixed-citation-checks-pattern"><rule context="mixed-citation" id="mixed-citation-checks">
        ...
        <assert test="normalize-space(@publication-type)!=''" role="error" sqf:fix="replace-to-preprint-ref" id="mixed-citation-publication-type-presence">[mixed-citation-publication-type-presence] <name/> must have a publication-type attribute with a non-empty value.</assert>
        ...
        <report test="@publication-type='other'" role="warning" sqf:fix="replace-to-preprint-ref" id="mixed-citation-other-publication-flag">[mixed-citation-other-publication-flag] <name/> in reference (id=<value-of select="ancestor::ref/@id"/>) has a publication-type='other'. Is that correct?</report>
  ...
  <pattern id="ack-tests-pattern"><rule context="ack" id="ack-tests">
    ...
    <pattern id="strike-checks-pattern"><rule context="strike" id="strike-checks">
        <report test="." role="warning" sqf:fix="strip-tags" id="strike-warning">[strike-warning] strike element is present. Is this tracked change formatting that's been erroneously retained? Should this text be deleted?</report>
     </rule></pattern>
    ...
    <pattern id="underline-checks-pattern"><rule context="underline" id="underline-checks">
        ...
        <report test="matches(lower-case(.),'www\.|(f|ht)tp|^link\s|\slink\s')" role="warning" sqf:fix="replace-to-ext-link strip-tags" id="underline-link-warning">[underline-link-warning] Should this underline element be a link (ext-link) instead? <value-of select="."/></report>
  ...
  <pattern id="sc-checks-pattern"><rule context="sc" id="sc-checks">
    ...
    <pattern id="fig-checks-pattern"><rule context="fig" id="fig-checks">
       ...
       <report test="not(label) and parent::sec and preceding-sibling::title and not(following-sibling::*)" role="warning" id="fig-sec-wrapper">[fig-sec-wrapper] Unlablled figure is entirely wrapped in a sec with a title. Is the sec redundant and the title actually the label (or title) for the figure?</report>
        ...
        <report test="not(title) and (count(p)=1) and (count(tokenize(p[1],'\.\p{Z}')) gt 1) and not(matches(lower-case(p[1]),'^\p{Z}*\p{P}?(a|a[–—\-][b-z]|i)\p{P}'))" role="warning" sqf:fix="replace-move-sentence-to-title" id="fig-caption-2">[fig-caption-2] Caption for <value-of select="$label"/> doesn't have a title, but there are mutliple sentences in the legend. Is the first sentence actually the title?</report>
        
        <report test="not(title) and (count(p)=1) and not(count(tokenize(p[1],'\.\p{Z}')) gt 1)" role="warning" sqf:fix="replace-p-to-title" id="fig-caption-3">[fig-caption-3] Caption for <value-of select="$label"/> doesn't have a title, but it does have a paragraph. Is the paragraph actually the title?</report>
     </rule></pattern>
    ...
    <pattern id="list-checks-pattern"><rule context="list" id="list-checks">
     ...
     <pattern id="graphic-checks-pattern"><rule context="graphic|inline-graphic" id="graphic-checks">
        ...
        <assert test="$file=$image-file-types" role="error" id="graphic-check-2">[graphic-check-2] <name/> must have an xlink:href attribute that ends with an image file type extension. <value-of select="if ($file!='') then $file else @*:href"/> is not one of <value-of select="string-join($image-file-types,', ')"/>.</assert>
        ...
        <report test="not(ancestor::sub-article) and preceding::graphic/@*:href/lower-case(.) = $link or preceding::inline-graphic/@*:href/lower-case(.) = $link" role="error" id="graphic-test-6">[graphic-test-6] Image file for <value-of select="if (parent::fig/label) then parent::fig/label else 'graphic'"/> (<value-of select="@*:href"/>) is the same as the one used for another graphic or inline-graphic.</report>
        
        <report test="ancestor::sub-article and preceding::graphic/@*:href/lower-case(.) = $link or preceding::inline-graphic/@*:href/lower-case(.) = $link" role="warning" id="graphic-test-9">[graphic-test-9] Image file in sub-article for <value-of select="if (parent::fig/label) then parent::fig/label else 'graphic'"/> (<value-of select="@*:href"/>) is the same as the one used for another graphic or inline-graphic. Is that correct?</report>
     ...
     </rule></pattern><pattern id="graphic-placement-pattern"><rule context="graphic" id="graphic-placement">
         <assert test="parent::fig or parent::table-wrap or parent::disp-formula or parent::alternatives[parent::table-wrap or parent::disp-formula]" role="error" id="graphic-test-8">[graphic-test-8] <name/> can only be placed as a child of fig, table-wrap, disp-formula or alternatives (alternatives must in turn must be a child of either table-wrap or disp-formula). This one is a child of <value-of select="parent::*/name()"/></assert>
       </rule></pattern><pattern id="inline-checks-pattern"><rule context="inline-graphic" id="inline-checks">
         <assert test="parent::inline-formula or parent::alternatives[parent::inline-formula] or ancestor::caption[parent::fig or parent::table-wrap]" role="warning" id="inline-graphic-test-1">[inline-graphic-test-1] <name/> is usually placed either in inline-formula (or alternatives which in turn is a child of inline-formula), or in the caption for a figure or table. This one is not (its a child of <value-of select="parent::*/name()"/>). Is that OK?</assert>
       </rule></pattern>
  
      <pattern id="media-checks-pattern"><rule context="media" id="media-checks">
        <let name="link" value="@*:href"/>
      
      <assert test="matches(@*:href,'\.[\p{L}\p{N}]{1,15}$')" role="error" id="media-test-3">[media-test-3] media must have an @xlink:href which contains a file reference.</assert>
    ...
    <pattern id="sec-checks-pattern"><rule context="sec" id="sec-checks">
        ...
        <report test="@sec-type='data-availability' and (not(p) or p[normalize-space(.)='' or matches(normalize-space(lower-case(.)),'^(not (available|applicable)|n/a)\.?$')])" role="warning" id="sec-data-availability-empty">[sec-data-availability-empty] Data availability section isn't populated with any meaningful content. Should it be removed?</report>
  ...
  <pattern id="app-checks-pattern"><rule context="app" id="app-checks">
    ...
    <pattern id="title-checks-pattern"><rule context="title" id="title-checks">
        ...
        <report test="lower-case(.)=." role="warning" sqf:fix="replace-sentence-case" id="title-lower-case">[title-lower-case] Content of &lt;title&gt; element is entirely in lower-case case: Is that correct? '<value-of select="."/>'</report>
    ...
    <pattern id="p-bold-checks-pattern"><rule context="p[not(ancestor::sub-article or ancestor::def) and (count(*)=1) and (child::bold or child::italic)]" id="p-bold-checks">
      ...
      </rule></pattern><pattern id="p-ref-checks-pattern"><rule context="article[descendant::xref[@ref-type='bibr'][matches(.,'\p{L}')]]//p[not(ancestor::sub-article) and not(ancestor::ack)]" id="p-ref-checks">
      ...
      </rule></pattern><pattern id="p-file-ref-checks-pattern"><rule context="article//p[not(ancestor::sub-article) and not(ancestor::ack)]" id="p-file-ref-checks">
        ...
        <report test="matches($text,$missing-file-regex)" role="warning" id="missing-file-in-text-test">[missing-file-in-text-test] <name/> element contains possible unlinked citation to a figure, table or file - search - <value-of select="string-join(e:analyze-string($text,$missing-file-regex)//*:match,'; ')"/></report>
      </rule></pattern>
  ...
  <pattern id="p-td-th-checks-pattern"><rule context="p|td|th" id="p-td-th-checks">
    ...
    <pattern id="general-article-meta-checks-pattern"><rule context="article/front/article-meta" id="general-article-meta-checks">
      ...
      <report test="@specific-use and number(substring-after(.,concat($article-id,'.'))) != (number($latest-rp-doi-version)+1)" role="error" id="final-prc-article-dois-8">[final-prc-article-dois-8] The version DOI for this Reviewed preprint version needs to end with a number that is one more than whatever number the last published reviewed preprint version DOI ends with. This version DOI ends with <value-of select="substring-after(.,concat($article-id,'.'))"/> (<value-of select="."/>), whereas <value-of select="if ($latest-rp-doi-version='0') then 'there is no previous reviewed preprint version in the pub-history' else concat('the latest reviewed preprint DOI in the publication history ends with ',$latest-rp-doi-version,' (',$latest-rp-doi,')')"/>. Either there is a missing reviewed preprint publication event in the publication history, or the version DOI is incorrect.</report>
     ...
     </rule></pattern><pattern id="author-notes-fn-checks-pattern"><rule context="article/front/article-meta/author-notes/fn" id="author-notes-fn-checks">
        <let name="id" value="@id"/>
        ...
        <report test="@fn-type='equal' and (count(ancestor::article-meta//contrib[@contrib-type='author'][xref/@rid = $id]) lt 2)" role="error" id="author-fn-2">[author-fn-2] Equal author type footnote (id=<value-of select="$id"/>) in author-notes is linked to from <value-of select="count(ancestor::article-meta//contrib[@contrib-type='author'][xref/@rid = $id])"/> author(s), which must be a mistake. "<value-of select="."/>"</report>
        ...
        <report test="parent::contrib-group[not(preceding-sibling::contrib-group)] and not(@contrib-type)" role="error" id="contrib-2">[contrib-2] Contrib without the attribute contrib-type="author" is present in author contrib-group (the first contrib-group within article-meta). This is not correct.</report>
      ...
      <assert test="self-uri[@content-type='editor-report']" role="error" id="rp-event-test-4">[rp-event-test-4] The event-desc for Reviewed preprint publication events must have a &lt;self-uri content-type="editor-report"&gt; (which has a DOI link to the eLife Assessment for that version).</assert>
        
     <assert test="self-uri[@content-type='referee-report']" role="error" id="rp-event-test-5">[rp-event-test-5] The event-desc for Reviewed preprint publication events must have at least one &lt;self-uri content-type="referee-report"&gt; (which has a DOI link to a public review for that version).</assert>
    </rule></pattern><pattern id="event-desc-tests-pattern"><rule context="event-desc" id="event-desc-tests">
      
      <report test="parent::event/self-uri[1][@content-type='preprint'] and .!='Preprint posted'" role="error" id="event-desc-content">[event-desc-content] <name/> that's a child of a preprint event must contain the text 'Preprint posted'. This one does not (<value-of select="."/>).</report>
      
      <report test="parent::event/self-uri[1][@content-type='reviewed-preprint'] and .!=concat('Reviewed preprint v',replace(parent::event[1]/self-uri[1][@content-type='reviewed-preprint']/@*:href,'^.*\.',''))" role="error" id="event-desc-content-2">[event-desc-content-2] <name/> that's a child of a Reviewed preprint event must contain the text 'Reviewed preprint v' followwd by the verison number for that Reviewed preprint version. This one does not (<value-of select="."/> != <value-of select="concat('Reviewed preprint v',replace(parent::event[1]/self-uri[1][@content-type='reviewed-preprint']/@*:href,'^.*\.',''))"/>).</report>
      ...
      <report test="matches(lower-case(@*:href),'(bio|med)rxiv')" role="error" id="event-self-uri-href-2">[event-self-uri-href-2] <name/> in event must have an xlink:href attribute containing a link to the preprint. Where possible this should be a doi. bioRxiv and medRxiv preprint have dois, and this one points to one of those, but it is not a doi - <value-of select="@*:href"/>.</report>
      ...
      <report test="@content-type='reviewed-preprint' and not(matches(@*:href,'^https://doi.org/10.7554/eLife.\d+\.[1-9]$'))" role="error" id="event-self-uri-href-4">[event-self-uri-href-4] <name/> in event has the attribute content-type="reviewed-preprint", but the xlink:href attribute does not contain an eLife version specific DOI - <value-of select="@*:href"/>.</report>
      
      <report test="(@content-type!='reviewed-preprint' or not(@content-type)) and matches(@*:href,'^https://doi.org/10.7554/eLife.\d+\.\d$')" role="error" id="event-self-uri-href-5">[event-self-uri-href-5] <name/> in event does not have the attribute content-type="reviewed-preprint", but the xlink:href attribute contains an eLife version specific DOI - <value-of select="@*:href"/>. If it's a preprint event, the link should be to a preprint. If it's an event for reviewed preprint publication, then it should have the attribute content-type!='reviewed-preprint'.</report>
      ...
      <report test="@content-type=('editor-report','referee-report','author-comment') and not(matches(@*:href,'^https://doi.org/10.7554/eLife.\d+\.[1-9]\.sa\d+$'))" role="error" id="event-self-uri-href-7">[event-self-uri-href-7] <name/> in event has the attribute content-type="<value-of select="@content-type"/>", but the xlink:href attribute does not contain an eLife peer review DOI - <value-of select="@*:href"/>.</report>
      ...
      <sqf:fix id="pick-funding-ror-3" use-when="count(descendant::institution-id) gt 2">
      ...
      </rule></pattern><pattern id="general-grant-doi-tests-pattern"><rule context="funding-group/award-group[award-id[not(@award-id-type='doi') and normalize-space(.)!=''] and funding-source/institution-wrap[count(institution-id)=1]/institution-id[not(.=$grant-doi-exception-funder-ids)]]" id="general-grant-doi-tests">
	   ...
	   </rule></pattern><pattern id="general-funding-no-award-id-tests-pattern"><rule context="funding-group/award-group[not(award-id) and funding-source/institution-wrap[count(institution-id)=1]/institution-id]" id="general-funding-no-award-id-tests">
        <let name="funder-id" value="funding-source/institution-wrap/institution-id"/>
        <let name="funder-entry" value="document($rors)//*:ror[*:id[@type='ror']=$funder-id]"/>
    ...
    </rule></pattern><pattern id="eu-horizon-grant-doi-tests-pattern"><rule context="funding-group/award-group[award-id[not(@award-id-type='doi')] and funding-source/institution-wrap/institution-id=($eu-horizon-fundref-ids,'https://ror.org/00k4n6c32')]" id="eu-horizon-grant-doi-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/funding-3sv64358#known-grant-funder-grant-doi-test-1" test="$grant-matches" role="warning" sqf:fix="add-grant-doi" id="known-grant-funder-grant-doi-test-1">[known-grant-funder-grant-doi-test-1] Funding entry from <value-of select="funding-source/institution-wrap/institution"/> has an award-id (<value-of select="$award-id-elem"/>) which could potentially be replaced with a grant DOI. The following grant DOIs are possibilities: <value-of select="string-join(for $grant in $grant-matches return concat('https://doi.org/',$grant/@doi),'; ')"/>.</report>
      ...
      <report test="matches(lower-case(.),'grant')" role="warning" id="award-id-test-13">[award-id-test-13] award-id contains the phrase 'grant' (<value-of select="."/>). Should it be removed?</report>
    ...
    </rule></pattern><pattern id="funding-institution-wrap-tests-pattern"><rule context="funding-source//institution-wrap" id="funding-institution-wrap-tests">
      
      <report test="count(institution-id)=1 and (normalize-space(string-join(text(),''))!='' or comment())" role="error" id="funding-institution-wrap-test-3">[funding-institution-wrap-test-3] institution-wrap cannot contain text content or comments. It can only contain elements and whitespace.</report>  
        
    </rule></pattern><pattern id="funding-ror-tests-pattern"><rule context="funding-source[count(institution-wrap/institution-id[@institution-id-type='ror'])=1]" id="funding-ror-tests">
      <let name="rors" value="'rors.xml'"/>
      <let name="ror" value="institution-wrap[1]/institution-id[@institution-id-type='ror'][1]"/>
      <let name="matching-ror" value="document($rors)//*:ror[*:id=$ror]"/>
    ...
    <pattern id="front-permissions-tests-pattern"><rule context="front[journal-meta/lower-case(journal-id[1])='elife']//permissions" id="front-permissions-tests">
	  ...
	  <assert see="https://elifeproduction.slab.com/posts/licensing-and-copyright-rqdavyty#permissions-test-4" test="*:free_to_read" role="error" id="permissions-test-4">[permissions-test-4] permissions must contain an ali:free_to_read element.</assert>
	
	  <assert test="license" role="error" id="permissions-test-5">[permissions-test-5] permissions must contain license.</assert>
	  
	  <assert see="https://elifeproduction.slab.com/posts/licensing-and-copyright-rqdavyty#permissions-test-9" test="$license-type = ('http://creativecommons.org/publicdomain/zero/1.0/', 'https://creativecommons.org/publicdomain/zero/1.0/', 'http://creativecommons.org/licenses/by/4.0/', 'https://creativecommons.org/licenses/by/4.0/')" role="error" id="permissions-test-9">[permissions-test-9] license does not have an @xlink:href which is equal to 'https://creativecommons.org/publicdomain/zero/1.0/' or 'https://creativecommons.org/licenses/by/4.0/'.</assert>
	...
	</rule></pattern><pattern id="cc-by-permissions-tests-pattern"><rule context="front[journal-meta/lower-case(journal-id[1])='elife']//permissions[contains(license[1]/@*:href,'creativecommons.org/licenses/by/')]" id="cc-by-permissions-tests">
      <let name="author-contrib-group" value="ancestor::article-meta/contrib-group[1]"/>
      <let name="copyright-holder" value="e:get-copyright-holder($author-contrib-group)"/>
      <let name="license-type" value="license/@*:href"/>
      <let name="is-first-version" value="if (ancestor::article-meta/article-id[@specific-use='version' and ends-with(.,'.1')]) then true()                                           else if (not(ancestor::article-meta/pub-history[event[date[@date-type='reviewed-preprint']]])) then true()                                           else false()"/>
      ...
      <assert see="https://elifeproduction.slab.com/posts/licensing-and-copyright-rqdavyty#permissions-test-6" test="copyright-year = $authoritative-year" role="error" id="permissions-test-6">[permissions-test-6] copyright-year must match the year of first reviewed preprint publication date. Currently copyright-year=<value-of select="copyright-year"/> and authoritative pub-date=<value-of select="$authoritative-year"/>.</assert>
    ...
    </rule></pattern><pattern id="cc-0-permissions-tests-pattern"><rule context="front[journal-meta/lower-case(journal-id[1])='elife']//permissions[contains(license[1]/@*:href,'creativecommons.org/publicdomain/zero')]" id="cc-0-permissions-tests">
      <let name="license-type" value="license/@*:href"/>
    ...
    </rule></pattern><pattern id="license-tests-pattern"><rule context="front[journal-meta/lower-case(journal-id[1])='elife']//permissions/license" id="license-tests">
	...
	</rule></pattern><pattern id="license-p-tests-pattern"><rule context="front[journal-meta/lower-case(journal-id[1])='elife']//permissions/license/license-p" id="license-p-tests">
    ...
    </rule></pattern><pattern id="license-link-tests-pattern"><rule context="permissions/license[@*:href]/license-p" id="license-link-tests">
      <let name="license-link" value="parent::license/@*:href"/>
    ...
    </rule></pattern><pattern id="license-ali-ref-link-tests-pattern"><rule context="permissions/license[*:license_ref]/license-p" id="license-ali-ref-link-tests">
  ...
  <pattern id="clintrial-related-object-pattern"><rule context="related-object[@content-type or @document-id]" id="clintrial-related-object">
      ...
      <assert test="@document-id[not(matches(.,'\p{Zs}'))]" role="error" id="clintrial-related-object-6">[clintrial-related-object-6] <name/> must have an @document-id with a value that does not contain a space character.</assert>
      
      <assert test="@*:href[not(matches(.,'\p{Zs}'))]" role="error" id="clintrial-related-object-7">[clintrial-related-object-7] <name/> must have an @xlink:href with a value that does not contain a space character.</assert>
      ...
      <report test="ends-with(@*:href,'.')" role="error" id="clintrial-related-object-14">[clintrial-related-object-14] <name/> has a @xlink:href attribute value which ends with a full stop, which is not correct - '<value-of select="@*:href"/>'.</report>
      
      <assert test="@*:href!=''" role="error" id="clintrial-related-object-17">[clintrial-related-object-17] <name/> must have an @xlink:href attribute with a non-empty value. This one does not.</assert>
      ...
      <assert test="@content-type=('pre-results','results','post-results')" role="error" id="clintrial-related-object-18">[clintrial-related-object-18] <name/> must have a content-type attribute with one of the following values: pre-results, results, or post-results.</assert>
      
      <assert test="ancestor::abstract or parent::article-meta" role="error" id="clintrial-related-object-parent-1">[clintrial-related-object-parent-1] <name/> element must either be a descendant of abstract or a child or article-meta. This one is not.</assert>
  ...
  <pattern id="inline-media-checks-pattern"><rule context="inline-media" id="inline-media-checks">
        <report test="." role="error" id="inline-media-flag">[inline-media-flag] The inline-media element is not permitted. Instead use inline-graphic for images or supplementary-material for downloadable files.</report>
     </rule></pattern>
  ...
  <pattern id="disp-quote-checks-pattern"><rule context="disp-quote" id="disp-quote-checks">
    ...
    <pattern id="xref-checks-pattern"><rule context="xref" id="xref-checks">
        ...
        <report test="parent::xref" role="error" sqf:fix="strip-tags" id="xref-parent">[xref-parent] This xref element containing '<value-of select="."/>' is a child of another xref. Nested xrefs are not supported - it must be either stripped or moved so that it is a child of another element.</report>
    ...
    <pattern id="ext-link-tests-pattern"><rule context="ext-link[@ext-link-type='uri']" id="ext-link-tests">
      ...
      <assert test="matches(@*:href,'^https?:..(www\.)?[-a-zA-Z0-9@:%.,_\+~#=!]{1,256}\.[a-z]{2,6}([-a-zA-Z0-9@:;%,_\\(\)\[\]+.~#?!&amp;&lt;&gt;//=]*)$|^ftp://.|^tel:.|^mailto:.')" role="warning" id="url-conformance-test">[url-conformance-test] @xlink:href doesn't look like a URL - '<value-of select="@*:href"/>'. Is this correct?</assert>
      ...
      <report test="matches(@*:href,'[\p{Zs}]')" role="error" id="url-space-report">[url-space-report] '<value-of select="@*:href"/>' - Link contains a space which is incorrect.</report>
      
      <report test="(.!=@*:href) and matches(.,'https?:|ftp:')" role="warning" id="ext-link-text">[ext-link-text] The text for a URL is '<value-of select="."/>' (which looks like a URL), but it is not the same as the actual embedded link, which is '<value-of select="@*:href"/>'.</report>

      <report test="matches(@*:href,'^https?://(dx\.)?doi\.org/[^1][^0]?')" role="error" id="ext-link-doi-check">[ext-link-doi-check] Embedded URL within text starts with the DOI prefix, but it is not a valid doi - <value-of select="@*:href"/>.</report>

    <report test="not(ancestor::fig/permissions[contains(.,'phylopic')]) and matches(@*:href,'phylopic\.org')" role="warning" id="phylopic-link-check">[phylopic-link-check] This link is to phylopic.org, which is a site where silhouettes/images are typically reproduced from. Please check whether any figures contain reproduced images from this site, and if so whether permissions have been obtained and/or copyright statements are correctly included.</report>

    <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#ext-link-child-test-5" test="contains(@*:href,'datadryad.org/review?')" role="warning" id="ext-link-child-test-5">[ext-link-child-test-5] ext-link looks like it points to a review dryad dataset - <value-of select="."/>. Should it be updated?</report>
    ...
    <pattern id="unallowed-symbol-tests-pattern"><rule context="p|td|th|title|xref|bold|italic|sub|sc|named-content|monospace|code|underline|fn|institution|ext-link|ref" id="unallowed-symbol-tests">
      ...
      <report test="contains(.,'⍰')" role="error" id="junk-character-presence-4">[junk-character-presence-4] <name/> element contains a junk character '⍰' which should be replaced or deleted.</report>
      ...
      <report test="matches(., '[԰-ۿ܀-ॿ฀-࿿]')" role="warning" id="non-roman-script-presence-1">[non-roman-script-presence-1] <name/> element contains the following non-roman script character(s): <value-of select="string-join(distinct-values(e:analyze-string(.,'[԰-ۿ܀-ॿ฀-࿿]')//*:match),'; ')"/>. It is unusual for these characters to be present in eLife content. Are they correct?</report>
    </rule></pattern>
    ...
    <pattern id="psyarxiv-journal-meta-checks-pattern"><rule context="article/front/journal-meta[lower-case(journal-id[1])='psyarxiv']" id="psyarxiv-journal-meta-checks">
      ...
      <assert test="publisher/publisher-name='Center for Open Science'" role="error" id="psyarxiv-publisher">[psyarxiv-publisher] PsyArXiv preprints must have a &lt;publisher-name&gt; element with the value 'Center for Open Science', inside a &lt;publisher&gt; element.</assert>
        ...
        <assert test="matches(.,'^10\.31234/osf\.io/[\da-z]+(_v\d+)?$')" role="error" id="psyarxiv-doi-conformance">[psyarxiv-doi-conformance] PsyArXiv preprints must have a &lt;article-id pub-id-type="doi"&gt; element with a value that matches the regex '^10\.31234/osf\.io/[\da-z]+(_v\d+)?$'. In other words, the current DOI listed is not a valid PsyArXiv DOI: '<value-of select="."/>'.</assert>
      </rule></pattern>
    ...
    <pattern id="osf-journal-meta-checks-pattern"><rule context="article/front/journal-meta[lower-case(journal-id[1])='osf preprints']" id="osf-journal-meta-checks">
      ...
      <assert test="publisher/publisher-name='Center for Open Science'" role="error" id="osf-publisher">[osf-publisher] Preprints on OSF must have a &lt;publisher-name&gt; element with the value 'Center for Open Science', inside a &lt;publisher&gt; element.</assert>
        ...
        <assert test="matches(.,'^10\.31219/osf\.io/[\da-z]+(_v\d+)?$')" role="error" id="osf-doi-conformance">[osf-doi-conformance] Preprints on OSF must have a &lt;article-id pub-id-type="doi"&gt; element with a value that matches the regex '^10/.31219/osf\.io/[\da-z]+(_v\d+)?$'. In other words, the current DOI listed is not a valid OSF Preprints DOI: '<value-of select="."/>'.</assert>
      </rule></pattern>
