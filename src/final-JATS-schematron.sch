 ...
 <pattern id="article-tests-pattern"><rule context="article" id="article-tests">
  ...
  <pattern id="p-tests-pattern"><rule context="p" id="p-tests">
    ...
    <report test="not(ancestor::fig/permissions[contains(.,'phylopic')]) and matches(@xlink:href,'phylopic\.org')" role="warning" id="phylopic-link-check">[phylopic-link-check] This link is to phylopic.org, which is a site where silhouettes/images are typically reproduced from. Please check whether any figures contain reproduced images from this site, and if so whether permissions have been obtained and/or copyright statements are correctly included.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#media-test-5" test="matches(label[1],'[Aa]nimation') and not(@mime-subtype='gif')" role="error" id="media-test-5">[media-test-5] <value-of select="label"/> media with animation type label must have a @mime-subtype='gif'.</report>    
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#media-test-6" test="matches(@xlink:href,'\.doc[x]?$|\.pdf$|\.xlsx$|\.xml$|\.xlsx$|\.mp4$|\.gif$')  and (@mime-subtype='octet-stream')" role="warning" id="media-test-6">[media-test-6] media has @mime-subtype='octet-stream', but the file reference ends with a recognised mime-type. Is this correct?</report>      
