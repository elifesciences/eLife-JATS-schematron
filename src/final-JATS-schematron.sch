<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:ali="http://www.niso.org/schemas/ali/1.0/" xmlns:file="java.io.File" xmlns:java="http://www.java.com/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" queryBinding="xslt2">
 ...
 <pattern id="article-tests-pattern">
    <rule context="article" id="article-tests">
      ...
      <assert test="@specific-use = ('version-of-record','corrected-version-of-record')" role="error" id="article-specific-use">[article-specific-use] The root article element must have a specific-use atribute with a value which is either 'version-of-record' or 'corrected-version-of-record'.</assert>
    ...
    <report test="(@article-type = ('article-commentary','discussion','editorial','research-article','review-article')) and count(back) != 1" role="error" id="test-article-back">[test-article-back] Article must have one child back. Currently there are <value-of select="count(back)"/>
      ...
      <report test="not(descendant::code) and ($line-count gt 1)" role="error" id="line-count">[line-count] Articles without code blocks must only have one line in the xml. The xml for this article has <value-of select="$line-count"/>.</report>
  ...
  <pattern id="research-article-pattern">
    <rule context="article[@article-type='research-article']" id="research-article">
	  ...
	  <report test="not($type-display = ('Scientific Correspondence','Feature Article')) and not(sub-article[@article-type='referee-report'])" role="error" flag="dl-ar" id="final-test-r-article-d-letter">[final-test-r-article-d-letter] A decision letter must be present for research articles.</report>
	  ...
	  <report test="($type-display != 'Scientific Correspondence') and not(sub-article[@article-type='author-comment'])" role="warning" flag="dl-ar" id="test-r-article-a-reply">[test-r-article-a-reply] Author response should usually be present for research articles, but this one does not have one. Is that correct?</report>
  ...
  <pattern id="test-article-metadata-pattern">
    ...
    <assert test="count(pub-date[@date-type='pub']) = 1" role="error" id="test-epub-date">[test-epub-date] <name/> must contain one and only one publication date (a child pub-date element with the attribute date-type='pub'). <value-of select="count(pub-date[@date-type='pub'])"/> is not allowed.</assert> 
    
    <assert test="count(pub-date[@date-type='update']) le 1" role="error" id="test-update-date">[test-update-date] <name/> must contain one update date (pub-date[@date-type='update']) or not one at all. <value-of select="count(pub-date[@date-type='update'])"/> is not allowed.</assert>
    ...
    <report test="not($article-type = $notice-article-types) and not(abstract)" role="error" id="test-abstract">[test-abstract] <value-of select="$article-type"/>s must have an abstract. This one does not.</report>
    
    <report test="not($article-type = $notice-article-types) and not(abstract[@abstract-type='toc'])" role="error" id="test-impact-statement">[test-impact-statement] <value-of select="$article-type"/>s must have an impact statement. This one does not.</report>
    
    <assert test="empty(e:indistinct-values(abstract/@abstract-type)) and count(abstract[not(@abstract-type)]) le 1" role="error" id="test-abstracts">[test-abstracts] There cannot be more than one abstract of the same type.</assert>
    ...
    <report test="not($article-type = $notice-article-types) and count(kwd-group[@kwd-group-type='author-generated']) != 1" role="error" id="test-auth-kwd-group-presence-1">[test-auth-kwd-group-presence-1] One author keyword group must be present in article-meta.</report>
    
    <report test="$article-type = $notice-article-types and count(kwd-group[@kwd-group-type='author-keywords']) != 0" role="error" id="test-auth-kwd-group-presence-2">[test-auth-kwd-group-presence-2] <value-of select="$subj-type"/> articles must not have any author keywords</report>
    ...
    <report test="($subj-type = ('Research Article', 'Research Advance', 'Replication Study', 'Research Communication')) and (count(kwd-group[@kwd-group-type='research-organism']) = 0)" role="warning" id="test-ro-kwd-group-presence-2">[test-ro-kwd-group-presence-2] <value-of select="$subj-type"/> does not contain a Research Organism keyword group. Is this correct?</report>
    
    <!-- VoRs most likely will have been published as preprints, but in rare cases they are not, 
      so we do not specify that pub-history should/shouldnt be present for them -->
    <report test="$version-type='corrected-version-of-record' and not(pub-history)" role="warning" id="pub-history-presence">[pub-history-presence] A new version must have a pub-history. This one does not.</report>
   </rule>
  </pattern>
  <pattern id="test-research-article-metadata-pattern">
    <rule context="article[@article-type='research-article']/front/article-meta" id="test-research-article-metadata">
     ...
     <assert test="contrib-group[@content-type='authors']" role="error" id="test-contrib-group-presence-1">[test-contrib-group-presence-1] contrib-group[@content-type='authors] must be present (as a child of article-meta) for research articles (this is the contrib-group which contains authors).</assert>
     
     <assert test="contrib-group[@content-type='peer-review']" role="error" id="test-contrib-group-presence-2">[test-contrib-group-presence-2] contrib-group[@content-type='peer-review'] must be present (as a child of article-meta) for research articles (this is the contrib-group which contains reviewers and editors).</assert>
  ...
  <pattern id="test-article-categories-pattern">
   ...
   <let name="template" value="parent::article-meta/custom-meta-group/custom-meta[meta-name='pdf-template']/meta-value[1]"/>
     ...
     <assert test="count(subj-group[@subj-group-type='heading']) = 1" role="error" id="type-subj-test">[type-subj-test] There must be one subj-group[@subj-group-type='heading'] which is a child of article-categories. Currently there are <value-of select="count(article-categories/subj-group[@subj-group-type='heading'])"/>.</assert>
  ...
  <pattern id="disp-channel-checks-pattern">
    ...
    <let name="article-type" value="ancestor::article/@article-type"/> 
      ...
      <assert test=". = $allowed-disp-subj" role="error" id="type-subj-value-test-1">[type-subj-value-test-1] Content of the article type subject should be one of the following: Research Article, Short Report, Tools and Resources, Research Advance, Registered Report, Replication Study, Research Communication, Feature Article, Insight, Editorial, Correction, Retraction, or Expression of concern. Currently it is <value-of select="."/>.</assert>
      
      <report test="($article-type = 'research-article') and not(.=($research-disp-channels,'Feature Article'))" role="error" id="type-subj-value-test-2">[type-subj-value-test-2] Article is an @article-type="<value-of select="$article-type"/>" but the article type subject is <value-of select="."/>. It should be one of 'Research Article', 'Short Report', 'Tools and Resources', 'Research Advance', 'Registered Report', 'Replication Study', 'Research Communication', or 'Scientific Correspondence' according to the article-type.</report>
      
      <report test="($article-type = 'article-commentary') and not(.='Insight')" role="error" id="type-subj-value-test-3">[type-subj-value-test-3] Article is an @article-type="<value-of select="$article-type"/>" but the article type subject is <value-of select="."/>. It should be 'Insight' according to the article-type.</report>
      
      <report test="($article-type = 'editorial') and not(.='Editorial')" role="error" id="type-subj-value-test-4">[type-subj-value-test-4] Article is an @article-type="<value-of select="$article-type"/>" but the article type subject is <value-of select="."/>. It should be 'Editorial' according to the article-type.</report>
      
      <report test="($article-type = 'correction') and not(.='Correction')" role="error" id="type-subj-value-test-5">[type-subj-value-test-5] Article is an @article-type="<value-of select="$article-type"/>" but the article type subject is <value-of select="."/>. It should be 'Correction' according to the article-type.</report>
      
      <report test="($article-type = 'discussion') and not(.='Feature Article')" role="error" id="type-subj-value-test-6">[type-subj-value-test-6] Article is an @article-type="<value-of select="$article-type"/>" but the article type subject is <value-of select="."/>. It should be 'Feature Article' according to the article-type.</report>
      
      <report test="($article-type = 'review-article') and not(.='Review Article')" role="error" id="type-subj-value-test-7">[type-subj-value-test-7] Article is an @article-type="<value-of select="$article-type"/>" but the article type subject is <value-of select="."/>. It should be 'Review Article' according to the article-type.</report>
      
      <report test="($article-type = 'retraction') and not(.='Retraction')" role="error" id="type-subj-value-test-8">[type-subj-value-test-8] Article is an @article-type="<value-of select="$article-type"/>" but the article type subject is <value-of select="."/>. It should be 'Retraction' according to the article-type.</report>
      
      <report test="($article-type = 'expression-of-concern') and not(.='Expression of concern')" role="error" id="type-subj-value-test-9">[type-subj-value-test-9] Article is an @article-type="<value-of select="$article-type"/>" but the article type subject is <value-of select="."/>. It should be 'Expression of concern' according to the article-type.</report>
  </rule>
  </pattern>
  <pattern id="MSA-checks-pattern">
    <rule context="article-categories/subj-group[@subj-group-type='major-subject']/subject" id="MSA-checks">
      
      <assert test=". = $MSAs" role="error" id="subj-msa-test">[subj-msa-test] Content of the subject must match one of the MSAs. <value-of select="."/> is not an eLife MSA.</assert>
    </rule>
  </pattern>
  <pattern id="head-subj-checks-pattern">
    <rule context="article-categories/subj-group[@subj-group-type='major-subject']" id="head-subj-checks">
      ...
      <assert test="count(subject) = (1,2)" role="error" id="msa-subj-test-1">[msa-subj-test-1] Each subj-group[@subj-group-type='major-subject'] must contain one or two subject elements. This one contains <value-of select="count(subject)"/>.</assert>
      
      <assert test="empty($indistinct-subjects)" role="error" id="indistinct-msa">[indistinct-msa] The subjects in subj-group[@subj-group-type='major-subject'] must be distinct. The following are duplicated - <value-of select="string-join($indistinct-subjects,', ')"/>.</assert>
    </rule>
  </pattern>
  ...
  <pattern id="auth-contrib-group-pattern">
    <rule context="article/front/article-meta/contrib-group[1]" id="auth-contrib-group">
      ...
      <assert test="@content-type='authors'" role="error" id="content-type-authors-test">[content-type-authors-test] The first contrib-group in article-meta must have the attribute content-type="authors", as it is the container for authors.</assert>
  ...
  <pattern id="test-editor-contrib-group-pattern">
    <rule context="article/front/article-meta/contrib-group[2]" id="test-editor-contrib-group">
      
      <assert test="@content-type='peer-review'" role="error" id="content-type-editors-test">[content-type-editors-test] The second contrib-group in article-meta must have the attribute content-type="peer-revew", as it is the container for Editors.</assert>
      
      <assert test="count(contrib[@contrib-type='senior_editor']) = 1" role="error" id="editor-conformance-1">[editor-conformance-1] contrib-group[@content-type='peer-review'] must contain one (and only 1) Senior Editor (contrib[@contrib-type='senior_editor']).</assert>
      
      <assert test="count(contrib[@contrib-type='editor']) = 1" role="error" id="editor-conformance-2">[editor-conformance-2] contrib-group[@content-type='peer-review'] must contain one (and only 1) Reviewing Editor (contrib[@contrib-type='editor']).</assert>
  ...
  <pattern id="auth-cont-tests-pattern">
    <rule context="article[@article-type='research-article']//article-meta//contrib[(@contrib-type='author') and not(ancestor::collab)]" id="auth-cont-tests">
      ...
      <assert test="role" role="error" id="final-auth-cont-test-1">[final-auth-cont-test-1] <value-of select="e:get-name(name[1])"/> has no contributions. Please ensure to query this with the authors.</assert>
    </rule>
  </pattern>
  <pattern id="collab-cont-tests-pattern">
    <rule context="article[@article-type='research-article']//article-meta//contrib[(@contrib-type='author') and child::collab]" id="collab-cont-tests">
      ...
      <assert test="role" role="error" id="final-collab-cont-test-1">[final-collab-cont-test-1] <value-of select="e:get-collab(child::collab[1])"/> has no contributions. Please ensure to query this with the authors.</assert>
  ...
  <pattern id="credit-role-tests-pattern">
    <rule context="article-meta//contrib[@contrib-type='author']/role[@vocab='credit']" id="credit-role-tests">
      ...
      <assert test="@vocab-identifier='http://credit.niso.org/'" role="error" id="vocab-identifier-conformance">[vocab-identifier-conformance] A CRediT taxonomy role must have a vocab-identifier attribute whose value is http://credit.niso.org/. The role containing <value-of select="."/> does not.</assert>
      
      <report test="not(@vocab-term-identifier) or ((count($credit-role) = 1) and ($vocab-term-id != $credit-role/@uri))" role="error" id="vocab-term-identifier">[vocab-term-identifier] A CRediT taxonomy role must have a @vocab-term-identifier, the value of which must be the URL of the specific CRediT term. <value-of select="if (empty($credit-role)) then concat('It must be one of these - ',string-join($credit-roles//*:item/@uri,', '))            else concat('In this case ',$credit-role/@uri,' (based on the @vocab-term of this role element)')"/>.</report>
      
      <report test="not(@vocab-term) or ((count($credit-role) = 1) and ($vocab-term != $credit-role/@term))" role="error" id="vocab-term-conformance">[vocab-term-conformance] A CRediT taxonomy role must have a @vocab-term, the value of which must be one of the CRediT terms - <value-of select="if (empty($credit-role)) then string-join($credit-roles//*:item/@term,', ')            else concat(' in this case ',$credit-role/@term,' (based on the @vocab-term-identifer of of this role element)')"/>.
      </report>
      
      <report test="count($credit-role) gt 1" role="error" id="vocab-term-and-identifier">[vocab-term-and-identifier] 
      ...
      <report test=". = following-sibling::role/text()" role="error" id="role-text-duplicate">[role-text-duplicate] <value-of select="e:get-name(parent::contrib/name[1])"/> has more than one CRediT role with the text <value-of select="."/>.</report>
      
      <report test="@vocab-term = following-sibling::role/@vocab-term" role="error" id="role-vocab-term-duplicate">[role-vocab-term-duplicate] <value-of select="e:get-name(parent::contrib/name[1])"/> has more than one CRediT role with the vocab-term attribute value <value-of select="@vocab-term"/>.</report>
      
      <report test="@vocab-term-identifier = following-sibling::role/@vocab-term-identifier" role="error" id="role-vocab-term-identifier-duplicate">[role-vocab-term-identifier-duplicate] <value-of select="e:get-name(parent::contrib/name[1])"/> has more than one CRediT role with the vocab-term-identifier attribute value <value-of select="@vocab-term-identifier"/>.</report>
     
    </rule>
  </pattern>
  <pattern id="non-credit-role-tests-pattern">
    <rule context="article-meta//contrib[@contrib-type='author']/role[not(@vocab='credit')]" id="non-credit-role-tests">
      <let name="credit-regex" value="'conceptualization|data curation|formal analysis|funding acquisition|investigation|methodology|project administration|resources|software|supervision|validation|visualization|writing [—–\-] original draft|writing [—–\-] review (&amp;|and) editing'"/>
      
      <report test="matches(lower-case(.),$credit-regex)" role="error" id="credit-role-not-tagged">[credit-role-not-tagged] Non-credit role for <value-of select="if (parent::contrib/collab) then parent::contrib/collab/text()[1] else (e:get-name(parent::contrib/name[1]))"/>, '<value-of select="."/>', contains a credit term. This should be captured as a proper credit role (role[@vocab="credit"]) instead.</report>
  ...
  <pattern id="author-xref-tests-pattern">
    <rule context="article-meta//contrib[@contrib-type='author']/xref" id="author-xref-tests">
      ...
      <assert test="@ref-type=('aff','author-notes')" role="error" id="author-xref-ref-type">[author-xref-ref-type] <value-of select="."/> is not permitted as a ref-type attribute value for author xref elements. The only permitted values are 'aff' or 'author-notes'.</assert>
      ...
      <report test="(@ref-type='author-notes') and contains(@rid,'equal') and preceding-sibling::xref[not(@ref-type='aff')]" role="error" id="author-xref-test-2">[author-xref-test-2] Equal contribution links from authors must appear after affiliation footnote links. For <value-of select="e:get-name(preceding-sibling::name[1])"/>, their equal contribution link (to <value-of select="idref(@rid)"/>) appears after another non-affiliation link, when it should appear before it.</report>
      
      <report test="(@ref-type='author-notes') and contains(@rid,'pa') and following-sibling::xref[@ref-type='aff' or contains(@rid,'equal')]" role="error" id="author-xref-test-3">[author-xref-test-3] Present address type footnote links from authors must appear after affiliation and equal contribution links (if there is one). For <value-of select="e:get-name(preceding-sibling::name[1])"/>, their present address link (to <value-of select="idref(@rid)"/>) appears before an affiliation link or equal contribution link.</report>
  ...
  <pattern id="given-names-tests-pattern">
    <rule context="contrib-group//name/given-names" id="given-names-tests">
      ...
      <report test="matches(normalize-space(.),'[A-Za-z]\s[A-za-z]\s[A-za-z]\s[A-za-z]|[A-Za-z]\s[A-za-z]\s[A-za-z]$|^[A-za-z]\s[A-za-z]$')" role="info" id="given-names-test-15">[given-names-test-15] given-names contains initials with spaces. Esnure that the space(s) is removed between initials - '<value-of select="."/>'.</report>
  ...
  <pattern id="contrib-tests-pattern">
    <rule context="article-meta//contrib" id="contrib-tests">
	  ...
	  <report test="self::*[@corresp='yes'][not(email) and not(contrib-id[@contrib-id-type='twitter'])]" role="error" id="contrib-contact-1">[contrib-contact-1] Corresponding authors must have contact information - either an email or a twitter handle.</report>
	  
	  <report test="not(@corresp='yes') and (not(ancestor::collab/parent::contrib[@corresp='yes'])) and (email or contrib-id[@contrib-id-type='twitter'])" role="error" id="contrib-contact-2">[contrib-contact-2] Non-corresponding authors must not have contact information (an email or twitter handle).</report>
  ...
  <pattern id="email-tests-pattern">
    <rule context="article-meta//email" id="email-tests">
    	...
    	<assert test="matches(upper-case(.),'^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$')" role="error" id="email-test">[email-test] email element must contain a valid email address. Currently it is <value-of select="."/>.</assert>
  ...
  <pattern id="front-permissions-tests-pattern">
	...
	<assert test="license" role="error" id="permissions-test-5">[permissions-test-5] permissions must contain license.</assert>
	  ...
	  <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/licensing-and-copyright#permissions-test-6" test="if (contains($license-type,'creativecommons.org/publicdomain/zero')) then ()            else not(copyright-year = ancestor::article-meta/pub-date[@publication-format='electronic'][@date-type='pub']/year)" role="error" id="permissions-test-6">[permissions-test-6] copyright-year must match the contents of the year in the pub-date[@publication-format='electronic'][@date-type='pub']. Currently, copyright-year=<value-of select="copyright-year"/> and pub-date=<value-of select="ancestor::article-meta/pub-date[@publication-format='electronic'][@date-type='pub']/year"/>.</report>
  ...
  <pattern id="impact-statement-abstract-tests-pattern">
    <rule context="article-meta/abstract[@abstract-type='toc']" id="impact-statement-abstract-tests">
      ...
      <report test="count(p) gt 1" role="error" id="impact-statement-abstract-test-1">[impact-statement-abstract-test-1] abstract[@abstract-type='toc'] (for the impact statement) cannot contain more than one p element. This one has <value-of select="count(p)"/>.</report>
      
      <report test="*[name() != 'p']" role="error" id="impact-statement-abstract-test-2">[impact-statement-abstract-test-2] abstract[@abstract-type='toc'] must only contain p elements. This one has other elements: <value-of select="string-join(for $x in child::* return $x/name(),', ')"/>.</report>
      
    </rule>
  </pattern>
  <pattern id="impact-statement-tests-pattern">
    <rule context="article-meta/abstract[@abstract-type='toc']/p[1]" id="impact-statement-tests">
      ...
      <report test="not(child::*) and normalize-space(.)=''" role="error" id="impact-statement-test-1">[impact-statement-test-1] The abstract for the impact statement cannot be empty.</report>
      
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#custom-meta-test-5" test="($count gt 30)" role="warning" id="impact-statement-test-2">[impact-statement-test-2] Impact statement contains more than 30 words (<value-of select="$count"/>). This is not allowed.</report>
      
      
      
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#final-custom-meta-test-6" test="matches(.,'[\.|\?]$')" role="error" id="final-impact-statement-test-3">[final-impact-statement-test-3] Impact statement must end with a full stop or question mark.</assert>
      
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#custom-meta-test-7" test="matches(replace(.,' et al\. ',' et al '),'[\p{L}][\p{L}]+\. .*$|[\p{L}\p{N}][\p{L}\p{N}]+\? .*$|[\p{L}\p{N}][\p{L}\p{N}]+! .*$')" role="warning" id="impact-statement-test-4">[impact-statement-test-4] Impact statement appears to be made up of more than one sentence. Please check, as more than one sentence is not allowed.</report>
      
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#custom-meta-test-8" test="not($subj = 'Replication Study') and matches(.,'[:;]')" role="warning" id="impact-statement-test-5">[impact-statement-test-5] Impact statement contains a colon or semi-colon, which is likely incorrect. It needs to be a proper sentence.</report>
      
      
      
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#final-custom-meta-test-9" test="matches(.,'[Ww]e show|[Ww]e present|[Tt]his study|[Tt]his paper')" role="error" id="final-impact-statement-test-6">[final-impact-statement-test-6] Impact statement contains a possessive phrase. This is not allowed.</report>
      
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#custom-meta-test-10" test="matches(.,'^[\d]+$')" role="error" id="impact-statement-test-7">[impact-statement-test-7] Impact statement is comprised entirely of numbers, which must be incorrect.</report>
      
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#custom-meta-test-11" test="matches(.,' [Oo]ur |^[Oo]ur ')" role="warning" id="impact-statement-test-8">[impact-statement-test-8] Impact statement contains 'our'. Is this possessive langauge relating to the article or research itself (which should be removed)?</report>
      
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#custom-meta-test-13" test="matches(.,' study ') and not(matches(.,'[Tt]his study'))" role="warning" id="impact-statement-test-9">[impact-statement-test-9] Impact statement contains 'study'. Is this a third person description of this article? If so, it should be changed to not include this.</report>
      
      
      
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#final-rep-study-custom-meta-test" test="($subj = 'Replication Study') and not(matches(.,'^Editors[\p{Po}] Summary: '))" role="error" id="final-rep-study-impact-statement-test">[final-rep-study-impact-statement-test] Impact statement in Replication studies must begin with 'Editors' summary: '. This does not - <value-of select="."/>.</report>
      
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#custom-meta-test-16" test="$we-token = $verbs" role="warning" id="impact-statement-test-11">[impact-statement-test-11] Impact statement contains 'we' followed by a verb - '<value-of select="concat('we ',$we-token)"/>' in '<value-of select="."/>'. Is this possessive language relating to the article or research itself (which should be removed)?</report>
    </rule>
  </pattern>
  ...
  <pattern id="author-aff-tests-pattern">
    <rule context="article-meta/contrib-group[@content-type='authors']/aff" id="author-aff-tests">
      ...
      <assert test="city" role="error" id="final-auth-aff-test-2">[final-auth-aff-test-2] Author affiliations must have a city. This one does not - <value-of select="$display"/>
      </assert>
  ...
  <pattern id="gen-aff-tests-pattern">
    <rule context="aff" id="gen-aff-tests">
     ...
     <report test="count(institution[@content-type='dept']) ge 1" role="warning" id="gen-aff-test-2">[gen-aff-test-2] Affiliation has <value-of select="count(institution[@content-type='dept'])"/> department field(s) - <value-of select="$display"/>. Is this correct?</report>
      ...
      <report test="count(city) gt 1" role="error" id="gen-aff-test-4">[gen-aff-test-4] Affiliations cannot have more than 1 city elements. <value-of select="$display"/> has <value-of select="count(city)"/>.</report>
      
      <report test="count(state) gt 1" role="error" id="gen-aff-test-7">[gen-aff-test-7] Affiliations cannot have more than 1 state elements. <value-of select="$display"/> has <value-of select="count(state)"/>.</report>
  ...
  <pattern id="aff-child-tests-pattern">
    <rule context="aff/*" id="aff-child-tests">
      <let name="allowed-elems" value="('label','institution','city','state','country')"/>
  ...
  <pattern id="institution-wrap-tests-pattern">
    <rule context="article-meta//award-group//institution-wrap" id="institution-wrap-tests">
      ...
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/funding-information#institution-id-test" test="institution-id[@institution-id-type='doi']" role="warning" id="institution-id-test">[institution-id-test] Whenever possible, a funder should have a doi - please check whether there is an appropriate doi in the open funder registry. (institution-id[@institution-id-type="doi"] is not present in institution-wrap).</assert>
  ...
  <pattern id="institution-id-tests-pattern">
    <rule context="institution-wrap/institution-id" id="institution-id-tests">
      ...
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/funding-information#institution-id-test-2" test="@institution-id-type='doi'" role="error" id="institution-id-test-2">[institution-id-test-2] <name/> element must have the attribute institution-id-type="doi".</assert>
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/funding-information#institution-id-test-5" test="(normalize-space(.) != '') and not(matches(.,'^10.13039/\d*$'))" role="error" id="institution-id-test-5">[institution-id-test-5] <name/> element in funding entry for <value-of select="parent::institution-wrap/institution"/> contains the following text - <value-of select="."/> - which is not a fundref doi.</report>
      
      <assert test="@vocab='open-funder-registry'" role="error" id="institution-id-test-6">[institution-id-test-6] <name/> element must have the attribute vocab="open-funder-registry".</assert>
      
      <assert test="@vocab-identifier='10.13039/open-funder-registry'" role="error" id="institution-id-test-7">[institution-id-test-7] <name/> element must have the attribute vocab-identifier="10.13039/open-funder-registry".</assert>
  ...
  <pattern id="par-tests-pattern">
    <rule context="funding-group//principal-award-recipient" id="par-tests">
      ...
      <assert test="count(name) = 1" role="error" id="par-test-1">[par-test-1] Every <name/> element must have one and only one name element. If there are mutliple award recipients, they must each be tagged in their own <name/> element.</assert>
      
    </rule>
  </pattern>
  <pattern id="funding-orcid-tests-pattern">
    <rule context="principal-award-recipient/contrib-id[@contrib-id-type='orcid']" id="funding-orcid-tests">
      <let name="orcid" value="."/>
      <let name="matching-author-name" value="e:get-name(ancestor::article-meta//contrib[@contrib-type='author' and contrib-id[@contrib-id-type='orcid' and text() = $orcid]]/name[1])"/>
      <let name="name" value="e:get-name(parent::*/name[1])"/>
      
      <!-- Needs updating to only allow https when this is implemented -->
      <assert test="matches(.,'^http[s]?://orcid.org/[\d]{4}-[\d]{4}-[\d]{4}-[\d]{3}[0-9X]$')" role="error" id="funding-orcid-test-1">[funding-orcid-test-1] contrib-id[@contrib-id-type="orcid"] must contain a valid ORCID URL in the format 'https://orcid.org/0000-0000-0000-0000'</assert>
      
      <assert test="$name = $matching-author-name" role="error" id="funding-orcid-test-2">[funding-orcid-test-2] In the funding section the ORCiD and name must be the same as in the author list. The ORCiD in the funding entry for <value-of select="$name"/> (<value-of select="$orcid"/>) does not match up with the ORCiD in the author section. Either the ORCiD is incorrect or the name is incorrect.</assert>
  ...
  <pattern id="kwd-group-tests-pattern">
    <rule context="article-meta/kwd-group[not(@kwd-group-type='research-organism')]" id="kwd-group-tests">
      ...
      <assert test="@kwd-group-type='author-generated'" role="error" id="kwd-group-type">[kwd-group-type] kwd-group must have a @kwd-group-type 'research-organism', or 'author-generated'.</assert>
  ...
  <pattern id="custom-meta-group-tests-pattern">
    <rule context="article-meta/custom-meta-group" id="custom-meta-group-tests">
      ...
      <report test="count(custom-meta[@specific-use='meta-only']) != 3" role="error" id="custom-meta-presence">[custom-meta-presence] There must be three custom-meta[@specific-use='meta-only'] elements in article-meta. One for the pdf-template, one for the source-file-type, and one for the elife-xml-version. Currently there are <value-of select="count(custom-meta[@specific-use='meta-only'])"/>.</report>
  ...
  <pattern id="custom-meta-tests-pattern">
    <rule context="article-meta/custom-meta-group/custom-meta" id="custom-meta-tests">
      ...
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#custom-meta-test-1" test="count(meta-name) = 1" role="error" id="custom-meta-test-1">[custom-meta-test-1] One meta-name must be present in custom-meta.</assert>
      
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#custom-meta-test-2" test="meta-name = ('pdf-template','source-file-type','elife-xml-version')" role="error" id="custom-meta-test-2">[custom-meta-test-2] The value of meta-name can only be 'pdf-template', 'source-file-type', or 'elife-xml-version'. Currently it is <value-of select="meta-name"/>.</assert>
      
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#custom-meta-test-3" test="count(meta-value) = 1" role="error" id="custom-meta-test-3">[custom-meta-test-3] One meta-value must be present in custom-meta.</assert>
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/impact-statement#custom-meta-test-15" test="($pos=2) and (meta-name != 'source-file-type')" role="error" id="custom-meta-test-15">[custom-meta-test-15] The value of the 2nd meta-name can only be 'source-file-type'. Currently it is <value-of select="meta-name"/>.</report>
      
      <report test="($pos=3) and (meta-name != 'elife-xml-version')" role="error" id="custom-meta-test-17">[custom-meta-test-17] The value of the 3rd meta-name can only be 'elife-xml-version'. Currently it is <value-of select="meta-name"/>.</report>
  ...
  <pattern id="meta-value-tests-pattern">
    <rule context="article-meta/custom-meta-group/custom-meta/meta-value" id="meta-value-tests">
      ...
      <report test="child::*" role="error" id="meta-value-test-1">[meta-value-test-1] <name/> cannot contain elements.</report>
      
    </rule>
  </pattern>
  <pattern id="pdf-template-meta-tests-pattern">
    <rule context="article-meta/custom-meta-group/custom-meta[meta-name='pdf-template']/meta-value" id="pdf-template-meta-tests">
      ...
      <assert test=". = ('1','2','3','4','5','6')" role="error" id="pdf-template-meta-test-1">[pdf-template-meta-test-1] pdf-template meta-value must one of '1','2','3','4','5', or '6'.</assert>
      
      <report test=". = ('1','2','3','4','5','6')" role="info" id="pdf-template-meta-test-info">[pdf-template-meta-test-info] Template <value-of select="."/>.</report>
      
      <report test="($type='Insight') and (. != '1')" role="error" id="pdf-template-meta-test-2">[pdf-template-meta-test-2] <value-of select="$type"/> must be a template 1. Currently it is a template <value-of select="."/>.</report>
      
      <report test="($type='Editorial') and (. != '2')" role="error" id="pdf-template-meta-test-3">[pdf-template-meta-test-3] <value-of select="$type"/> must be a template 2. Currently it is a template <value-of select="."/>.</report>
      
      <report test="($type='Feature Article') and not(.=('3','4','5'))" role="error" id="pdf-template-meta-test-4">[pdf-template-meta-test-4] <value-of select="$type"/> must be a template 3, 4, or 5. Currently it is a template <value-of select="."/>.</report>
      
      <report test="($type=$research-subj) and (. != '6')" role="error" id="pdf-template-meta-test-5">[pdf-template-meta-test-5] <value-of select="$type"/> must be a template 6. Currently it is a template <value-of select="."/>.</report>
      
    </rule>
  </pattern>
  <pattern id="source-file-meta-tests-pattern">
    <rule context="article-meta/custom-meta-group/custom-meta[meta-name='source-file-type']/meta-value" id="source-file-meta-tests">
      
      <assert test=". = ('latex','word')" role="error" id="source-file-meta-test-1">[source-file-meta-test-1] The meta-value for the source-file-type must be either 'latex', or 'word'. <value-of select="meta-name"/> is not allowed.</assert>
      
    </rule>
  </pattern>
  <pattern id="xml-version-meta-tests-pattern">
    <rule context="article-meta/custom-meta-group/custom-meta[meta-name='elife-xml-version']/meta-value" id="xml-version-meta-tests">
      
      <assert test=". = 2.0" role="error" id="xml-version-meta-test-1">[xml-version-meta-test-1] The meta-value for the elife-xml-version must be '2.0'. <value-of select="meta-name"/> is not allowed.</assert>
  ...
  <pattern id="related-object-tests-pattern">
    <rule context="related-object" id="related-object-tests">
      ...
      <assert test="ancestor::abstract[not(@abstract-type)] or ancestor::sec[@sec-type='ethics-statement']" role="error" id="related-object-ancestor">[related-object-ancestor] <name/> is not allowed outside of the main abstract (abstract[not(@abstract-type)]) or ethics statements (sec[@sec-type='ethics-statement']).</assert>
    </rule>
  </pattern>
  
  <pattern id="pub-date-tests-pattern">
    <rule context="article-meta/pub-date" id="pub-date-tests">
      <let name="date" value="e:get-iso-pub-date(.)"/>
      
      <assert test="@publication-format='electronic'" role="error" id="pub-date-1">[pub-date-1] pub-date must have the attribute publication-format='electronic'.</assert>
      
      <assert test="@iso-8601-date = $date" role="error" id="pub-date-2">[pub-date-2] pub-date must have the attribute iso-8601-date with a value equivalent to the data in the year, month and day fields, in this case '<value-of select="$date"/>'.</assert>
      
      <assert test="@date-type=('pub','update')" role="error" id="pub-date-3">[pub-date-3] pub-date must have the attribute date-type with a value which is either 'pub' (for the publication date) or 'update' (for the update date if there is one, for example VoR publication date for an article that has been PoA'd).</assert>
      
      <report test="@date-type='pub' and preceding-sibling::pub-date" role="error" id="pub-date-4">[pub-date-4] There can only be one pub-date with the attribute date-type='pub', and it must be the first pub-date appearing in article-meta. This has <value-of select="count(preceding-sibling::pub-date)"/> pub-date(s) preceding it with the date-type(s) <value-of select="string-join(preceding-sibling::pub-date/@date-type,' and ')"/>.</report>
      
      <report test="@date-type='update' and not(preceding-sibling::pub-date[@date-type='pub'])" role="error" id="pub-date-5">[pub-date-5] The update date (pub-date[@date-type='update']), must be placed directly after the publication date (pub-date[@date-type='pub']) in article-meta. Either this article is missing a publication date, or the update date is placed incorrectly.</report>
    </rule>
  </pattern>
  <pattern id="event-tests-pattern">
    <rule context="event" id="event-tests">
      
      <assert test="event-desc" role="error" id="event-1">[event-1] event must contain an event description (event-desc).</assert>
      
      <assert test="date" role="error" id="event-2">[event-2] event must contain at least one date.</assert>
  ...
  <pattern id="xref-target-tests-pattern">
    <rule context="xref" id="xref-target-tests">
      ...
      <report test="(@ref-type='list') and ($target/local-name() != 'def-item')" role="error" id="list-xref-target-test">[list-xref-target-test] xref with @ref-type='<value-of select="@ref-type"/>' points to <value-of select="$target/local-name()"/>. This is not correct.</report>
  ...
  <pattern id="fig-group-child-tests-pattern">
    <rule context="fig-group/*" id="fig-group-child-tests">
      ...
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/figures#fig-group-child-test-1" test="local-name() = ('fig','media','caption')" role="error" id="fig-group-child-test-1">[fig-group-child-test-1] <name/> is not allowed as a child of fig-group.</assert>
  ...
  <pattern id="fig-group-caption-pattern">
    <rule context="fig-group/caption" id="fig-group-caption">
      ...
      <assert test="@specific-use='print'" role="error" id="fig-group-child-test-3">[fig-group-child-test-3] <name/> which is a child of fig-group, must have an specific-use='print' attribute so that it does not display on the eLife website.</assert>
      
      <assert test="p[1] = $text" role="error" id="fig-group-caption-conformance">[fig-group-caption-conformance] fig-group caption contains '<value-of select="p[1]"/>', when according to the contents of the fig-group, the text should be '<value-of select="$text"/>'.</assert>
      
      <assert test="count(p) = 1" role="error" id="fig-group-caption-child-1">[fig-group-caption-child-1] fig-group caption must contain one and only one p element.</assert>
      
      <report test="*[not(name() = 'p')]" role="error" id="fig-group-caption-child-2">[fig-group-caption-child-2] fig-group caption can only contain a sinlge p element. <value-of select="string-join(distinct-values(*[not(name() = 'p')]/name()),' and ')"/> is not permitted.</report>
  ...
  <pattern id="fig-tests-pattern">
    <rule context="fig[not(ancestor::sub-article)]" id="fig-tests">
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/figures#final-fig-test-5" test="not($article-type = $notice-article-types) and not(caption/title)" role="error" id="final-fig-test-5">[final-fig-test-5] fig caption must have a title.</report>
      ...
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/figures#final-fig-test-7" test="graphic or alternatives[graphic[not(@specific-use='print')]]" role="error" id="final-fig-test-7">[final-fig-test-7] fig must have a graphic.</assert>
    </rule>
  </pattern>
  ...
  <pattern id="graphic-tests-pattern">
    <rule context="graphic|inline-graphic" id="graphic-tests">
      ...
      <report test="contains(@mime-subtype,'postscript') and not(ends-with($file,'.eps')) and not(ends-with($file,'.ps'))" role="error" id="graphic-test-2">[graphic-test-2] <name/> has postscript mime-subtype but filename does not end with '.eps' or '.ps'. This cannot be correct.</report>
  ...
  <pattern id="back-supplementary-file-tests-pattern">
    <rule context="sec[@sec-type='supplementary-material']/supplementary-material[contains(label[1],'upplementary file')]" id="back-supplementary-file-tests">
      ...
      <assert test="string($pos) = $no" role="error" id="back-supplementary-file-position">[back-supplementary-file-position] <value-of select="replace(label,'\.$','')"/> id ends with <value-of select="$no"/>, but it is placed <value-of select="$pos"/>. Either it is mislabelled, the id is incorrect, or it should be moved to a different position.</assert>
  ...
  <pattern id="back-source-data-tests-pattern">
    <rule context="sec[@sec-type='supplementary-material']/supplementary-material[contains(label[1],'ource data')]" id="back-source-data-tests">
      ...
      <assert test="string($pos) = $no" role="error" id="back-source-data-position">[back-source-data-position] <value-of select="replace(label,'\.$','')"/> id ends with <value-of select="$no"/>, but it is placed <value-of select="$pos"/>. Either it is mislabelled, the id is incorrect, or it should be moved to a different position.</assert>
  ...
  <pattern id="fig-source-data-tests-pattern">
    <rule context="fig//supplementary-material[not(ancestor::media) and contains(label[1],' data ')]" id="fig-source-data-tests">
      ...
      <assert test="$number = $pos" role="error" id="fig-data-test-2">[fig-data-test-2] '<value-of select="$label"/>' ends with <value-of select="$number"/>, but it is placed <value-of select="$pos"/>. Either it is misnumbered or it should be moved to a different position.</assert>
  ...
  <pattern id="fig-source-code-tests-pattern">
    <rule context="fig//supplementary-material[not(ancestor::media) and contains(label[1],' code ')]" id="fig-source-code-tests">
      ...
      <assert test="$number = $pos" role="error" id="fig-code-test-2">[fig-code-test-2] '<value-of select="$label"/>' ends with <value-of select="$number"/>, but it is placed <value-of select="$pos"/>. Either it is misnumbered or it should be moved to a different position.</assert>
  ...
  <pattern id="vid-source-data-tests-pattern">
    <rule context="media//supplementary-material[not(ancestor::fig) and contains(label[1],' data ')]" id="vid-source-data-tests">
      ...
      <assert test="$number = $pos" role="error" id="vid-data-test-2">[vid-data-test-2] '<value-of select="$label"/>' ends with <value-of select="$number"/>, but it is placed <value-of select="$pos"/>. Either it is misnumbered or it should be moved to a different position.</assert>
  ...
  <pattern id="vid-source-code-tests-pattern">
    <rule context="media//supplementary-material[not(ancestor::fig) and contains(label[1],' code ')]" id="vid-source-code-tests">
      ...
      <assert test="$number = $pos" role="error" id="vid-code-test-2">[vid-code-test-2] '<value-of select="$label"/>' ends with <value-of select="$number"/>, but it is placed <value-of select="$pos"/>. Either it is misnumbered or it should be moved to a different position.</assert>
  ...
  <pattern id="disp-formula-child-tests-pattern">
    <rule context="disp-formula/*" id="disp-formula-child-tests">
      ...
      <report test="not(name()=('label','mml:math'))" role="error" id="disp-formula-child-test-1">[disp-formula-child-test-1] <name/> element is not allowed as a child of disp-formula.</report>
    </rule>
  </pattern>
  <pattern id="inline-formula-child-tests-pattern">
    <rule context="inline-formula/*" id="inline-formula-child-tests">
      
      <report test="name()!='mml:math'" role="error" id="inline-formula-child-test-1">[inline-formula-child-test-1] <name/> element is not allowed as a child of inline-formula.</report>
    </rule>
  </pattern>
  ...
  <pattern id="list-tests-pattern">
    <rule context="list[@continued-from]" id="list-tests">
      <let name="rid" value="@continued-from"/>
      <let name="type" value="@list-type"/>
      
      <assert test="preceding::list[(@id = $rid) and @list-type = $type]" role="error" id="continued-from-test-1">[continued-from-test-1] List (with the type <value-of select="$type"/>) has the continued from attribute which references a list with the id '<value-of select="$rid"/>', but there is no list before it with that id and the same type. Either the continued-from value is incorrect, the types need to be made the same, or the lists need to be moved so that they reference each other correctly.</assert>
  ...
  <pattern id="code-tests-pattern">
    <rule context="code" id="code-tests">
      ...
      <assert test="parent::p" role="error" id="code-parent-test">[code-parent-test] code element (containing the content <value-of select="."/>) is directly preceded by another code element (containing the content <value-of select="preceding::*[1]"/>). If the content is part of the same code block, then it should be captured using only 1 code element and line breaks added in the xml. If these are separate code blocks (uncommon, but possible), then this markup is fine.</assert>
  ...
  <pattern id="code-tests-2-pattern">
    <rule context="p[count(code) gt 1]/code[2]" id="code-tests-2">
      ...
      <report test="normalize-space(preceding-sibling::text()[preceding-sibling::*[1]/local-name()='code'][1])=''" role="warning" id="code-sibling-test">[code-sibling-test] code element (containing the content <value-of select="."/>) is directly preceded by another code element (containing the content <value-of select="preceding::*[1]"/>). If the content is part of the same code block, then it should be captured using only 1 code element and line breaks added in the xml. If these are separate code blocks (uncommon, but possible), then this markup is fine.</report>
  ...
  <pattern id="code-tests-3-pattern">
    <rule context="p[count(code) = 1]/code" id="code-tests-3">
      ...
      <report test="$previous-parent/*[last()][(local-name()='code') and not(following-sibling::text())] and not(preceding-sibling::*) and not(preceding-sibling::text())" role="warning" id="code-sibling-test-2">[code-sibling-test-2] code element (containing the content <value-of select="."/>) is directly preceded by another code element (containing the content <value-of select="preceding::*[1]"/>). If the content is part of the same code block, then it should be captured using only 1 code element and line breaks added in the xml. If these are separate code blocks (uncommon, but possible), then this markup is fine.</report>
  ...
  <pattern id="disp-quote-tests-pattern">
    <rule context="disp-quote" id="disp-quote-tests">
      ...
      <report test="ancestor::sub-article[@article-type='referee-report']" role="warning" flag="dl-ar" id="disp-quote-test-1">[disp-quote-test-1] Content is tagged as a display quote, which is almost definitely incorrect, since it's in a decision letter - <value-of select="."/>
      </report>
  ...
  <pattern id="fig-sup-tests-pattern">
    <rule context="article/body//fig[@specific-use='child-fig']" id="fig-sup-tests">
      ...
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/figures#fig-sup-test-2" test="$label-conform = true()" role="error" id="fig-sup-test-2">[fig-sup-test-2] fig in the body of the article which has a @specific-use='child-fig' must have a label in the format 'Figure 0—figure supplement 0.' (where 0 is one or more digits).</assert>
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/figures#fig-sup-test-4" test="not($article-type = $notice-article-types) and $no != string($pos)" role="error" id="fig-sup-test-4">[fig-sup-test-4] <value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other figures it is placed in position <value-of select="$pos"/>.</report>
      
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/figures#fig-sup-test-5" test="not($article-type = $notice-article-types) and (($label-conform = true()) and ($label-no != string($pos)))" role="error" id="fig-sup-test-5">[fig-sup-test-5] <value-of select="label"/> is in position <value-of select="$pos"/>, which means either the label or the placement incorrect.</report>
      
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/figures#fig-sup-test-6" test="($label-conform = true()) and ($no != $label-no)" role="error" id="fig-sup-test-6">[fig-sup-test-6] <value-of select="label"/> label ends with <value-of select="$label-no"/>, but the id (<value-of select="@id"/>) ends with <value-of select="$no"/>, so one must be incorrect.</report>
  ...
  <pattern id="permissions-2-pattern">
    <rule context="permissions[not(parent::article-meta) and copyright-statement and not(license[1]/ali:license_ref[1][contains(.,'creativecommons.org')]) and not(contains(license[1]/@xlink:href,'creativecommons.org'))]" id="permissions-2">
      ...
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/licensing-and-copyright#fig-permissions-test-12" test="matches(license[1]/license-p[1],'[Ff]urther reproduction of (this|these) (panels?|illustrations?) would need permission from the copyright holder\.$|[Ff]urther reproduction of this figure would need permission from the copyright holder\.$')" role="warning" id="fig-permissions-test-12">[fig-permissions-test-12] <value-of select="$label"/> permissions - the &lt;license-p&gt; for all rights reserved type permissions should usually end with 'further reproduction of this panel/illustration/figure would need permission from the copyright holder.', but <value-of select="$label"/>'s doesn't. Is this correct? (There is no 'https://creativecommons.org/' type link on the license element or in an ali:license_ref so presumed ARR.)</assert>
  ...
  <pattern id="top-level-sec-tests-pattern">
    <rule context="body/sec" id="top-level-sec-tests">
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/article-structure#sec-conformity" test="not($type = ($features-subj,'Review Article', $notice-display-types)) and not(replace(title,' ',' ') = $allowed-titles)" role="warning" id="sec-conformity">[sec-conformity] top level sec with title - <value-of select="title"/> - is not a usual title for <value-of select="$type"/> content. Should this be captured as a sub-level of <value-of select="preceding-sibling::sec[1]/title"/>?</report>
  ...
  <pattern id="fig-ids-pattern">
    <rule context="article/body//fig[not(@specific-use='child-fig')][not(ancestor::boxed-text)]" id="fig-ids">
      ...
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/figures#fig-id-test-1" test="matches(@id,'^fig[0-9]{1,3}$|^chem[0-9]{1,3}$|^scheme[0-9]{1,3}$')" role="error" id="fig-id-test-1">[fig-id-test-1] fig must have an @id in the format fig0 (or C0 for chemical structures, or S0 for Schemes). <value-of select="@id"/> does not conform to this.</assert>
  ...
  <pattern id="fn-ids-pattern">
    <rule context="fn" id="fn-ids">
      ...
      <report test="if ($parent = 'table-wrap-foot') then ()         else if ($type = 'conflict') then not(matches(@id,'^conf[0-9]{1,3}$'))         else if ($type = 'con') then           if ($parent = 'author-notes') then not(matches(@id,'^equal-contrib[0-9]{1,3}$'))           else not(matches(@id,'^con[0-9]{1,3}$'))         else if ($type = 'present-address') then not(matches(@id,'^pa[0-9]{1,3}$'))         else if ($type = 'COI-statement') then not(matches(@id,'^con[0-9]{1,3}$'))         else if ($type = 'fn') then not(matches(@id,'^fn[0-9]{1,3}$'))         else ()" role="error" id="fn-id-test">[fn-id-test] fn @id is not in the correct format. Refer to eLife kitchen sink for correct format.</report>
    </rule>
  </pattern>
  <pattern id="body-disp-formula-ids-pattern">
    <rule context="article/body//disp-formula[not(ancestor::boxed-text)]" id="body-disp-formula-ids">
      
      <assert test="matches(@id,'^equ[0-9]{1,9}$')" role="error" id="body-disp-formula-id-test">[body-disp-formula-id-test] disp-formula @id must be in the format 'equ0'.</assert>
    </rule>
  </pattern>
  <pattern id="box-disp-formula-ids-pattern">
    <rule context="boxed-text//disp-formula" id="box-disp-formula-ids">
      
      <report test="not(ancestor::app) and not(matches(@id,'^box[0-9]{1,9}equ[0-9]{1,9}$'))" role="error" id="box-disp-formula-id-test-1">[box-disp-formula-id-test-1] disp-formula @id in boxed-text must be in the format 'box0equ0'.</report>
      
      <report test="ancestor::app and not(matches(@id,'^app[0-9]{1,9}box[0-9]{1,9}equ[0-9]{1,9}$'))" role="error" id="box-disp-formula-id-test-2">[box-disp-formula-id-test-2] disp-formula @id in boxed-text which is also in an appendix must be in the format 'app0box0equ0'.</report>
    </rule>
  </pattern>
  <pattern id="mml-math-ids-pattern">
    <rule context="disp-formula/mml:math" id="mml-math-ids">
      
      <report test="not(ancestor::sub-article) and not(ancestor::app) and not(matches(@id,'^m[0-9]{1,9}$'))" role="error" id="mml-math-id-test">[mml-math-id-test] mml:math @id in disp-formula must be in the format 'm0'.  <value-of select="@id"/> does not conform to this.</report>
      
      <report test="(ancestor::app) and not(matches(@id,'^app\d+m[0-9]{1,9}$'))" role="error" flag="dl-ar" id="app-mml-math-id-test">[app-mml-math-id-test] mml:math @id in disp-formula must be in the format 'app0m0'.  <value-of select="@id"/> does not conform to this.</report>
  ...
  <pattern id="app-table-wrap-ids-pattern">
    <rule context="app//table-wrap[label]" id="app-table-wrap-ids">
      ...
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/tables#app-table-wrap-id-test-1" test="matches(@id, '^app[0-9]{1,3}table[0-9]{1,3}$|^app[0-9]{1,3}keyresource$')" role="error" id="app-table-wrap-id-test-1">[app-table-wrap-id-test-1] table-wrap @id in appendix must be in the format 'app0table0' for normal tables, or 'keyresource' for key resources tables in appendices. <value-of select="@id"/> does not conform to this.</assert>
  ...
  <pattern id="low-level-sec-ids-pattern">
    <rule context="article/body/sec//sec[not(ancestor::boxed-text)]|article/back/sec//sec[not(ancestor::boxed-text)]" id="low-level-sec-ids">
  ...
  <pattern id="boxed-text-upper-sec-ids-pattern">
    <rule context="article/body//boxed-text/sec" id="boxed-text-upper-sec-ids">
      ...
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/article-structure#low-level-sec-id-test" test="@id = concat($box-id,'s',$pos)" role="error" id="boxed-text-upper-sec-id">[boxed-text-upper-sec-id] The id for a sec which is a child of boxed-text must be a concatenation of the box id, 's', and this element's position relative to its sibling secs. It must be <value-of select="concat($box-id,'-s',$pos)"/>.</assert>
    </rule>
  </pattern>
  <pattern id="boxed-text-lower-sec-ids-pattern">
    <rule context="article/body//boxed-text/sec//sec" id="boxed-text-lower-sec-ids">
      <let name="parent-sec" value="parent::sec/@id"/>
      <let name="pos" value="count(parent::sec/sec) - count(following-sibling::sec)"/>
      
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/article-structure#low-level-sec-id-test" test="@id = concat($parent-sec,'-',$pos)" role="error" id="boxed-text-lower-sec-id">[boxed-text-lower-sec-id] The id for a sec in boxed-text must be a concatenation of the box id, its parent sec id and this element's position relative to its sibling secs. It must be <value-of select="concat($parent-sec,'-',$pos)"/>.</assert>
    </rule>
  </pattern>
  ...
  <pattern id="def-term-ids-pattern">
    <rule context="def-item" id="def-term-ids">
      
      <assert test="matches(@id,'^def\d{1,4}$')" role="error" id="def-item-id-test">[def-item-id-test] def-term must have an id in the format 'def0'. '<value-of select="@id"/>' is not in this format.</assert>
  ...
  <pattern id="fig-children-pattern">
    <rule context="fig/*" id="fig-children">
      <let name="allowed-children" value="('label', 'caption', 'graphic', 'permissions', 'attrib', 'alternatives')"/>
  ...
  <pattern id="supplementary-material-children-pattern">
    <rule context="supplementary-material/*" id="supplementary-material-children">
      ...
      <assert test="local-name() = $allowed-children" role="error" id="supplementary-material-child-conformance">[supplementary-material-child-conformance] <name/> is not allowed as a child of supplementary-material.</assert>
    </rule>
  </pattern>
  ...
  <pattern id="res-ethics-sec-pattern">
    <rule context="article[@article-type='research-article']//sec[not(@sec-type='ethics-statement') and not(descendant::xref[@ref-type='bibr'])]" id="res-ethics-sec">
  ...
  <pattern id="back-tests-pattern">
    <rule context="back" id="back-tests">
      ...
      <report test="fn-group" role="error" id="back-test-7">[back-test-7] fn-group is not permitted as a child of back.</report>
  ...
  <pattern id="ethics-tests-pattern">
    <rule context="sec[@sec-type='ethics-statement']" id="ethics-tests">
      ...
      <report test="ancestor::article[not(@article-type='discussion')] and not(parent::sec[@sec-type='additional-information'])" role="error" id="ethics-test-1">[ethics-test-1] Ethics statements can only be captured as a child of a sec [@sec-type='additional-information']</report>
 
      <report test="count(p) gt 3" role="error" id="ethics-test-2">[ethics-test-2] Ethics statement may not have more than 3 p elements. Currently there are <value-of select="count(p)"/>.</report>
      
      <report test="count(p) = 0" role="error" id="ethics-test-3">[ethics-test-3] Ethics statements must have at least one fn element.</report>
    </rule>
  </pattern>
  <pattern id="ethics-child-tests-pattern">
    <rule context="sec[@sec-type='ethics-statement']/*" id="ethics-child-tests">
      
      <assert test="name() = ('p','title')" role="error" id="ethics-child-test-1">[ethics-child-test-1] Ethics statements can only contain p and title elements. <value-of select="name()"/> is not permitted.</assert>
    </rule>
  </pattern>
  <pattern id="ethics-p-tests-pattern">
    <rule context="sec[@sec-type='ethics-statement']/p" id="ethics-p-tests">
      
      <assert test="@content-type=('ethics-approval-human','ethics-approval-animal') or related-object[@document-id-type='clinical-trial-number']" role="error" id="ethics-test-4">[ethics-test-4] p element in ethics statements must either have the attribute content-type with a value of 'ethics-approval-human', or 'ethics-approval-animal' (for human or animal ethics statements respectively), or it must have a clincal trial number tagged using a related-object element. This one does not meet this requirement.</assert>
      
    </rule>
  </pattern>
  <pattern id="glossary-tests-pattern">
    <rule context="glossary" id="glossary-tests">
      
      <assert test="parent::back" role="error" id="glossary-test-1">[glossary-test-1] glossary must be captured as a child of back. This one is a child of <value-of select="parent::*/name()"/>.</assert>
      
      <assert test="title" role="error" id="glossary-test-2">[glossary-test-2] glossary must contain a child title element.</assert>
      
      <assert test="def-list" role="error" id="glossary-test-3">[glossary-test-3] glossary must contains a child def-list element.</assert>
    </rule>
  </pattern>
  <pattern id="glossary-child-tests-pattern">
    <rule context="glossary/*" id="glossary-child-tests">
      
      <report test="name()='title' and .!= 'Abbreviations'" role="error" id="glossary-title-test">[glossary-title-test] The title for a glossary must be 'Abbreviations'. This one is <value-of select="."/>.</report>
      
      <assert test="name()=('title','def-list')" role="error" id="glossary-child-test">[glossary-child-test] The only elements permitted within glossary are title and def-list. <name/> is not allowed.</assert>
      
    </rule>
  </pattern>
  <pattern id="def-list-tests-pattern">
    <rule context="def-list" id="def-list-tests">
      
      <assert test="def-item" role="error" id="def-list-empty-test">[def-list-empty-test] def-list must contain at least one definition (def-item).</assert>
      
    </rule>
  </pattern>
  <pattern id="def-item-tests-pattern">
    <rule context="def-item" id="def-item-tests">
      
      <assert test="count(term) = 1" role="error" id="def-item-term-test">[def-item-term-test] def-item must contain one and only one term.</assert>
      
      <assert test="count(def) = 1" role="error" id="def-item-def-test">[def-item-def-test] def-item must contain one and only one def.</assert>
      
    </rule>
  </pattern>
  <pattern id="term-child-tests-pattern">
    <rule context="term/*" id="term-child-tests">
      <let name="allowed-list" value="('italic','monospace','sup','sub')"/>
      
      <assert test="name()=$allowed-list" role="error" id="term-child">[term-child] <name/> is not permitted in term. The only allowed elements are italic, monospace, sup, and sub.</assert>
  ...
  <pattern id="sub-article-contrib-tests-pattern">
    <rule context="sub-article/front-stub/contrib-group/contrib" id="sub-article-contrib-tests">
      ...
      <assert test="role" role="error" flag="dl-ar" id="sub-article-contrib-test-4">[sub-article-contrib-test-4] sub-article contrib must have a child role element.</assert>
  ...
  <pattern id="dec-letter-editor-tests-2-pattern">
    <rule context="sub-article[@article-type='referee-report']/front-stub/contrib-group[1]/contrib[role[@specific-use='editor']]" id="dec-letter-editor-tests-2">
      <let name="name" value="e:get-name(name[1])"/>
      <let name="role" value="role[1]"/>
      <!--<let name="top-role" value="ancestor::article//article-meta/contrib-group[@content-type='peer-review']/contrib[e:get-name(name[1])=$name]/role"/>-->
      <!--<let name="top-name" value="e:get-name(ancestor::article//article-meta/contrib-group[@content-type='peer-review']/contrib[role=$role]/name[1])"/>-->
  ...
  <pattern id="insight-test-pattern">
    <rule context="article[descendant::article-meta/article-categories/subj-group[@subj-group-type='heading']/subject = 'Insight']//article-meta" id="insight-test">
  ...
  <pattern id="expression-of-concern-test-pattern">
    <rule context="article[@article-type='expression-of-concern']//article-meta" id="expression-of-concern-test">
  ...
  <pattern id="elem-citation-data-pattern">
    <rule context="ref/element-citation[@publication-type='data']" id="elem-citation-data">
      ...
      <report test="count(person-group[@person-group-type='author']) gt 1" role="error" id="err-elem-cit-data-3-1">[err-elem-cit-data-3-1] Data references must have one and only one &lt;person-group person-group-type='author'&gt;. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(person-group[@person-group-type='author'])"/>.</report>
      
      
      
      <report test="count(person-group) lt 1" role="error" id="final-err-elem-cit-data-3-2">[final-err-elem-cit-data-3-2] Data references must have one and only one &lt;person-group person-group-type='author'&gt;. Reference '<value-of select="ancestor::ref/@id"/>' has 0.</report>
      
      
      
      <assert test="count(data-title)=1" role="error" id="final-err-elem-cit-data-10">[final-err-elem-cit-data-10] Data reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(source)"/> data-title elements. It must contain one (and only one).</assert>
      
      
      
      <assert test="count(source)=1" role="error" id="final-err-elem-cit-data-11-2">[final-err-elem-cit-data-11-2] Data reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(source)"/> source elements. It must contain one (and only one).</assert>
      ...
      <report test="pub-id and ext-link" role="error" id="elem-cit-data-pub-id-ext-link">[elem-cit-data-pub-id-ext-link] Dataset reference '<value-of select="ancestor::ref/@id"/>' has both &lt;pub-id&gt; &lt;ext-link&gt; elements. There can only be one or the other, not both.</report>
      
      <assert test="count(*) = count(person-group| data-title| source| year| pub-id| version| ext-link)" role="error" id="err-elem-cit-data-18">[err-elem-cit-data-18] The only tags that are allowed as children of &lt;element-citation&gt; with the publication-type="data" are: &lt;person-group&gt;, &lt;data-title&gt;, &lt;source&gt;, &lt;year&gt;, &lt;pub-id&gt;, &lt;ext-link&gt; and &lt;version&gt;. Reference '<value-of select="ancestor::ref/@id"/>' has other elements.</assert>
      
      <assert test="@specific-use=('generated','analyzed')" role="error" id="data-cit-specific-use">[data-cit-specific-use] Every element-citation[@publication-type='data'] must have a specific-use attribute with the value 'generated', or 'analyzed'.</assert>
  ...
  <pattern id="elem-citation-data-pub-id-doi-pattern">
    <rule context="ref/element-citation[@publication-type='data']/pub-id[@pub-id-type='doi']" id="elem-citation-data-pub-id-doi">
      ...
      <assert test="not(@xlink:href)" role="error" id="err-elem-cit-data-14-2">[err-elem-cit-data-14-2] If the pub-id is of pub-id-type doi, it may not have an @xlink:href. Reference '<value-of select="ancestor::ref/@id"/>' has a &lt;pub-id element with type doi and an @link-href with value '<value-of select="@link-href"/>'.</assert>
  ...
  <pattern id="elem-citation-data-pub-id-pattern">
    <rule context="ref/element-citation[@publication-type='data']/pub-id" id="elem-citation-data-pub-id">
      ...
      <assert test="@pub-id-type=('accession','doi')" role="error" id="err-elem-cit-data-13-2">[err-elem-cit-data-13-2] Each pub-id element must have a pub-id-type which is either accession or doi. Reference '<value-of select="ancestor::ref/@id"/>' has a &lt;pub-id element with the type '<value-of select="@pub-id-type"/>'.</assert>
      
      <report test="if (@pub-id-type != 'doi') then not(@xlink:href) else ()" role="error" id="err-elem-cit-data-14-1">[err-elem-cit-data-14-1] If the pub-id is of any pub-id-type except doi, it must have an @xlink:href. Reference '<value-of select="ancestor::ref/@id"/>' has a &lt;pub-id element with type '<value-of select="@pub-id-type"/>' but no @xlink-href.</report>
  ...
  <pattern id="elem-citation-software-article-title-pattern">
    <rule context="element-citation[@publication-type = 'software']/article-title" id="elem-citation-software-article-title">
  ...
  <pattern id="elem-citation-software-pub-id-pattern">
    <rule context="element-citation[@publication-type='software']/pub-id" id="elem-citation-software-pub-id">
      
      <assert test="@pub-id-type='doi'" role="error" id="err-elem-cit-software-pub-id">[err-elem-cit-software-pub-id] If present, the &lt;pub-id&gt; element must contain @pub-id-type='doi'. The &lt;pub-id&gt; element in Reference '<value-of select="ancestor::ref/@id"/>' contains @pub-id-type='<value-of select="@pub-id-type"/>'.</assert>
 ...
 <pattern id="feature-title-tests-pattern">
    <rule context="article-meta[descendant::subj-group[@subj-group-type='heading']/subject = $features-subj]//title-group/article-title" id="feature-title-tests">
  ...
  <pattern id="feature-article-category-tests-pattern">
    <rule context="article-categories[subj-group[@subj-group-type='heading']/subject = $features-subj]" id="feature-article-category-tests">
     ...
     <assert test="$count = 1" role="error" id="feature-article-category-test-1">[feature-article-category-test-1] article categories for <value-of select="subj-group[@subj-group-type='heading']/subject"/>s must contain one, and only one, subj-group[@subj-group-type='sub-display-channel']</assert>
     
   </rule>
  </pattern>
  <pattern id="feature-author-tests-pattern">
    <rule context="article//article-meta[article-categories//subj-group[@subj-group-type='heading']/subject=$features-subj]//contrib[@contrib-type='author']" id="feature-author-tests">
  ...
  <pattern id="feature-template-tests-pattern">
    <rule context="article[descendant::article-meta/article-categories/subj-group[@subj-group-type='heading']/subject = $features-subj]" id="feature-template-tests">
     ...
     <report test="($template = '5') and not(child::sub-article[@article-type='referee-report'])" role="warning" id="feature-template-test-3">[feature-template-test-3] <value-of select="$type"/> is a template <value-of select="$template"/> but it does not (currently) have a decision letter. Is that OK?</report>
     
     <report test="($template = '5') and not(child::sub-article[@article-type='author-comment'])" role="warning" id="feature-template-test-4">[feature-template-test-4] <value-of select="$type"/> is a template <value-of select="$template"/> but it does not (currently) have an author response. Is that OK?</report>
     
     <report test="front/article-meta/contrib-group[@content-type='peer-review'] and ($template != '5')" role="error" id="feature-templates-no-bre">[feature-templates-no-bre] <value-of select="$type"/> is a template <value-of select="$template"/>, which means that it should not have any BREs. This <value-of select="$type"/> has <value-of select="           string-join(           for $x in front/article-meta/contrib-group[@content-type='peer-review']/contrib           return concat(e:get-name($x/name[1]),' as ',$x/role[1])           ,           ' and '           )           "/>. Please remove any senior/reviewing editors.</report>
     
     <report test="($template != '5') and front/article-meta//contrib[@contrib-type='author']/role" role="warning" id="feature-templates-author-cont">[feature-templates-author-cont] <value-of select="$type"/> is a template <value-of select="$template"/>, which means that it should very likely not have any Author contributions. This <value-of select="$type"/> has <value-of select="           string-join(for $x in front/article-meta//contrib[@contrib-type='author']/role           return concat('&quot;', $x,'&quot;')           ,           '; '           )           "/>. Please check with eLife production whether author contributions should be present.</report>
   </rule>
  </pattern>
  ...
  <pattern id="feature-comment-tests-pattern">
    <rule context="article[descendant::article-meta[descendant::subj-group[@subj-group-type='heading']/subject = $features-subj]]//p|      article[descendant::article-meta[descendant::subj-group[@subj-group-type='heading']/subject = $features-subj]]//td|      article[descendant::article-meta[descendant::subj-group[@subj-group-type='heading']/subject = $features-subj]]//th" id="feature-comment-tests">
  ...
  <pattern id="correction-tests-pattern">
    <rule context="article[@article-type = 'correction']" id="correction-tests">
      ...
      <report test="descendant::contrib-group[@content-type='peer-review']" role="error" id="corr-SE-BRE">[corr-SE-BRE] Correction notices must not contain any Senior or Reviewing Editors.</report>
  ...
  <pattern id="retraction-tests-pattern">
    <rule context="article[@article-type = 'retraction']" id="retraction-tests">
      ...
      <report test="descendant::contrib-group[@content-type='peer-review']" role="error" id="retr-SE-BRE">[retr-SE-BRE] Retractions must not contain any Senior or Reviewing Editors.</report>
  ...
  <pattern id="vid-xref-conformance-pattern">
    <rule context="xref[@ref-type='video']" id="vid-xref-conformance">
      ...
      <report test="matches($post-text,'^[\)][A-Za-z0-9]')" role="error" id="vid-xref-test-7">[vid-xref-test-7] citation is followed by a ')' which in turns is immediately followed by a letter or number. Is there a space missing after the ')'?  - '<value-of select="concat(.,$post-text)"/>'.</report>
      ...
      <report test="matches($pre-text,'[Ff]igure [0-9]{1,3}[\s]?[\s—\-][\s]?$')" role="error" id="vid-xref-test-9">[vid-xref-test-9] Incomplete citation. Video citation is preceded by text which suggests it should instead be a link to a figure level video - '<value-of select="concat($pre-text,.)"/>'.</report>
  ...
  <pattern id="fig-xref-conformance-pattern">
    <rule context="xref[@ref-type='fig' and @rid]" id="fig-xref-conformance">
      ...
      <report test="matches(.,'[A-Z]$') and matches($post-text,'^\s?and [A-Z] |^\s?and [A-Z]\.')" role="warning" id="fig-xref-test-17">[fig-xref-test-17] Figure citation - '<value-of select="."/>' - is followed by the text '<value-of select="substring($post-text,1,7)"/>' - should this text be included in the link text too (i.e. '<value-of select="concat(.,substring($post-text,1,6))"/>')?</report>
      
      <report test="matches($post-text,'^\-[A-Za-z0-9]')" role="warning" id="fig-xref-test-18">[fig-xref-test-18] Figure citation - '<value-of select="."/>' - is followed by the text '<value-of select="substring($post-text,1,10)"/>' - should some or all of that text be included in the citation text?</report>
    </rule>
  </pattern>
  <pattern id="table-xref-conformance-pattern">
    <rule context="xref[@ref-type='table']" id="table-xref-conformance">
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/asset-citations#table-xref-test-3" test="matches($post-text,'^[\)][A-Za-z0-9]')" role="error" id="table-xref-test-3">[table-xref-test-3] citation is followed by a ')' which in turns is immediately followed by a letter or number. Is there a space missing after the ')'?  - '<value-of select="concat(.,$post-text)"/>'.</report>
  ...
  <pattern id="supp-file-xref-conformance-pattern">
    <rule context="xref[@ref-type='supplementary-material']" id="supp-file-xref-conformance">
      ...
      <report test="matches($post-text,'^[\)][A-Za-z0-9]')" role="error" id="supp-xref-test-3">[supp-xref-test-3] citation is followed by a ')' which in turns is immediately followed by a letter or number. Is there a space missing after the ')'?  - '<value-of select="concat(.,$post-text)"/>'.</report>
  ...
  <pattern id="def-item-xref-conformance-pattern">
    <rule context="xref[@ref-type='list']" id="def-item-xref-conformance">
      <let name="rid" value="@rid"/>
      <let name="label" value="ancestor::article//def-item[@id = $rid]/term"/>
      
      <assert test="(text()|*) = ($label/text()|$label/*)" role="error" id="def-xref-conformity-1">[def-xref-conformity-1] A link to a definition must contain the same content as the term of that definition. This link has <value-of select="."/>, but the term it points to contains <value-of select="$label"/>.</assert>
  ...
  <pattern id="journal-title-tests-pattern">
    <rule context="element-citation[@publication-type='journal']/source" id="journal-title-tests">
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/references/journal-references#ampersand-check" test="matches(.,'\s?[Aa]mp[;]?\s?') and (. != 'Hippocampus')" role="warning" id="ampersand-check">[ampersand-check] ref '<value-of select="ancestor::ref/@id"/>' appears to contain the text 'amp', is this a broken ampersand?</report>
  ...
  <pattern id="data-availability-statement-pattern">
    <rule context="sec[@sec-type='data-availability']/p[1]" id="data-availability-statement">
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/data-availability#final-das-dryad-conformity" test="matches(.,'[Dd]ryad') and not(xref[@rid = ancestor::back/ref-list/ref[element-citation[@publication-type='data' and contains(source[1],'Dryad')]]/@id])" role="error" id="final-das-dryad-conformity">[final-das-dryad-conformity] Data Availability Statement contains the word Dryad, but there is no data citation in the dataset section with a database name containing 'Dryad'.</report>
  ...
  <pattern id="colour-table-pattern">
    <rule context="th[@style]|td[@style]" id="colour-table">
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/tables#colour-check-table" test="@style=$allowed-values" role="warning" id="colour-check-table">[colour-check-table] <name/> element has colour background. Is this correct? It contains <value-of select="."/>.</report>
  ...
  <pattern id="named-content-pattern">
    <rule context="named-content" id="named-content">
      
      <assert test="@content-type='sequence'" role="error" id="named-content-check">[named-content-check] <name/> element must have the attribute content-type='sequence', but the element containing '<value-of select="."/>' does not.</assert>
  ...
  <pattern id="colour-styled-content-pattern">
    <rule context="styled-content" id="colour-styled-content">
      ...
      <report test="." role="warning" id="styled-content-check">[styled-content-check] <value-of select="."/> has colour formatting. Is this correct? Preceding text - <value-of select="substring(preceding-sibling::text()[1],string-length(preceding-sibling::text()[1])-25)"/>
      </report>
  ...
  <pattern id="math-colour-tests-pattern">
    <rule context="mml:*[@mathcolor]" id="math-colour-tests">
      ...
      <assert test="@mathcolor = $allowed-values" role="error" id="final-mathcolor-test-1">[final-mathcolor-test-1] math (<value-of select="name()"/> element) containing '<value-of select="."/>' has a color style which is not red, blue or purple - '<value-of select="@mathcolor"/>' - which is not allowed. Only 'red', 'blue' and 'purple' are allowed.</assert>
  ...
  <pattern id="xref-formatting-pattern">
    <rule context="xref[not(@ref-type='bibr')]" id="xref-formatting">
      ...
      <report test="not(@ref-type='list') and $child = $formatting-elems" role="warning" id="xref-child-test">[xref-child-test] xref - <value-of select="."/> - has a formatting child element - <value-of select="$child"/> - which is likely not correct.</report>
    </rule>
  </pattern>
  ...
  <pattern id="auth-kwd-style-pattern">
    <rule context="kwd-group[@kwd-group-type='author-keywords']/kwd" id="auth-kwd-style">
      ...
      <report test="not(ancestor::article-meta/article-categories/subj-group[@subj-group-type='heading']/subject[1] = $features-subj) and count(tokenize(.,'\s')) gt 4" role="warning" id="auth-kwd-check-6">[auth-kwd-check-6] Keyword contains more than 4 words - <value-of select="."/>. Should these be split out into separate keywords?</report>
