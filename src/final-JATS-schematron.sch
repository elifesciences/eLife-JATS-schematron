<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:ali="http://www.niso.org/schemas/ali/1.0/" xmlns:file="java.io.File" xmlns:java="http://www.java.com/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" queryBinding="xslt2">
   ...
   <xsl:function name="e:alter-award-id">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$funder-id=('http://dx.doi.org/10.13039/100004412','https://ror.org/02ebx7v45')">
        <!-- HSFP grant DOIs are registered in many ways: RGP0027/2013, LT000376/2014-L, RG00088/1999-M -->
  ...
  <pattern id="covid-prologue-pattern"><rule context="article[front/article-meta//article-title[matches(lower-case(.),'sars-cov-2|covid-19|coronavirus')]]" id="covid-prologue">
      ...
      <assert see="https://elifeproduction.slab.com/posts/article-content-house-style-k617122o#h1vp1-covid-processing-instruction" test="preceding::processing-instruction('covid-19-tdm')" role="warning" id="covid-processing-instruction">[covid-processing-instruction] The article title (<value-of select="front/article-meta//article-title"/>) suggests that this article should probably have the covid processing instruction - '&lt;?covid-19-tdm?&gt;' - but it does not. Should it?</assert>
 ...
 <pattern id="article-tests-pattern"><rule context="article" id="article-tests">
    ...
    <report test="(@article-type = ('article-commentary','discussion','editorial','research-article','review-article')) and count(back) != 1" role="error" id="test-article-back">[test-article-back] Article must have one child back. Currently there are <value-of select="count(back)"/></report>
      ...
      <report test="not(@article-type='research-article') and not(@article-type='review-article' and e:is-prc(.)) and sub-article" role="error" id="non-r-article-sub-article">[non-r-article-sub-article] <value-of select="@article-type"/> type articles cannot have sub-articles (peer review materials).</report>
  ...
  <pattern id="test-article-metadata-pattern"><rule context="article/front/article-meta" id="test-article-metadata">
    ...
    <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#test-auth-kwd-group-presence-1" test="if ($subj-type = $notice-display-types) then ()       else count(kwd-group[@kwd-group-type='author-keywords']) != 1" role="error" id="test-auth-kwd-group-presence-1">[test-auth-kwd-group-presence-1] One author keyword group must be present in article-meta.</report>
    
    <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#test-auth-kwd-group-presence-2" test="if ($subj-type = $notice-display-types) then (count(kwd-group[@kwd-group-type='author-keywords']) != 0)       else ()" role="error" id="test-auth-kwd-group-presence-2">[test-auth-kwd-group-presence-2] <value-of select="$subj-type"/> articles must not have any author keywords</report>
    
    <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#test-ro-kwd-group-presence-1" test="count(kwd-group[@kwd-group-type='research-organism']) gt 1" role="error" id="test-ro-kwd-group-presence-1">[test-ro-kwd-group-presence-1] More than 1 Research organism keyword group is present in article-meta. This is incorrect.</report>
    
    <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#test-ro-kwd-group-presence-2" test="if ($subj-type = ('Research Article', 'Research Advance', 'Replication Study', 'Research Communication'))       then (count(kwd-group[@kwd-group-type='research-organism']) = 0)       else ()" role="warning" id="test-ro-kwd-group-presence-2">[test-ro-kwd-group-presence-2] <value-of select="$subj-type"/> does not contain a Research Organism keyword group. Is this correct?</report>
    ...
    <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#test-contrib-group-presence-1" test="contrib-group" role="error" id="test-contrib-group-presence-1">[test-contrib-group-presence-1] contrib-group (with no attributes containing authors) must be present (as a child of article-meta) for research articles.</assert>
     
     <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#test-contrib-group-presence-2" test="contrib-group[@content-type='section']" role="error" id="test-contrib-group-presence-2">[test-contrib-group-presence-2] contrib-group[@content-type='section'] must be present (as a child of article-meta) for research articles (this is the contrib-group which contains reviewers and editors).</assert>
   ...
   <let name="template" value="parent::article-meta/custom-meta-group/custom-meta[meta-name='Template']/meta-value[1]"/>
    ...
    <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#contrib-presence-test" test="contrib" role="error" id="contrib-presence-test">[contrib-presence-test] contrib-group must contain at least one contrib.</assert>
    ...
    <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#equal-count-test" test="count(contrib[@equal-contrib='yes']) = 1" role="error" id="equal-count-test">[equal-count-test] There is one contrib with the attribute equal-contrib='yes'. This cannot be correct. Either 2 or more contribs within the same contrib-group should have this attribute, or none. Check <value-of select="contrib[@equal-contrib='yes']/name"/></report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#corresp-presence-test" test="contrib[@contrib-type='author' and @corresp='yes']" role="error" id="corresp-presence-test">[corresp-presence-test] There must be at least one corresponding author (a contrib[@contrib-type='author' and @corresp='yes'] in the first contrib-group).</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#non-contrib-contribs" test="$article-type=$non-contribs and descendant::contrib[@contrib-type='author' and role]" role="error" id="non-contrib-contribs">[non-contrib-contribs] <value-of select="$article-type"/> type articles should not contain author contributions.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#editor-conformance-1" test="count(contrib[@contrib-type='senior_editor']) = 1" role="error" id="editor-conformance-1">[editor-conformance-1] contrib-group[@content-type='section'] must contain one (and only 1) Senior Editor (contrib[@contrib-type='senior_editor']).</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#editor-conformance-2" test="count(contrib[@contrib-type='editor']) = 1" role="warning" id="editor-conformance-2">[editor-conformance-2] contrib-group[@content-type='section'] should contain one (and only 1) Reviewing Editor (contrib[@contrib-type='editor']). This one doesn't which is almost definitely incorrect and needs correcting.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#editor-conformance-3" test="(@contrib-type='senior_editor') and ($role!='Senior Editor')" role="error" id="editor-conformance-3">[editor-conformance-3] <value-of select="$name"/> has a @contrib-type='senior_editor' but their role is not 'Senior Editor' (<value-of select="$role"/>), which is incorrect.</report>
      
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#editor-conformance-4" test="(@contrib-type='editor') and ($role!='Reviewing Editor')" role="error" id="editor-conformance-4">[editor-conformance-4] <value-of select="$name"/> has a @contrib-type='editor' but their role is not 'Reviewing Editor' (<value-of select="$role"/>), which is incorrect.</report>

      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#editor-conformance-5" test="count($matching-author-names)=0" role="error" id="editor-conformance-5">[editor-conformance-5] <value-of select="$name"/> is listed both as an author and as a <value-of select="$role"/>, which must be incorrect.</assert>
      
    </rule></pattern><pattern id="auth-cont-tests-pattern"><rule context="article[@article-type=('research-article','review-article') and e:get-version(.)='1']//article-meta//contrib[(@contrib-type='author') and not(collab or collab-wrap) and not(ancestor::collab or ancestor::collab-wrap)]" id="auth-cont-tests">
      
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#auth-cont-test-1" test="child::xref[@ref-type='fn' and matches(@rid,'^con[0-9]{1,3}$')]" role="warning" flag="version-1" id="auth-cont-test-1">[auth-cont-test-1] <value-of select="e:get-name(name[1])"/> has no contributions. Please ensure to query this with the authors.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#dupe-cont-test-1" test="empty($indistinct-conts)" role="error" flag="version-1" id="dupe-cont-test-1">[dupe-cont-test-1] Author <value-of select="if (name) then e:get-name(name[1]) else if (collab or collab-wrap) then e:get-surname(.) else ('with no name')"/> has duplicated contributions which is incorrect. The indistinct contributions are: <value-of select="string-join($indistinct-conts,'; ')"/>.</assert>
    </rule></pattern><pattern id="auth-cont-tests-v2-pattern"><rule context="article[@article-type=('research-article','review-article') and e:get-version(.)!='1']//article-meta//contrib[(@contrib-type='author') and not(collab or collab-wrap) and not(ancestor::collab or ancestor::collab-wrap)]" id="auth-cont-tests-v2">
      
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#auth-cont-test-1-v2" test="role" role="warning" flag="version-2" id="auth-cont-test-1-v2">[auth-cont-test-1-v2] <value-of select="e:get-name(name[1])"/> has no contributions. Please ensure to query this with the authors.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#auth-cont-test-2-v2" test="role and not(role[@vocab='credit'])" role="warning" flag="version-2" id="auth-cont-test-2-v2">[auth-cont-test-2-v2] <value-of select="e:get-name(name[1])"/> has no CRediT contributions. Is that correct?</report>
    </rule></pattern><pattern id="collab-cont-tests-pattern"><rule context="article[@article-type=('research-article','review-article') and e:get-version(.)='1']//article-meta//contrib[(@contrib-type='author') and *[name()=('collab','collab-wrap')]]" id="collab-cont-tests">
      
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#collab-cont-test-1" test="child::xref[@ref-type='fn' and matches(@rid,'^con[0-9]{1,3}$')]" role="warning" flag="version-1" id="collab-cont-test-1">[collab-cont-test-1] <value-of select="e:get-surname(.)"/> has no contributions. Please ensure to query this with the authors.</assert>
    </rule></pattern><pattern id="collab-cont-tests-v2-pattern"><rule context="article[@article-type=('research-article','review-article') and e:get-version(.)!='1']//article-meta//contrib[(@contrib-type='author') and (collab or collab-wrap)]" id="collab-cont-tests-v2">
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#final-collab-cont-test-1-v2" test="role" role="error" flag="version-2" id="final-collab-cont-test-1-v2">[final-collab-cont-test-1-v2] <value-of select="e:get-surname(.)"/> has no contributions. Please ensure to query this with the authors.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#final-collab-cont-test-2-v2" test="role and not(role[@vocab='credit'])" role="warning" flag="version-2" id="final-collab-cont-test-2-v2">[final-collab-cont-test-2-v2] <value-of select="e:get-surname(.)"/> has no CRediT contributions. Is that correct?</report>
      ...
      <report test="not(ancestor::element-citation) and not(ancestor::article-meta)" role="error" id="collab-name-test-3">[collab-name-test-3] <name/> must only be captured as a descendant of ref or article-meta. This one is not.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#author-xref-test-2" test="(@ref-type='fn') and contains(@rid,'equal') and preceding-sibling::xref[not(@ref-type='aff')]" role="error" id="author-xref-test-2">[author-xref-test-2] Equal contribution links from authors must appear after affiliation footnote links. For <value-of select="e:get-name(preceding-sibling::name[1])"/>, their equal contribution link (to <value-of select="idref(@rid)"/>) appears after another non-affiliation link, when it should appear before it.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#author-xref-test-4" test="contains(@rid,'dataset')" role="error" id="author-xref-test-4">[author-xref-test-4] Author footnote links to datasets are not needed. Please remove this - &lt;xref <value-of select="string-join(for $x in self::*/@* return concat($x/name(),'=&quot;',$x,'&quot;'),' ')"/>/&gt;</report>
    </rule></pattern><pattern id="name-tests-pattern"><rule context="contrib-group//name" id="name-tests">
		
    	<assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#surname-test-1" test="count(surname) = 1" role="error" id="surname-test-1">[surname-test-1] Each name must contain only one surname.</assert>
	  
	  <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-1" test="count(given-names) gt 1" role="error" id="given-names-test-1">[given-names-test-1] Each name must contain only one given-names element.</report>
	  
	  <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-2" test="given-names" role="warning" id="given-names-test-2">[given-names-test-2] This name - <value-of select="."/> - does not contain a given-name. Please check with eLife staff that this is correct.</assert>
	  
	</rule></pattern><pattern id="surname-tests-pattern"><rule context="contrib-group//name/surname" id="surname-tests">
		
	  <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#surname-test-2" test="normalize-space(.)=''" role="error" id="surname-test-2">[surname-test-2] surname must not be empty.</report>
		
    <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#surname-test-3" test="descendant::bold or descendant::sub or descendant::sup or descendant::italic or descendant::sc" role="error" id="surname-test-3">[surname-test-3] surname must not contain any formatting (bold, or italic emphasis, or smallcaps, superscript or subscript).</report>
		
	  <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#surname-test-4" test="matches(.,&quot;^[\p{L}\p{M}\s'’-]*$&quot;)" role="error" id="surname-test-4">[surname-test-4] surname should usually only contain letters, spaces, or hyphens. <value-of select="."/> contains other characters.</assert>
		
	  <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#surname-test-5" test="matches(.,'^\p{Ll}') and not(matches(.,'^(de[lrn]?|van|von|el|te[rn]|d[ai]|dos)\s'))" role="warning" id="surname-test-5">[surname-test-5] surname doesn't begin with a capital letter - <value-of select="."/>. Is this correct?</report>
	  
	  <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#surname-test-6" test="matches(.,'^\p{Zs}')" role="error" id="surname-test-6">[surname-test-6] surname starts with a space, which cannot be correct - '<value-of select="."/>'.</report>
	  
	  <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#surname-test-7" test="matches(.,'\p{Zs}$')" role="error" id="surname-test-7">[surname-test-7] surname ends with a space, which cannot be correct - '<value-of select="."/>'.</report>
	    
	    <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#surname-test-8" test="matches(.,'^[A-Z]{1,2}\p{Zs}') and (string-length(.) gt 3)" role="warning" id="surname-test-8">[surname-test-8] surname looks to start with initial - '<value-of select="."/>'. Should '<value-of select="substring-before(.,' ')"/>' be placed in the given-names field?</report>
	    
	    <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#surname-test-9" test="matches(.,'[\(\)\[\]]')" role="warning" id="surname-test-9">[surname-test-9] surname contains brackets - '<value-of select="."/>'. Should the bracketed text be placed in the given-names field instead?</report>
	    
	    <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#surname-test-10" test="matches(.,'\p{Zs}(III?|I?V)$')" role="warning" id="surname-test-10">[surname-test-10] surname ends with what might be roman numerals - '<value-of select="."/>'. Should these be placed in a suffix element instead?</report>
		
	  </rule></pattern><pattern id="given-names-tests-pattern"><rule context="name/given-names" id="given-names-tests">
		
	  <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-3" test="normalize-space(.)=''" role="error" id="given-names-test-3">[given-names-test-3] given-names must not be empty.</report>
		
    	<report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-4" test="descendant::bold or descendant::sub or descendant::sup or descendant::italic or descendant::sc" role="error" id="given-names-test-4">[given-names-test-4] given-names must not contain any formatting (bold, or italic emphasis, or smallcaps, superscript or subscript) - '<value-of select="."/>'.</report>
		
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-5" test="matches(.,&quot;^[\p{L}\p{M}\(\)\s'’-]*$&quot;)" role="error" id="given-names-test-5">[given-names-test-5] given-names should usually only contain letters, spaces, or hyphens. <value-of select="."/> contains other characters.</assert>
		
	  <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-6" test="matches(.,'^\p{Lu}')" role="warning" id="given-names-test-6">[given-names-test-6] given-names doesn't begin with a capital letter - '<value-of select="."/>'. Is this correct?</assert>
	  
    <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-7" test="matches(.,'^[\p{L}]{1}\.$|^[\p{L}]{1}\.\p{Zs}?[\p{L}]{1}\.\p{Zs}?$')" role="error" id="given-names-test-7">[given-names-test-7] given-names contains initialised full stop(s) which is incorrect - <value-of select="."/></report>
	  
    <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-8" test="matches(.,'^\p{Zs}')" role="error" id="given-names-test-8">[given-names-test-8] given-names starts with a space, which cannot be correct - '<value-of select="."/>'.</report>
	  
    <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-9" test="matches(.,'\p{Zs}$')" role="error" id="given-names-test-9">[given-names-test-9] given-names ends with a space, which cannot be correct - '<value-of select="."/>'.</report>
	  
	  <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-10" test="matches(.,'[A-Za-z] [Dd]e[rn]?$')" role="warning" id="given-names-test-10">[given-names-test-10] given-names ends with de, der, or den - should this be captured as the beginning of the surname instead? - '<value-of select="."/>'.</report>
		
	  <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-11" test="matches(.,'[A-Za-z] [Vv]an$')" role="warning" id="given-names-test-11">[given-names-test-11] given-names ends with ' van' - should this be captured as the beginning of the surname instead? - '<value-of select="."/>'.</report>
	  
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-12" test="matches(.,'[A-Za-z] [Vv]on$')" role="warning" id="given-names-test-12">[given-names-test-12] given-names ends with ' von' - should this be captured as the beginning of the surname instead? - '<value-of select="."/>'.</report>
	  
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-13" test="matches(.,'[A-Za-z] [Ee]l$')" role="warning" id="given-names-test-13">[given-names-test-13] given-names ends with ' el' - should this be captured as the beginning of the surname instead? - '<value-of select="."/>'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-14" test="matches(.,'[A-Za-z] [Tt]e[rn]?$')" role="warning" id="given-names-test-14">[given-names-test-14] given-names ends with te, ter, or ten - should this be captured as the beginning of the surname instead? - '<value-of select="."/>'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-15" test="matches(normalize-space(.),'[A-Za-z]\p{Zs}[A-za-z]\p{Zs}[A-za-z]\p{Zs}[A-za-z]|[A-Za-z]\p{Zs}[A-za-z]\p{Zs}[A-za-z]$|^[A-za-z]\p{Zs}[A-za-z]$')" role="info" id="given-names-test-15">[given-names-test-15] given-names contains initials with spaces. Ensure that the space(s) is removed between initials - '<value-of select="."/>'.</report>
		
	</rule></pattern><pattern id="suffix-tests-pattern"><rule context="contrib-group//name/suffix" id="suffix-tests">
      
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#suffix-assert" test=".=('Jr', 'Jnr', 'Sr', 'Snr', 'I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X')" role="error" id="suffix-assert">[suffix-assert] suffix can only have one of these values - 'Jr', 'Jnr', 'Sr', 'Snr', 'I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X'.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#suffix-child-test" test="*" role="error" id="suffix-child-test">[suffix-child-test] suffix cannot have any child elements - <value-of select="*/local-name()"/></report>
      
    </rule></pattern><pattern id="name-child-tests-pattern"><rule context="contrib-group//name/*" id="name-child-tests">
      
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#disallowed-child-assert" test="local-name() = ('surname','given-names','suffix')" role="error" id="disallowed-child-assert">[disallowed-child-assert] <value-of select="local-name()"/> is not allowed as a child of name.</assert>
	  ...
	  <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hjuk3-contrib-test-4" test="($type = 'editor') and (count(xref[@ref-type='aff']) + count(aff) = 0)" role="error" id="contrib-test-4">[contrib-test-4] The <value-of select="role[1]"/> (<value-of select="$name"/>) must have an affiliation. Exeter: If it is not present in the eJP output, please check with eLife production. Production: Please check eJP or ask Editorial for the correct affiliation.</report>
	     ...
	     <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#contrib-test-3" test="name and (collab or collab-wrap)" role="error" id="contrib-test-3">[contrib-test-3] author contains both a child name element and a child collab-wrap (or collab) element. This is not correct.</report>
	  
	     <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#name-test" test="if (collab or collab-wrap) then ()         else count(name) != 1" role="error" id="name-test">[name-test] Contrib contains no collab-wrap but has <value-of select="count(name)"/> name(s). This is not correct.</report>
	  
	     <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#contrib-email-1" test="self::*[@corresp='yes'][not(child::*:email)]" role="error" id="contrib-email-1">[contrib-email-1] Corresponding authors must have an email.</report>
	  
	  <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#contrib-email-2" test="not(@corresp='yes') and (not(ancestor::*[name()=('collab','collab-wrap')]/parent::contrib[@corresp='yes'])) and (child::email)" role="error" id="contrib-email-2">[contrib-email-2] Non-corresponding authors must not have an email.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#credit-role-check-1" test="@vocab-identifier='http://credit.niso.org/'" role="error" id="credit-role-check-1">[credit-role-check-1] A CRediT taxonomy role must have a @vocab-identifier whose value is http://credit.niso.org/.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#credit-role-check-2" test="not(@vocab-term-identifier) or ((count($credit-role) = 1) and ($vocab-term-id != $credit-role/@uri))" role="error" flag="version-2" id="credit-role-check-2">[credit-role-check-2] A CRediT taxonomy role must have a @vocab-term-identifier, the value of which must be the URL of the specific CRediT term. <value-of select="if (empty($credit-role)) then concat('It must be one of these - ',string-join(document($credit-roles)//*:item/@uri,', ')) else concat('In this case ',$credit-role/@uri,' (based on the @vocab-term of this role element)')"/>.</report>
	  ...
	  <let name="text" value="."/>
    	...
    	<assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#orcid-test-1" test="@authenticated='true'" role="error" id="orcid-test-1">[orcid-test-1] contrib-id[@contrib-id-type="orcid"] must have an @authenticated="true"</assert>
		
		<!-- Needs updating to only allow https when this is implemented -->
	  <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#orcid-test-2" test="matches(.,'^http[s]?://orcid.org/[\d]{4}-[\d]{4}-[\d]{4}-[\d]{3}[0-9X]$')" role="error" id="orcid-test-2">[orcid-test-2] contrib-id[@contrib-id-type="orcid"] must contain a valid ORCID URL in the format 'https://orcid.org/0000-0000-0000-0000'</assert>
	  
	  
	  
	  <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#final-orcid-test-3" test="count(ancestor::contrib-group//contrib-id[@contrib-id-type='orcid' and .=$text]) = 1" role="error" id="final-orcid-test-3">[final-orcid-test-3] <value-of select="e:get-name(parent::*/name[1])"/>'s ORCiD is the same as another author's - <value-of select="."/>. Duplicated ORCiDs are not allowed. If it is clear who the ORCiD belongs to, remove the duplicate. If it is not clear please raise a query with production so that they can raise it with the authors.</assert>
    	...
    	<assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#email-test" test="matches(upper-case(.),'^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]+$')" role="error" id="email-test">[email-test] email element must contain a valid email address. Currently it is <value-of select="self::*"/>.</assert>
      ...
      <report test="ancestor::article/@article-type=('article-commentary','editorial',$notice-article-types)" role="error" id="history-exception-1">[history-exception-1] <name/> is not permitted in articles with the article-type <value-of select="ancestor::article/@article-type"/>. Either correct the article type if it is wrong or remove the <name/> element.</report>
	   ...
	   <let name="dtd-version" value="ancestor::article/@dtd-version"/>
    	...
    	<report test="($dtd-version le '1.3') and not(date[@date-type='accepted'])" role="error" id="history-date-test-2">[history-date-test-2] history in JATS version <value-of select="$dtd-version"/> must contain date[@date-type='accepted']</report>
	...
	</rule></pattern><pattern id="prc-history-tests-pattern"><rule context="article[(not(@article-type) or @article-type=('research-article','review-article','discussion','')) and e:is-prc(.)]//article-meta/history" id="prc-history-tests">
      <let name="dtd-version" value="ancestor::article/@dtd-version"/>
      ...
      <report test="($dtd-version le '1.3') and date[@date-type!='sent-for-review' or not(@date-type)]" role="error" id="prc-history-date-test-2">[prc-history-date-test-2] PRC articles can only have sent-for-review dates in their history. This one has a <value-of select="if (date[@date-type!='sent-for-review']) then date[@date-type!='sent-for-review']/@date-type else 'undefined'"/> date.</report>
      
      <report test="($dtd-version ge '1.4')" role="error" id="history-1.4-test-2">[history-1.4-test-2] history in article-meta is deprecated in JATS version <value-of select="$dtd-version"/>. Please move the items to pub-history and remove the history element.</report>
    ...
    <assert test="name()=$allowed-children" role="error" id="pub-date-child-test-1">[pub-date-child-test-1] <value-of select="name()"/> is not allowed as a child of pub-date. The only permitted elements are <value-of select="string-join($allowed-children,'; ')"/>.</assert>
      ...
      <report test="not(e:is-prc(.)) and ($dtd-version le '1.3') and count(event) gt 1" role="error" id="pub-history-child">[pub-history-child] <name/> must have one, and only one, event element in in JATS 1.3 (or lower) non-PRC content. This one has <value-of select="count(event)"/>.</report>
      
      <report test="not(e:is-prc(.)) and ($dtd-version ge '1.4') and count(event) gt 3" role="error" id="pub-history-child-1.4">[pub-history-child-1.4] <name/> cannot have more than 3 event elements in JATS 1.4 (or later) non-PRC content. This one has <value-of select="count(event)"/>.</report>
      
      <report test="not(e:is-prc(.)) and ($dtd-version ge '1.4') and not(event[date[@date-type='received']])" role="error" id="pub-history-recieved">[pub-history-recieved] In legacy model content tagged in JATS 1.4 (or later) <name/> must contain a recieved date (an event containing a date with the date-type 'received'). This one does not.</report>
      
      <report test="not(e:is-prc(.)) and ($dtd-version ge '1.4') and not(event[date[@date-type='accepted']])" role="error" id="pub-history-accepted">[pub-history-accepted] In legacy model content tagged in JATS 1.4 (or later) <name/> must contain an accepted date (an event containing a date with the date-type 'accepted'). This one does not.</report>
      ...
      <report test="e:is-prc(.) and count(event[self-uri[@content-type='preprint']]) != 1" role="error" id="pub-history-events-2">[pub-history-events-2] <name/> must contain one, and only one preprint event (an event with a self-uri[@content-type='preprint'] element). This one has <value-of select="count(event[self-uri[@content-type='preprint']])"/> preprint event elements.</report>
      
      <report test="e:is-prc(.) and count(event[date[@date-type='preprint']]) != 1" role="error" id="pub-history-events-2a">[pub-history-events-2a] <name/> must contain one, and only one preprint event (an event with a date[@date-type='preprint'] element). This one has <value-of select="count(event[date[@date-type='preprint']])"/> preprint event elements.</report>
      ...
      <report test="matches(parent::event/self-uri[1]/@xlink:href,'elifesciences\.org|10.7554/e[lL]ife') and not(matches(.,'^Reviewed preprint v\d$') or .=$template-descriptions)" role="error" id="event-desc-content-2">[event-desc-content-2] <name/> that's a child of an event with an eLife DOI must contain the text 'Reviewed preprint v0' (or in older content one of the folllowing template descriptions: '<value-of select="string-join($template-descriptions,'; ')"/>'). This one has '<value-of select="."/>'.</report>
    ...
    <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h5tdf-aff-test-1" test="parent::contrib-group//contrib//xref/@rid = @id" role="error" id="aff-test-1">[aff-test-1] aff elements that are direct children of contrib-group must have an xref in that contrib-group pointing to them.</assert>
      ...
      <report test="$ror='https://ror.org/05t99sp05' and matches(institution-wrap[1]/institution[1]/lower-case(.),'univ(\.|ersity) of california')" role="error" id="aff-ror-cal-coast">[aff-ror-cal-coast] Affiliation has the ROR id for California Coast University (https://ror.org/05t99sp05), but the institution name contains 'University of California' (or similar) - <value-of select="institution-wrap[1]/institution[1]"/>. The ROR id is incorrect.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#kwd-group-type" test="@kwd-group-type='author-keywords'" role="error" id="kwd-group-type">[kwd-group-type] kwd-group must have a @kwd-group-type 'research-organism', or 'author-keywords'.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#non-ro-kwd-presence-test" test="kwd" role="warning" id="non-ro-kwd-presence-test">[non-ro-kwd-presence-test] kwd-group must contain at least one kwd</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#kwd-group-title" test="title = 'Research organism'" role="error" id="kwd-group-title">[kwd-group-title] kwd-group title is <value-of select="title"/>, which is wrong. It should be 'Research organism'.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#ro-kwd-presence-test" test="kwd" role="warning" id="ro-kwd-presence-test">[ro-kwd-presence-test] kwd-group must contain at least one kwd</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#kwd-upper-case" test="substring(.,1,1) = upper-case(substring(.,1,1))" role="error" id="kwd-upper-case">[kwd-upper-case] research-organism kwd elements should start with an upper-case letter.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#kwd-child-test" test="*[local-name() != 'italic']" role="error" id="kwd-child-test">[kwd-child-test] research-organism keywords cannot have child elements such as <value-of select="*/local-name()"/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#kwd-dupe-test" test="preceding-sibling::kwd = ." role="error" id="kwd-dupe-test">[kwd-dupe-test] research-organism keywords must be distinct. This one containing <value-of select="."/> is not.</report>
  ...
  <pattern id="equal-author-tests-pattern"><rule context="article-meta//contrib[@contrib-type='author']" id="equal-author-tests">
    ...
    <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#equal-author-test-1" test="@equal-contrib='yes' and not(xref[matches(@rid,'^equal-contrib[0-9]$')])" role="error" id="equal-author-test-1">[equal-author-test-1] Equal authors must contain an xref[@ref-type='fn'] with an @rid that starts with 'equal-contrib' and ends in a digit.</report>
    
    <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#equal-author-test-2" test="xref[matches(@rid,'^equal-contrib[0-9]$')] and not(@equal-contrib='yes')" role="error" id="equal-author-test-2">[equal-author-test-2] author contains an xref[@ref-type='fn'] with a 'equal-contrib0' type @rid, but the contrib has no @equal-contrib='yes'.</report>
  ...
  <pattern id="p-tests-pattern"><rule context="p" id="p-tests">
    ...
    <report test="not(ancestor::fig/permissions[contains(.,'phylopic')]) and matches(@xlink:href,'phylopic\.org')" role="warning" id="phylopic-link-check">[phylopic-link-check] This link is to phylopic.org, which is a site where silhouettes/images are typically reproduced from. Please check whether any figures contain reproduced images from this site, and if so whether permissions have been obtained and/or copyright statements are correctly included.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-group-child-test-2" test="(local-name() = 'media') and not(contains(@mimetype,'video'))" role="error" id="fig-group-child-test-2">[fig-group-child-test-2] <name/> which is a child of fig-group, must have an @mimetype containing 'video' - i.e. only video type media is allowed as a child of fig-group.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#graphic-test-1" test="contains($mime-subtype,'tiff') and not(matches($file,'\.tif$|\.tiff$'))" role="error" id="graphic-test-1">[graphic-test-1] <name/> has tif mime-subtype but filename does not end with '.tif' or '.tiff'. This cannot be correct.</report>
      
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#graphic-test-2" test="contains($mime-subtype,'postscript') and not(ends-with($file,'.eps'))" role="error" id="graphic-test-2">[graphic-test-2] <name/> has postscript mime-subtype but filename does not end with '.eps'. This cannot be correct.</report>
      
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#graphic-test-3" test="contains($mime-subtype,'jpeg') and not(matches($file,'\.jpg$|\.jpeg$'))" role="error" id="graphic-test-3">[graphic-test-3] <name/> has jpeg mime-subtype but filename does not end with '.jpg' or '.jpeg'. This cannot be correct.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#graphic-test-4" test="$dtd-version le '1.3' and not(@mimetype=('image','application'))" role="error" id="graphic-test-4">[graphic-test-4] <name/> must have a @mimetype='image' or 'application'.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#graphic-test-5" test="matches(@xlink:href,'\.[\p{L}\p{N}]{1,6}$')" role="error" id="graphic-test-5">[graphic-test-5] <name/> must have an @xlink:href which contains a file reference.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#graphic-test-6" test="preceding::graphic/@xlink:href = $link" role="error" id="graphic-test-6">[graphic-test-6] Image file for <value-of select="if (name()='inline-graphic') then 'inline-graphic' else replace(parent::fig/label,'\.','')"/> (<value-of select="$link"/>) is the same as the one used for <value-of select="replace(preceding::graphic[@xlink:href=$link][1]/parent::fig/label,'\.','')"/>.</report>
      ...
      <report test="($dtd-version ge '1.4') and @mime-subtype" role="error" id="media-test-2a">[media-test-2a] media has a mime-subtype attribute, but this is depcreated in JATS v<value-of select="$dtd-version"/>. Add it to the existing mimetype, i.e. <value-of select="concat(@mimetype,'/',@mime-subtype)"/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-video-title" test="not(ancestor::sub-article) and not(caption/title)" role="error" id="final-video-title">[final-video-title] <value-of select="replace(label,'\.$,','')"/> does not have a title, which is incorrect.</report>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-video-title-sa" test="ancestor::sub-article and not(caption/title)" role="warning" id="final-video-title-sa">[final-video-title-sa] <value-of select="replace(label,'\.$,','')"/> does not have a title, which is incorrect.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#inline-formula-test-2" test="not($pre-text/following-sibling::*[1]/local-name()='disp-formula') and not(parent::*[name()=('td','th')] and preceding-sibling::node()[1]/name()='break') and matches($pre-text,'[\p{L}\p{N}\p{M}]$')" role="warning" id="inline-formula-test-2">[inline-formula-test-2] There is no space between inline-formula and the preceding text - <value-of select="concat(substring($pre-text,string-length($pre-text)-15),descendant::mml:math[1])"/> - Is this correct?</report>
      
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#inline-formula-test-3" test="not($post-text/preceding-sibling::*[1]/local-name()='disp-formula') and not(parent::*[name()=('td','th')] and following-sibling::node()[1]/name()='break') and matches($post-text,'^[\p{L}\p{N}\p{M}]')" role="warning" id="inline-formula-test-3">[inline-formula-test-3] There is no space between inline-formula and the following text - <value-of select="concat(descendant::mml:math[1],substring($post-text,1,15))"/> - Is this correct?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#math-test-19" test="descendant::mml:mi[(.='') and preceding-sibling::*[1][(local-name() = 'mi') and matches(.,'[A-Za-z]')] and following-sibling::*[1][(local-name() = 'mi') and matches(.,'[A-Za-z]')]]" role="warning" id="math-test-19">[math-test-19] Maths containing '<value-of select="."/>' has what looks like words or terms which need separating with a space. With it's current markup the space will not be preserved on the eLife website. Please add in the space(s) using the latex '\;' in the appropriate place(s), so that the space is preserved in the HTML.</report>
      
      <report test="matches(.,'\p{Zs}\p{Zs}\p{Zs}\p{Zs}+$') and not(matches(.,'\s\s\s\s+$'))" role="error" id="math-test-20">[math-test-20] <value-of select="ancestor::*[name()=('inline-formula','disp-formula')][1]/name()"/> ends with 4 or more spaces. These types of spaces may cause the equation to break over numerous lines in the HTML or shift the equation to the left. Please ensure they are removed.</report>
      
      <report test="matches(.,'^\p{Zs}\p{Zs}\p{Zs}\p{Zs}+') and not(matches(.,'^\s\s\s\s+'))" role="error" id="math-test-21">[math-test-21] <value-of select="ancestor::*[name()=('inline-formula','disp-formula')][1]/name()"/> starts with 4 or more spaces. These types of spaces may cause the equation to break over numerous lines in the HTML or shift the equation to the right. Please ensure they are removed.</report>
      ...
      <report test="descendant::mml:mstyle[@displaystyle='true'] and ancestor::inline-formula and not(ancestor::sub-article) and not(ancestor::table-wrap) and (descendant::*[name()=('mml:mfrac','mml:mroot','mml:msqrt')] or descendant::mml:mo[matches(.,'[∑∫∏⋃⋂\(\){}\[\]\|]')])" role="warning" id="math-test-22">[math-test-22] Inline formula has display style. Is that correct? (inline-formula has a descendant mml:mstyle with the attribute displaystyle="true").</report>
      
      <report test="@display='block' and ancestor::inline-formula" role="error" id="math-block-inline">[math-block-inline] <name/> is a descendant of an inline-formula but it has the attribute display="block". This is incorrect. Either the inline-formula is really a disp-formula, or the display="block" attribute should be removed (or changed to "inline").</report>
      ...
      <report test="exists($arrays-without-spaces)" role="warning" id="tex-math-test-8">[tex-math-test-8] <name/> contains array(s) without horizontal or vertical spacing - <value-of select="string-join($arrays-without-spaces,' ---- ')"/></report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-video-cite" test="$xrefs//*:match" role="warning" id="final-video-cite">[final-video-cite] There is no citation to <value-of select="$label"/>. Ensure this is added.</assert>
      
      
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-video-placement-1" test="($xrefs//*:match) and ($sec-id != $sec1/@id)" role="error" id="final-video-placement-1">[final-video-placement-1] <value-of select="$label"/> does not appear in the same section as where it is first cited (sec with title '<value-of select="$sec1/title"/>'), which is incorrect.</report>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-placement-2" test="($xref-sib = 'p') and ($xref1//following::media/@id = $id)" role="warning" id="video-placement-2">[video-placement-2] <value-of select="$label"/> appears after its first citation but not directly after its first citation. Is this correct?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#label-fig-group-conformance-2" test="not(ancestor::fig-group) and parent::media and matches(.,'[Ff]igure')" role="error" id="label-fig-group-conformance-2">[label-fig-group-conformance-2] <value-of select="$label"/> contains the string 'Figure' but it's not placed in a &lt;fig-group&gt; element, which is incorrect. Either the label needs updating, or it needs moving into the &lt;fig-group&gt;.</report>
      
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#distinct-label-conformance" test="some $x in preceding::label satisfies (replace($x,'\p{P}','') = $label-2)" role="error" id="distinct-label-conformance">[distinct-label-conformance] Duplicated labels - <value-of select="$label"/> is present more than once in the text.</report>
  ...
  <pattern id="body-video-specific-pattern"><rule context="article[not(@article-type = $notice-article-types)]/body//media[starts-with(@mimetype,'video')]" id="body-video-specific">
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-body-video-position-test-1" test="not(ancestor::fig-group) and (matches(label[1],'[Vv]ideo')) and ($no != string($pos))" role="error" id="final-body-video-position-test-1">[final-body-video-position-test-1] <value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other videos it is placed in position <value-of select="$pos"/>.</report>
      
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#fig-video-label-test" test="starts-with(label[1],$fig-label)" role="error" id="fig-video-label-test">[fig-video-label-test] <value-of select="label"/> does not begin with its parent figure label - <value-of select="$fig-label"/> - which is incorrect.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#fig-video-position-test" test="(ancestor::fig-group) and ($no != string($fig-pos))" role="error" id="fig-video-position-test">[fig-video-position-test] <value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other fig-level videos it is placed in position <value-of select="$fig-pos"/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#fig-video-check-1" test="(not(ancestor::fig-group)) and (descendant::xref[@ref-type='fig'][contains(.,'igure') and not(contains(.,'supplement'))])" role="warning" id="fig-video-check-1">[fig-video-check-1] <value-of select="label"/> contains a link to <value-of select="descendant::xref[@ref-type='fig'][contains(.,'igure') and not(contains(.,'supplement'))][1]"/>, but it is not a captured as a child of that fig. Should it be captured as <value-of select="concat(descendant::xref[@ref-type='fig'][contains(.,'igure') and not(contains(.,'supplement'))][1],'—video x')"/> instead?</report>
      
      <report test="$is-explainer and not(caption/p[matches(lower-case(.),'explainer videos are not peer reviewed')])" role="error" id="explainer-video-check-1">[explainer-video-check-1] <value-of select="label"/> is an author explainer video, but the caption does not include the text 'Explainer videos are not peer reviewed'.</report>
      
      <report test="$is-explainer and not(caption//supplementary-material[contains(label[1],'data')])" role="warning" id="explainer-video-check-2">[explainer-video-check-2] <value-of select="label"/> is an author explainer video, but it does not have any source data. All author explainer videos should be accompanied by a source data file with a transcript.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-app-video-position-test" test="$no = string($pos)" role="error" id="final-app-video-position-test">[final-app-video-position-test] <value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other appendix videos it is placed in position <value-of select="$pos"/>.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-dl-video-position-test" test="$no = string($pos)" role="error" id="final-dl-video-position-test">[final-dl-video-position-test] <value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other DL videos it is placed in position <value-of select="$pos"/>.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-ar-video-position-test" test="$no = string($pos)" role="error" id="final-ar-video-position-test">[final-ar-video-position-test] <value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other AR videos it is placed in position <value-of select="$pos"/>.</assert>
  ...
  <pattern id="fig-specific-tests-pattern"><rule context="article/body//fig[not(@specific-use='child-fig')][not(ancestor::boxed-text)]" id="fig-specific-tests">
      ...
      <report test="replace(.,'^https?|/$','')=replace($article-license,'^https?|/$','')" role="error" id="block-permish-ali-license">[block-permish-ali-license] ali:license_ref in permissions for <value-of select="$label"/> is the same as the license link for the article - <value-of select="."/> - which is incorrect.</report>
      
      <assert test="contains(.,'creativecommons.org')" role="error" id="block-permish-ali-license-2">[block-permish-ali-license-2] ali:license_ref should only contain creativecommons.org licenses. This one in permissions for <value-of select="$label"/> is not a creative commons link - <value-of select="."/> - which is incorrect.</assert>
  ...
  <pattern id="article-title-tests-pattern"><rule context="article-meta//article-title" id="article-title-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-1" test="matches(.,'^\([A-Za-z]|^[A-Za-z]\)')" role="warning" id="video-title-test-1">[video-title-test-1] '<value-of select="$label"/>' appears to have a title which is the beginning of a caption. Is this correct?</report>
      
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-2" test="matches(.,'\.$|\?$')" role="error" id="video-title-test-2">[video-title-test-2] title for <value-of select="$label"/> must end with a full stop.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-3" test="matches(.,' vs\.$')" role="warning" id="video-title-test-3">[video-title-test-3] title for <value-of select="$label"/> ends with 'vs.', which indicates that the title sentence may be split across title and caption.</report>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-4" test="matches(.,'^\p{Zs}')" role="error" id="video-title-test-4">[video-title-test-4] title for <value-of select="$label"/> begins with a space, which is not allowed.</report>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-7" test="string-length(.) gt 250" role="warning" id="video-title-test-7">[video-title-test-7] title for <value-of select="$label"/> is longer than 250 characters. Is it a caption instead?</report>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-8" test="$sentence-count gt 1" role="warning" id="video-title-test-8">[video-title-test-8] title for <value-of select="$label"/> contains <value-of select="$sentence-count"/> sentences. Should the sentence(s) after the first be moved into the caption? Or is the title itself a caption (in which case, please ask the authors for a title)?</report>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-9" test="matches(.,'\p{Zs}$')" role="error" id="video-title-test-9">[video-title-test-9] The title for <value-of select="$label"/> ends with space(s) which is incorrect - '<value-of select="."/>'.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#comp-int-title-test" test="title = 'Competing interests'" role="error" id="comp-int-title-test">[comp-int-title-test] fn-group[@content-type='competing-interests'] must have a title that contains 'Competing interests'. Currently it is '<value-of select="title"/>'.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#auth-cont-title-test" test="title = 'Author contributions'" role="error" id="auth-cont-title-test">[auth-cont-title-test] fn-group[@content-type='author-contribution'] must have a title that contains 'Author contributions'. Currently it is '<value-of select="title"/>'.</assert>
  ...
  <pattern id="award-group-ids-pattern"><rule context="funding-group/award-group" id="award-group-ids">
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#app-video-sup-id-test-1" test="matches(@id,'^app[0-9]{1,3}fig[0-9]{1,3}video[0-9]{1,3}$')" role="error" id="app-video-sup-id-test-1">[app-video-sup-id-test-1] video supplement must have an @id in the format app0fig0video0.  <value-of select="@id"/> does not conform to this.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#box-vid-id-2" test="starts-with(@id,$box-id)" role="error" id="box-vid-id-2">[box-vid-id-2] video id does not start with its ancestor boxed-text id. Please ensure the first part of the id contains '<value-of select="$box-id"/>'.</assert>
  ...
  <pattern id="back-tests-pattern"><rule context="back" id="back-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#additional-info-test-2" test="if ($author-count = 0) then ()         else not(fn-group[@content-type='competing-interest'])" role="error" id="additional-info-test-2">[additional-info-test-2] This type of sec must have a child fn-group[@content-type='competing-interest'].</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#additional-info-test-4" test="$article-type=$non-contribs and fn-group[@content-type='author-contribution']" role="error" id="additional-info-test-4">[additional-info-test-4] <value-of select="$article-type"/> type articles should not contain author contributions.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#comp-int-fn-test-1" test="count(fn) gt 0" role="error" id="comp-int-fn-test-1">[comp-int-fn-test-1] At least one child fn element should be present in fn-group[@content-type='competing-interest'].</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#comp-int-fn-group-test-1" test="ancestor::back" role="error" id="comp-int-fn-group-test-1">[comp-int-fn-group-test-1] This fn-group must be a descendant of back.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#comp-int-fn-test-2" test="@fn-type='COI-statement'" role="error" id="comp-int-fn-test-2">[comp-int-fn-test-2] fn element must have an @fn-type='COI-statement' as it is a child of fn-group[@content-type='competing-interest'].</assert>
      
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#comp-int-fn-test-3" test="contains(lower-case(.),'the other authors')" role="error" id="comp-int-fn-test-3">[comp-int-fn-test-3] Competing interests footnote contains information about other authors - '<value-of select="."/>'. These footnotes should only contain information about that specific author.</report>
      
      <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#comp-int-fn-test-4" test="matches(.,'\.\p{Zs}*$')" role="error" id="comp-int-fn-test-4">[comp-int-fn-test-4] Competing interests footnote ends with full stop - <value-of select="."/> - Please remove the full stop.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#auth-cont-fn-test-1" test="@fn-type='con'" role="error" id="auth-cont-fn-test-1">[auth-cont-fn-test-1] This fn must have an @fn-type='con'.</assert>
  ...
  <pattern id="ref-report-front-pattern"><rule context="sub-article[@article-type='referee-report']/front-stub" id="ref-report-front">
      ...
      <assert test="name or anonymous or collab or collab-wrap" role="error" flag="dl-ar" id="sub-article-contrib-test-2">[sub-article-contrib-test-2] sub-article contrib must have either a child name or a child anonymous element.</assert>
      
      <report test="(name and anonymous) or ((collab or collab-wrap) and anonymous) or (name and (collab or collab-wrap))" role="error" flag="dl-ar" id="sub-article-contrib-test-3">[sub-article-contrib-test-3] sub-article contrib can only have a child name element or a child anonymous element or a child collab element (with descendant group members as required), it cannot have more than one of these elements. This has <value-of select="string-join(for $x in *[name()=('name','anonymous','collab','collab-wrap')] return concat('a ',$x/name()),' and ')"/>.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#prc-reviewer-test-1" test="name or collab or collab-wrap" role="error" id="prc-reviewer-test-1">[prc-reviewer-test-1] A reviewer contrib in a PRC article cannot have a child <value-of select="*[name()=('name','collab','collab-wrap')]/name()"/> element, since all reviewers are captured as anonymous. They must have an anonymous element instead.</report>
  ...
  <pattern id="elem-citation-general-pattern"><rule context="element-citation" id="elem-citation-general">
    ...
    </rule></pattern><pattern id="elem-citation-gen-name-3-2-pattern"><rule context="element-citation/person-group/collab | element-citation/person-group/collab-name" id="elem-citation-gen-name-3-2">
      ...
      <assert see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#err-elem-cit-gen-date-1-6" test="not(concat($YYYY, 'a')=.) or (concat($YYYY, 'a')=. and         (some $y in //element-citation/descendant::year         satisfies (normalize-space($y) = concat($YYYY,'b'))         and (ancestor::element-citation/person-group[1]/name[1]/surname/replace(.,'[-‐–—]','-') = $y/ancestor::element-citation/person-group[1]/name[1]/surname/replace(.,'[-‐–—]','-')         or ancestor::element-citation/person-group[1]/collab[1] = $y/ancestor::element-citation/person-group[1]/collab[1]         or ancestor::element-citation/person-group[1]/collab-name[1] = $y/ancestor::element-citation/person-group[1]/collab-name[1]         )))" role="error" id="err-elem-cit-gen-date-1-6">[err-elem-cit-gen-date-1-6] If the &lt;year&gt; element contains the letter 'a' after the digits, there must be another reference with the same first author surname (or collab-name) with a letter "b" after the year. Reference '<value-of select="ancestor::ref/@id"/>' does not fulfill this requirement.</assert>
    ...
    </rule></pattern><pattern id="collab-content-pattern"><rule context="ref/element-citation//collab | ref/element-citation//collab-name" id="collab-content">
      
      <report test="matches(.,'[\[\]\(\)]')" role="warning" id="collab-brackets">[collab-brackets] <name/> in reference '<value-of select="ancestor::ref/@id"/>' contains brackets - <value-of select="."/>. Are the brackets necessary?</report>
  ...
  <pattern id="ref-list-ordering-pattern"><rule context="ref[preceding-sibling::ref]" id="ref-list-ordering">
      ...
      <assert see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#err-elem-cit-high-2-2" test="($order-value gt $preceding-ref-order-value)" role="error" id="err-elem-cit-high-2-2">[err-elem-cit-high-2-2] The order of &lt;element-citation&gt;s in the reference list should be name and date, arranged alphabetically by the first author’s surname, or by the value of the first &lt;collab&gt; element. In the case of two authors, the sequence should be arranged by both authors' surnames, then date. For three or more authors, the sequence should be the first author's surname, then date. Reference '<value-of select="@id"/>' appears to be in a different order.</assert>
  ...
  <pattern id="elem-citation-patent-pattern"><rule context="element-citation[@publication-type='patent']" id="elem-citation-patent">
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-2-3" test="every $type in person-group/@person-group-type         satisfies $type = ('assignee','inventor')" role="error" id="err-elem-cit-patent-2-3">[err-elem-cit-patent-2-3] The only allowed types of person-group elements are "assignee" and "inventor". Reference '<value-of select="ancestor::ref/@id"/>' has &lt;person-group&gt; elements of other types.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-2A" test="count(person-group[@person-group-type='assignee']) le 1" role="error" id="err-elem-cit-patent-2A">[err-elem-cit-patent-2A] There may be zero or one person-group elements with @person-group-type="assignee". Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(person-group[@person-group-type='assignee'])"/> &lt;person-group&gt; elements of type 'assignee'.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#final-err-elem-cit-patent-9-1" test="count(source)=1" role="error" id="final-err-elem-cit-patent-9-1">[final-err-elem-cit-patent-9-1] Each &lt;element-citation&gt; of type 'patent' must contain one and only one &lt;source&gt; elements. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(source)"/> &lt;source&gt; elements.</assert>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#final-err-elem-cit-patent-10-1-1" test="patent" role="error" id="final-err-elem-cit-patent-10-1-1">[final-err-elem-cit-patent-10-1-1] The  &lt;patent&gt; element is required. Reference '<value-of select="ancestor::ref/@id"/>' has no &lt;patent&gt; elements.</assert>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#final-err-elem-cit-patent-11-1" test="ext-link" role="error" id="final-err-elem-cit-patent-11-1">[final-err-elem-cit-patent-11-1] The &lt;ext-link&gt; element is required in a patent reference. Reference '<value-of select="ancestor::ref/@id"/>' has no &lt;ext-link&gt; elements.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-18" test="count(*) = count(person-group| article-title| source| year| patent| ext-link)" role="error" id="err-elem-cit-patent-18">[err-elem-cit-patent-18] The only tags that are allowed as children of &lt;element-citation&gt; with the publication-type="patent" are: &lt;person-group&gt;, &lt;article-title&gt;, &lt;source&gt;, &lt;year&gt;, &lt;patent&gt;, and &lt;ext-link&gt;. Reference '<value-of select="ancestor::ref/@id"/>' has other elements.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-8-2-1" test="./string-length() + sum(*/string-length()) ge 2" role="error" id="err-elem-cit-patent-8-2-1">[err-elem-cit-patent-8-2-1] A  &lt;article-title&gt; element within a &lt;element-citation&gt; of type 'patent' must contain at least two characters. Reference '<value-of select="ancestor::ref/@id"/>' has too few characters.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-8-2-2" test="count(*)=count(italic | sub | sup)" role="error" id="err-elem-cit-patent-8-2-2">[err-elem-cit-patent-8-2-2] A &lt;article-title&gt; element within a &lt;element-citation&gt; of type 'patent' may only contain the child elements &lt;italic&gt;, &lt;sub&gt;, and &lt;sup&gt;. No other elements are allowed. Reference '<value-of select="ancestor::ref/@id"/>' has disallowed child elements.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-9-2-2" test="count(*)=count(italic | sub | sup)" role="error" id="err-elem-cit-patent-9-2-2">[err-elem-cit-patent-9-2-2] A &lt;source&gt; element within a &lt;element-citation&gt; of type 'patent' may only contain the child elements &lt;italic&gt;, &lt;sub&gt;, and &lt;sup&gt;. No other elements are allowed. Reference '<value-of select="ancestor::ref/@id"/>' has disallowed child elements.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-10-1-2" test="count(*)=0" role="error" id="err-elem-cit-patent-10-1-2">[err-elem-cit-patent-10-1-2] The &lt;patent&gt; element may not have child elements. Reference '<value-of select="ancestor::ref/@id"/>' has child elements.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-10-2" test="some $x in document($countries)/countries/country/@country satisfies ($x=@country)" role="error" id="err-elem-cit-patent-10-2">[err-elem-cit-patent-10-2] The &lt;patent&gt; element must have a country attribute, the value of which must be a 2 letter ISO 3166-1 country code. Reference '<value-of select="ancestor::ref/@id"/>' has a patent/@country attribute with the value '<value-of select="@country"/>', which is not in the list.</assert>
  ...
  <pattern id="elem-citation-web-pattern"><rule context="element-citation[@publication-type='web']" id="elem-citation-web">
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-2-1" test="count(person-group)=1" role="error" id="err-elem-cit-web-2-1">[err-elem-cit-web-2-1] There must be one and only one person-group. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(person-group)"/> &lt;person-group&gt; elements.</assert>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#final-err-elem-cit-web-8-1" test="count(article-title)=1" role="error" id="final-err-elem-cit-web-8-1">[final-err-elem-cit-web-8-1] Each  &lt;element-citation&gt; of type 'web' must contain one and only one &lt;article-title&gt; element. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(article-title)"/> &lt;article-title&gt; elements.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-9-1" test="count(source) &gt; 1" role="error" id="err-elem-cit-web-9-1">[err-elem-cit-web-9-1] Each  &lt;element-citation&gt; of type 'web' may contain one and only one &lt;source&gt; element. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(source)"/> &lt;source&gt; elements.</report>
      
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-10-1" test="count(ext-link)=1" role="error" id="err-elem-cit-web-10-1">[err-elem-cit-web-10-1] One and only one &lt;ext-link&gt; element is required. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(ext-link)"/> &lt;ext-link&gt; elements.</assert>
      
      
      
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#final-err-elem-cit-web-11-1" test="count(date-in-citation) lt 1" role="error" id="final-err-elem-cit-web-11-1">[final-err-elem-cit-web-11-1] Web Reference '<value-of select="ancestor::ref/@id"/>' has no accessed date (&lt;date-in-citation&gt; element) which is required.</report>
      
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-11-1-1" test="count(date-in-citation) gt 1" role="error" id="err-elem-cit-web-11-1-1">[err-elem-cit-web-11-1-1] Web Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(date-in-citation)"/> &lt;date-in-citation&gt; elements. One and only one &lt;date-in-citation&gt; element is required.</report>
      
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-12" test="count(*) = count(person-group|article-title|source|year|ext-link|date-in-citation)" role="error" id="err-elem-cit-web-12">[err-elem-cit-web-12] The only tags that are allowed as children of &lt;element-citation&gt; with the publication-type="web" are: &lt;person-group&gt;, &lt;article-title&gt;, &lt;source&gt;, &lt;year&gt;, &lt;ext-link&gt; and &lt;date-in-citation&gt;. Reference '<value-of select="ancestor::ref/@id"/>' has other elements.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-2-2" test="@person-group-type='author'" role="error" id="err-elem-cit-web-2-2">[err-elem-cit-web-2-2] The &lt;person-group&gt; element must contain @person-group-type='author'. The &lt;person-group&gt; element in Reference '<value-of select="ancestor::ref/@id"/>' contains @person-group-type='<value-of select="@person-group-type"/>'.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-8-2-1" test="./string-length() + sum(*/string-length()) ge 2" role="error" id="err-elem-cit-web-8-2-1">[err-elem-cit-web-8-2-1] A  &lt;article-title&gt; element within a &lt;element-citation&gt; of type 'web' must contain 
        at least two characters. Reference '<value-of select="ancestor::ref/@id"/>' has too few characters.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-8-2-2" test="count(*)=count(italic | sub | sup)" role="error" id="err-elem-cit-web-8-2-2">[err-elem-cit-web-8-2-2] A  &lt;article-title&gt; element within a &lt;element-citation&gt; of type 'web' may only contain the child elements &lt;italic&gt;, &lt;sub&gt;, and &lt;sup&gt;. No other elements are allowed. Reference '<value-of select="ancestor::ref/@id"/>' has disallowed child elements.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-9-2-2" test="count(*)=count(italic | sub | sup)" role="error" id="err-elem-cit-web-9-2-2">[err-elem-cit-web-9-2-2] A  &lt;source&gt; element within a &lt;element-citation&gt; of type 'web' may only contain the child elements &lt;italic&gt;, &lt;sub&gt;, and &lt;sup&gt;. No other elements are allowed. Reference '<value-of select="ancestor::ref/@id"/>' has disallowed child elements.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-11-2-1" test="./@iso-8601-date" role="error" id="err-elem-cit-web-11-2-1">[err-elem-cit-web-11-2-1]
        The &lt;date-in-citation&gt; element must have an @iso-8601-date attribute.
        Reference '<value-of select="ancestor::ref/@id"/>' does not.
      </assert>
      
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-11-2-2" test="matches(./@iso-8601-date,'^\d{4}-\d{2}-\d{2}$')" role="error" id="err-elem-cit-web-11-2-2">[err-elem-cit-web-11-2-2]
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-11-3" test="matches(normalize-space(.),'^(January|February|March|April|May|June|July|August|September|October|November|December) \d{1,2}, \d{4}')" role="error" id="err-elem-cit-web-11-3">[err-elem-cit-web-11-3]
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-11-5" test="(matches(@iso-8601-date,replace($date-regex,'\p{Zs}','')))" role="error" id="err-elem-cit-web-11-5">[err-elem-cit-web-11-5] 
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-11-4" test="if (matches(@iso-8601-date,replace($date-regex,'\p{Zs}',''))) then format-date(xs:date(@iso-8601-date), '[MNn] [D], [Y]')!=.         else ()" role="error" id="err-elem-cit-web-11-4">[err-elem-cit-web-11-4] 
  ...
  <pattern id="elem-citation-thesis-pattern"><rule context="element-citation[@publication-type='thesis']" id="elem-citation-thesis"> 
      ...
      <assert see="https://elifeproduction.slab.com/posts/thesis-references-27x10ocq#err-elem-cit-thesis-11-1" test="count(pub-id) le 1" role="error" id="err-elem-cit-thesis-11-1">[err-elem-cit-thesis-11-1]
        A maximum of one &lt;pub-id&gt; element is allowed.
        Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(pub-id)"/>
        &lt;pub-id&gt; elements.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/thesis-references-27x10ocq#err-elem-cit-thesis-2-2" test="@person-group-type='author'" role="error" id="err-elem-cit-thesis-2-2">[err-elem-cit-thesis-2-2]
        Each &lt;person-group&gt; must have a @person-group-type attribute of type 'author'.
        Reference '<value-of select="ancestor::ref/@id"/>' has a &lt;person-group&gt; 
        element with @person-group-type attribute '<value-of select="@person-group-type"/>'.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/thesis-references-27x10ocq#err-elem-cit-thesis-8-2" test="count(*) = count(sub|sup|italic)" role="error" id="err-elem-cit-thesis-8-2">[err-elem-cit-thesis-8-2]
        An &lt;article-title&gt; element in a reference may contain characters and &lt;italic&gt;, &lt;sub&gt;, and &lt;sup&gt;. 
        No other elements are allowed.
        Reference '<value-of select="ancestor::ref/@id"/>' does not meet this requirement.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/thesis-references-27x10ocq#err-elem-cit-thesis-10-2" test="count(*)=0" role="error" id="err-elem-cit-thesis-10-2">[err-elem-cit-thesis-10-2]
 ...
 <pattern id="feature-title-tests-pattern"><rule context="article-meta[descendant::subj-group[@subj-group-type='display-channel']/subject = $features-subj]//title-group/article-title" id="feature-title-tests">
     ...
     <assert see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-author-test-1" test="bio or collab or ancestor::collab or collab-wrap or ancestor::collab-wrap" role="error" id="feature-author-test-1">[feature-author-test-1] Author must contain child bio in feature content.</assert>
     ...
     <report test="@related-article-type='commentary-article' and not(ancestor::article//ref-list/ref//pub-id[@pub-id-type]/lower-case(.) = lower-case($doi))" role="error" id="insight-related-article-test-2">[insight-related-article-test-2] This Insight is related to <value-of select="$doi"/>, but there is no reference in the ref-list with that doi.</report>
     
     <report test="$related-ref-year!='' and not($current-year = $related-ref-year)" role="warning" id="insight-related-article-test-3">[insight-related-article-test-3] This Insight is related to <value-of select="$doi"/>, but the reference in the ref-list with that doi has a year (<value-of select="$related-ref-year"/>) which is not the same as the current year (<value-of select="$current-year"/>). Is that correct?</report>
  ...
  <pattern id="unlinked-object-cite-pattern"><rule context="fig[not(ancestor::sub-article) and label]|       table-wrap[not(ancestor::sub-article) and label[not(contains(.,'ey resources table'))]]|       media[not(ancestor::sub-article) and label]|       supplementary-material[not(ancestor::sub-article) and label]" id="unlinked-object-cite">
      ...
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#text-v-object-cite-test" test="matches(lower-case($article-text),lower-case($regex))" role="warning" id="text-v-object-cite-test">[text-v-object-cite-test] <value-of select="$cite1"/> has possible unlinked citations in the text.</report>
  ...
  <pattern id="vid-xref-conformance-pattern"><rule context="xref[@ref-type='video']" id="vid-xref-conformance">
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#vid-xref-test-2" test="not(parent::*[name()=('td','th')] and preceding-sibling::node()[1]/name()='break') and matches($pre-text,'[\p{L}\p{N}\p{M}\p{Pe},;]$')" role="warning" id="vid-xref-test-2">[vid-xref-test-2] There is no space between citation and the preceding text - <value-of select="concat(substring($pre-text,string-length($pre-text)-15),.)"/> - Is this correct?</report>
  ...
  <pattern id="fig-xref-conformance-pattern"><rule context="xref[@ref-type='fig' and @rid]" id="fig-xref-conformance">
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-test-2" test="not(parent::*[name()=('td','th')] and preceding-sibling::node()[1]/name()='break') and matches($pre-text,'[\p{L}\p{N}\p{M}\p{Pe},;]$')" role="warning" id="fig-xref-test-2">[fig-xref-test-2] There is no space between citation and the preceding text - <value-of select="concat(substring($pre-text,string-length($pre-text)-15),.)"/> - Is this correct?</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-test-3" test="not(parent::*[name()=('td','th')] and following-sibling::node()[1]/name()='break') and matches($post-text,'^[\p{L}\p{N}\p{M}\p{Ps}]')" role="warning" id="fig-xref-test-3">[fig-xref-test-3] There is no space between citation and the following text - <value-of select="concat(.,substring($post-text,1,15))"/> - Is this correct?</report>
  ...
  <pattern id="org-ref-article-book-title-pattern"><rule context="element-citation/article-title|       element-citation/chapter-title|       element-citation/source|       element-citation/data-title" id="org-ref-article-book-title">
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#hrmnk-article-title-organism-check" test="$organisms//*:organism" role="info" id="article-title-organism-check">[article-title-organism-check] <name/> contains an organism - <value-of select="string-join($organisms//*:organism,'; ')"/> - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#hxrsi-italic-genus-oocytes" test="matches(lower-case(.),concat($regex-prefix,'\p{Zs}*oocytes'))" role="error" id="italic-genus-oocytes">[italic-genus-oocytes] <name/> contains a genus name followed by 'oocytes'. 'oocytes' should not be in italics.</report>
  ...
  <pattern id="unallowed-symbol-tests-pattern"><rule context="p|td|th|title|xref|bold|italic|sub|sc|named-content|monospace|code|underline|fn|institution|ext-link" id="unallowed-symbol-tests">		
      ...
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#replacement-character-presence" test="contains(.,'�')" role="error" id="replacement-character-presence">[replacement-character-presence] <name/> element contains the replacement character '�' which is not allowed.</report>
      
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#junk-character-presence" test="contains(.,'')" role="error" id="junk-character-presence">[junk-character-presence] <name/> element contains a junk character '' which should be replaced.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#inverterted-question-presence" test="contains(.,'¿')" role="warning" id="inverterted-question-presence">[inverterted-question-presence] <name/> element contains an inverted question mark '¿' which should very likely be replaced/removed.</report>
      
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#empty-parentheses-presence" test="some $x in self::*[not(local-name() = ('monospace','code'))]/text() satisfies matches($x,'\(\)|\[\]')" role="warning" id="empty-parentheses-presence">[empty-parentheses-presence] <name/> element contains empty parentheses ('[]', or '()'). Is there a missing citation within the parentheses? Or perhaps this is a piece of code that needs formatting?</report>
      
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#broken-unicode-presence" test="matches(.,'&amp;#x\d')" role="warning" id="broken-unicode-presence">[broken-unicode-presence] <name/> element contains what looks like a broken unicode - <value-of select="."/>.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#operating-system-command-presence" test="contains(.,'&#x9D;')" role="error" id="operating-system-command-presence">[operating-system-command-presence] <name/> element contains an operating system command character '&#x9D;' (unicode string: &amp;#x9D;) which should very likely be replaced/removed. - <value-of select="."/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#webreplacement-character-presence" test="matches(.,'�')" role="error" id="webreplacement-character-presence">[webreplacement-character-presence] web citation contains the replacement character '�' which is unallowed - <value-of select="."/></report>
      
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#psyarxiv-web-test" test="matches($link,'psyarxiv.org')" role="error" id="psyarxiv-web-test">[psyarxiv-web-test] web ref '<value-of select="ancestor::ref/@id"/>' has a link which points to a preprint server, PsyArXiv, therefore it should be captured as a preprint type ref - <value-of select="ext-link"/></report>
      
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#arxiv-web-test" test="matches($link,'/arxiv.org')" role="error" id="arxiv-web-test">[arxiv-web-test] web ref '<value-of select="ancestor::ref/@id"/>' has a link which points to a preprint server, arXiv, therefore it should be captured as a preprint type ref - <value-of select="ext-link"/></report>
      
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#biorxiv-web-test" test="matches($link,'biorxiv.org')" role="warning" id="biorxiv-web-test">[biorxiv-web-test] web ref '<value-of select="ancestor::ref/@id"/>' has a link which points to a preprint server, bioRxiv, therefore it should almost certainly be captured as a preprint type ref - <value-of select="ext-link"/></report>
      
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#chemrxiv-web-test" test="matches($link,'chemrxiv.org')" role="error" id="chemrxiv-web-test">[chemrxiv-web-test] web ref '<value-of select="ancestor::ref/@id"/>' has a link which points to a preprint server, ChemRxiv, therefore it should be captured as a preprint type ref - <value-of select="ext-link"/></report>
      
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#peerj-preprints-web-test" test="matches($link,'peerj.com/preprints/')" role="error" id="peerj-preprints-web-test">[peerj-preprints-web-test] web ref '<value-of select="ancestor::ref/@id"/>' has a link which points to a preprint server, PeerJ Preprints, therefore it should be captured as a preprint type ref - <value-of select="ext-link"/></report>
      
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#paleorxiv-web-test" test="matches($link,'paleorxiv.org')" role="error" id="paleorxiv-web-test">[paleorxiv-web-test] web ref '<value-of select="ancestor::ref/@id"/>' has a link which points to a preprint server, PaleorXiv, therefore it should be captured as a preprint type ref - <value-of select="ext-link"/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/software-references-aymhzmlh#r-test-1" test="matches($lc,'r: a language and environment for statistical computing') and not(matches(person-group[@person-group-type='author']/*[name()=('collab','collab-name')][1],'^R Development Core Team$'))" role="error" id="R-test-1">[R-test-1] software ref '<value-of select="ancestor::ref/@id"/>' has a data-title '<value-of select="data-title[1]"/>' but it does not have one collab-name element containing 'R Development Core Team'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/software-references-aymhzmlh#r-test-2" test="matches($lc,'r: a language and environment for statistical computing') and (count(person-group[@person-group-type='author']/*[name()=('collab','collab-name')]) != 1)" role="error" id="R-test-2">[R-test-2] software ref '<value-of select="ancestor::ref/@id"/>' has a data-title '<value-of select="data-title[1]"/>' but it has <value-of select="count(person-group[@person-group-type='author']/*[name()=('collab','collab-name')])"/> collab-name element(s).</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#surname-number-check" test="matches(.,'[0-9]')" role="warning" id="surname-number-check">[surname-number-check] name in ref '<value-of select="ancestor::ref/@id"/>' contains numbers - <value-of select="."/>. Should this be captured as a collab-name?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#PMCID-link-test" test="matches(.,'PMCID[:]?\p{Zs}?PMC[0-9][0-9][0-9]+') and (count(ext-link[matches(@xlink:href,'ncbi.nlm.nih.gov/pmc|pmc.ncbi.nlm.nih.gov')]) = 0)" role="error" id="PMCID-link-test">[PMCID-link-test] td element containing - '<value-of select="."/>' - looks like it contains a PMCID, but it contains no link pointing to PMC, which is incorrect.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#auth-kwd-check" test="not(matches(.,'RNA|[Cc]ryoEM|[34]D')) and (. != $lower) and not(contains($t,.))" role="warning" id="auth-kwd-check">[auth-kwd-check] Keyword - '<value-of select="."/>' - does not appear in the article text with this capitalisation. Should it be <value-of select="$lower"/> instead?</report>
      
      <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#auth-kwd-check-2" test="matches(.,'&amp;#x\d')" role="warning" id="auth-kwd-check-2">[auth-kwd-check-2] Keyword contains what looks like a broken unicode - <value-of select="."/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#auth-kwd-check-3" test="contains(.,'&lt;') or contains(.,'&gt;')" role="error" id="auth-kwd-check-3">[auth-kwd-check-3] Keyword contains markup captured as text - <value-of select="."/>. Please remove it and ensure that it is marked up properly (if necessary).</report>
      
      <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#auth-kwd-check-4" test="matches(.,'[\(\)\[\]]') or contains(.,'{') or contains(.,'}')" role="warning" id="auth-kwd-check-4">[auth-kwd-check-4] Keyword contains brackets - <value-of select="."/>. These should either simply be removed, or added as two keywords (with the brackets still removed).</report>
      
      <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#auth-kwd-check-5" test="contains($lower,' and ')" role="warning" id="auth-kwd-check-5">[auth-kwd-check-5] Keyword contains 'and' - <value-of select="."/>. These should be split out into two keywords.</report>
      
      <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#auth-kwd-check-6" test="not(ancestor::article-meta/article-categories/subj-group[@subj-group-type='display-channel']/subject[1] = $features-subj) and count(tokenize(.,'\p{Zs}')) gt 4" role="warning" id="auth-kwd-check-6">[auth-kwd-check-6] Keyword contains more than 4 words - <value-of select="."/>. Should these be split out into separate keywords?</report>
      
      <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#auth-kwd-check-7" test="not(italic) and matches($lower,$org-regex)" role="warning" id="auth-kwd-check-7">[auth-kwd-check-7] Keyword contains an organism name which is not in italics - <value-of select="."/>. Please italicise the organism name in the keyword.</report>
    </rule></pattern><pattern id="general-kwd-pattern"><rule context="kwd" id="general-kwd">
      
      <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#auth-kwd-check-8" test="contains(.,', ')" role="warning" id="auth-kwd-check-8">[auth-kwd-check-8] Keyword contains a comma - '<value-of select="."/>'. Should this be split into multiple keywords?</report>
      
      <report see="https://elifeproduction.slab.com/posts/keywords-and-research-organisms-mq0bozp9#auth-kwd-check-9" test="matches(.,'[”“‘’&quot;]')" role="warning" id="auth-kwd-check-9">[auth-kwd-check-9] Keyword contains a quotation mark - '<value-of select="."/>'. Should this be removed and/or should the keyword be split into multiple keywords?</report>
