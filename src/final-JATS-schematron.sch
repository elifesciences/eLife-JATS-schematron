 ...
 <pattern id="article-tests-pattern">
  ...
  <pattern id="ext-link-tests-pattern">
    <rule context="ext-link[@ext-link-type='uri']" id="ext-link-tests">
      ...
      <assert test="matches(@xlink:href,'^https?:..(www\.)?[-a-zA-Z0-9@:%.,_\+~#=!]{1,256}\.[a-z]{2,6}([-a-zA-Z0-9@:;%,_\\(\)+.~#?!&amp;&lt;&gt;//=]*)$|^ftp://.|^git://.|^tel:.|^mailto:.')" role="warning" id="url-conformance-test">[url-conformance-test] @xlink:href doesn't look like a URL - '<value-of select="@xlink:href"/>'. Is this correct?</assert>
  ...
  <pattern id="software-heritage-tests-pattern">
    <rule context="ext-link[contains(@xlink:href,'softwareheritage')]" id="software-heritage-tests">
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/toolkit/archiving-code#software-heritage-test-4" test="ancestor::body and not(some $x in preceding-sibling::ext-link[position() le 3] satisfies lower-case($x/@xlink:href) = $origin)" role="warning" id="software-heritage-test-4">[software-heritage-test-4] A Software heritage link must follow the original link for the software. The Software heritage link with the text '<value-of select="."/>' has '<value-of select="$origin"/>' as its origin URL, but there is no preceding link with that same URL.</report>
  ...
  <pattern id="supplementary-material-tests-pattern">
    <rule context="supplementary-material" id="supplementary-material-tests">
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/additional-files#supplementary-material-test-2" test="if (contains(label,'Transparent reporting form')) then ()         else not(caption)" role="warning" id="supplementary-material-test-2">[supplementary-material-test-2] <value-of select="label"/> is missing a title/caption - is this correct?  (supplementary-material should have a child caption.)</report>
  ...
  <pattern id="elem-citation-general-pattern">
    <rule context="element-citation" id="elem-citation-general">
      ...
      <report test="descendant::etal" role="error" id="err-elem-cit-gen-name-5">[err-elem-cit-gen-name-5] The &lt;etal&gt; element in a reference is not allowed. Reference '<value-of select="ancestor::ref/@id"/>' contains it.</report>
      
      <report test="count(year) &gt; 1 " role="error" id="err-elem-cit-gen-date-1-9">[err-elem-cit-gen-date-1-9] There may be at most one &lt;year&gt; element. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(year)"/> &lt;year&gt; elements.</report>
      
      <report test="(fpage) and not(lpage)" role="warning" id="fpage-lpage-test-1">[fpage-lpage-test-1] <value-of select="e:citation-format1(.)"/> has a first page <value-of select="fpage"/>, but no last page. Is this correct? Should it be an elocation-id instead?</report>
  ...
  <pattern id="elem-citation-year-pattern">
    <rule context="ref/element-citation/year" id="elem-citation-year">
      ...
      <assert test="(1700 le number($YYYY)) and (number($YYYY) le ($current-year + 5))" role="warning" id="err-elem-cit-gen-date-1-2">[err-elem-cit-gen-date-1-2] The numeric value of the first 4 digits of the &lt;year&gt; element must be between 1700 and the current year + 5 years (inclusive). Reference '<value-of select="ancestor::ref/@id"/>' does not meet this requirement as it contains the value '<value-of select="."/>'.</assert>
      
      <assert test="./@iso-8601-date" role="error" id="err-elem-cit-gen-date-1-3">[err-elem-cit-gen-date-1-3] All &lt;year&gt; elements must have @iso-8601-date attributes. Reference '<value-of select="ancestor::ref/@id"/>' does not.</assert>
      
      <assert test="not(./@iso-8601-date) or (1700 le number(substring(normalize-space(@iso-8601-date),1,4)) and number(substring(normalize-space(@iso-8601-date),1,4)) le ($current-year + 5))" role="warning" id="err-elem-cit-gen-date-1-4">[err-elem-cit-gen-date-1-4] The numeric value of the first 4 digits of the @iso-8601-date attribute on the &lt;year&gt; element must be between 1700 and the current year + 5 years (inclusive). Reference '<value-of select="ancestor::ref/@id"/>' does not meet this requirement as the attribute contains the value '<value-of select="./@iso-8601-date"/>'.</assert>
      ...
      <assert test="not(concat($YYYY, 'a')=.) or (concat($YYYY, 'a')=. and         (some $y in //element-citation/descendant::year         satisfies (normalize-space($y) = concat($YYYY,'b'))         and (ancestor::element-citation/person-group[1]/name[1]/surname = $y/ancestor::element-citation/person-group[1]/name[1]/surname         or ancestor::element-citation/person-group[1]/collab[1] = $y/ancestor::element-citation/person-group[1]/collab[1]         )))" role="error" id="err-elem-cit-gen-date-1-6">[err-elem-cit-gen-date-1-6] If the &lt;year&gt; element contains the letter 'a' after the digits, there must be another reference with the same first author surname (or collab) with a letter "b" after the year. Reference '<value-of select="ancestor::ref/@id"/>' does not fulfill this requirement.</assert>
  ...
  <pattern id="ref-pattern">
    <rule context="ref" id="ref">
      ...
      <assert test="matches(normalize-space(@id) ,'^bib\d+$')" role="error" id="err-elem-cit-high-3-2">[err-elem-cit-high-3-2] Each &lt;ref&gt; element must have an @id attribute that starts with 'bib' and ends with a number. Reference '<value-of select="@id"/>' has the value '<value-of select="@id"/>', which is incorrect.</assert>
      
      <assert test="count(preceding-sibling::ref)=0 or number(substring(@id,4)) gt number(substring(preceding-sibling::ref[1]/@id,4))" role="error" id="err-elem-cit-high-3-3">[err-elem-cit-high-3-3] The sequence of ids in the &lt;ref&gt; elements must increase monotonically (e.g. 1,2,3,4,5, . . . ,50,51,52,53, . . . etc). Reference '<value-of select="@id"/>' has the value  '<value-of select="@id"/>', which does not fit this pattern.</assert>
  ...
  <pattern id="pub-id-tests-pattern">
    <rule context="element-citation/pub-id" id="pub-id-tests">
      ...
      <report test="(@xlink:href) and not(matches(@xlink:href,'^http[s]?://|^ftp://'))" role="error" id="final-pub-id-test-1">[final-pub-id-test-1] @xlink:href must start with an http:// or ftp:// protocol. - <value-of select="@xlink:href"/> does not.</report>
      
      
      
      <report test="(@pub-id-type='doi') and not(matches(.,'^10\.\d{4,9}/[-._;\+()#/:A-Za-z0-9&lt;&gt;\[\]]+$'))" role="error" id="final-pub-id-test-2">[final-pub-id-test-2] pub-id is tagged as a doi, but it is not one - <value-of select="."/>
      </report>
      ...
      <report test="ends-with(.,'.')" role="error" id="pub-id-test-5">[pub-id-test-5] <value-of select="@pub-id-type"/> pub-id ends with a full stop - <value-of select="."/> - which is not correct. Please remove the full stop.</report>
 ...
 <pattern id="feature-title-tests-pattern">
  ...
  <pattern id="vid-xref-conformance-pattern">
    <rule context="xref[@ref-type='video']" id="vid-xref-conformance">
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/asset-citations#vid-xref-conformity-2" test="not(contains(.,'nimation')) and not(contains(.,$target-no))" role="error" id="vid-xref-conformity-2">[vid-xref-conformity-2] video citation does not match the video that it links to. Target video label number is <value-of select="$target-no"/>, but that number is not in the citation text - <value-of select="."/>.</report>
  ...
  <pattern id="org-ref-article-book-title-pattern">
    <rule context="element-citation/article-title|element-citation/chapter-title|element-citation/source|element-citation/data-title" id="org-ref-article-book-title">	
      ...
      <report test="matches($lc,'t\.\s?brucei') and not(italic[contains(text() ,'T. brucei')])" role="info" id="tbrucei-ref-article-title-check">[tbrucei-ref-article-title-check] <name/> contains an organism - 'T. brucei' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'trypanosoma\s?brucei') and not(italic[contains(text() ,'Trypanosoma brucei')])" role="info" id="trypanosomabrucei-ref-article-title-check">[trypanosomabrucei-ref-article-title-check] <name/> contains an organism - 'Trypanosoma brucei' - but there is no italic element with that correct capitalisation or spacing.</report>
  ...
  <pattern id="publisher-name-tests-pattern">
    <rule context="element-citation/publisher-name" id="publisher-name-tests">
      ...
      <report test="matches(lower-case(.),'guardian|the independent|times|post|news')" role="warning" id="pub-name-newspaper">[pub-name-newspaper] <name/> contains the text 'guardian', 'independent', 'times' or 'post' - <value-of select="."/> - is it a newspaper reference? If so, it should be captured as a web or a periodical reference.</report>
    </rule>
  </pattern>
