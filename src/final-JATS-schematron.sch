 ...
 <pattern id="article-tests-pattern">
  ...
  <pattern id="table-wrap-tests-pattern">
    <rule context="table-wrap" id="table-wrap-tests">
      ...
      <report test="matches($id,'^app[0-9]{1,4}keyresource$') and not(matches($lab,'^Appendix [0-9]{1,4}—key resources table$'))" role="error" id="kr-table-wrap-test-2">[kr-table-wrap-test-2] table-wrap has an id '<value-of select="$id"/>' but its label is not in the format 'Appendix 0—key resources table', which is incorrect. More information here - https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/tables#kr-table-wrap-test-2</report>
