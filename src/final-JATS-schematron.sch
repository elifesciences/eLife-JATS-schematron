<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:ali="http://www.niso.org/schemas/ali/1.0/" xmlns:file="java.io.File" xmlns:java="http://www.java.com/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" queryBinding="xslt2">
 ...
 <pattern id="article-tests-pattern"><rule context="article" id="article-tests">
  ...
  <pattern id="test-article-metadata-pattern"><rule context="article/front/article-meta" id="test-article-metadata">
   ...
   <let name="template" value="parent::article-meta/custom-meta-group/custom-meta[meta-name='Template']/meta-value[1]"/>
    ...
    <assert test="parent::contrib-group//contrib//xref/@rid = @id" role="error" id="aff-test-1">[aff-test-1] aff elements that are direct children of contrib-group must have an xref in that contrib-group pointing to them.</assert>
      ...
      <assert test="institution[not(@*)] or institution-wrap[institution[not(@*)]]" role="error" id="final-auth-aff-test-3">[final-auth-aff-test-3] Author affiliations must have a top level institution. This one (with the id <value-of select="@id"/>) does not - <value-of select="$display"/></assert>
    ...
    </rule></pattern><pattern id="wellcome-fund-statement-tests-pattern"><rule context="article-meta/funding-group[descendant::institution[lower-case(.)=('wellcome','wellcome trust')]]/funding-statement" id="wellcome-fund-statement-tests">
  ...
  <pattern id="p-tests-pattern"><rule context="p" id="p-tests">
      ...
      <report test="name()=('msubsup','munderover') and not(*[3][normalize-space(descendant-or-self::*[text()])])" role="error" id="math-empty-second-script-check">[math-empty-second-script-check] <name/> element must not have an empty <value-of select="if (name()='msubsup') then 'superscript' else 'overscript'"/> expression.</report>
  ...
  <pattern id="fig-specific-tests-pattern"><rule context="article/body//fig[not(@specific-use='child-fig')][not(ancestor::boxed-text)]" id="fig-specific-tests">
    ...
    <report test="matches(lower-case(.),'biorend[eo]r') and not(ancestor::fig/permissions[matches(lower-case(.),'biorend[eo]r')])" role="error" id="final-fig-caption-test-4">[final-fig-caption-test-4] Caption or attrib for <value-of select="$label"/> contains what looks like a mention of bioRender and does not have a permissions statement that refers to bioRender. bioRender do not permit content to be published under a CC-BY license (https://help.biorender.com/en/articles/8601313-creative-commons-licensing-for-biorender-figures-premium-only). Once the authors have obtained permission, this figure needs to contain a copyright statement indicating which license it is available under (e.g. CC BY-NC-ND).</report>
      ...
      <assert test=".='BioRender Inc'" role="error" id="biorender-permissions-tes-1">[biorender-permissions-tes-1] permissions element mentions bioRender, but the copyright holder is not 'BioRender Inc' which is incorrect. The current copyright holder is: <value-of select="."/>. The license text is: <value-of select="parent::permissions/license[1]/license-p[1]"/>.</assert>
 ...
 <pattern id="feature-title-tests-pattern"><rule context="article-meta[descendant::subj-group[@subj-group-type='display-channel']/subject = $features-subj]//title-group/article-title" id="feature-title-tests">
  ...
  <pattern id="unallowed-symbol-tests-pattern"><rule context="p|td|th|title|xref|bold|italic|sub|sc|named-content|monospace|code|underline|fn|institution|ext-link" id="unallowed-symbol-tests">		
      ...
      <report test="matches($link,'paleorxiv.org')" role="error" id="paleorxiv-web-test">[paleorxiv-web-test] web ref '<value-of select="ancestor::ref/@id"/>' has a link which points to a preprint server, bioRxiv, therefore it should be captured as a preprint type ref - <value-of select="ext-link"/></report>
  ...
  <pattern id="fundref-rule-pattern"><rule context="article[not(@article-type='article-commentary')]//ack" id="fundref-rule">
      <let name="ack" value="."/>   
