 ...
 <pattern id="article-tests-pattern">
  ...
  <pattern id="math-tests-pattern">
    <rule context="mml:math" id="math-tests">
      ...
      <report test="not(descendant::mml:msqrt) and not(descendant::mml:mfrac) and matches($data,'^±\d+%$|^+\d+%$|^-\d+%$|^\d+%$|^±\d+$|^+\d+$|^-\d+$')" role="warning" id="math-test-13">[math-test-13] mml:math only contains '<value-of select="."/>', which is likely unnecessary. Should this be captured as normal text instead?</report>
