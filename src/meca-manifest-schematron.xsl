<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:saxon="http://saxon.sf.net/" xmlns:schold="http://www.ascc.net/xml/schematron" xmlns:iso="http://purl.oclc.org/dsdl/schematron" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:meca="http://manuscriptexchange.org" version="2.0"><!--Implementers: please note that overriding process-prolog or process-root is 
   ...
   <xsl:template match="/">
      <svrl:schematron-output xmlns:svrl="http://purl.oclc.org/dsdl/svrl" title="eLife MECA manifest schematron" schemaVersion="">
         ...
         <svrl:active-pattern>
            ...
            <xsl:attribute name="id">indistinct-files</xsl:attribute>
            <xsl:attribute name="name">indistinct-files</xsl:attribute>
            <xsl:apply-templates/>
         </svrl:active-pattern>
         ...
         <svrl:active-pattern>
            ...
            <xsl:attribute name="id">item</xsl:attribute>
   ...
   <xsl:template match="meca:instance" priority="1000" mode="M2">
		...
		<!--REPORT error-->
      <xsl:if test="@href = preceding::meca:instance/@href">
         <svrl:successful-report xmlns:svrl="http://purl.oclc.org/dsdl/svrl" test="@href = preceding::meca:instance/@href">
            <xsl:attribute name="id">indictinct-file</xsl:attribute>
            <xsl:attribute name="role">error</xsl:attribute>
            <xsl:attribute name="location">
               <xsl:apply-templates select="." mode="schematron-select-full-path"/>
            </xsl:attribute>
            <svrl:text>Instance for <xsl:text/>
               <xsl:value-of select="parent::meca:item/meca:title"/>
               <xsl:text/> has the same file as another item within the manifest - <xsl:text/>
   ...
   <xsl:template match="meca:item" priority="1000" mode="M3">
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" test="@type">
               <xsl:attribute name="id">item-type-presence</xsl:attribute>
               <xsl:attribute name="role">error</xsl:attribute>
               <xsl:attribute name="location">
                  <xsl:apply-templates select="." mode="schematron-select-full-path"/>
               </xsl:attribute>
               <svrl:text>Item element must have a type attribute with one of the following permitted values: <xsl:text/>
                  <xsl:value-of select="string-join($allowed-types,'; ')"/>
                  <xsl:text/>. This one does not have a type value.</svrl:text>
            </svrl:failed-assert>
         </xsl:otherwise>
      </xsl:choose>
      ...
      <xsl:if test="@type and not(@type = $allowed-types)">
         <svrl:successful-report xmlns:svrl="http://purl.oclc.org/dsdl/svrl" test="@type and not(@type = $allowed-types)">
            ...
            <svrl:text>Item element has a type attribute with the value '<xsl:text/>
               <xsl:value-of select="@type"/>
               <xsl:text/>' which is not one of the permitted values: <xsl:text/>
               <xsl:value-of select="string-join($allowed-types,'; ')"/>
               <xsl:text/>.</svrl:text>
         </svrl:successful-report>
      </xsl:if>
