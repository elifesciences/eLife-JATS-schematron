...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
  ...
  <sqf:fixes>
    ...
    <report test="descendant::bold or descendant::sub or descendant::sup or descendant::italic or descendant::sc" 
      ...
      <report test="matches(.,'\s') and not(matches(lower-case(.),'^de[lrn]? |^v[ao]n |^el |^te[rn] |^l[ae] |^zur |^d[ia] |^dos '))" 
	      role="warning" 
    ...
    <report test="matches(.,'\p{Zs}$')" 
      ...
      <report test="not(ancestor::ref) and matches(.,'^[\p{Lu}\s]+$')" 
        role="warning" 
    ...
    <rule context="article/front/article-meta/contrib-group[1]" id="orcid-name-checks">
     ...
     <rule context="contrib-id[@contrib-id-type='orcid']" id="orcid-tests">
       ...
       <report test="following-sibling::contrib-id[@contrib-id-type='orcid']" 
          role="error" 
          id="orcid-test-5"><value-of select="parent::*/name()"/> <value-of select="if (parent::*/name) then concat('for ',e:get-name(parent::*/name[1])) else ''"/> has two or more ORCID IDs. An author should only have one ORCID ID. Please check which is correct, remove any incorrect or duplicate ORCID IDs and (if appropriate) preference the authenticated ORCID ID.</report>
  ...
  <pattern id="confproc-ref">
    ...
    <rule context="collab-name" id="collab-name-tests">
      ...
      <report test="ancestor::article-meta and not(parent::collab-wrap)" 
        role="error" 
        id="collab-name-test-1"><name/> that is a descendant of article-meta must be captured as a child of collab-wrap. This one has the parent <value-of select="parent::*/name()"/>.</report>
      
      <report test="ancestor::ref and not(parent::person-group)" 
        role="error" 
  ...
  <pattern id="math">
    <rule context="*:math" id="math-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#math-test-2"
        test="descendant::*:merror" 
        role="error" 
        id="math-test-2">math contains an mml:merror with '<value-of select="descendant::*:merror[1]/*"/>'. This will almost certainly not render correctly.</report>
      ...
      <report test="@display='inline' and ancestor::disp-formula" 
        role="warning" 
        id="math-display-inline"><name/> is a descendant of an disp-formula but it has the attribute display="inline". This is likely incorrect. Either the disp-formula is really a inline-formula, or the display="inline" attribute should be removed (or changed to "block").</report>
      
      <assert test="@display" 
        role="warning" 
        id="math-display-absence"><name/> does not have a display attribute. Since this math is a descendant of <value-of select="ancestor::*[name()=('disp-formula','inline-formula')]/name()"/>, is should be display="<value-of select="if (ancestor::disp-formula) then 'block' else 'inline'"/>".</assert>
      
      <report test="matches($data,'^\s*=')" 
        role="warning" 
        id="math-starts-with-equals"><name/> starts with an equal sign. Is that correct? Or has some processing error occurred?</report>
    </rule>
    ...
    <rule context="*:msub|*:msup|*:msubsup|*:munder|*:mover|*:munderover|*:mfrac|*:mroot" id="math-empty-child-tests">
      ...
      <let name="second-name" value="if (local-name() = 'msub') then 'subscript'
      ...
      <report test="*[1][matches(.,'^\p{Z}*$')]"
        role="warning"
        id="math-empty-base-check"><name/> element should not have a missing or empty <value-of select="$first-name"/> expression.</report>

      <report test="*[2][matches(.,'^\p{Z}*$')]"
        role="error"
        id="math-empty-script-check"><name/> element must not have a missing or empty <value-of select="$second-name"/> expression.</report>
    ...
    <rule context="*:mtable" id="math-mtable-tests">
      <assert test="*:mtr or *:mlabeledtr"
        role="error"
        id="math-mtable-check-1"><name/> element must have either a child mml:mtr or mml:mlabeledtr element. This one has neither.</assert>
    </rule>
  </pattern>
    ...
    <pattern id="list">
     ...
     <pattern id="graphic">
      ...
      <report test="matches(lower-case(@*:href),'\.(xml|XML)\s*$')" 
        role="error" 
        id="media-test-9">media points to an xml file. This cannot be handled currently. Please download the file, place it in a zip and replace the file with this zip, and update the xlink:href accordingly.</report>  
     </rule>
    ...
    <pattern id="sec">
      ...
      <rule context="sec/label" id="sec-label-checks">
        <report test="matches(.,'[2-4]D')" 
          role="warning" 
          sqf:fix="move-to-title delete-node"
...
<!-- Top level section titles that will appear in the table of contents -->
      <rule context="article/body/sec/title | article/back/sec/title | article/back/app/title | article/back/app-group/app/title" id="title-toc-checks">
        <report test="xref" 
          role="error" 
          id="toc-title-contains-citation"><name/> element contains a citation and will appear within the table of contents on EPP. This will cause images not to load. Please either remove the citation or make it plain text.</report>
      </rule>
    </pattern>
  ...
  <pattern id="p-td-th">
    ...
    <pattern id="article-metadata">
      <rule context="article/front/article-meta" id="general-article-meta-checks">
        ...
        <report test="$is-reviewed-preprint and ($dtd le '1.3') and not(count(history)=1)" 
          role="error" 
          id="history-presence">Reviewed preprints (dtd version <value-of select="$dtd"/>) must have (and only one) history. This one has <value-of select="count(history)"/>.</report>
        
        <report test="$is-reviewed-preprint and ($dtd ge '1.4') and history" 
          role="error" 
          id="history-absence">Reviewed preprints (dtd version <value-of select="$dtd"/>) must not have a history element.</report>
    ...
    <rule context="event/self-uri" id="event-self-uri-tests">
      ...
      <rule context="funding-group/award-group" id="award-group-tests">
        ...
        <report see="https://elifeproduction.slab.com/posts/funding-3sv64358#award-group-test-6" 
          test="count(funding-source//institution[normalize-space(.)!='']) = 0" 
          role="error" 
  ...
  <pattern id="refinery-comments">
    <rule context="ref//comment()" id="refinery-comment-checks">
      ...
      <report test="matches(lower-case(.),'refinery:') and not(matches(lower-case(.),'refinery:.*?(doi|pmid).*?suggested|refinery: existing (doi|pmid) could not be verified'))"
        role="warning"
        sqf:fix="dismiss-refinery"
        id="refinery-unknown-suggestion">Ref (with id <value-of select="ancestor::ref/@id"/>) has a suggested change '<value-of select="normalize-space(.)"/>'.</report>
      
      <assert test="matches(lower-case(.),'refinery:')"
        role="error"
        sqf:fix="delete-node"
        id="ref-comment-2">Ref (with id <value-of select="ancestor::ref/@id"/>) has comment node with the content '<value-of select="."/>'. Comments should be removed.</assert>
  ...
  <!-- These are purely for oXygen validation -->
    <pattern id="api-calls">
        <rule context="article[descendant::article-meta/pub-history/event/self-uri[@content-type='reviewed-preprint']]/sub-article[@article-type='editor-report']" flag="local-only" id="assessment-api-check">
          ...
          <let name="epp-assessment-paragraphs" value="if (exists($epp-assessment-data)) then
            $epp-assessment-data?content?*[?type = 'paragraph']?text
            else ()"/>
          <let name="prev-assessment-text" value="if (exists($epp-assessment-paragraphs)) then normalize-space(string-join($epp-assessment-paragraphs,' '))
            else ()"/>
          <let name="prev-strength-terms" value="if (exists($epp-assessment-data)) then $epp-assessment-data?strength?* else ()"/>
          <let name="prev-strength-rank" value="if (exists($prev-strength-terms)) then sum(for $term in $prev-strength-terms[.!=''] return e:assessment-term-to-number($term))
            else ()"/>
          <let name="prev-significance-terms" value="if (exists($epp-assessment-data)) then $epp-assessment-data?significance?* else ()"/>
          <let name="prev-significance-rank" value="if (exists($prev-significance-terms)) then sum(for $term in $prev-significance-terms[.!=''] return e:assessment-term-to-number($term))
            else ()"/>
          
          <let name="curr-strength-terms" value="if (front-stub/kwd-group[@kwd-group-type='evidence-strength']/kwd) 
            then front-stub/kwd-group[@kwd-group-type='evidence-strength']/kwd
            else '(None)'"/>
          <let name="curr-strength-rank" value="sum(for $term in $curr-strength-terms
            return e:assessment-term-to-number($term))"/>
          <let name="curr-significance-terms" value="if (front-stub/kwd-group[@kwd-group-type='claim-importance']/kwd) 
            then front-stub/kwd-group[@kwd-group-type='claim-importance']/kwd
            else '(None)'"/>
          ...
          <report test="($prev-strength-rank gt $curr-strength-rank)" 
            role="warning" 
            id="str-kwd-api-check">The Assessment strength term(s) in this revised Reviewed Preprint (version <value-of select="$rp-version"/>) are lower than those in the previous version (version <value-of select="$prev-version"/>). Is that correct? Current: <value-of select="string-join($curr-strength-terms,'; ')"/>. Previous: <value-of select="string-join($prev-strength-terms,'; ')"/>.</report>
          
          <report test="($prev-significance-rank gt $curr-significance-rank)" 
            role="warning" 
            id="sig-kwd-api-check">The Assessment significance term(s) in this revised Reviewed Preprint (version <value-of select="$rp-version"/>) are lower than those in the previous version (version <value-of select="$prev-version"/>). Is that correct? Current: <value-of select="string-join($curr-significance-terms,'; ')"/>. Previous: <value-of select="string-join($prev-significance-terms,'; ')"/>.</report>
          
          <report test="$prev-assessment-text = $curr-assessment-text" 
            role="warning" 
            id="assessment-text-api-check">The Assessment text in this revised Reviewed Preprint (version <value-of select="$rp-version"/>) is the exact same as in the previous version (version <value-of select="$prev-version"/>). Is that correct?</report>
      </rule>
    </pattern>
