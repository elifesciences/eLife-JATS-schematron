<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:ali="http://www.niso.org/schemas/ali/1.0/" xmlns:file="java.io.File" xmlns:java="http://www.java.com/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" queryBinding="xslt2">
  ...
  <pattern id="test-article-metadata-pattern"><rule context="article/front/article-meta" id="test-article-metadata">
   ...
   <let name="template" value="parent::article-meta/custom-meta-group/custom-meta[meta-name='Template']/meta-value[1]"/>
    ...
    <assert test="parent::contrib-group//contrib//xref/@rid = @id" role="error" id="aff-test-1">aff elements that are direct children of contrib-group must have an xref in that contrib-group pointing to them.</assert>
      ...
      <assert test="addr-line[named-content[@content-type='city']]" role="error" id="final-auth-aff-test-2">Author affiliations must have a city. This one does not - <value-of select="$display"/></assert>
      
      
      
      <assert test="institution[not(@*)] or institution-wrap[institution[not(@*)]]" role="error" id="final-auth-aff-test-3">Author affiliations (&lt;aff&gt;) must include an &gt;institution&gt; tag. This one (with the id <value-of select="@id"/>) does not - <value-of select="$display"/></assert>
      ...
      <assert test="name()=$allowed-elems" role="error" id="aff-child-conformity"><value-of select="name()"/> is not allowed as a child of &lt;aff&gt;.</assert>
      ...
      <assert test="parent::aff" role="error" id="addr-line-parent"><value-of select="name()"/> is not allowed as a child of &lt;<value-of select="parent::*[1]/local-name()"/>&gt;.</assert>
    </rule></pattern><pattern id="addr-line-child-tests-pattern"><rule context="addr-line/*" id="addr-line-child-tests">
      
      <assert test="name()='named-content'" role="error" id="addr-line-child-1"><value-of select="name()"/> is not allowed as a child of &lt;addr-line&gt;.</assert>
      
      <report test="(name()='named-content') and not(@content-type='city')" role="error" id="addr-line-child-2"><value-of select="name()"/> in &lt;addr-line&gt; must have the attribute content-type="city". <value-of select="."/> does not.</report>
 ...
 <pattern id="feature-title-tests-pattern"><rule context="article-meta[descendant::subj-group[@subj-group-type='display-channel']/subject = $features-subj]//title-group/article-title" id="feature-title-tests">
  ...
  <pattern id="unallowed-symbol-tests-pattern"><rule context="p|td|th|title|xref|bold|italic|sub|sc|named-content|monospace|code|underline|fn|institution|ext-link" id="unallowed-symbol-tests">		
      ...
      <report test="$text = 'United States of America'" role="error" id="united-states-test-1"><value-of select="."/> is not allowed. This should be 'United States'.</report>
      
      <report test="$text = 'USA'" role="error" id="united-states-test-2"><value-of select="."/> is not allowed. This should be 'United States'</report>
      
      <report test="$text = 'UK'" role="error" id="united-kingdom-test-2"><value-of select="."/> is not allowed. This should be 'United Kingdom'</report>
      ...
      <report test="(. = 'Singapore') and ($city != 'Singapore')" role="error" id="singapore-test-1"><value-of select="ancestor::aff/@id"/> has 'Singapore' as its country but '<value-of select="$city"/>' as its city, which must be incorrect.</report>
      ...
      <report test="(. = 'Singapore') and (ancestor::aff/country/text() != 'Singapore')" role="error" id="singapore-test-2"><value-of select="ancestor::aff/@id"/> has 'Singapore' as its city but '<value-of select="ancestor::aff/country/text()"/>' as its country, which must be incorrect.</report>
      
      <report test="(lower-case(.) = 'washington') and (ancestor::aff/country/text() = 'United States')" role="error" id="wash-dc-test-1"><value-of select="ancestor::aff/@id"/> has 'Washington' as its city. Either it should be changed to 'Washington, DC' or if referring to the US state then changed to the correct city.</report>
      
      <report test="matches(.,'�')" role="error" id="city-replacement-character-presence"><name/> element contains the replacement character '�' which is unallowed.</report>
      ...
      <report test="matches(.,'�')" role="error" id="institution-replacement-character-presence"><name/> element contains the replacement character '�' which is unallowed.</report>
      ...
      <report test="contains(.,'�')" role="error" id="dept-replacement-character-presence"><name/> element contains the replacement character '�' which is unallowed.</report>
