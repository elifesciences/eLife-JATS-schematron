<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:ali="http://www.niso.org/schemas/ali/1.0/" xmlns:file="java.io.File" xmlns:java="http://www.java.com/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" queryBinding="xslt2">
 ...
 <pattern id="article-tests-pattern"><rule context="article" id="article-tests">
	  ...
	  <report test="@dtd-version" role="info" id="dtd-info">DTD version is <value-of select="@dtd-version"/></report>
	  
	  <assert test="@article-type = $allowed-article-types" role="error" id="test-article-type">article-type must be equal to 'article-commentary', 'correction', 'discussion', 'editorial', or 'research-article'. Currently it is <value-of select="@article-type"/></assert>
		
	  <assert test="count(front) = 1" role="error" id="test-article-front">Article must have one child front. Currently there are <value-of select="count(front)"/></assert>
		
	  <assert test="count(body) = 1" role="error" id="test-article-body">Article must have one child body. Currently there are <value-of select="count(body)"/></assert>
      
    <report test="(@article-type = ('article-commentary','discussion','editorial','research-article','review-article')) and count(back) != 1" role="error" id="test-article-back">Article must have one child back. Currently there are <value-of select="count(back)"/></report>
      ...
      <report test="@article-type!='research-article' and sub-article" role="error" id="non-r-article-sub-article"><value-of select="@article-type"/> type articles cannot have sub-articles (peer review materials).</report>
     ...
     <report test="$disp-channel != 'Scientific Correspondence' and not(sub-article[not(@article-type=('reply','author-comment'))])" role="error" id="r-article-sub-articles"><value-of select="$disp-channel"/> type articles cannot have only an Author response. The following combinations of peer review-material are permitted: Editor's evaluation, Decision letter, and Author response; Decision letter, and Author response; Editor's evaluation and Decision letter; Editor's evaluation and Author response; or Decision letter.</report>
     
   </rule></pattern>

  <pattern id="test-front-pattern"><rule context="article/front" id="test-front">
	 
  	  <assert test="count(journal-meta) = 1" role="error" id="test-front-jmeta">There must be one journal-meta that is a child of front. Currently there are <value-of select="count(journal-meta)"/></assert>
		  
  	  <assert test="count(article-meta) = 1" role="error" id="test-front-ameta">There must be one article-meta that is a child of front. Currently there are <value-of select="count(article-meta)"/></assert>
	 
	 </rule></pattern>

  <pattern id="test-journal-meta-pattern"><rule context="article/front/journal-meta" id="test-journal-meta">
	 
		<assert test="journal-id[@journal-id-type='nlm-ta'] = 'elife'" role="error" id="test-journal-nlm">journal-id[@journal-id-type='nlm-ta'] must only contain 'elife'. Currently it is <value-of select="journal-id[@journal-id-type='nlm-ta']"/></assert>
		  
		<assert test="journal-id[@journal-id-type='publisher-id'] = 'eLife'" role="error" id="test-journal-pubid-1">journal-id[@journal-id-type='publisher-id'] must only contain 'eLife'. Currently it is <value-of select="journal-id[@journal-id-type='publisher-id']"/></assert>
	 
		<assert test="journal-title-group/journal-title = 'eLife'" role="error" id="test-journal-pubid-2">journal-meta must contain a journal-title-group with a child journal-title which must be equal to 'eLife'. Currently it is <value-of select="journal-id[@journal-id-type='publisher-id']"/></assert>
		  
		<assert test="issn = '2050-084X'" role="error" id="test-journal-pubid-3">ISSN must be 2050-084X. Currently it is <value-of select="issn"/></assert>
      ...
      <assert test="@journal-id-type=('nlm-ta','publisher-id')" role="error" id="test-journal-id">The only journal-id-types permitted are 'nlm-ta' or 'publisher-id'. <value-of select="@journal-id-type"/> is not permitted.</assert>
    </rule></pattern>
  ...
  <pattern id="test-article-metadata-pattern"><rule context="article/front/article-meta" id="test-article-metadata">
	...
	<assert test="matches($article-id,'^\d{5,6}$')" role="error" id="test-article-id">article-id must consist only of 5 or 6 digits. Currently it is <value-of select="article-id[@pub-id-type='publisher-id']"/></assert>
    
    <assert test="count(article-version) = 1" role="error" id="test-article-version-presence">There must be one article-version element in the article-meta. Currently there are <value-of select="count(article-version)"/></assert>
	   
     <assert test="count(article-categories) = 1" role="error" id="test-article-presence">There must be one article-categories element in the article-meta. Currently there are <value-of select="count(article-categories)"/></assert>
    ...
    <report test="not($article-type = ($notice-article-types,'article-commentary','editorial')) and count(history) != 1" role="error" id="test-history-presence">There must be one and only one history element in the article-meta. Currently there are <value-of select="count(history)"/></report>
    ...
    <report test="if ($subj-type = $notice-display-types) then (count(kwd-group[@kwd-group-type='author-keywords']) != 0)       else ()" role="error" id="test-auth-kwd-group-presence-2"><value-of select="$subj-type"/> articles must not have any author keywords</report>
    ...
    <report test="if ($subj-type = ('Research Article', 'Research Advance', 'Replication Study', 'Research Communication'))       then (count(kwd-group[@kwd-group-type='research-organism']) = 0)       else ()" role="warning" id="test-ro-kwd-group-presence-2"><value-of select="$subj-type"/> does not contain a Research Organism keyword group. Is this correct?</report>
    ...
    <report test="($subj-type=('Research Article','Research Advance','Tools and Resources','Short Report')) and (history/date[@date-type='received']/@iso-8601-date gt '2021-07-01') and not(pub-history[event[self-uri[@content-type='preprint']]])" role="warning" id="preprint-presence">This <value-of select="$subj-type"/> was received on '<value-of select="history/date[@date-type='received']/@iso-8601-date"/>' (after the preprint mandate) but does not have preprint information. Is that correct?</report>
      ...
      <assert test="starts-with(.,'10.7554/eLife.')" role="error" id="nprc-article-dois-1">Article level DOI must start with '10.7554/eLife.'. Currently it is <value-of select="."/></assert>
      
      <assert test="substring-after(.,'10.7554/eLife.') = $article-id" role="error" id="nprc-article-dois-2">Article level DOI must be a concatenation of '10.7554/eLife.' and the article-id. Currently it is <value-of select="."/></assert>
      
      <report test="preceding-sibling::article-id[@pub-id-type='doi']" role="error" id="nprc-article-dois-3">A non PRC article cannot have more than one DOI in article-meta.</report>
      
      <report test="@*[name()!='pub-id-type']" role="error" id="nprc-article-dois-4">The DOI article-id in a non PRC article cannot have any attributes others than pub-id-type. This one has the following unpermitted attributes: <value-of select="string-join(@*[name()!='pub-id-type']/name(),'; ')"/></report>
      ...
      <assert test="starts-with(.,'10.7554/eLife.')" role="error" id="prc-article-dois-1">Article level DOI must start with '10.7554/eLife.'. Currently it is <value-of select="."/></assert>
      
      <report test="not(@specific-use) and substring-after(.,'10.7554/eLife.') != $article-id" role="error" id="prc-article-dois-2">Article level concept DOI must be a concatenation of '10.7554/eLife.' and the article-id. Currently it is <value-of select="."/></report>
      
      <report test="@specific-use and not(contains(.,$article-id))" role="error" id="prc-article-dois-3">Article level specific version DOI must contain the article-id (<value-of select="$article-id"/>). Currently it does not <value-of select="."/></report>
      
      <report test="@specific-use and not(matches(.,'^10.7554/eLife\.\d{5,6}\.\d$'))" role="error" id="prc-article-dois-4">Article level specific version DOI must be in the format 10.7554/eLife.00000.0. Currently it is <value-of select="."/></report>
    ...
    <assert test="contrib-group" role="error" id="test-contrib-group-presence-1">contrib-group (with no attributes containing authors) must be present (as a child of article-meta) for research articles.</assert>
     ...
     <assert test="contrib-group[@content-type='section']" role="error" id="test-contrib-group-presence-2">contrib-group[@content-type='section'] must be present (as a child of article-meta) for research articles (this is the contrib-group which contains reviewers and editors).</assert>
      ...
      <report test="funding-group" role="error" id="funding-exception"><value-of select="descendant::subj-group[@subj-group-type='display-channel'][1]/subject[1]"/>s cannot have funding, but this one has a funding-group element. Please remove it.</report>
      
    </rule></pattern><pattern id="article-version-pattern"><rule context="article-version" id="article-version">
      
      <assert test="parent::article-meta" role="error" id="article-version-test-1"><name/> must be a child of article-meta. This one is a child of <value-of select="parent::*/name()"/>.</assert>
      
      <assert test="@article-version-type='publication-state'" role="error" id="article-version-test-2"><name/> must a article-version-type="publication-state" attribute. This one does not.</assert>
      ...
      <assert test=".='version of record'" role="error" id="article-version-test-4"><name/> must contain the text 'version of record'. This one contains '<value-of select="."/>'.</assert>
   ...
   <let name="template" value="parent::article-meta/custom-meta-group/custom-meta[meta-name='Template']/meta-value[1]"/>
    ...
    <report test="article-title//bold" role="error" id="article-title-test-6">Article title must not contain bold.</report>
	  ...
	  <report test="matches(article-title[1],'-Based ')" role="error" id="article-title-test-9">Article title contains the string '-Based '. this should be lower-case, '-based '. - <value-of select="article-title"/></report>
	  ...
	  <report test="matches(article-title[1],' [Bb]ased ') and not(matches(article-title[1],' [Bb]ased on '))" role="warning" id="article-title-test-12">Article title contains the string ' based'. Should the preceding space be replaced by a hyphen - '-based'. - <value-of select="article-title"/></report>
    ...
    <report test="count(contrib[@equal-contrib='yes']) = 1" role="error" id="equal-count-test">There is one contrib with the attribute equal-contrib='yes'. This cannot be correct. Either 2 or more contribs within the same contrib-group should have this attribute, or none. Check <value-of select="contrib[@equal-contrib='yes']/name"/></report>
      ...
      <report test="$article-type=$non-contribs and descendant::contrib[@contrib-type='author' and role]" role="error" id="non-contrib-contribs"><value-of select="$article-type"/> type articles should not contain author contributions.</report>
      ...
      <report test="(@contrib-type='editor') and ($role!='Reviewing Editor')" role="error" id="editor-conformance-4"><value-of select="$name"/> has a @contrib-type='editor' but their role is not 'Reviewing Editor' (<value-of select="$role"/>), which is incorrect.</report>

      <assert test="count($matching-author-names)=0" role="error" id="editor-conformance-5"><value-of select="$name"/> is listed both as an author and as a <value-of select="$role"/>, which must be incorrect.</assert>
      ...
      <assert test="child::xref[@ref-type='fn' and matches(@rid,'^con[0-9]{1,3}$')]" role="warning" id="auth-cont-test-1"><value-of select="e:get-name(name[1])"/> has no contributions. Please ensure to query this with the authors.</assert>
      ...
      <assert test="empty($indistinct-conts)" role="error" id="dupe-cont-test-1">Author <value-of select="if (name) then e:get-name(name[1]) else if (collab) then (e:get-collab(collab[1])) else ('with no name')"/> has duplicated contributions which is incorrect. The indistinct contributions are: <value-of select="string-join($indistinct-conts,'; ')"/>.</assert>
      ...
      <assert test="role" role="warning" id="auth-cont-test-1-v2"><value-of select="e:get-name(name[1])"/> has no contributions. Please ensure to query this with the authors.</assert>
      
      <report test="role and not(role[@vocab='credit'])" role="warning" id="auth-cont-test-2-v2"><value-of select="e:get-name(name[1])"/> has no CRediT contributions. Is that correct?</report>
      ...
      <assert test="child::xref[@ref-type='fn' and matches(@rid,'^con[0-9]{1,3}$')]" role="warning" id="collab-cont-test-1"><value-of select="e:get-collab(child::collab[1])"/> has no contributions. Please ensure to query this with the authors.</assert>
      ...
      <assert test="role" role="error" id="final-collab-cont-test-1-v2"><value-of select="e:get-collab(child::collab[1])"/> has no contributions. Please ensure to query this with the authors.</assert>
      
      <report test="role and not(role[@vocab='credit'])" role="warning" id="final-collab-cont-test-2-v2"><value-of select="e:get-collab(child::collab[1])"/> has no CRediT contributions. Is that correct?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hzwb3-author-xref-test-1" test="(@ref-type='aff') and preceding-sibling::xref[not(@ref-type='aff')]" role="error" id="author-xref-test-1">Affiliation footnote links (xrefs) from authors must be the first type of link to be listed. For <value-of select="e:get-name(preceding-sibling::name[1])"/>, their affiliation link - <value-of select="."/> - appears after another non-affiliation link, when it should appear before it.</report>
    ...
    <report test="matches(.,'\p{Zs}$')" role="error" id="given-names-test-9">given-names ends with a space, which cannot be correct - '<value-of select="."/>'.</report>
      ...
      <report test="*" role="error" id="suffix-child-test">suffix cannot have any child elements - <value-of select="*/local-name()"/></report>
      
    </rule></pattern><pattern id="name-child-tests-pattern"><rule context="contrib-group//name/*" id="name-child-tests">
      
      <assert test="local-name() = ('surname','given-names','suffix')" role="error" id="disallowed-child-assert"><value-of select="local-name()"/> is not allowed as a child of name.</assert>
	  ...
	  <report test="(@contrib-type='author') and ($coi = 'No competing interests declared') and (matches($inst,$comp-regex))" role="warning" id="COI-test"><value-of select="$name"/> is affiliated with what looks like a company, but contains no COI statement. Is this correct?</report>
	  
	  <report see="https://elifeproduction.slab.com/posts/deceased-status-8gs60uqk#deceased-test-1" test="matches($fn,'[Dd]eceased') and not(@deceased='yes')" role="error" id="deceased-test-1"><value-of select="$name"/> has a linked footnote '<value-of select="$fn"/>', but not @deceased="yes" which is incorrect.</report>
	  
	  <report see="https://elifeproduction.slab.com/posts/deceased-status-8gs60uqk#deceased-test-2" test="(@deceased='yes') and not(matches($fn,'[Dd]eceased'))" role="error" id="deceased-test-2"><value-of select="$name"/> has the attribute deceased="yes", but no footnote which contains the text 'Deceased', which is incorrect.</report>
      ...
      <report test="$normalized-name != $name" role="warning" id="corresp-author-initial-test"><value-of select="$name"/> has a name with letters that have diacritics, marks, or a name with special characters. Please ensure that their initials display correctly in the PDF in the 'For correspondence' section on the first page.</report>
		  ...
		  <assert test="if (ancestor::collab) then self::*[local-name() = ($allowed-contrib-blocks,'aff')]       else if ($template = '5') then self::*[local-name() = $allowed-contrib-blocks-features]       else if ($article-type = ($features-article-types,'expression-of-concern')) then self::*[local-name() = $allowed-contrib-blocks-features]       else self::*[local-name() = $allowed-contrib-blocks]" role="error" id="author-children-test"><value-of select="self::*/local-name()"/> is not allowed as a child of author.</assert>
      ...
      <!-- Exception included for group authors -->
      <assert test="if (ancestor::collab) then self::*[local-name() = ($allowed-contrib-blocks,'aff')]         else if ($template = '5') then self::*[local-name() = $allowed-contrib-blocks-features]         else if ($article-type = $features-article-types) then self::*[local-name() = $allowed-contrib-blocks-features]         else self::*[local-name() = $allowed-contrib-blocks]" role="error" id="author-children-test-v2"><value-of select="self::*/local-name()"/> is not allowed as a child of author.</assert>
      ...
      <report test="@vocab-term or @vocab-term-identifier or @vocab-identifier" role="error" id="author-role-check-1">role with <value-of select="string-join(@*[name()=('vocab-term','vocab-term-identifier','vocab-identifier')]/name(),'; ')"/> attributes must have a vocab="credit" attribute. This one does not.</report>
      
      <report test="matches(lower-case(.),'^\p{Zs}*(conceptuali[sz]ation|data\p{Zs}+curation|formal\p{Zs}+analysis|funding\p{Zs}+acquisition|investigation|methodology|project\p{Zs}+administration|resources|software|supervision|validation|visualization|writing\p{Zs}+[-–—]\p{Zs}+original\p{Zs}+draft|writing\p{Zs}+[-–—]\p{Zs}+review(ing)?\p{Zs}+(&amp;|and)\p{Zs}+editing)\p{Zs}*$')" role="error" id="author-role-check-2">role with content '<value-of select="."/>' exactly matches one of the CRediT taxonomy terms, but it does not have a vocab="credit" attribute.</report>
    ...
    <assert test="name()=$allowed-children" role="error" id="pub-date-child-test-1"><value-of select="name()"/> is not allowed as a child of pub-date. The only permitted elements are <value-of select="string-join($allowed-children,'; ')"/>.</assert>
      ...
      <report test="not(e:is-prc(.)) and count(event) gt 1" role="error" id="pub-history-child"><name/> must have one, and only one, event element in non-PRC content. This one has <value-of select="count(event)"/>.</report>
      
      <report test="e:is-prc(.) and count(event) le 1" role="error" id="pub-history-events-1"><name/> in PRC articles must have more than one event element, at least one for the preprint, and at least one for the reviewed preprint (there may be numerous reviewed preprint events). This one has <value-of select="count(event)"/> event elements.</report>
      
      <report test="count(event[self-uri[@content-type='preprint']]) != 1" role="error" id="pub-history-events-2"><name/> must contain one, and only one preprint event (an event with a self-uri[@content-type='preprint'] element). This one has <value-of select="count(event[self-uri[@content-type='preprint']])"/> preprint event elements.</report>
      
      <report test="e:is-prc(.) and count(event[self-uri[@content-type='reviewed-preprint']]) lt 1" role="error" id="pub-history-events-3"><name/> in PRC articles must have at least one event element for reviewed preprint publication (an event with a self-uri[@content-type='reviewed-preprint'] element). This one has none.</report>
      
      <report test="e:is-prc(.) and count(event[self-uri[@content-type='reviewed-preprint']]) gt 3" role="warning" id="pub-history-events-4"><name/> has <value-of select="count(event[self-uri[@content-type='reviewed-preprint']])"/> reviewed preprint event elements, which is unusual. Is this correct?</report>
    </rule></pattern><pattern id="event-tests-pattern"><rule context="event" id="event-tests">
      <let name="date" value="date[1]/@iso-8601-date"/>
      
      <assert test="event-desc" role="error" id="event-test-1"><name/> must contain an event-desc element. This one does not.</assert>
      
      <assert test="date[@date-type=('preprint','reviewed-preprint')]" role="error" id="event-test-2"><name/> must contain a date element with the attribute date-type="preprint" or date-type="reviewed-preprint". This one does not.</assert>
      
      <assert test="self-uri" role="error" id="event-test-3"><name/> must contain a self-uri element. This one does not.</assert>
      ...
      <assert test="name()=$allowed-elems" role="error" id="event-child"><name/> is not allowed in an event element. The only permitted children of event are <value-of select="string-join($allowed-elems,', ')"/>.</assert>
    </rule></pattern><pattern id="event-desc-tests-pattern"><rule context="event-desc" id="event-desc-tests">
      
      <report test="not(matches(parent::event/self-uri[1]/@xlink:href,'elifesciences\.org|10.7554/e[lL]ife')) and not(starts-with(.,'This manuscript was published as a preprint at ') or .='This manuscript was published as a preprint.')" role="error" id="event-desc-content"><name/> that's a child of an event without an eLife DOI must contain the text 'This manuscript was published as a preprint at ' followed by the preprint server name. This one does not.</report>
      
      <report test="matches(parent::event/self-uri[1]/@xlink:href,'elifesciences\.org|10.7554/e[lL]ife') and not(.=('This manuscript was published as a reviewed preprint.','The reviewed preprint was revised.'))" role="error" id="event-desc-content-2"><name/> that's a child of an event with an eLife DOI must contain the text 'This manuscript was published as a reviewed preprint.' or 'The reviewed preprint was revised.'</report>
      
      <report test="*" role="error" id="event-desc-elems"><name/> cannot contain elements. This one has the following: <value-of select="string-join(distinct-values(*/name()),', ')"/>.</report>
      
    </rule></pattern><pattern id="event-date-tests-pattern"><rule context="event/date" id="event-date-tests">
      
      <assert test="day and month and year" role="error" id="event-date-child"><name/> in event must have a day, month and year element. This one does not.</assert>
      
      <assert test="@date-type=('preprint','reviewed-preprint')" role="error" id="event-date-type"><name/> in event must have a date-type attribute with the value 'preprint' or 'reviewed-preprint'.</assert>
      ...
      <assert test="@content-type=('preprint','reviewed-preprint')" role="error" id="event-self-uri-content-type"><name/> in event must have the attribute content-type="preprint" or content-type="reviewed-preprint". This one does not.</assert>
      
      <assert test="not(*) and normalize-space(.)=''" role="error" id="event-self-uri-content"><name/> in event must be empty. This one contains elements and/or text.</assert>
      
      <assert test="matches(@xlink:href,'^https?:..(www\.)?[-a-zA-Z0-9@:%.,_\+~#=!]{2,256}\.[a-z]{2,6}([-a-zA-Z0-9@:;%,_\\(\)+.~#?!&amp;&lt;&gt;//=]*)$')" role="error" id="event-self-uri-href-1"><name/> in event must have an xlink:href attribute containing a link to the preprint. This one does not have a valid URI - <value-of select="@xlink:href"/>.</assert>
      
      <report test="matches(lower-case(@xlink:href),'(bio|med)rxiv')" role="error" id="event-self-uri-href-2"><name/> in event must have an xlink:href attribute containing a link to the preprint. Where possible this should be a doi. bioRxiv and medRxiv preprint have dois, and this one points to one of those, but it is not a doi - <value-of select="@xlink:href"/>.</report>
      
      <assert test="matches(@xlink:href,'https?://(dx.doi.org|doi.org)/')" role="warning" id="event-self-uri-href-3"><name/> in event must have an xlink:href attribute containing a link to the preprint. Where possible this should be a doi. This one is not a doi - <value-of select="@xlink:href"/>. Please check whether there is a doi that can be used instead.</assert>
      
      <report test="@content-type='reviewed-preprint' and not(matches(@xlink:href,'^https://doi.org/10.7554/eLife.\d+\.[1-9]$'))" role="error" id="event-self-uri-href-4"><name/> in event has the attribute content-type="reviewed-preprint", but the xlink:href attribute does not contain an eLife version specific DOI - <value-of select="@xlink:href"/>.</report>
      
      <report test="(@content-type!='reviewed-preprint' or not(@content-type)) and matches(@xlink:href,'^https://doi.org/10.7554/eLife.\d+\.\d$')" role="error" id="event-self-uri-href-5"><name/> in event does not have the attribute content-type="reviewed-preprint", but the xlink:href attribute contains an eLife version specific DOI - <value-of select="@xlink:href"/>. If it's a preprint event, the link should be to a preprint. If it's an event for reviewed preprint publication, then it should have the attribute content-type!='reviewed-preprint'.</report>
	...
	<assert test="license" role="error" id="permissions-test-5">permissions must contain license.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#medicine-abstract-conformance" test="sec" role="warning" id="medicine-abstract-conformance">Medicine articles with a colon in their title should likely have a structured abstract. If there is no note in eJP about this, either the colon in the title is incorrect, or the abstract should be changed to a structured format.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#medicine-abstract-conformance-2" test="parent::article-meta/history/date[@date-type='received']/@iso-8601-date lt '2021-04-05'" role="warning" id="medicine-abstract-conformance-2"><value-of select="parent::article-meta/article-categories/subj-group[@subj-group-type='heading']/subject[. = ('Medicine','Epidemiology and Global Health')]"/> articles submitted after 4th April 2021 should have a structured abstract, but this one does not. eLife: please check this with Editorial if there are no related notes from eJP. Exeter: Please flag this to the eLife Production team.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#abstract-child-test-1" test="local-name() = $allowed-elems" role="error" id="abstract-child-test-1"><name/> is not allowed as a child of abstract.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#clintrial-conformance-1" test="($pos = 1) and (title != 'Background:')" role="error" id="clintrial-conformance-1">First section title is '<value-of select="title"/>' - but the only allowed value is 'Background:'.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#clintrial-conformance-4" test="($pos = 4) and (title != 'Conclusions:')" role="error" id="clintrial-conformance-4">Fourth section title is '<value-of select="title"/>' - but the only allowed value is 'Conclusions:'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#clintrial-conformance-5" test="($pos = 6) and (title != 'Clinical trial number:')" role="error" id="clintrial-conformance-5">Sixth section title is '<value-of select="title"/>' - but the only allowed value is 'Clinical trial number:'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#clintrial-conformance-6" test="($pos = 5) and (title != 'Funding:')" role="error" id="clintrial-conformance-6">Fifth section title is '<value-of select="title"/>' - but the only allowed value is 'Funding:'.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#clintrial-related-object-1" test="ancestor::sec[title = 'Clinical trial number:']" role="error" id="clintrial-related-object-1"><name/> in abstract must be placed in a section whose title is 'Clinical trial number:'</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#clintrial-related-object-2" test="@source-type='clinical-trials-registry'" role="error" id="clintrial-related-object-2"><name/> must have an @source-type='clinical-trials-registry'.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#clintrial-related-object-3" test="@source-id!=''" role="error" id="clintrial-related-object-3"><name/> must have an @source-id with a non-empty value.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#clintrial-related-object-4" test="@source-id-type='registry-name'" role="error" id="clintrial-related-object-4"><name/> must have an @source-id-type='registry-name'.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#clintrial-related-object-5" test="@document-id-type='clinical-trial-number'" role="error" id="clintrial-related-object-5"><name/> must have an @document-id-type='clinical-trial-number'.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#clintrial-related-object-6" test="@document-id[not(matches(.,'\p{Zs}'))]" role="error" id="clintrial-related-object-6"><name/> must have an @document-id with a value that does not contain a space character.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#clintrial-related-object-10" test="parent::p" role="error" id="clintrial-related-object-10"><name/> in abstract must be a child of a &lt;p&gt; element.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#clintrial-related-object-11" test="some $x in document($registries)/registries/registry satisfies ($x/subtitle/string()=@source-id)" role="warning" id="clintrial-related-object-11"><name/> @source-id value should almost always be one of the subtitles of the Crossref clinical trial registries. "<value-of select="@source-id"/>" is not one of the following <value-of select="string-join(for $x in document($registries)/registries/registry return concat('&quot;',$x/subtitle/string(),'&quot; (',$x/doi/string(),')'),', ')"/>. Is that correct?</assert>
      ...
      <report test="ends-with(@xlink:href,'.')" role="error" id="clintrial-related-object-14"><name/> has a @xlink:href attribute value which ends with a full stop, which is not correct - '<value-of select="@xlink:href"/>'.</report>

      <report test="ends-with(@document-id,'.')" role="error" id="clintrial-related-object-15"><name/> has an @document-id attribute value which ends with a full stop, which is not correct - '<value-of select="@document-id"/>'.</report>
    ...
    <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h5tdf-aff-test-1" test="parent::contrib-group//contrib//xref/@rid = @id" role="error" id="aff-test-1">aff elements that are direct children of contrib-group must have an xref in that contrib-group pointing to them.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hzqn1-final-auth-aff-test-2" test="addr-line[named-content[@content-type='city']]" role="error" id="final-auth-aff-test-2">Author affiliations must have a city. This one does not - <value-of select="$display"/></assert>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h60a2-final-auth-aff-test-3" test="institution[not(@*)] or institution-wrap[institution[not(@*)]]" role="error" id="final-auth-aff-test-3">Author affiliations (&lt;aff&gt;) must include an &gt;institution&gt; tag. This one (with the id <value-of select="@id"/>) does not - <value-of select="$display"/></assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hqr61-aff-institution-id-test-2" test="matches(.,'^https?://ror\.org/[a-z0-9]{9}$')" role="error" id="aff-institution-id-test-2">institution-id in aff must a value which is a valid ROR id. '<value-of select="."/>' is not a valid ROR id.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hrsm6-aff-child-conformity" test="name()=$allowed-elems" role="error" id="aff-child-conformity"><value-of select="name()"/> is not allowed as a child of &lt;aff&gt;.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h1gbm-addr-line-child-1" test="name()='named-content'" role="error" id="addr-line-child-1"><value-of select="name()"/> is not allowed as a child of &lt;addr-line&gt;.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hoyaj-addr-line-child-2" test="(name()='named-content') and not(@content-type='city')" role="error" id="addr-line-child-2"><value-of select="name()"/> in &lt;addr-line&gt; must have the attribute content-type="city". <value-of select="."/> does not.</report>
		...
		<report see="https://elifeproduction.slab.com/posts/funding-3sv64358#funding-group-test-2" test="count(award-group) = 0" role="warning" id="funding-group-test-2">There is no funding for this article. Is this correct?</report>
		
	  <report see="https://elifeproduction.slab.com/posts/funding-3sv64358#funding-group-test-3" test="(count(award-group) = 0) and (funding-statement!='No external funding was received for this work.')" role="warning" id="funding-group-test-3">Is this funding-statement correct? - '<value-of select="funding-statement"/>' Usually it should be 'No external funding was received for this work.'</report>
      ...
      <report test="$grant-matches" role="warning" id="known-grant-funder-grant-doi-test-1">Funding entry from <value-of select="funding-source/institution-wrap/institution"/> has an award-id (<value-of select="$award-id-elem"/>) which could potentially be replaced with a grant DOI. The following grant DOIs are possibilities: <value-of select="string-join(for $grant in $grant-matches return concat('https://doi.org/',$grant/@doi),'; ')"/>.</report>

      <assert test="$grant-matches" role="warning" id="known-grant-funder-grant-doi-test-2">Funding entry from <value-of select="funding-source/institution-wrap/institution"/> has an award-id (<value-of select="$award-id-elem"/>). The award id hasn't exactly matched the details of a known grant DOI, but the funder is known to mint grant DOIs (for example in the format <value-of select="$grants[1]/@doi"/> for ID <value-of select="$grants[1]/@award"/>). Does the award ID in the article contain a number/string within it that can be used to find a match here: https://api.crossref.org/works?filter=type:grant,award.number:[insert-grant-number]</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/funding-3sv64358#award-id-test-2" test="matches(.,'^\p{Zs}?[Nn][/]?[\.]?[Aa][.]?\p{Zs}?$')" role="error" id="award-id-test-2">Award id contains - <value-of select="."/> - This entry should be empty.</report>
      ...
      <assert test="@vocab='open-funder-registry'" role="error" id="institution-id-test-6"><name/> in funding must have a vocab="open-funder-registry" attribute. This one does not.</assert>
      
      <assert test="@vocab-identifier='10.13039/open-funder-registry'" role="error" id="institution-id-test-7"><name/> in funding must have a vocab-identifier="10.13039/open-funder-registry" attribute. This one does not.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/funding-3sv64358#par-test-1" test="normalize-space(.)='' and not(*)" role="error" id="par-test-1"><name/> cannot be empty.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#final-custom-meta-test-6" test="matches(.,'[\.|\?]$')" role="error" id="final-custom-meta-test-6">Impact statement must end with a full stop or question mark.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#custom-meta-child-test-1" test="local-name() = $allowed-elements" role="error" id="custom-meta-child-test-1"><name/> is not allowed in impact statement.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feat-custom-meta-test-1" test="child::*" role="error" id="feat-custom-meta-test-1"><value-of select="child::*[1]/name()"/> is not allowed in a Template type meta-value.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feat-custom-meta-test-info" test=". = ('1','2','3','4','5')" role="info" id="feat-custom-meta-test-info">Template <value-of select="."/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feat-custom-meta-test-3" test="($type='Insight') and (. != '1')" role="error" id="feat-custom-meta-test-3"><value-of select="$type"/> must be a template 1. Currently it is a template <value-of select="."/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feat-custom-meta-test-4" test="($type='Editorial') and (. != '2')" role="error" id="feat-custom-meta-test-4"><value-of select="$type"/> must be a template 2. Currently it is a template <value-of select="."/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feat-custom-meta-test-5" test="($type='Feature Article') and not(.=('3','4','5'))" role="error" id="feat-custom-meta-test-5"><value-of select="$type"/> must be a template 3, 4, or 5. Currently it is a template <value-of select="."/>.</report>
  ...
  <pattern id="volume-test-pattern"><rule context="article-meta/volume" id="volume-test">
      ...
      <report test=".='' or (. != (number($pub-date) - 2011))" role="error" id="volume-test-1">Journal volume is incorrect. It should be <value-of select="number($pub-date) - 2011"/>.</report>
    </rule></pattern>
  ...
  <pattern id="p-tests-pattern"><rule context="p" id="p-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/general-layout-and-formatting-wq0m31at#hbssk-p-test-7" test="matches(.,'^\p{Zs}?•') and not(ancestor::sub-article)" role="warning" id="p-test-7">p element starts with a bullet point. It is very likely that this should instead be captured as a list-item in a list[@list-type='bullet']. - <value-of select="."/></report>
      ...
      <assert test="if (ancestor::sec[@sec-type='data-availability']) then self::*/local-name() = ($allowed-p-blocks,'element-citation')  else self::*/local-name() = $allowed-p-blocks" role="error" id="allowed-p-test">p element cannot contain <value-of select="self::*/local-name()"/>. Only the following elements are allowed - <value-of select="string-join($allowed-p-blocks,', ')"/>.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hkftz-aff-xref-target-test" test="(@ref-type='aff') and ($target/local-name() != 'aff')" role="error" id="aff-xref-target-test">xref with @ref-type='<value-of select="@ref-type"/>' points to <value-of select="$target/local-name()"/>. This is not correct.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-target-test" test="(@ref-type='fig') and ($target/local-name() != 'fig')" role="error" id="fig-xref-target-test">xref with @ref-type='<value-of select="@ref-type"/>' points to <value-of select="$target/local-name()"/>. This is not correct.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supplementary-material-xref-target-test" test="(@ref-type='supplementary-material') and ($target/local-name() != 'supplementary-material')" role="error" id="supplementary-material-xref-target-test">xref with @ref-type='<value-of select="@ref-type"/>' points to <value-of select="$target/local-name()"/>. This is not correct.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#disp-formula-xref-target-test" test="(@ref-type='disp-formula') and ($target/local-name() != 'disp-formula')" role="error" id="disp-formula-xref-target-test">xref with @ref-type='<value-of select="@ref-type"/>' points to <value-of select="$target/local-name()"/>. This is not correct.</report>
      ...
      <report test="matches(.,'^[Dd][Oo][Ii]:|^[Dd][Oo][Ii]\p{Zs}')" role="warning" id="ext-link-child-test-4">ext-link text - <value-of select="."/> - appears to start with the string 'Doi:' or 'Doi ' (or similar), which is unnecessary.</report>
    ...
    <report test="not(ancestor::fig/permissions[contains(.,'phylopic')]) and matches(@xlink:href,'phylopic\.org')" role="warning" id="phylopic-link-check">This link is to phylopic.org, which is a site where silhouettes/images are typically reproduced from. Please check whether any figures contain reproduced images from this site, and if so whether permissions have been obtained and/or copyright statements are correctly included.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-group-child-test-1" test="local-name() = ('fig','media')" role="error" id="fig-group-child-test-1"><name/> is not allowed as a child of fig-group.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-group-child-test-2" test="(local-name() = 'media') and not(@mimetype='video')" role="error" id="fig-group-child-test-2"><name/> which is a child of fig-group, must have an @mimetype='video' - i.e. only video type media is allowed as a child of fig-group.</report>
      
    </rule></pattern><pattern id="fig-tests-pattern"><rule context="fig[not(ancestor::sub-article)]" id="fig-tests">
      <let name="article-type" value="ancestor::article/@article-type"/>
      
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-test-2" test="@position" role="error" id="fig-test-2">fig must have a @position.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#feat-fig-test-3" test="($article-type = $features-article-types) and not(label)" role="warning" id="feat-fig-test-3">fig doesn't have a label. Is this correct?</report>
      
      
      
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#final-fig-test-4" test="not($article-type = ('discussion',$notice-article-types)) and not(caption)" role="error" id="final-fig-test-4"><value-of select="label"/> has no title or caption (caption element).</report>
      
      
      
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#final-fig-test-5" test="not($article-type = ('discussion',$notice-article-types)) and not(caption/title)" role="error" id="final-fig-test-5">fig caption must have a title.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#ar-fig-test-2" test="if ($article-type = ($features-article-types,$notice-article-types)) then ()         else not(label)" role="error" id="ar-fig-test-2">Author Response fig must have a label.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#final-ar-fig-position-test" test="$no = string($pos)" role="error" id="final-ar-fig-position-test"><value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other AR images it is placed in position <value-of select="$pos"/>.</assert>
      ...
      <report test="contains(@mime-subtype,'tiff') and not(matches($file,'\.tif$|\.tiff$'))" role="error" id="graphic-test-1"><name/> has tif mime-subtype but filename does not end with '.tif' or '.tiff'. This cannot be correct.</report>
      
      <report test="contains(@mime-subtype,'postscript') and not(ends-with($file,'.eps'))" role="error" id="graphic-test-2"><name/> has postscript mime-subtype but filename does not end with '.eps'. This cannot be correct.</report>
      
      <report test="contains(@mime-subtype,'jpeg') and not(matches($file,'\.jpg$|\.jpeg$'))" role="error" id="graphic-test-3"><name/> has jpeg mime-subtype but filename does not end with '.jpg' or '.jpeg'. This cannot be correct.</report>
      ...
      <assert test="@mimetype=('image','application')" role="error" id="graphic-test-4"><name/> must have a @mimetype='image' or 'application'.</assert>
      
      <assert test="matches(@xlink:href,'\.[\p{L}\p{N}]{1,6}$')" role="error" id="graphic-test-5"><name/> must have an @xlink:href which contains a file reference.</assert>
      ...
      <report test="matches(label[1],'[Aa]nimation') and not(@mime-subtype='gif')" role="error" id="media-test-5"><value-of select="label"/> media with animation type label must have a @mime-subtype='gif'.</report>    
      ...
      <report test="not(ancestor::sub-article) and not(caption/title)" role="error" id="final-video-title"><value-of select="replace(label,'\.$,','')"/> does not have a title, which is incorrect.</report>
      
      <report test="ancestor::sub-article and not(caption/title)" role="warning" id="final-video-title-sa"><value-of select="replace(label,'\.$,','')"/> does not have a title, which is incorrect.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#supplementary-material-test-1" test="label" role="error" id="supplementary-material-test-1">supplementary-material must have a label.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#supplementary-material-test-2" test="not(matches(label[1],'Transparent reporting form|MDAR checklist')) and not(caption)" role="warning" id="supplementary-material-test-2"><value-of select="label"/> is missing a title/caption - is this correct? (supplementary-material should have a child caption.)</report>
      
      
      
      <report see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#final-supplementary-material-test-3" test="if (caption) then not(caption/title)         else ()" role="warning" id="final-supplementary-material-test-3"><value-of select="label"/> doesn't have a title. Is this correct?</report>
      
      		
      
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#final-supplementary-material-test-5" test="media" role="error" id="final-supplementary-material-test-5"><value-of select="label"/> is missing a file (supplementary-material must have a media).</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#supplementary-material-test-8" test="matches(label[1],'^Inclusion in global research form$|^MDAR checklist$|^Transparent reporting form$|^Supplementary file \d{1,4}\.$|^Source data \d{1,4}\.$|^Source code \d{1,4}\.$|^Reporting standard \d{1,4}\.$') and not(ancestor::sec[@sec-type='supplementary-material'])" role="error" id="supplementary-material-test-8"><value-of select="label"/> has an article level label but it is not captured in the additional files section - This must be incorrect.</report>
      
      <report see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#supplementary-material-test-9" test="count(media) gt 1" role="error" id="supplementary-material-test-9"><value-of select="label"/> has <value-of select="count(media)"/> media elements which is incorrect.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#source-code-test-1" test="($file = $code-files) and not(matches(label[1],'[Ss]ource code \d{1,4}\.$'))" role="warning" id="source-code-test-1"><value-of select="label"/> has a file which looks like code - <value-of select="$link"/>, but it's not labelled as code.</report>
      
      <report see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#supplementary-material-test-11" test="contains(lower-case(caption[1]/title[1]),'key resource')" role="warning" id="supplementary-material-test-11"><value-of select="if (self::*/label) then replace(label,'\.$','') else self::*/local-name()"/> has a title '<value-of select="caption[1]/title[1]"/>'. Is it a Key resources table? If so, it should be captured as a table in an appendix for the article.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#back-supplementary-file-position" test="string($pos) = $no" role="error" id="back-supplementary-file-position"><value-of select="replace(label,'\.$','')"/> id ends with <value-of select="$no"/>, but it is placed <value-of select="e:get-ordinal($pos)"/>. Either it is mislabelled, the id is incorrect, or it should be moved to a different position.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#back-source-data-position" test="string($pos) = $no" role="error" id="back-source-data-position"><value-of select="replace(label,'\.$','')"/> id ends with <value-of select="$no"/>, but it is placed <value-of select="e:get-ordinal($pos)"/>. Either it is mislabelled, the id is incorrect, or it should be moved to a different position.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#back-source-code-position" test="string($pos) = $no" role="error" id="back-source-code-position"><value-of select="replace(label,'\.$','')"/> id ends with <value-of select="$no"/>, but it is placed <value-of select="e:get-ordinal($pos)"/>. Either it is mislabelled, the id is incorrect, or it should be moved to a different position.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#fig-data-test-1" test="matches(label[1],'^Figure \d{1,4}—source data \d{1,4}|^Appendix \d{1,4}—figure \d{1,4}—source data \d{1,4}') and (count(descendant::xref[@ref-type='fig'])=1) and (descendant::xref[(@ref-type='fig') and contains(.,'upplement')])" role="warning" id="fig-data-test-1"><value-of select="label"/> is figure level source data, but contains 1 figure citation which is a link to a figure supplement - should it be figure supplement level source data?</report>
      
      <report see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#fig-code-test-1" test="matches(label[1],'^Figure \d{1,4}—source code \d{1,4}|^Appendix \d{1,4}—figure \d{1,4}—source code \d{1,4}') and (count(descendant::xref[@ref-type='fig'])=1) and (descendant::xref[(@ref-type='fig') and contains(.,'upplement')])" role="warning" id="fig-code-test-1"><value-of select="label"/> is figure level source code, but contains 1 figure citation which is a link to a figure supplement - should it be figure supplement level source code?</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#fig-data-test-2" test="$number = $pos" role="error" id="fig-data-test-2">'<value-of select="$label"/>' ends with <value-of select="$number"/>, but it is placed <value-of select="e:get-ordinal($pos)"/>. Either it is misnumbered or it should be moved to a different position.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#fig-code-id" test="@id=concat($fig-id,'scode',$pos)" role="error" id="fig-code-id">The id for figure level source code must be the id of its ancestor fig, followed by 'scode', followed by its position relative to other source data for the same figure. The id for <value-of select="$label"/>, '<value-of select="@id"/>' is not in this format. It should be '<value-of select="concat($fig-id,'scode',$pos)"/>' instead.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#fig-code-label" test="$label = concat($fig-label,'—source code ',$pos,'.')" role="error" id="fig-code-label">Figure source data label (<value-of select="$label"/>) is incorrect based on its position. Either it has been placed under the wrong figure or the label is incorrect. Should the label be <value-of select="concat($fig-label,'—source code ',$pos,'.')"/> instead?</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#vid-data-label" test="$label = concat($vid-label,'—source data ',$pos,'.')" role="error" id="vid-data-label">Video source data label (<value-of select="$label"/>) is incorrect based on its position. Either it has been placed under the wrong video or the label is incorrect. Should the label be <value-of select="concat($vid-label,'—source data ',$pos,'.')"/> instead?</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#vid-code-label" test="$label = concat($vid-label,'—source code ',$pos,'.')" role="error" id="vid-code-label">Video source code label (<value-of select="$label"/>) is incorrect based on its position. Either it has been placed under the wrong video or the label is incorrect. Should the label be <value-of select="concat($vid-label,'—source code ',$pos,'.')"/> instead?</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#table-data-test-2" test="$number = $pos" role="error" id="table-data-test-2">'<value-of select="$label"/>' ends with <value-of select="$number"/>, but it is placed <value-of select="e:get-ordinal($pos)"/>. Either it is misnumbered or it should be moved to a different position.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#table-data-id" test="@id=concat($table-id,'sdata',$pos)" role="error" id="table-data-id">The id for table level source data must be the id of its ancestor table-wrap, followed by 'sdata', followed by its position relative to other source data for the same table. The id for <value-of select="$label"/>, '<value-of select="@id"/>' is not in this format. It should be '<value-of select="concat($table-id,'sdata',$pos)"/>' instead.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#table-code-test-2" test="$number = $pos" role="error" id="table-code-test-2">'<value-of select="$label"/>' ends with <value-of select="$number"/>, but it is placed <value-of select="e:get-ordinal($pos)"/>. Either it is misnumbered or it should be moved to a different position.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#table-code-id" test="@id=concat($table-id,'scode',$pos)" role="error" id="table-code-id">The id for table level source code must be the id of its ancestor table, followed by 'scode', followed by its position relative to other source data for the same table. The id for <value-of select="$label"/>, '<value-of select="@id"/>' is not in this format. It should be '<value-of select="concat($table-id,'scode',$pos)"/>' instead.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#table-code-label" test="$label = concat($table-label,'—source code ',$pos,'.')" role="error" id="table-code-label">Table source code label (<value-of select="$label"/>) is incorrect based on its position. Either it has been placed under the wrong table or the label is incorrect. Should the label be <value-of select="concat($table-label,'—source code ',$pos,'.')"/> instead?</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#math-test-14" test="not(matches($data,'^±$|^±[\d]+$|^±[\d]+\.[\d]+$|^×$|^~$|^~[\d]+$|^~[\d]+\.[\d]+$|^%[\d]+$|^%[\d]+\.[\d]+$|^%$|^±\d+%$|^+\d+%$|^-\d+%$|^\d+%$|^±\d+$|^+\d+$|^-\d+$')) and ($children='')" role="warning" id="math-test-14">mml:math only contains numbers and/or operators - '<value-of select="$data"/>'. Is it necessary for this to be set as a formula, or can it be captured with as normal text instead?</report>
      
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#math-test-3" test="$data = '±'" role="error" id="math-test-3">mml:math only contains '±', which is unnecessary. Capture this as a normal text '±' instead.</report>
      
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#math-test-4" test="matches($data,'^±[\d]+$|^±[\d]+\.[\d]+$')" role="error" id="math-test-4">mml:math only contains '±' followed by digits, which is unnecessary. Capture this as a normal text instead.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#math-test-6" test="$data = '~'" role="error" id="math-test-6">mml:math only contains '~', which is unnecessary. Capture this as a normal text '~' instead.</report>
      
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#math-test-7" test="matches($data,'^~[\d]+$|^~[\d]+\.[\d]+$')" role="error" id="math-test-7">mml:math only contains '~' and digits, which is unnecessary. Capture this as a normal text instead.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#math-test-11" test="contains($data,'○')" role="warning" id="math-test-11">mml:math contains '○' (the white circle symbol). Should this be the degree symbol instead - '°', or '∘' (the ring operator symbol)?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#math-test-16" test="matches($data,'^H\-$|^Cl\-$|^Br\-$|^I\-$|^OH\-$|^NO3\-$|^NO2\-$|^HCO3\-$|^HSO4\-$|^CN\-$|^MnO4\-$|^ClO[3]?\-$|^O2\-$|^S2\-$|^SO42\-$|^SO32\-$|^S2O32\-$|^SiO32\-$|^CO32\-$|^CrO42\-$|^Cr2O72\-$|^N3\-$|^P3\-$|^PO43\-$')" role="warning" id="math-test-16">mml:math seems to only contain the formula for an anion - '<value-of select="."/>' - which is likely unnecessary. Should this be captured as normal text instead?</report>
      ...
      <report test="matches(.,'\p{Zs}\p{Zs}\p{Zs}\p{Zs}+$') and not(matches(.,'\s\s\s\s+$'))" role="error" id="math-test-20"><value-of select="parent::*/name()"/> ends with 4 or more spaces. These types of spaces may cause the equation to break over numerous lines in the HTML or shift the equation to the left. Please ensure they are removed.</report>
      
      <report test="matches(.,'^\p{Zs}\p{Zs}\p{Zs}\p{Zs}+') and not(matches(.,'^\s\s\s\s+'))" role="error" id="math-test-21"><value-of select="parent::*/name()"/> starts with 4 or more spaces. These types of spaces may cause the equation to break over numerous lines in the HTML or shift the equation to the right. Please ensure they are removed.</report>
      ...
      <report test="*[1][matches(.,'^\p{Z}*$')]" role="error" id="math-empty-base-check"><name/> element must not have a missing or empty base expression.</report>

      <report test="*[2][matches(.,'^\p{Z}*$')]" role="error" id="math-empty-script-check"><name/> element must not have a missing or empty <value-of select="$script-name"/> expression.</report>

      <report test="local-name()=('msubsup','munderover') and *[3][matches(.,'^\p{Z}*$')]" role="error" id="math-empty-second-script-check"><name/> element must not have a missing or empty <value-of select="if (local-name()='msubsup') then 'superscript' else 'overscript'"/> expression.</report>
      ...
      <assert test="count(*) ge 3" role="error" id="math-multiscripts-check-1"><name/> element must at least 3 child elements. This one has <value-of select="count(*)"/>.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#table-wrap-test-1" test="table" role="error" id="table-wrap-test-1">table-wrap must have one table.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#table-wrap-test-4" test="(matches($id,'^table[0-9]{1,3}$')) and (normalize-space($lab) = '')" role="error" id="table-wrap-test-4">table-wrap with id <value-of select="$id"/> has no label which is not correct.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#final-table-title-test-1" test="caption/title" role="error" id="final-table-title-test-1"><value-of select="replace(label[1],'\.$','')"/> does not have a title. Please ensure to query the authors for one.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#table-title-test-4" test="ends-with(.,' vs.')" role="warning" id="table-title-test-4">title for <value-of select="replace(ancestor::table-wrap[1]/label[1],'\.$','')"/> ends with 'vs.', which indicates that the title sentence may be split across title and caption - <value-of select="."/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#table-title-test-5" test="string-length(.) gt 250" role="warning" id="table-title-test-5">title for <value-of select="replace(ancestor::table-wrap[1]/label[1],'\.$','')"/> is longer than 250 characters. Is it a caption instead?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#kr-table-header-4" test="tr[1]/th[1] and (normalize-space(tr[1]/th[1]) != 'Reagent type (species) or resource')" role="warning" id="kr-table-header-4">The first column header in a Key resources table is usually 'Reagent type (species) or resource' but this one has '<value-of select="tr[1]/th[1]"/>'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#kr-table-header-5" test="tr[1]/th[2] and (normalize-space(tr[1]/th[2]) != 'Designation')" role="warning" id="kr-table-header-5">The second column header in a Key resources table is usually 'Designation' but this one has '<value-of select="tr[1]/th[2]"/>'.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#kr-table-header-7" test="tr[1]/th[4] and (normalize-space(tr[1]/th[4]) != 'Identifiers')" role="warning" id="kr-table-header-7">The fourth column header in a Key resources table is usually 'Identifiers' but this one has '<value-of select="tr[1]/th[4]"/>'.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#body-table-label-test-1" test="matches(.,'^Table \d{1,4}\.$|^Key resources table$|^Author response table \d{1,4}\.$|^Decision letter table \d{1,4}\.$|^Review table \d{1,4}\.$')" role="error" id="body-table-label-test-1"><value-of select="."/> - Table label does not conform to the usual format.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#app-table-label-test-1" test="matches(.,'^Appendix \d{1,4}—table \d{1,4}\.$|^Appendix \d{1,4}—key resources table$')" role="error" id="app-table-label-test-1"><value-of select="."/> - Table label does not conform to the usual format.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#app-table-label-test-2" test="starts-with(.,$app)" role="error" id="app-table-label-test-2"><value-of select="."/> - Table label does not begin with the title of the appendix it sits in. Either the table is in the incorrect appendix or the table has been labelled incorrectly.</assert>
      
    </rule></pattern><pattern id="table-tests-pattern"><rule context="table" id="table-tests">
      
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#table-test-1" test="count(tbody) = 0" role="error" id="table-test-1">table must have at least one body (tbody).</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#thead-test-1" test="count(tr) = 0" role="error" id="thead-test-1">thead must have at least one row (tr).</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#tr-test-2" test="th and (parent::tbody)" role="warning" id="tr-test-2">table row in body contains a th element (a header). Please check that this is correct.</report>
      
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#tr-test-3" test="td and (parent::thead)" role="error" id="tr-test-3">table row in header contains a td element (table data), which is not allowed. Only th elements (table heading cells) are allowed in a row in the table header.</report>
      ...
      <report test="($type='order') and matches(.,'^\p{Zs}?\d+')" role="warning" id="order-test-1">list-item is part of an ordered list, but it begins with a number. Is this correct? <value-of select="."/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/general-layout-and-formatting-wq0m31at#hd4i0-list-item-test-1" test="matches(.,'^\p{Zs}?\p{Ll}[\p{Zs}\)\.]')" role="warning" id="list-item-test-1">list-item begins with a single lowercase letter, is this correct? - <value-of select="."/></report>
      ...
      <report test="($xrefs//*:match) and ($sec-id != $sec1/@id)" role="error" id="final-video-placement-1"><value-of select="$label"/> does not appear in the same section as where it is first cited (sec with title '<value-of select="$sec1/title"/>'), which is incorrect.</report>
      
      <report test="($xref-sib = 'p') and ($xref1//following::media/@id = $id)" role="warning" id="video-placement-2"><value-of select="$label"/> appears after its first citation but not directly after its first citation. Is this correct?</report>
      
    </rule></pattern><pattern id="code-tests-pattern"><rule context="code" id="code-tests">
      
      <report see="https://elifeproduction.slab.com/posts/code-blocks-947pcamv#code-child-test" test="child::*" role="error" id="code-child-test">code contains a child element, which will display in HTML with its tagging, i.e. '&lt;<value-of select="child::*[1]/name()"/><value-of select="if (child::*[1]/@*) then for $x in child::*[1]/@* return concat(' ',$x/name(),'=&quot;',$x/string(),'&quot;') else ()"/>&gt;<value-of select="child::*[1]"/>&lt;/<value-of select="child::*[1]/name()"/>&gt;'. Strip any child elements.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/code-blocks-947pcamv#code-sibling-test" test="normalize-space(preceding-sibling::text()[preceding-sibling::*[1]/local-name()='code'][1])=''" role="warning" id="code-sibling-test">code element (containing the content <value-of select="."/>) is directly preceded by another code element (containing the content <value-of select="preceding::*[1]"/>). If the content is part of the same code block, then it should be captured using only 1 code element and line breaks added in the xml. If these are separate code blocks (uncommon, but possible), then this markup is fine.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#label-fig-group-conformance-1" test="not(ancestor::fig-group) and parent::fig[@specific-use='child-fig']" role="error" id="label-fig-group-conformance-1"><value-of select="$label"/> is not placed in a &lt;fig-group&gt; element, which is incorrect. Either the label needs updating, or it needs moving into the &lt;fig-group&gt;.</report>
      
      <report test="not(ancestor::fig-group) and parent::media and matches(.,'[Ff]igure')" role="error" id="label-fig-group-conformance-2"><value-of select="$label"/> contains the string 'Figure' but it's not placed in a &lt;fig-group&gt; element, which is incorrect. Either the label needs updating, or it needs moving into the &lt;fig-group&gt;.</report>
      ...
      <report test="ancestor::sub-article[not(@article-type=('reply','author-comment'))]" role="warning" id="disp-quote-test-1">Content is tagged as a display quote, which is almost definitely incorrect, since it's within peer review material that is not an author response - <value-of select="."/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h29wm-bracket-test-1" test="$open-curly gt $close-curly" role="warning" id="bracket-test-1"><name/> element contains more left '(' than right ')' parentheses (<value-of select="$open-curly"/> and <value-of select="$close-curly"/> respectively). Is that correct? Possibly troublesome section(s) are <value-of select="string-join(for $sentence in tokenize(.,'\. ') return if (string-length(replace($sentence,'[^\(]','')) gt string-length(replace($sentence,'[^\)]',''))) then $sentence else (),' ---- ')"/></report>
      
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h29wm-bracket-test-2" test="not(matches(.,'^\p{Zs}?(\d+|[A-Za-z]|[Ii]?[Xx]|[Ii]?[Vv]|[Vv]?[Ii]{1,3})\)')) and ($open-curly lt $close-curly)" role="warning" id="bracket-test-2"><name/> element contains more right ')' than left '(' parentheses (<value-of select="$close-curly"/> and <value-of select="$open-curly"/> respectively). Is that correct? Possibly troublesome section(s) are <value-of select="string-join(for $sentence in tokenize(.,'\. ') return if (string-length(replace($sentence,'[^\(]','')) lt string-length(replace($sentence,'[^\)]',''))) then $sentence else (),' ---- ')"/></report>
      ...
      <assert test="matches(label[1],'^Appendix \d{1,2}—box \d{1,2}\.$')" role="error" id="app-box-label-test"><name/> element must have a label in the format "Appendix 0—box 0.".</assert>
      
      <assert test="starts-with(label[1],$app-title)" role="error" id="app-box-label-test-2"><name/> label must start with the title for the appendix it sits in, <value-of select="$app-title"/>. This one does not - "<value-of select="label[1]"/>".</assert>
      ...
      <report test="count(descendant::p) = (0,1)" role="warning" id="app-little-content"><value-of select="title"/> has no sibling appendices, contains no assets (figures, tables, videos, or display formula), and only has one paragraph. Does it need to be an appendix?</report>
      ...
      <assert test="$parent=$allowed-parents" role="error" id="attrib-parent">attrib is a child of <value-of select="$parent"/>, which is not allowed. It can only be a child of the following elements: <value-of select="string-join($allowed-parents,'; ')"/></assert>
      ...
      <assert test="name()=$allowed-children" role="error" id="attrib-child"><value-of select="name()"/> is not permitted as a child of attrib. Only the following elements are: <value-of select="string-join($allowed-children,'; ')"/></assert>
  ...
  <pattern id="body-video-specific-pattern"><rule context="article[not(@article-type = $notice-article-types)]/body//media[@mimetype='video']" id="body-video-specific">
      ...
      <report test="not(ancestor::fig-group) and (matches(label[1],'[Vv]ideo')) and ($no != string($pos))" role="error" id="final-body-video-position-test-1"><value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other videos it is placed in position <value-of select="$pos"/>.</report>
      
      <assert test="starts-with(label[1],$fig-label)" role="error" id="fig-video-label-test"><value-of select="label"/> does not begin with its parent figure label - <value-of select="$fig-label"/> - which is incorrect.</assert>
      
      <report test="(ancestor::fig-group) and ($no != string($fig-pos))" role="error" id="fig-video-position-test"><value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other fig-level videos it is placed in position <value-of select="$fig-pos"/>.</report>
      
      <report test="(not(ancestor::fig-group)) and (descendant::xref[@ref-type='fig'][contains(.,'igure') and not(contains(.,'supplement'))])" role="warning" id="fig-video-check-1"><value-of select="label"/> contains a link to <value-of select="descendant::xref[@ref-type='fig'][contains(.,'igure') and not(contains(.,'supplement'))][1]"/>, but it is not a captured as a child of that fig. Should it be captured as <value-of select="concat(descendant::xref[@ref-type='fig'][contains(.,'igure') and not(contains(.,'supplement'))][1],'—video x')"/> instead?</report>
      ...
      <assert test="$no = string($pos)" role="error" id="final-app-video-position-test"><value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other AR videos it is placed in position <value-of select="$pos"/>.</assert>
      ...
      <assert test="$no = string($pos)" role="error" id="final-app-fig-video-position-test"><value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other AR videos it is placed in position <value-of select="$pos"/>.</assert>
    </rule></pattern><pattern id="fig-video-specific-pattern"><rule context="fig-group/media[@mimetype='video']" id="fig-video-specific">
      
      <report test="following-sibling::fig" role="error" id="fig-video-position-test-2"><value-of select="replace(label,'\.$','')"/> is placed before <value-of select="following-sibling::fig[1]/label[1]"/> Figure level videos should always be placed after figures and figure supplements in their figure group.</report>
      ...
      <assert test="$no = string($pos)" role="error" id="final-dl-video-position-test"><value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other DL videos it is placed in position <value-of select="$pos"/>.</assert>
      ...
      <assert test="$no = string($pos)" role="error" id="final-ar-video-position-test"><value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other AR videos it is placed in position <value-of select="$pos"/>.</assert>
  ...
  <pattern id="fig-specific-tests-pattern"><rule context="article/body//fig[not(@specific-use='child-fig')][not(ancestor::boxed-text)]" id="fig-specific-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-specific-test-6" test="($fol-sib/local-name() = 'p') and ($fol-sib/*/local-name() = 'disp-formula') and (count($fol-sib/*[1]/preceding-sibling::text()) = 0) and (not(matches($pre-sib,'\.\p{Zs}*?$|\?\p{Zs}*?$|!\p{Zs}*?$')))" role="warning" id="fig-specific-test-6"><value-of select="$lab"/> is immediately followed by a display formula, and preceded by a paragraph which does not end with punctuation. Should it should be moved after the display formula or after the para following the display formula?</report>
      
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-specific-test-5" test="($fol-sib/local-name() = 'disp-formula') and (not(matches($pre-sib,'\.\p{Zs}*?$|\?\p{Zs}*?$|!\p{Zs}*?$')))" role="warning" id="fig-specific-test-5"><value-of select="$lab"/> is immediately followed by a display formula, and preceded by a paragraph which does not end with punctuation. Should it should be moved after the display formula or after the para following the display formula?</report>
      
      <report test="not($article-type = $notice-article-types) and ancestor::article//xref[(ancestor::caption or ancestor::table-wrap) and @rid = $id] and not(ancestor::article//xref[(@rid = $id) and not(ancestor::caption) and not(ancestor::table-wrap)])" role="warning" id="fig-specific-test-7"><value-of select="$lab"/> is only cited in a table or the caption of an object. Please ask the authors for citation in the main text.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-sup-test-1" test="parent::fig-group" role="error" id="fig-sup-test-1">fig supplement is not a child of fig-group. This cannot be correct.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-sup-test-3" test="starts-with(label[1],concat('Figure ',$parent-fig-no))" role="error" id="fig-sup-test-3"><value-of select="label"/> does not start with the main figure number it is associated with - <value-of select="concat('Figure ',$parent-fig-no)"/>.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-sup-test-4" test="if ($article-type = $notice-article-types) then ()         else $no != string($pos)" role="error" id="fig-sup-test-4"><value-of select="label"/> does not appear in sequence which is incorrect. Relative to the other figures it is placed in position <value-of select="$pos"/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-sup-test-5" test="if ($article-type = $notice-article-types) then ()         else ($label-conforms and ($label-no != string($pos)))" role="error" id="fig-sup-test-5"><value-of select="label"/> is in position <value-of select="$pos"/>, which means either the label or the placement incorrect.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#dec-fig-test-1" test="label" role="error" id="dec-fig-test-1">fig must have a label.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#dec-fig-test-2" test="matches(label[1],'^Decision letter image [0-9]{1,3}\.$')" role="error" id="dec-fig-test-2">fig label in author response must be in the format 'Decision letter image 1.'.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#app-fig-sup-test-1" test="matches(.,'^Appendix \d{1,4}—figure \d{1,4}—figure supplement \d{1,4}\.$|^Appendix—figure \d{1,4}—figure supplement \d{1,4}\.$')" role="error" id="app-fig-sup-test-1">label for fig inside appendix must be in the format 'Appendix 1—figure 1—figure supplement 1.'.</assert>
      ...
      <report test="if ($count = 0) then ()         else if (not(matches($id,'^app[0-9]{1,3}fig[0-9]{1,3}$'))) then ()         else $no != string($pos)" role="error" id="final-app-fig-pos-test"><value-of select="replace(label[1],'\.','')"/> does not appear in sequence which is incorrect. Relative to the other figures in the same appendix it is placed in position <value-of select="$pos"/>.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/licensing-and-copyright-rqdavyty#fig-permissions-test-2" test="copyright-year and (not(copyright-statement) or not(copyright-holder))" role="error" id="fig-permissions-test-2">permissions for <value-of select="$label"/> has a copyright-year, but not a copyright-statement or copyright-holder which is incorrect.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/licensing-and-copyright-rqdavyty#fig-permissions-test-4" test="license/license-p" role="error" id="fig-permissions-test-4">permissions for <value-of select="$label"/> must contain a license-p element.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/licensing-and-copyright-rqdavyty#fig-permissions-test-11" test="copyright-statement or license" role="error" id="fig-permissions-test-11">Asset level permissions must either have a &lt;copyright-statement&gt; and/or a &lt;license&gt; element, but those for <value-of select="$label"/> have neither.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/licensing-and-copyright-rqdavyty#permissions-notification" test="." role="info" id="permissions-notification"><value-of select="$label"/> has permissions - '<value-of select="if (license/license-p) then license/license-p else if (copyright-statement) then copyright-statement else ()"/>'.</report>
      
      <assert see="https://elifeproduction.slab.com/posts/licensing-and-copyright-rqdavyty#permissions-parent" test="parent::*/local-name() = ('fig', 'media', 'table-wrap', 'boxed-text', 'supplementary-material')" role="error" id="permissions-parent">permissions is not allowed as a child of <value-of select="parent::*/local-name()"/></assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/licensing-and-copyright-rqdavyty#fig-permissions-test-12" test="matches(license[1]/license-p[1],'[Ff]urther reproduction of (this|these) (panels?|illustrations?) would need permission from the copyright holder\.$|[Ff]urther reproduction of this figure would (need|require) permission from the copyright holder\.$')" role="warning" id="fig-permissions-test-12"><value-of select="$label"/> permissions - the &lt;license-p&gt; for all rights reserved type permissions should usually end with 'further reproduction of this panel/illustration/figure would need permission from the copyright holder.', but <value-of select="$label"/>'s doesn't. Is this correct? (There is no 'https://creativecommons.org/' type link on the license element or in an ali:license_ref so presumed ARR.)</assert>
      
      <report see="https://elifeproduction.slab.com/posts/licensing-and-copyright-rqdavyty#fig-permissions-test-13" test="license//ext-link[contains(@xlink:href,'creativecommons.org')]" role="warning" id="fig-permissions-test-13"><value-of select="$label"/> permissions - the &lt;license-p&gt; contains a CC link, but the license does not have an ali:license_ref element, which is very likely incorrect.</report>
  ...
  <pattern id="ra-body-tests-pattern"><rule context="article[@article-type='research-article']/body" id="ra-body-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#ra-sec-test-2" test="if ($type = ('Short Report','Scientific Correspondence','Feature Article')) then ()         else count(sec[@sec-type='intro']) != 1" role="warning" id="ra-sec-test-2"><value-of select="$type"/> doesn't have child sec[@sec-type='intro'] in the main body. Is this correct?</report>
      
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#ra-sec-test-3" test="if ($type = ('Short Report','Scientific Correspondence','Feature Article')) then ()         else $method-count != 1" role="warning" id="ra-sec-test-3">main body in <value-of select="$type"/> content doesn't have a child sec with @sec-type whose value is either 'materials|methods', 'methods' or 'model'. Is this correct?.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#medicine-methods" test="$pos=2 and not(title[1]=('Methods','Materials and methods'))" role="warning" id="medicine-methods">The second top level section in a Medicine article should be 'Methods' or 'Materials and methods', but this one is '<value-of select="title[1]"/>'. Is that correct?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#medicine-discussion" test="$pos=4 and title[1]!='Discussion'" role="warning" id="medicine-discussion">The fourth top level section in a Medicine article should be 'Discussion', but this one is '<value-of select="title[1]"/>'. Is that correct?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#conclusion-test-2" test="matches($title,'conclusions?')" role="warning" id="conclusion-test-2">Level <value-of select="count(ancestor::sec) + 1"/> section with the title '<value-of select="."/>' should likely be made a level 2 section.</report>
  ...
  <pattern id="article-title-tests-pattern"><rule context="article-meta//article-title" id="article-title-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-title-test-1" test="matches(.,'^\([A-Za-z]|^[A-Za-z]\)')" role="warning" id="fig-title-test-1">'<value-of select="$label"/>' appears to have a title which is the beginning of a caption. Is this correct?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-title-test-5" test="matches(.,'^\p{P}')" role="warning" id="fig-title-test-5">title for <value-of select="$label"/> begins with punctuation. Is this correct? - <value-of select="."/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-title-test-7" test="string-length(.) gt 250" role="warning" id="fig-title-test-7">title for <value-of select="$label"/> is longer than 250 characters. Is it a caption instead?</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#supplementary-material-title-test-2" test="matches(.,'[\.\?]$')" role="error" id="supplementary-material-title-test-2">title for <value-of select="$label"/> must end with a full stop.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#supplementary-material-title-test-4" test="matches(.,'^\p{Zs}')" role="error" id="supplementary-material-title-test-4">title for <value-of select="$label"/> begins with a space, which is not allowed.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#supplementary-material-title-test-8" test="$sentence-count gt 1" role="warning" id="supplementary-material-title-test-8">title for <value-of select="$label"/> contains <value-of select="$sentence-count"/> sentences. Should the sentence(s) after the first be moved into the caption? Or is the title itself a caption (in which case, please ask the authors for a title)?</report>
      ...
      <report test="matches(.,'^\p{Zs}')" role="error" id="video-title-test-4">title for <value-of select="$label"/> begins with a space, which is not allowed.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#ref-list-title-test" test="title = 'References'" role="warning" id="ref-list-title-test">reference list usually has a title that is 'References', but currently it is '<value-of select="title"/>' - is that correct?</assert>
      ...
      <assert test="parent::back or parent::sec[@sec-type='additional-information' and parent::back]" role="error" id="fn-parent">fn-group is only allowed as a child of back or as a child of sec[@sec-type='additional-information'] (which in turn is in back). This one is placed as a child of <value-of select="parent::*/name()"/></assert>
      ...
      <assert test="article-title = (&quot;Editor's evaluation&quot;,'eLife assessment','eLife Assessment')" role="error" id="ed-eval-title-test">A sub-article[@article-type='editor-report'] must have the title "eLife Assessment" or "Editor's evaluation". Currently it is <value-of select="article-title"/>.</assert>
      ...
      <assert test="name()=$allowed-elems" role="error" id="title-child-conformance"><name/> is not allowed in a title element. The only permitted elements are <value-of select="string-join($allowed-elems,', ')"/>.</assert>
  ...
  <pattern id="award-group-ids-pattern"><rule context="funding-group/award-group" id="award-group-ids">
      ...
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-id-test-1" test="matches(@id,'^fig[0-9]{1,3}$|^C[0-9]{1,3}$|^S[0-9]{1,3}$')" role="error" id="fig-id-test-1">fig must have an @id in the format fig0 (or C0 for chemical structures, or S0 for Schemes). <value-of select="@id"/> does not conform to this.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#fig-sup-id-test" test="matches(@id,'^fig[0-9]{1,3}s[0-9]{1,3}$')" role="error" id="fig-sup-id-test">figure supplement must have an @id in the format fig0s0. <value-of select="@id"/> does not conform to this.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#resp-fig-id-test" test="matches(@id,'^respfig[0-9]{1,3}$|^sa[0-9]fig[0-9]{1,3}$')" role="error" id="resp-fig-id-test">fig in decision letter/author response must have @id in the format respfig0, or sa0fig0. <value-of select="@id"/> does not conform to this.</assert>
    </rule></pattern><pattern id="rep-fig-sup-ids-pattern"><rule context="sub-article//fig[@specific-use='child-fig']" id="rep-fig-sup-ids">
      
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#resp-fig-sup-id-test" test="matches(@id,'^respfig[0-9]{1,3}s[0-9]{1,3}$|^sa[0-9]{1}fig[0-9]{1,3}s[0-9]{1,3}$')" role="error" id="resp-fig-sup-id-test">figure supplement in decision letter/author response must have @id in the format respfig0s0 or sa0fig0s0. <value-of select="@id"/> does not conform to this.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#disp-formula-id-test" test="not(ancestor::sub-article) and not(matches(@id,'^equ[0-9]{1,9}$'))" role="error" id="disp-formula-id-test">disp-formula @id must be in the format 'equ0'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#sub-disp-formula-id-test" test="(ancestor::sub-article) and not(matches(@id,'^sa[0-9]equ[0-9]{1,9}$|^equ[0-9]{1,9}$'))" role="error" id="sub-disp-formula-id-test">disp-formula @id must be in the format 'sa0equ0' when in a sub-article.  <value-of select="@id"/> does not conform to this.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#sub-mml-math-id-test" test="(ancestor::sub-article) and not(matches(@id,'^sa[0-9]m[0-9]{1,9}$|^m[0-9]{1,9}$'))" role="error" id="sub-mml-math-id-test">mml:math @id in disp-formula must be in the format 'sa0m0'.  <value-of select="@id"/> does not conform to this.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#resp-table-wrap-id-test" test="if (label) then matches(@id, '^resptable[0-9]{1,3}$|^sa[0-9]table[0-9]{1,3}$')         else matches(@id, '^respinlinetable[0-9]{1,3}$||^sa[0-9]inlinetable[0-9]{1,3}$')" role="warning" id="resp-table-wrap-id-test">table-wrap @id in a sub-article must be in the format 'resptable0' or 'sa0table0' if it has a label, or in the format 'respinlinetable0' or 'sa0inlinetable0' if it does not.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#table-wrap-id-test" test="if (label = 'Key resources table') then @id='keyresource'         else if (label) then matches(@id, '^table[0-9]{1,3}$')         else matches(@id, '^inlinetable[0-9]{1,3}$')" role="error" id="table-wrap-id-test">table-wrap @id must be in the format 'table0', unless it doesn't have a label, in which case it must be 'inlinetable0' or it is the key resource table which must be 'keyresource'.</assert>
  ...
  <pattern id="fig-children-pattern"><rule context="fig/*" id="fig-children">
      ...
      <assert test="local-name() = $allowed-children" role="error" id="fig-child-conformance"><name/> is not allowed as a child of fig.</assert>
      ...
      <assert test="local-name() = $allowed-children" role="error" id="table-wrap-child-conformance"><name/> is not allowed as a child of table-wrap.</assert>
      ...
      <assert test="local-name() = $allowed-children" role="error" id="media-child-conformance"><name/> is not allowed as a child of media.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#supplementary-material-child-conformance" test="local-name() = $allowed-children" role="error" id="supplementary-material-child-conformance"><name/> is not allowed as a child of supplementary-material.</assert>
    </rule></pattern><pattern id="author-notes-children-pattern"><rule context="author-notes/*" id="author-notes-children">
      
      <assert test="local-name() = 'fn'" role="error" id="author-notes-child-conformance"><name/> is not allowed as a child of author-notes.</assert>
    </rule></pattern>
  ...
  <pattern id="back-tests-pattern"><rule context="back" id="back-tests">
      ...
      <report test="($article-type = ('article-commentary', 'editorial', 'book-review', 'discussion')) and sec[@sec-type='additional-information']" role="error" id="back-test-11"><value-of select="$article-type"/> type articles cannot contain additional information sections (sec[@sec-type="additional-information"]).</report>
      ...
      <report test="count(p[matches(.,'^The following previously published datasets? w(as|ere) used:\s?$')]) gt 1" role="error" id="data-used-p-presence">Data availabilty section contains more than one p element describing that previously published datasets were used. Either the content of one (or more) is incorrect, or the duplicated sentence needs removing.</report>
      ...
      <report test="$article-type=$non-contribs and fn-group[@content-type='author-contribution']" role="error" id="additional-info-test-4"><value-of select="$article-type"/> type articles should not contain author contributions.</report>
      ...
      <assert test="name()=('title','supplementary-material')" role="error" id="add-files-3"><value-of select="name()"/> is not allowed as a child element in the additional files section (sec[@sec-type='supplementary-material']).</assert>
      ...
      <report test="preceding::fn[lower-case(.)=$lower-case]" role="error" id="comp-int-fn-test-5">Competing interests footnotes must be distinct. This one (with the id <value-of select="@id"/>) is the same as another one (with the id <value-of select="string-join(preceding::fn[lower-case(.)=$lower-case]/@id,'; ')"/>): <value-of select="."/></report>
      ...
      <report test="matches(.,'\.\s*$')" role="error" id="auth-cont-fn-test-2">Author contribution must not end with a full stop. This one does: <value-of select="."/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/ethics-se0ia1cs#ethics-test-3" test="count(fn) = 0" role="error" id="ethics-test-3">Ethics fn-group must have at least one fn element.</report>
  ...
  <pattern id="dec-letter-reply-tests-pattern"><rule context="article/sub-article" id="dec-letter-reply-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-reply-test-3" test="count(front-stub) = 1" role="error" id="dec-letter-reply-test-3">sub-article must contain one and only one front-stub.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-reply-test-4" test="count(body) = 1" role="error" id="dec-letter-reply-test-4">sub-article must contain one and only one body.</assert>
      
      <report test="not($is-prc) and @article-type='referee-report'" role="error" id="sub-article-1">'<value-of select="@article-type"/>' is not permitted as the article-type for a sub-article in a non-PRC article. Provided this is in fact a non-PRC article, the article-type should be 'decision-letter'.</report>
      
      <report test="not($is-prc) and @article-type='author-comment'" role="error" id="sub-article-2">'<value-of select="@article-type"/>' is not permitted as the article-type for a sub-article in a non-PRC article. Provided this is in fact a non-PRC article, the article-type should be 'response'.</report>
      
      <report test="$is-prc and @article-type='decision-letter'" role="error" id="sub-article-3">'<value-of select="@article-type"/>' is not permitted as the article-type for a sub-article in PRC articles. Provided this is in fact a PRC article, the article-type should be 'referee-report'.</report>
      
      <report test="$is-prc and @article-type='reply'" role="error" id="sub-article-4">'<value-of select="@article-type"/>' is not permitted as the article-type for a sub-article in a non-PRC article. Provided this is in fact a non-PRC article, the article-type should be 'author-comment'.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-reply-test-5" test="matches(.,'&lt;[/]?[Aa]uthor response')" role="error" id="dec-letter-reply-test-5"><value-of select="ancestor::sub-article/@article-type"/> paragraph contains what looks like pseudo-code - <value-of select="."/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-reply-test-6" test="matches(.,'&lt;\p{Zs}?/?\p{Zs}?[a-z]*\p{Zs}?/?\p{Zs}?&gt;')" role="warning" id="dec-letter-reply-test-6"><value-of select="ancestor::sub-article/@article-type"/> paragraph contains what might be pseudo-code or tags which should likely be removed - <value-of select="."/>.</report>
      ...
      <assert test="count(contrib-group) = 1" role="error" id="ed-eval-front-test-2">editor report front-stub must contain 1 (and only 1) contrib-group element. This one has <value-of select="count(contrib-group)"/>.</assert>
      
      <report test="count(related-object) gt 1" role="error" id="ed-eval-front-test-3">editor report front-stub must contain 1 or 0 related-object elements. This one has <value-of select="count(related-object)"/>.</report>

      <report test="e:is-prc(.) and not(kwd-group[@kwd-group-type='evidence-strength'])" role="error" id="ed-eval-front-test-4">eLife Assessment front-stub does not contain a strength term keyword group, which must be incorrect.</report>

      <report test="e:is-prc(.) and not(kwd-group[@kwd-group-type='claim-importance'])" role="warning" id="ed-eval-front-test-5">eLife Assessment front-stub does not contain a significance term keyword group, which is very unusual. Is that correct?</report>
      ...
      <assert test="name()=('article-id','title-group','contrib-group','kwd-group','related-object')" role="error" id="ed-eval-front-child-test-1"><name/> element is not allowed in the front-stub for an Editor's evaluation. Only the following elements are permitted: article-id, title-group, contrib-group, kwd-group, related-object.</assert>
      ...
      <report test="@kwd-group-type='evidence-strength' and count(kwd) = 2" role="warning" id="ed-report-kwd-group-2"><value-of select="@kwd-group-type"/> type kwd-group has <value-of select="count(kwd)"/> keywords: <value-of select="string-join(kwd,'; ')"/>. Please check this is correct.</report>
      
      <report test="@kwd-group-type='evidence-strength' and count(kwd) gt 2" role="error" id="ed-report-kwd-group-4"><value-of select="@kwd-group-type"/> type kwd-group has <value-of select="count(kwd)"/> keywords: <value-of select="string-join(kwd,'; ')"/>. This is incorrect.</report>
      ...
      <assert test="lower-case(.)=$allowed-vals" role="error" id="ed-report-bold-terms-1">Bold phrase in eLife Assessment - <value-of select="."/> - is not one of the permitted terms from the vocabulary. Should the bold formatting be removed? These are currently bolded terms <value-of select="string-join($allowed-vals,', ')"/></assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-front-test-1" test="count(article-id[@pub-id-type='doi']) = 1" role="error" id="dec-letter-front-test-1">sub-article front-stub must contain article-id[@pub-id-type='doi'].</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-front-test-2" test="$count gt 0" role="error" id="dec-letter-front-test-2">decision letter front-stub must contain at least 1 contrib-group element.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-front-test-3" test="$count gt 2" role="error" id="dec-letter-front-test-3">decision letter front-stub contains more than 2 contrib-group elements.</report>
      
      <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-front-test-4" test="($count = 1) and not(matches(parent::sub-article[1]/body[1],'(All|The) reviewers have opted to remain anonymous|The reviewer has opted to remain anonymous')) and not(parent::sub-article[1]/body[1]//ext-link[matches(@xlink:href,'http[s]?://www.reviewcommons.org/|doi.org/10.24072/pci.evolbiol')])" role="warning" id="dec-letter-front-test-4">decision letter front-stub has only 1 contrib-group element. Is this correct? i.e. were all of the reviewers (aside from the reviewing editor) anonymous? The text 'The reviewers have opted to remain anonymous' or 'The reviewer has opted to remain anonymous' is not present and there is no link to Review commons or a Peer Community in Evolutionary Biology doi in the decision letter.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-editor-test-1" test="count(contrib[@contrib-type='editor']) = 1" role="warning" id="dec-letter-editor-test-1">First contrib-group in decision letter must contain 1 and only 1 editor (contrib[@contrib-type='editor']).</assert>
      
      <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-editor-test-2" test="contrib[not(@contrib-type) or @contrib-type!='editor']" role="warning" id="dec-letter-editor-test-2">First contrib-group in decision letter contains a contrib which is not marked up as an editor (contrib[@contrib-type='editor']).</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-reviewer-test-1" test="count(contrib[@contrib-type='reviewer']) gt 0" role="error" id="dec-letter-reviewer-test-1">Second contrib-group in decision letter must contain a reviewer (contrib[@contrib-type='reviewer']).</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-reviewer-test-6" test="count(contrib[@contrib-type='reviewer']) gt 5" role="warning" id="dec-letter-reviewer-test-6">Second contrib-group in decision letter contains more than five reviewers. Is this correct? Exeter: Please check with eLife. eLife: check eJP to ensure this is correct.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-body-test-1" test="child::*[1]/local-name() = 'boxed-text'" role="error" id="dec-letter-body-test-1">First child element in decision letter is not boxed-text. This is certainly incorrect.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-body-test-3" test="contains(lower-case(.),'reviewed and recommended by peer community in evolutionary biology') and not(child::ext-link[matches(@xlink:href,'doi.org/10.24072/pci.evolbiol')])" role="error" id="dec-letter-body-test-3">The decision letter indicates that this article was reviewed by PCI evol bio, but there is no doi link with the prefix '10.24072/pci.evolbiol' which must be incorrect.</report>
      ...
      <report test="matches(.,'[Dd]ecision letter image \d')" role="error" id="sub-article-dl-image-1">Decision letter contains bold text that mentions an image: <value-of select="."/>. This should be changed to a citation (and if the decision letter image is missing, then it should be added).</report>
      
      <report test="matches(.,'[Dd]ecision letter table \d')" role="error" id="sub-article-dl-image-2">Decision letter contains bold text that mentions an table: <value-of select="."/>. This should be changed to a citation (and if the decision letter table is missing, then it should be added).</report>
      ...
      <report test="matches(.,'[Rr]eview image \d')" role="error" id="sub-article-pr-image-1">Public review contains bold text that mentions an image: <value-of select="."/>. This should be changed to a citation (and if the review image is missing, then it should be added).</report>
      
      <report test="matches(.,'[Rr]eview table \d')" role="error" id="sub-article-pr-image-2">Public review contains bold text that mentions an table: <value-of select="."/>. This should be changed to a citation (and if the review table is missing, then it should be added).</report>
      ...
      <report test="matches(.,'[Aa]uthor response image \d')" role="error" id="sub-article-ar-image-1">Author response contains bold text that mentions an image: <value-of select="."/>. This should be changed to a citation (and if the author response image is missing, then it should be added).</report>
      
      <report test="matches(.,'[Aa]uthor response table \d')" role="error" id="sub-article-ar-image-2">Author response contains bold text that mentions an table: <value-of select="."/>. This should be changed to a citation (and if the author response table is missing, then it should be added).</report>
      ...
      <report test="matches(.,'image \d')" role="error" id="sub-article-gen-image-1"><value-of select="ancestor::sub-article/front-stub//article-title"/> contains bold text that mentions an image: <value-of select="."/>. Is there a missing image?</report>
      
      <report test="matches(.,'table \d')" role="error" id="sub-article-gen-image-2"><value-of select="ancestor::sub-article/front-stub//article-title"/> contains bold text that mentions a table: <value-of select="."/>. Is there a missing table?</report>
    </rule></pattern>
  
  <pattern id="ref-report-front-pattern"><rule context="sub-article[@article-type='referee-report']/front-stub" id="ref-report-front">
      <let name="count" value="count(contrib-group)"/>
      
      <assert see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#dec-letter-front-test-1" test="count(article-id[@pub-id-type='doi']) = 1" role="error" id="ref-report-front-1">sub-article front-stub must contain article-id[@pub-id-type='doi'].</assert>
      
      <assert test="$count = 1" role="error" id="ref-report-front-2">sub-article front-stub must contain one, and only one contrib-group elements.</assert>
      ...
      <assert test="@contrib-type='author'" role="error" id="sub-article-contrib-test-1">contrib inside sub-article with article-type '<value-of select="ancestor::sub-article/@article-type"/>' must have the attribute contrib-type='author'.</assert>
      
      <assert test="name or anonymous or collab" role="error" id="sub-article-contrib-test-2">sub-article contrib must have either a child name or a child anonymous element.</assert>
      
      <report test="(name and anonymous) or (collab and anonymous) or (name and collab)" role="error" id="sub-article-contrib-test-3">sub-article contrib can only have a child name element or a child anonymous element or a child collab element (with descendant group members as required), it cannot have more than one of these elements. This has <value-of select="string-join(for $x in *[name()=('name','anonymous','collab')] return concat('a ',$x/name()),' and ')"/>.</report>
      
      <assert test="role" role="error" id="sub-article-contrib-test-4">contrib inside sub-article with article-type '<value-of select="ancestor::sub-article/@article-type"/>' must have a child role element.</assert>
      ...
      <report test="lower-case($sub-title)='recommendations for authors' and not(parent::contrib/preceding-sibling::contrib) and not(@specific-use='editor')" role="error" id="sub-article-role-test-1">The role element for the first contributor in <value-of select="$sub-title"/> must have the attribute specific-use='editor'.</report>
      
      <report test="$sub-article-type='referee-report' and (lower-case($sub-title)!='recommendations for authors' or parent::contrib/preceding-sibling::contrib) and not(@specific-use='referee')" role="error" id="sub-article-role-test-2">The role element for this contributor must have the attribute specific-use='referee'.</report>
      
      <report test="$sub-article-type='author-comment' and not(@specific-use='author')" role="error" id="sub-article-role-test-3">The role element for contributors in the author response must have the attribute specific-use='author'.</report>
      
      <report test="@specific-use='author' and .!='Author'" role="error" id="sub-article-role-test-4">A role element with the attribute specific-use='author' must contain the text 'Author'. This one has '<value-of select="."/>'.</report>
      
      <report test="@specific-use='editor' and not(.=('Senior and Reviewing Editor','Reviewing Editor'))" role="error" id="sub-article-role-test-5">A role element with the attribute specific-use='editor' must contain the text 'Senior and Reviewing Editor' or 'Reviewing Editor'. This one has '<value-of select="."/>'.</report>
      
      <report test="@specific-use='referee' and .!='Reviewer'" role="error" id="sub-article-role-test-6">A role element with the attribute specific-use='referee' must contain the text 'Reviewer'. This one has '<value-of select="."/>'.</report>
      ...
      <assert test="count(descendant::contrib[role[@specific-use='editor']]) = 1" role="error" id="ref-report-editor-1">The Recommendations for authors must contain 1 and only 1 editor (a contrib with a role[@specific-use='editor']). This one has <value-of select="count(descendant::contrib[role[@specific-use='editor']])"/>.</assert>
      
      <assert test="count(descendant::contrib[role[@specific-use='referee']]) &gt; 0" role="error" id="ref-report-reviewer-1">The Recommendations for authors must contain 1 or more reviewers (a contrib with a role[@specific-use='referee']). This one has 0.</assert>
      ...
      <assert test="count(descendant::contrib[role[@specific-use='referee']])=1" role="error" id="ref-report-reviewer-test-1">A public review must contain a single contributor which is a reviewer (a contrib with a child role[@specific-use='referee']). This one contains <value-of select="count(descendant::contrib[role[@specific-use='referee']])"/>.</assert>
      
      <report test="descendant::contrib[not(role[@specific-use='referee'])]" role="error" id="ref-report-reviewer-test-2">A public review cannot contain a contributor which is not a reviewer (i.e. a contrib without a child role[@specific-use='referee']).</report>
  ...
  <pattern id="research-advance-test-pattern"><rule context="article[descendant::article-meta/article-categories/subj-group[@subj-group-type='display-channel']/subject = 'Research Advance']//article-meta" id="research-advance-test">
      ...
      <assert test="matches(@xlink:href,'^10\.7554/e[lL]ife\.[\d]{5,6}$')" role="error" id="related-articles-test-6">related-article element must contain a @xlink:href, the value of which should be in the form 10.7554/eLife.00000.</assert>
  ...
  <pattern id="video-parent-conformance-pattern"><rule context="media[@mimetype='video']" id="video-parent-conformance">
      ...
      <assert test="$parent = ('sec','fig-group','body','boxed-text','app')" role="error" id="video-parent-test"><value-of select="replace(label[1],'\.$','')"/> is a child of a &lt;<value-of select="$parent"/>&gt; element. It can only be a child of sec, fig-group, body, boxed-text, or app.</assert>
      
    </rule></pattern>
  
  <pattern id="elem-citation-general-pattern"><rule context="element-citation" id="elem-citation-general">
      
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#err-elem-cit-gen-name-5" test="descendant::etal" role="error" id="err-elem-cit-gen-name-5">The &lt;etal&gt; element in a reference is not allowed. Reference '<value-of select="ancestor::ref/@id"/>' contains it.</report>
      
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#err-elem-cit-gen-date-1-9" test="count(year) &gt; 1 " role="error" id="err-elem-cit-gen-date-1-9">There may be at most one &lt;year&gt; element. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(year)"/> &lt;year&gt; elements.</report>
      
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#fpage-lpage-test-1" test="(fpage) and not(lpage)" role="warning" id="fpage-lpage-test-1"><value-of select="e:citation-format1(.)"/> has a first page <value-of select="fpage"/>, but no last page. Is this correct? Should it be an elocation-id instead?</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#err-elem-cit-gen-date-1-2" test="(1700 le number($YYYY)) and (number($YYYY) le ($current-year + 5))" role="warning" id="err-elem-cit-gen-date-1-2">The numeric value of the 4 digits in the &lt;year&gt; element must be between 1700 and the current year + 5 years (inclusive). Reference '<value-of select="ancestor::ref/@id"/>' does not meet this requirement as it contains the value '<value-of select="."/>'.</assert>
  ...
  <pattern id="ref-list-ordering-pattern"><rule context="ref[preceding-sibling::ref]" id="ref-list-ordering">
      ...
      <assert see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#err-elem-cit-high-3-1" test="@id" role="error" id="err-elem-cit-high-3-1">Each &lt;ref&gt; element must have an @id attribute.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#final-empty-elem-cit-des" test="not(*) and (normalize-space(.)='')" role="error" id="final-empty-elem-cit-des"><name/> element is empty - this is not allowed. It must contain content.</report>
      
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#tagging-elem-cit-des" test="matches(.,'&lt;/?[a-z]*/?&gt;')" role="error" id="tagging-elem-cit-des"><name/> element contains tagging, which should be removed - '<value-of select="."/>'.</report>
  ...
  <pattern id="elem-citation-journal-pattern"><rule context="element-citation[@publication-type='journal']" id="elem-citation-journal">
      ...
      <assert see="https://elifeproduction.slab.com/posts/journal-references-i098980k#err-elem-cit-journal-5-1-3" test="count(volume) le 1" role="error" id="err-elem-cit-journal-5-1-3">There may be no more than one  &lt;volume&gt; element within a &lt;element-citation&gt; of type 'journal'. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(volume)"/> &lt;volume&gt; elements.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#err-elem-cit-journal-6-5-1" test="lpage and not(fpage)" role="error" id="err-elem-cit-journal-6-5-1">&lt;lpage&gt; is only allowed if &lt;fpage&gt; is present. Reference '<value-of select="ancestor::ref/@id"/>' has &lt;lpage&gt; but no &lt;fpage&gt;.</report>
  ...
  <pattern id="elem-citation-book-pattern"><rule context="element-citation[@publication-type='book']" id="elem-citation-book">
      ...
      <assert see="https://elifeproduction.slab.com/posts/book-references-x4trb0n2#hifu4-final-err-elem-cit-book-10-1" test="count(source)=1" role="error" id="final-err-elem-cit-book-10-1">Each  &lt;element-citation&gt; of type 'book' must contain one and only one &lt;source&gt; element. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(source)"/> &lt;source&gt; elements.</assert>
  ...
  <pattern id="elem-citation-data-pattern"><rule context="ref/element-citation[@publication-type='data']" id="elem-citation-data">
      ...
      <report see="https://elifeproduction.slab.com/posts/data-references-4jxukxzy#err-elem-cit-data-3-1" test="count(person-group[@person-group-type='author']) gt 1" role="error" id="err-elem-cit-data-3-1">Data references must have one and only one &lt;person-group person-group-type='author'&gt;. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(person-group[@person-group-type='author'])"/>.</report>
      ...
      <assert test="@specific-use=('generated','analyzed')" role="error" id="err-elem-cit-data-1-v2">element-citation[@publication-type='data'] must have a specific-use attribute is either 'generated' or 'analyzed'. Reference '<value-of select="ancestor::ref/@id"/>' does not have this attribute or the value is incorrect.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/data-references-4jxukxzy#err-elem-cit-data-13-2" test="@pub-id-type=('accession','doi')" role="error" id="err-elem-cit-data-13-2">Each pub-id element must have a pub-id-type which is either accession or doi. Reference '<value-of select="ancestor::ref/@id"/>' has a &lt;pub-id element with the type '<value-of select="@pub-id-type"/>'.</assert>
      ...
      <report test="($diff gt 1) or ($diff lt -1)" role="warning" id="final-data-old-and-gend">Dataset reference <value-of select="if (parent::ref) then parent::ref/@id else 'in data availability section'"/> is marked as generated but the year is <value-of select="$year"/>. Is this correct?</report>
  ...
  <pattern id="elem-citation-software-pattern"><rule context="element-citation[@publication-type = 'software']" id="elem-citation-software">
      ...
      <assert see="https://elifeproduction.slab.com/posts/software-references-aymhzmlh#err-elem-cit-software-2-1" test="count(person-group[@person-group-type='author']) = 1" role="error" id="err-elem-cit-software-2-1">Each &lt;element-citation&gt; of type 'software' must contain one &lt;person-group&gt; element with attribute person-group-type = author. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(person-group[@person-group-type='author'])"/> &lt;person-group&gt; elements.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/software-references-aymhzmlh#err-elem-cit-software-10-1" test="count(data-title) &gt; 1" role="error" id="err-elem-cit-software-10-1">Each &lt;element-citation&gt; of type 'software' may contain one and only one &lt;data-title&gt; element. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(data-title)"/> &lt;data-title&gt; elements.</report>
      ...
      <report test="pub-id and ext-link" role="error" id="elem-cit-software-pub-id-ext-link">Software reference '<value-of select="ancestor::ref/@id"/>' has both &lt;pub-id&gt; &lt;ext-link&gt; elements. There can only be one or the other, not both.</report>
  ...
  <pattern id="elem-citation-preprint-pattern"><rule context="element-citation[@publication-type='preprint']" id="elem-citation-preprint">
      ...
      <assert see="https://elifeproduction.slab.com/posts/preprint-references-okxjjp9i#err-elem-cit-preprint-2-1" test="count(person-group)=1" role="error" id="err-elem-cit-preprint-2-1">There must be one and only one person-group. <value-of select="ancestor::ref/@id"/>' has <value-of select="count(person-group)"/> &lt;person-group&gt; elements.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/preprint-references-okxjjp9i#err-elem-cit-preprint-8-1" test="count(article-title)=1" role="error" id="err-elem-cit-preprint-8-1">Each  &lt;element-citation&gt; of type 'preprint' must contain one and only one &lt;article-title&gt; element. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(article-title)"/> &lt;article-title&gt; elements.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/preprint-references-okxjjp9i#err-elem-cit-preprint-13" test="count(*) = count(person-group| article-title| source| year| pub-id| ext-link)" role="error" id="err-elem-cit-preprint-13">The only tags that are allowed as children of &lt;element-citation&gt; with the publication-type="preprint" are: &lt;person-group&gt;, &lt;article-title&gt;, &lt;source&gt;, &lt;year&gt;, &lt;pub-id&gt;, and &lt;ext-link&gt;. Reference '<value-of select="ancestor::ref/@id"/>' has other elements.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/preprint-references-okxjjp9i#err-elem-cit-preprint-10-2" test="@pub-id-type='doi'" role="error" id="err-elem-cit-preprint-10-2">If present, the &lt;pub-id&gt; element must contain @pub-id-type='doi'. The &lt;pub-id&gt; element in Reference '<value-of select="ancestor::ref/@id"/>' contains @pub-id-type='<value-of select="@pub-id-type"/>'.</assert>
  ...
  <pattern id="elem-citation-web-pattern"><rule context="element-citation[@publication-type='web']" id="elem-citation-web">
      ...
      <report test="if (matches(@iso-8601-date,replace($date-regex,'\p{Zs}',''))) then format-date(xs:date(@iso-8601-date), '[MNn] [D], [Y]')!=.         else ()" role="error" id="err-elem-cit-web-11-4">
  ...
  <pattern id="gen-das-tests-pattern"><rule context="sec[@sec-type='data-availability']//element-citation[@publication-type='data']" id="gen-das-tests">
      ...
      <report test="@pub-id-type='doi' and (@xlink:href)" role="error" id="das-pub-id-3">A pub-id with the type doi does not need an xlink:href attribute. <value-of select="concat('xlink:href=&quot;',.,'&quot;')"/> should be removed from the pub-id containing <value-of select="."/>.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#das-elem-citation-child-1" test="name()=$allowed-elems" role="error" id="das-elem-citation-child-1">Reference in the data availability section has a <value-of select="name()"/> element in a <value-of select="parent::*/name()"/> element which is not allowed.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#das-elem-citation-year-1" test="(.!='') and (@iso-8601-date!=$digits)" role="error" id="das-elem-citation-year-1">Every year in a reference must have an @iso-8601-date attribute equal to the numbers in the year. Reference with id <value-of select="parent::*/@id"/> has a year '<value-of select="."/>' but an @iso-8601-date '<value-of select="@iso-8601-date"/>'.</report>
  ...
  <pattern id="pub-id-tests-pattern"><rule context="element-citation/pub-id" id="pub-id-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#final-pub-id-test-2" test="(@pub-id-type='doi') and not(matches(.,'^10\.\d{4,9}/[-._;\+()#/:A-Za-z0-9%&lt;&gt;\[\]]+$'))" role="error" id="final-pub-id-test-2">pub-id is tagged as a doi, but it is not one - <value-of select="."/></report>
      
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#pub-id-test-3" test="(@pub-id-type='pmid') and not(matches(.,'^\d{3,10}$'))" role="error" id="pub-id-test-3">pub-id is tagged as a pmid, but it is not a number made up of between 3 and 10 digits - <value-of select="."/></report>
      
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#pub-id-doi-test-1" test="(@pub-id-type != 'doi') and matches(@xlink:href,'https?://(dx.doi.org|doi.org)/')" role="error" id="pub-id-doi-test-1">pub-id has a doi link - <value-of select="@xlink:href"/> - but its pub-id-type is <value-of select="@pub-id-type"/> instead of doi.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#pub-id-test-5" test="ends-with(.,'.')" role="error" id="pub-id-test-5"><value-of select="@pub-id-type"/> pub-id ends with a full stop - <value-of select="."/> - which is not correct. Please remove the full stop.</report>
      
      <report test="matches(.,'\p{Zs}$')" role="error" id="pub-id-test-6"><value-of select="@pub-id-type"/> pub-id ends with space(s) which is incorrect - '<value-of select="."/>'.</report>
      ...
      <report test="matches(@xlink:href,'[\p{Zs}]')" role="error" id="pub-id-url-space-report">'<value-of select="@xlink:href"/>' - Link contains a space which is incorrect.</report>
 ...
 <pattern id="feature-title-tests-pattern"><rule context="article-meta[descendant::subj-group[@subj-group-type='display-channel']/subject = $features-subj]//title-group/article-title" id="feature-title-tests">
     ...
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-title-test-1" test="(count(ancestor::article-meta/article-categories/subj-group[@subj-group-type='sub-display-channel']/subject) = 1) and starts-with(.,$sub-disp-channel)" role="error" id="feature-title-test-1">title starts with the sub-display-channel. This is certainly incorrect.</report>
     ...
     <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#feature-abstract-test-2" test="title = 'eLife digest'" role="error" id="feature-abstract-test-2">abstract title must contain 'eLife digest'. Possible superfluous characters - <value-of select="replace(title,'eLife digest','')"/></assert>
     ...
     <report see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#final-digest-test-2" test="matches(.,'\[[Oo][Kk]\??\]')" role="error" id="final-digest-test-2">digest paragraph contains [OK] or [OK?] which should be removed - <value-of select="."/></report>
     
     <report test="matches(.,'\[[Qq][Uu][Ee][Rr][Yy]')" role="error" id="final-digest-query-test"><value-of select="name()"/> element contains [Query] or [QUERY] which should be removed - <value-of select="."/></report>
     ...
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-subj-test-2" test=". != e:titleCase(.)" role="error" id="feature-subj-test-2">The content of the sub-display-channel should be in title case - <value-of select="e:titleCase(.)"/></report>
     
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-subj-test-3" test="ends-with(.,':')" role="error" id="feature-subj-test-3">sub-display-channel ends with a colon. This is incorrect.</report>
     ...
     <assert see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-bio-test-1" test="p[1]/bold = $name" role="error" id="feature-bio-test-1">bio must contain a bold element that contains the name of the author - <value-of select="$name"/>.</assert>
     ...
     <assert see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-bio-test-4" test="(count(p) = 1)" role="error" id="feature-bio-test-4">One and only 1 &lt;p&gt; is allowed as a child of bio. <value-of select="."/></assert>
     
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-bio-test-5" test="*[local-name()!='p']" role="error" id="feature-bio-test-5"><value-of select="*[local-name()!='p'][1]/local-name()"/> is not allowed as a child of &lt;bio&gt;. - <value-of select="."/></report>
     ...
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-template-test-1" test="($template = ('1','2','3')) and child::sub-article" role="error" id="feature-template-test-1"><value-of select="$type"/> is a template <value-of select="$template"/> but it has a decision letter or author response, which cannot be correct, as only template 5s are allowed these.</report>
     
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-template-test-2" test="($template = '5') and not(@article-type='research-article')" role="error" id="feature-template-test-2"><value-of select="$type"/> is a template <value-of select="$template"/> so the article element must have a @article-type="research-article". Instead the @article-type="<value-of select="@article-type"/>".</report>
     
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-template-test-3" test="($template = '5') and not(child::sub-article[@article-type='decision-letter'])" role="warning" id="feature-template-test-3"><value-of select="$type"/> is a template <value-of select="$template"/> but it does not (currently) have a decision letter. Is that OK?</report>
     
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-template-test-4" test="($template = '5') and not(child::sub-article[@article-type='reply'])" role="warning" id="feature-template-test-4"><value-of select="$type"/> is a template <value-of select="$template"/> but it does not (currently) have an author response. Is that OK?</report>
     
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-templates-no-bre" test="front/article-meta/contrib-group[@content-type='section'] and ($template != '5')" role="error" id="feature-templates-no-bre"><value-of select="$type"/> is a template <value-of select="$template"/>, which means that it should not have any BREs. This <value-of select="$type"/> has <value-of select="           string-join(           for $x in front/article-meta/contrib-group[@content-type='section']/contrib           return concat(e:get-name($x/name[1]),' as ',$x/role[1])           ,           ' and '           )           "/>. Please remove any senior/reviewing editors.</report>
     
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-templates-author-cont" test="back/fn-group[@content-type='author-contribution'] and $template = '1'" role="error" id="feature-templates-author-cont"><value-of select="$type"/> articles should not have any Author contributions. This <value-of select="$type"/> has <value-of select="           string-join(for $x in back/fn-group[@content-type='author-contribution']/fn           return concat('&quot;', $x,'&quot;')           ,           '; '           )           "/>. Please remove the author contributions.</report>
     
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-templates-author-cont" test="back/fn-group[@content-type='author-contribution'] and $template = '2'" role="warning" id="feature-templates-author-cont-3"><value-of select="$type"/> articles should not usually have any Author contributions. This <value-of select="$type"/> has <value-of select="          string-join(for $x in back/fn-group[@content-type='author-contribution']/fn          return concat('&quot;', $x,'&quot;')          ,          '; '          )          "/>. Are they required?</report>
     
     
     
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#final-feature-templates-author-cont-2" test="$template = ('3','4') and not(back/fn-group[@content-type='author-contribution'])" role="error" id="final-feature-templates-author-cont-2"><value-of select="$type"/>s should have Author contributions. This one does not. Exeter please check with the Production team who will check with the Features team.</report>
     ...
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#final-feat-ok-test" test="matches(.,'\[[Oo][Kk]\??\]')" role="error" id="final-feat-ok-test"><value-of select="name()"/> element contains [OK] or [OK?] which should be removed - <value-of select="."/></report>
     
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#hb9uk-final-feat-query-test" test="matches(.,'\[[Qq][Uu][Ee][Rr][Yy]')" role="error" id="final-feat-query-test"><value-of select="name()"/> element contains [Query] or [QUERY] which should be removed - <value-of select="."/></report>
  ...
  <pattern id="correction-tests-pattern"><rule context="article[@article-type = 'correction']" id="correction-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#h577w-retr-aff-presence" test="descendant::article-meta//aff" role="error" id="retr-aff-presence"><value-of select="$display-subject"/> notices should not contain affiliations.</report>
      
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#heu8l-retr-coi-presence" test="descendant::fn-group[@content-type='competing-interest']" role="error" id="retr-COI-presence"><value-of select="$display-subject"/> notices should not contain competing interests.</report>
      
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hmmm1-retr-self-uri-presence" test="descendant::self-uri" role="error" id="retr-self-uri-presence"><value-of select="$display-subject"/> notices should not contain a self-uri element (as the PDF is not published).</report>
      
      <report see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#retr-abstract-presence" test="descendant::abstract" role="error" id="retr-abstract-presence"><value-of select="$display-subject"/> notices should not contain abstracts.</report>
      
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hwi7f-retr-back" test="back/*" role="error" id="retr-back"><value-of select="$display-subject"/> notices should not contain any content in the back.</report>
      
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hgn3c-retr-impact-statement" test="descendant::meta-name[text() = 'Author impact statement']" role="error" id="retr-impact-statement"><value-of select="$display-subject"/> notices should not contain an impact statement.</report>
  ...
  <pattern id="rrid-org-code-pattern"><rule context="p|td|th" id="rrid-org-code">
      ...
      <report see="https://elifeproduction.slab.com/posts/code-blocks-947pcamv#code-test" test="not(descendant::monospace) and not(descendant::code) and ($code-text != '')" role="warning" id="code-test"><name/> element contains what looks like unformatted code - '<value-of select="$code-text"/>' - does this need tagging with &lt;monospace/&gt; or &lt;code/&gt;?</report>
      
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h9zhw-cell-spacing-test" test="($unequal-equal-text != '') and not(disp-formula[contains(.,'=')]) and not(inline-formula[contains(.,'=')]) and not(child::code) and not(child::monospace)" role="warning" id="cell-spacing-test"><name/> element contains an equal sign with content directly next to one side, but a space on the other, is this correct? - <value-of select="$unequal-equal-text"/></report>
      
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h7eko-equal-spacing-test" test="matches(.,'\+cell[s]?|±cell[s]?') and not(descendant::p[matches(.,'\+cell[s]?|±cell[s]?')]) and not(descendant::td[matches(.,'\+cell[s]?|±cell[s]?')]) and not(descendant::th[matches(.,'\+cell[s]?|±cell[s]?')])" role="warning" id="equal-spacing-test"><name/> element contains the text '+cells' or '±cells' which is very likely to be incorrect spacing - <value-of select="."/></report>
      ...
      <report test="contains(.,'git://')" role="error" id="git-protocol"><name/> contains the git:// protocol - <value-of select="."/>. This is no longer widely supported, and should be replaced with the appropriate https:// protocol (or similar) equivalent.</report>
      
      <report test="matches(.,'user-?name\s*:|password\s*:') or (matches(.,'\suser-?name\s') and matches(.,'\spassword\s'))" role="warning" id="user-name-password"><name/> contains what may be a username and password - <value-of select="."/>. If these are access ceredentials for a dataset depositsed by the authors, it should be made publicly available (unless approved by editors) and the credentials removed/deleted.</report>
    </rule></pattern>
  ...
  <pattern id="duplicate-ref-pattern"><rule context="ref-list//ref" id="duplicate-ref">
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#duplicate-ref-test-4" test="some $x in preceding-sibling::ref/element-citation satisfies (         (($x/article-title = $a-title) and not($x/source = $source))         or         (($x/chapter-title = $c-title) and not($x/source = $source))         )" role="warning" id="duplicate-ref-test-4">ref '<value-of select="@id"/>' has the same title as another reference, but a different source. Is this correct? - '<value-of select="$a-title"/>'</report>
  ...
  <pattern id="ref-xref-conformance-pattern"><rule context="xref[@ref-type='bibr']" id="ref-xref-conformance">
      ...
      <report see="https://elifeproduction.slab.com/posts/reference-citations-vv87m87l#ref-xref-test-3" test="matches($post-text,'^[\p{L}\p{N}\p{M}\p{Ps}]')" role="warning" id="ref-xref-test-3">There is no space between citation and the following text - <value-of select="concat(.,substring($post-text,1,15))"/> - Is this correct?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/reference-citations-vv87m87l#ref-xref-test-14" test="matches($pre-sentence,$cite3)" role="warning" id="ref-xref-test-14">citation is preceded by text containing much of the citation text which is possibly unnecessary - <value-of select="concat($pre-sentence,.)"/>.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/reference-citations-vv87m87l#final-ref-xref-test-19" test="matches(.,'^et al|^ and|^\(\d|^,')" role="error" id="final-ref-xref-test-19"><value-of select="."/> - citation doesn't start with an author's name which is incorrect.</report>
  ...
  <pattern id="missing-ref-cited-pattern"><rule context="article[not(@article-type=('correction','retraction'))]//p[(ancestor::app or ancestor::body[parent::article]) and not(child::table-wrap) and not(child::supplementary-material)]|td[ancestor::app or ancestor::body[parent::article]]|th[ancestor::app or ancestor::body[parent::article]]" id="missing-ref-cited">
      ...
      <report test="matches($text,$missing-ref-regex)" role="warning" id="missing-ref-in-text-test"><name/> element contains possible citation which is unlinked or a missing reference - search - <value-of select="concat(tokenize(substring-before($text,' et al'),' ')[last()],' et al ',tokenize(substring-after($text,' et al'),' ')[2])"/></report>
      
      <report test="matches(lower-case($text),$missing-file-regex)" role="warning" id="missing-file-in-text-test"><name/> element contains possible citation to a file which is unlinked or missing. If you are unsure what object needs to be cited then please add the following author query (replacing XXXX as appropriate): Please confirm which XXXXXX this refers to, or confirm that this citation refers to another article.</report>
  ...
  <pattern id="unlinked-object-cite-pattern"><rule context="fig[not(ancestor::sub-article) and label]|       table-wrap[not(ancestor::sub-article) and label[not(contains(.,'ey resources table'))]]|       media[not(ancestor::sub-article) and label]|       supplementary-material[not(ancestor::sub-article) and label]" id="unlinked-object-cite">
      ...
      <report test="matches(lower-case($article-text),lower-case($regex))" role="warning" id="text-v-object-cite-test"><value-of select="$cite1"/> has possible unlinked citations in the text.</report>
  ...
  <pattern id="vid-xref-conformance-pattern"><rule context="xref[@ref-type='video']" id="vid-xref-conformance">
      ...
      <assert see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#vid-xref-conformity-1" test="matches(.,'\p{N}')" role="error" id="vid-xref-conformity-1"><value-of select="."/> - video citation does not contain any numbers which must be incorrect.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#vid-xref-test-3" test="matches($post-text,'^[\p{L}\p{N}\p{M}\p{Ps}]')" role="warning" id="vid-xref-test-3">There is no space between citation and the following text - <value-of select="concat(.,substring($post-text,1,15))"/> - Is this correct?</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#vid-xref-test-4" test="(ancestor::media[@mimetype='video']/@id = $rids)" role="warning" id="vid-xref-test-4"><value-of select="."/> - video citation is in the caption of the video that it links to. Is it correct or necessary?</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#vid-xref-test-5" test="(matches($post-text,'^ in $|^ from $|^ of $')) and (following-sibling::*[1]/@ref-type='bibr')" role="error" id="vid-xref-test-5"><value-of select="concat(.,$post-text,following-sibling::*[1])"/> - Video citation is in a reference to a video from a different paper, and therefore must be unlinked.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#vid-xref-test-6" test="matches($pre-text,'[A-Za-z0-9][\(]$')" role="error" id="vid-xref-test-6">citation is preceded by a letter or number immediately followed by '('. Is there a space missing before the '('?  - '<value-of select="concat($pre-text,.)"/>'.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#vid-xref-test-8" test="matches($post-text,'^[\p{Zs}]?[\p{Zs}—\-][\p{Zs}]?[Ss]ource')" role="error" id="vid-xref-test-8">Incomplete citation. Video citation is followed by text which suggests it should instead be a link to source data or code - <value-of select="concat(.,$post-text)"/>'.</report>
  ...
  <pattern id="fig-xref-conformance-pattern"><rule context="xref[@ref-type='fig' and @rid]" id="fig-xref-conformance">
      ...
      <assert see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-conformity-1" test="matches(.,'\p{N}')" role="error" id="fig-xref-conformity-1"><value-of select="."/> - figure citation does not contain any numbers which must be incorrect.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-conformity-2" test="($type = ('Figure','Chemical structure','Scheme')) and not(contains($no,$target-no))" role="error" id="fig-xref-conformity-2"><value-of select="."/> - figure citation does not appear to link to the same place as the content of the citation suggests it should.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-conformity-3" test="($type = ('Figure','Chemical structure','Scheme')) and ($no != $target-no)" role="warning" id="fig-xref-conformity-3"><value-of select="."/> - figure citation does not appear to link to the same place as the content of the citation suggests it should.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-conformity-4" test="($type = 'Figure') and matches(.,'[Ss]upplement')" role="error" id="fig-xref-conformity-4"><value-of select="."/> - figure citation links to a figure, but it contains the string 'supplement'. It cannot be correct.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-conformity-5" test="($type = 'Figure supplement') and (not(matches(.,'[Ss]upplement'))) and (not(matches(preceding-sibling::text()[1],'–[\p{Zs}]?$| and $| or $|,[\p{Zs}]?$')))" role="warning" id="fig-xref-conformity-5">figure citation stands alone, contains the text <value-of select="."/>, and links to a figure supplement, but it does not contain the string 'supplement'. Is it correct? Preceding text - '<value-of select="substring(preceding-sibling::text()[1],string-length(preceding-sibling::text()[1])-25)"/>'</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-test-2" test="matches($pre-text,'[\p{L}\p{N}\p{M}\p{Pe},;]$')" role="warning" id="fig-xref-test-2">There is no space between citation and the preceding text - <value-of select="concat(substring($pre-text,string-length($pre-text)-15),.)"/> - Is this correct?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-test-4" test="not(ancestor::supplementary-material) and not(ancestor::license-p) and (ancestor::fig/@id = $rid)" role="warning" id="fig-xref-test-4"><value-of select="."/> - Figure citation is in the caption of the figure that it links to. Is it correct or necessary?</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-test-5" test="($type = 'Figure') and (matches($post-text,'^ in $|^ from $|^ of $')) and (following-sibling::*[1]/@ref-type='bibr')" role="error" id="fig-xref-test-5"><value-of select="concat(.,$post-text,following-sibling::*[1])"/> - Figure citation refers to a figure from a different paper, and therefore must be unlinked.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-test-6" test="matches($pre-text,'[A-Za-z0-9][\(]$')" role="error" id="fig-xref-test-6">citation is preceded by a letter or number immediately followed by '('. Is there a space missing before the '('?  - '<value-of select="concat($pre-text,.)"/>'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-test-7" test="matches($post-text,'^[\)][A-Za-z0-9]')" role="error" id="fig-xref-test-7">citation is followed by a ')' which in turn is immediately followed by a letter or number. Is there a space missing after the ')'?  - '<value-of select="concat(.,$post-text)"/>'.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#fig-xref-test-11" test="matches($post-text,'^[\p{Zs}]?[\p{Zs}—\-][\p{Zs}]?[Vv]ideo')" role="warning" id="fig-xref-test-11">Incomplete citation. Figure citation is followed by text which suggests it should instead be a link to a video supplement - <value-of select="concat(.,$post-text)"/>'.</report>
  ...
  <pattern id="table-xref-conformance-pattern"><rule context="xref[@ref-type='table']" id="table-xref-conformance">
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#table-xref-conformity-1" test="not(matches(.,'Table')) and ($pre-text != ' and ') and ($pre-text != '–') and ($pre-text != ', ') and not(contains($rid,'app')) and not(contains($rid,'resp')) and not(contains($rid,'sa'))" role="warning" id="table-xref-conformity-1"><value-of select="."/> - citation points to table, but does not include the string 'Table', which is very unusual.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#table-xref-conformity-2" test="not(matches(.,'table')) and ($pre-text != ' and ') and ($pre-text != '–') and ($pre-text != ', ') and contains($rid,'app')" role="warning" id="table-xref-conformity-2"><value-of select="."/> - citation points to an Appendix table, but does not include the string 'table', which is very unusual.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#table-xref-conformity-3" test="(not(contains($rid,'app') or contains($rid,'sa'))) and ($text-no != $rid-no) and not(contains(.,'–')) and not(contains(.,' and '))" role="warning" id="table-xref-conformity-3"><value-of select="."/> - Citation content does not match what it directs to.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#table-xref-conformity-4" test="(contains($rid,'app')) and (not(ends-with($text-no,substring($rid-no,2)))) and not(contains(.,'–'))" role="warning" id="table-xref-conformity-4"><value-of select="."/> - Citation content does not match what it directs to.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#table-xref-conformity-5" test="(contains($rid,'sa')) and (not(ends-with($text-no,substring($rid-no,2)))) and not(contains(.,'–')) and not(contains(.,' and '))" role="warning" id="table-xref-conformity-5"><value-of select="."/> - Citation content does not match what it directs to.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#table-xref-test-1" test="(ancestor::table-wrap/@id = $rid) and not(ancestor::supplementary-material)" role="warning" id="table-xref-test-1"><value-of select="."/> - Citation is in the caption of the Table that it links to. Is it correct or necessary?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#table-xref-test-3" test="matches($post-text,'^[\)][A-Za-z0-9]')" role="error" id="table-xref-test-3">citation is followed by a ')' which in turn is immediately followed by a letter or number. Is there a space missing after the ')'?  - '<value-of select="concat(.,$post-text)"/>'.</report>
  ...
  <pattern id="supp-file-xref-conformance-pattern"><rule context="xref[@ref-type='supplementary-material']" id="supp-file-xref-conformance">
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supp-file-xref-conformity-1" test="contains($rid,'data') and not(matches(.,'[Ss]ource data')) and ($pre-text != ' and ') and ($pre-text != '–') and ($pre-text != ', ')" role="warning" id="supp-file-xref-conformity-1"><value-of select="."/> - citation points to source data, but does not include the string 'source data', which is very unusual.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supp-file-xref-conformity-2" test="contains($rid,'code') and not(matches(.,'[Ss]ource code')) and ($pre-text != ' and ') and ($pre-text != '–') and ($pre-text != ', ')" role="warning" id="supp-file-xref-conformity-2"><value-of select="."/> - citation points to source code, but does not include the string 'source code', which is very unusual.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supp-file-xref-conformity-3" test="contains($rid,'supp') and not(matches(.,'[Ss]upplementary file')) and ($pre-text != ' and ') and ($pre-text != '–') and ($pre-text != ', ')" role="warning" id="supp-file-xref-conformity-3"><value-of select="."/> - citation points to a supplementary file, but does not include the string 'Supplementary file', which is very unusual.</report>
      
      <assert see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supp-file-xref-conformity-4" test="contains(.,$last-rid-no)" role="warning" id="supp-file-xref-conformity-4"><value-of select="."/> - It looks like the citation content does not match what it directs to. The only case where this can be ignored is if this points to an audio file.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supp-file-xref-conformity-5" test="$last-text-no = $last-rid-no" role="warning" id="supp-file-xref-conformity-5"><value-of select="."/> - It looks like the citation content does not match what it directs to. Check that it is correct.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supp-file-xref-test-1" test="ancestor::supplementary-material/@id = $rid" role="warning" id="supp-file-xref-test-1"><value-of select="."/> - Citation is in the caption of the Supplementary file that it links to. Is it correct or necessary?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supp-xref-test-3" test="matches($post-text,'^[\)][A-Za-z0-9]')" role="error" id="supp-xref-test-3">citation is followed by a ')' which in turn is immediately followed by a letter or number. Is there a space missing after the ')'?  - '<value-of select="concat(.,$post-text)"/>'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supp-xref-test-4" test="matches($pre-text,'[Ff]igure [\d]{1,2}[\p{Zs}]?[\p{Zs}\p{P}][\p{Zs}]?$|[Vv]ideo [\d]{1,2}[\p{Zs}]?[\p{Zs}\p{P}][\p{Zs}]?$|[Tt]able [\d]{1,2}[\p{Zs}]?[\p{Zs}\p{P}][\p{Zs}]?$')" role="error" id="supp-xref-test-4">Incomplete citation. <value-of select="."/> citation is preceded by text which suggests it should instead be a link to Figure/Video/Table level source data or code - <value-of select="concat($pre-text,.)"/>'.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supp-xref-test-6" test="contains(.,'—Source')" role="warning" id="supp-xref-test-6">citation contains '—Source' (<value-of select="."/>). If it refers to asset level source data or code, then 'Source' should be spelled with a lowercase s, as in the label for that file.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supp-file-xref-conformity-6" test="contains($rid,'data') and matches(.,'[Ss]ource datas')" role="error" id="supp-file-xref-conformity-6"><value-of select="."/> - citation points to source data but contains the string 'source datas', which is grammatically incorrect. It should be source data instead.</report>
  ...
  <pattern id="equation-xref-conformance-pattern"><rule context="xref[@ref-type='disp-formula']" id="equation-xref-conformance">
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#equ-xref-conformity-1" test="not(matches(.,'[Ee]quation')) and ($prec-text != ' and ') and ($prec-text != '–')" role="warning" id="equ-xref-conformity-1"><value-of select="."/> - link points to equation, but does not include the string 'Equation', which is unusual. Is it correct?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#equ-xref-conformity-3" test="(matches($post-text,'^ in $|^ from $|^ of $')) and (following-sibling::*[1]/@ref-type='bibr')" role="error" id="equ-xref-conformity-3"><value-of select="concat(.,$post-text,following-sibling::*[1])"/> - Equation citation appears to be a reference to an equation from a different paper, and therefore must be unlinked.</report>
  ...
  <pattern id="org-ref-article-book-title-pattern"><rule context="element-citation/article-title|element-citation/chapter-title|element-citation/source|element-citation/data-title" id="org-ref-article-book-title">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#escoli-ref-article-title-check" test="matches($lc,'e\.\p{Zs}?coli') and not(italic[contains(.,'E. coli')])" role="info" id="escoli-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'E. coli' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#escherichiascoli-ref-article-title-check" test="matches($lc,'escherichia\p{Zs}?coli') and not(italic[contains(.,'Escherichia coli')])" role="info" id="escherichiascoli-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Escherichia coli' - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#schizosaccharomycesspombe-ref-article-title-check" test="matches($lc,'schizosaccharomyces\p{Zs}?pombe') and not(italic[contains(.,'Schizosaccharomyces pombe')])" role="info" id="schizosaccharomycesspombe-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Schizosaccharomyces pombe' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#sscerevisiae-ref-article-title-check" test="matches($lc,'s\.\p{Zs}?cerevisiae') and not(italic[contains(.,'S. cerevisiae')])" role="info" id="sscerevisiae-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'S. cerevisiae' - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#cselegans-ref-article-title-check" test="matches($lc,'c\.\p{Zs}?elegans') and not(italic[contains(.,'C. elegans')])" role="info" id="cselegans-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'C. elegans' - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#myceliophthorasthermophila-ref-article-title-check" test="matches($lc,'myceliophthora\p{Zs}?thermophila') and not(italic[contains(.,'Myceliophthora thermophila')])" role="info" id="myceliophthorasthermophila-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Myceliophthora thermophila' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#dictyostelium-ref-article-title-check" test="matches($lc,'dictyostelium') and not(italic[contains(.,'Dictyostelium')])" role="info" id="dictyostelium-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Dictyostelium' - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#plasmodiumsfalciparum-ref-article-title-check" test="matches($lc,'plasmodium\p{Zs}?falciparum') and not(italic[contains(.,'Plasmodium falciparum')])" role="info" id="plasmodiumsfalciparum-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Plasmodium falciparum' - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#sspyogenes-ref-article-title-check" test="matches($lc,'s\.\p{Zs}?pyogenes') and not(italic[contains(.,'S. pyogenes')])" role="info" id="sspyogenes-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'S. pyogenes' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#streptococcusspyogenes-ref-article-title-check" test="matches($lc,'streptococcus\p{Zs}?pyogenes') and not(italic[contains(.,'Streptococcus pyogenes')])" role="info" id="streptococcusspyogenes-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Streptococcus pyogenes' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#psdumerilii-ref-article-title-check" test="matches($lc,'p\.\p{Zs}?dumerilii') and not(italic[contains(.,'P. dumerilii')])" role="info" id="psdumerilii-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'P. dumerilii' - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#osfasciatus-ref-article-title-check" test="matches($lc,'o\.\p{Zs}?fasciatus') and not(italic[contains(.,'O. fasciatus')])" role="info" id="osfasciatus-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'O. fasciatus' - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#encephalitozoonscuniculi-ref-article-title-check" test="matches($lc,'encephalitozoon\p{Zs}?cuniculi') and not(italic[contains(.,'Encephalitozoon cuniculi')])" role="info" id="encephalitozoonscuniculi-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Encephalitozoon cuniculi' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#hssalinarum-ref-article-title-check" test="matches($lc,'h\.\p{Zs}?salinarum') and not(italic[contains(.,'H. salinarum')])" role="info" id="hssalinarum-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'H. salinarum' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#halobacteriumssalinarum-ref-article-title-check" test="matches($lc,'halobacterium\p{Zs}?salinarum') and not(italic[contains(.,'Halobacterium salinarum')])" role="info" id="halobacteriumssalinarum-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Halobacterium salinarum' - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#escarotovora-ref-article-title-check" test="matches($lc,'e\.\p{Zs}?carotovora') and not(italic[contains(.,'E. carotovora')])" role="info" id="escarotovora-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'E. carotovora' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#erwiniascarotovora-ref-article-title-check" test="matches($lc,'erwinia\p{Zs}?carotovora') and not(italic[contains(.,'Erwinia carotovora')])" role="info" id="erwiniascarotovora-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Erwinia carotovora' - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#hsapiens-ref-article-title-check" test="matches($lc,'h\.\p{Zs}?sapiens') and not(italic[contains(.,'H. sapiens')])" role="info" id="hsapiens-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'H. sapiens' - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#enterococcussfaecalis-ref-article-title-check" test="matches($lc,'enterococcus\p{Zs}?faecalis') and not(italic[contains(.,'Enterococcus faecalis')])" role="info" id="enterococcussfaecalis-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Enterococcus faecalis' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#xlaevis-ref-article-title-check" test="matches($lc,'x\.\p{Zs}?laevis') and not(italic[contains(.,'X. laevis')])" role="info" id="xlaevis-ref-article-title-check">
        <name/> contains an organism - 'X. laevis' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#xenopuslaevis-ref-article-title-check" test="matches($lc,'xenopus\p{Zs}?laevis') and not(italic[contains(.,'Xenopus laevis')])" role="info" id="xenopuslaevis-ref-article-title-check">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#mmusculus-ref-article-title-check" test="matches($lc,'m\.\p{Zs}?musculus') and not(italic[contains(.,'M. musculus')])" role="info" id="mmusculus-ref-article-title-check">
        <name/> contains an organism - 'M. musculus' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#musmusculus-ref-article-title-check" test="matches($lc,'mus\p{Zs}?musculus') and not(italic[contains(.,'Mus musculus')])" role="info" id="musmusculus-ref-article-title-check">
        <name/> contains an organism - 'Mus musculus' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#dimmigrans-ref-article-title-check" test="matches($lc,'d\.\p{Zs}?immigrans') and not(italic[contains(.,'D. immigrans')])" role="info" id="dimmigrans-ref-article-title-check">
        <name/> contains an organism - 'D. immigrans' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#drosophilaimmigrans-ref-article-title-check" test="matches($lc,'drosophila\p{Zs}?immigrans') and not(italic[contains(.,'Drosophila immigrans')])" role="info" id="drosophilaimmigrans-ref-article-title-check">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#dsubobscura-ref-article-title-check" test="matches($lc,'d\.\p{Zs}?subobscura') and not(italic[contains(.,'D. subobscura')])" role="info" id="dsubobscura-ref-article-title-check">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#daffinis-ref-article-title-check" test="matches($lc,'d\.\p{Zs}?affinis') and not(italic[contains(.,'D. affinis')])" role="info" id="daffinis-ref-article-title-check">
        <name/> contains an organism - 'D. affinis' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#drosophilaaffinis-ref-article-title-check" test="matches($lc,'drosophila\p{Zs}?affinis') and not(italic[contains(.,'Drosophila affinis')])" role="info" id="drosophilaaffinis-ref-article-title-check">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#dobscura-ref-article-title-check" test="matches($lc,'d\.\p{Zs}?obscura') and not(italic[contains(.,'D. obscura')])" role="info" id="dobscura-ref-article-title-check">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#pplantaginis-ref-article-title-check" test="matches($lc,'p\.\p{Zs}?plantaginis') and not(italic[contains(.,'P. plantaginis')])" role="info" id="pplantaginis-ref-article-title-check">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#podosphaeraplantaginis-ref-article-title-check" test="matches($lc,'podosphaera\p{Zs}?plantaginis') and not(italic[contains(.,'Podosphaera plantaginis')])" role="info" id="podosphaeraplantaginis-ref-article-title-check">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#medulis-ref-article-title-check" test="matches($lc,'m\.\p{Zs}?edulis') and not(italic[contains(.,'M. edulis')])" role="info" id="medulis-ref-article-title-check">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#mchilensis-ref-article-title-check" test="matches($lc,'m\.\p{Zs}?chilensis') and not(italic[contains(.,'M. chilensis')])" role="info" id="mchilensis-ref-article-title-check">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#pknowlesi-ref-article-title-check" test="matches($lc,'p\.\p{Zs}?knowlesi') and not(italic[contains(.,'P. knowlesi')])" role="info" id="pknowlesi-ref-article-title-check">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#paeruginosa-ref-article-title-check" test="matches($lc,'p\.\p{Zs}?aeruginosa') and not(italic[contains(.,'P. aeruginosa')])" role="info" id="paeruginosa-ref-article-title-check">
        <name/> contains an organism - 'P. aeruginosa' - but there is no italic element with that correct capitalisation or spacing.</report>

    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#pseudomonasaeruginosa-ref-article-title-check" test="matches($lc,'pseudomonas\p{Zs}?aeruginosa') and not(italic[contains(.,'Pseudomonas aeruginosa')])" role="info" id="pseudomonasaeruginosa-ref-article-title-check">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#trypanosomabrucei-ref-article-title-check" test="matches($lc,'trypanosoma\p{Zs}?brucei') and not(italic[contains(.,'Trypanosoma brucei')])" role="info" id="trypanosomabrucei-ref-article-title-check">
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#tbrucei-ref-article-title-check" test="matches($lc,'t\.\p{Zs}?gondii') and not(italic[contains(.,'T. gondii')])" role="info" id="tgondii-ref-article-title-check">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#drerio-ref-article-title-check" test="matches($lc,'d\.\p{Zs}?rerio') and not(italic[contains(.,'D. rerio')])" role="info" id="drerio-ref-article-title-check">
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#daniorerio-ref-article-title-check" test="matches($lc,'danio\p{Zs}?rerio') and not(italic[contains(.,'Danio rerio')])" role="info" id="daniorerio-ref-article-title-check">
      ...
      <report test="matches($lc,'yimenosaurus') and not(italic[contains(.,'Yimenosaurus')])" role="info" id="yimenosaurus-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Yimenosaurus' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'haya\p{Zs}?griva') and not(italic[contains(.,'Haya griva')])" role="info" id="hayagriva-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Haya griva' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'polacanthus\p{Zs}?foxii') and not(italic[contains(.,'Polacanthus foxii')])" role="info" id="polacanthusfoxii-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Polacanthus foxii' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'p\.\p{Zs}?foxii') and not(italic[contains(.,'P. foxii')])" role="info" id="pfoxii-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'P. foxii' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'s\.\p{Zs}?lawleri') and not(italic[contains(.,'S. lawleri')])" role="info" id="slawleri-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'S. lawleri' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'europelta\p{Zs}?carbonensis') and not(italic[contains(.,'Europelta carbonensis')])" role="info" id="europeltacarbonensis-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Europelta carbonensis' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'e\.\p{Zs}?carbonensis') and not(italic[contains(.,'E. carbonensis')])" role="info" id="ecarbonensis-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'E. carbonensis' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'s\.\p{Zs}?stenops') and not(italic[contains(.,'S. stenops')])" role="info" id="sstenops-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'S. stenops' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'pinacosaurus\p{Zs}?grangeri') and not(italic[contains(.,'Pinacosaurus grangeri')])" role="info" id="pinacosaurusgrangeri-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Pinacosaurus grangeri' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'p\.\p{Zs}?grangeri') and not(italic[contains(.,'P. grangeri')])" role="info" id="pgrangeri-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'P. grangeri' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'hungarosaurus\p{Zs}?tormai') and not(italic[contains(.,'Hungarosaurus tormai')])" role="info" id="Hungarosaurustormai-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Hungarosaurus tormai' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'h\.\p{Zs}?tormai') and not(italic[contains(.,'H. tormai')])" role="info" id="htormai-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'H. tormai' - but there is no italic element with that correct capitalisation or spacing.</report>  
      
      <report test="matches($lc,'bienosaurus\p{Zs}?lufengensis') and not(italic[contains(.,'Bienosaurus lufengensis')])" role="info" id="bienosauruslufengensis-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Bienosaurus lufengensis' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'c\.\p{Zs}?chunghoensis') and not(italic[contains(.,'C. chunghoensis')])" role="info" id="cchunghoensis-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'C. chunghoensis' - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#xenopus-ref-article-title-check" test="matches($lc,'xenopus') and not(italic[contains(.,'Xenopus')])" role="info" id="xenopus-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Xenopus' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'g\.\p{Zs}?beringei') and not(italic[contains(.,'G. beringei')])" role="info" id="gberingei-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'G. beringei' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'gorilla\p{Zs}?beringei') and not(italic[contains(.,'Gorilla beringei')])" role="info" id="gorillaberingei-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Gorilla beringei' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'m\.\p{Zs}?assamensis') and not(italic[contains(.,'M. assamensis')])" role="info" id="massamensis-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'M. assamensis' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'macaca\p{Zs}?fuscata') and not(italic[contains(.,'Macaca fuscata')])" role="info" id="macacafuscata-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Macaca fuscata' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'m\.\p{Zs}?mulatta') and not(italic[contains(.,'M. mulatta')])" role="info" id="mmulatta-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'M. mulatta' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'macaca\p{Zs}?mulatta') and not(italic[contains(.,'Macaca mulatta')])" role="info" id="macacamulatta-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Macaca mulatta' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'macaca\p{Zs}?nemestrina') and not(italic[contains(.,'Macaca nemestrina')])" role="info" id="macacanemestrina-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Macaca nemestrina' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'p\.\p{Zs}?anubis') and not(italic[contains(.,'P. anubis')])" role="info" id="panubis-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'P. anubis' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'papio\p{Zs}?anubis') and not(italic[contains(.,'Papio anubis')])" role="info" id="papioanubis-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Papio anubis' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'papio\p{Zs}?hamadryas') and not(italic[contains(.,'Papio hamadryas')])" role="info" id="papiohamadryas-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Papio hamadryas' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'p\.\p{Zs}?paniscus') and not(italic[contains(.,'P. paniscus')])" role="info" id="ppaniscus-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'P. paniscus' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'pan\p{Zs}?paniscus') and not(italic[contains(.,'Pan paniscus')])" role="info" id="panpaniscus-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Pan paniscus' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'p\.\p{Zs}?troglodytes') and not(italic[contains(.,'P. troglodytes')])" role="info" id="ptroglodytes-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'P. troglodytes' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'m\.\p{Zs}?fortuitum') and not(italic[contains(.,'M. fortuitum')])" role="info" id="mfortuitum-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'M. fortuitum' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'mycobacterium\p{Zs}?fortuitum') and not(italic[contains(.,'Mycobacterium fortuitum')])" role="info" id="mycobacteriumfortuitum-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'Mycobacterium fortuitum' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report test="matches($lc,'m\.\p{Zs}?tuberculosis') and not(italic[contains(.,'M. tuberculosis')])" role="info" id="mtuberculosis-ref-article-title-check">ref <value-of select="ancestor::ref/@id"/> references an organism - 'M. tuberculosis' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-bssubtilis-article-title-check" test="matches($lc,'b\.\p{Zs}?subtilis') and not(italic[contains(.,'B. subtilis')])" role="warning" id="bssubtilis-article-title-check"><name/> contains an organism - 'B. subtilis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-bacillusssubtilis-article-title-check" test="matches($lc,'bacillus\p{Zs}?subtilis') and not(italic[contains(.,'Bacillus subtilis')])" role="warning" id="bacillusssubtilis-article-title-check"><name/> contains an organism - 'Bacillus subtilis' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-drosophilasmelanogaster-article-title-check" test="matches($lc,'drosophila\p{Zs}?melanogaster') and not(italic[contains(.,'Drosophila melanogaster')])" role="warning" id="drosophilasmelanogaster-article-title-check"><name/> contains an organism - 'Drosophila melanogaster' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-escoli-article-title-check" test="matches($lc,'e\.\p{Zs}?coli') and not(italic[contains(.,'E. coli')])" role="warning" id="escoli-article-title-check"><name/> contains an organism - 'E. coli' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-escherichiascoli-article-title-check" test="matches($lc,'escherichia\p{Zs}?coli') and not(italic[contains(.,'Escherichia coli')])" role="warning" id="escherichiascoli-article-title-check"><name/> contains an organism - 'Escherichia coli' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-schizosaccharomycesspombe-article-title-check" test="matches($lc,'schizosaccharomyces\p{Zs}?pombe') and not(italic[contains(.,'Schizosaccharomyces pombe')])" role="warning" id="schizosaccharomycesspombe-article-title-check"><name/> contains an organism - 'Schizosaccharomyces pombe' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-sscerevisiae-article-title-check" test="matches($lc,'s\.\p{Zs}?cerevisiae') and not(italic[contains(.,'S. cerevisiae')])" role="warning" id="sscerevisiae-article-title-check"><name/> contains an organism - 'S. cerevisiae' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-saccharomycesscerevisiae-article-title-check" test="matches($lc,'saccharomyces\p{Zs}?cerevisiae') and not(italic[contains(.,'Saccharomyces cerevisiae')])" role="warning" id="saccharomycesscerevisiae-article-title-check"><name/> contains an organism - 'Saccharomyces cerevisiae' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-cselegans-article-title-check" test="matches($lc,'c\.\p{Zs}?elegans') and not(italic[contains(.,'C. elegans')])" role="warning" id="cselegans-article-title-check"><name/> contains an organism - 'C. elegans' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-caenorhabditisselegans-article-title-check" test="matches($lc,'caenorhabditis\p{Zs}?elegans') and not(italic[contains(.,'Caenorhabditis elegans')])" role="warning" id="caenorhabditisselegans-article-title-check"><name/> contains an organism - 'Caenorhabditis elegans' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-asthaliana-article-title-check" test="matches($lc,'a\.\p{Zs}?thaliana') and not(italic[contains(.,'A. thaliana')])" role="warning" id="asthaliana-article-title-check"><name/> contains an organism - 'A. thaliana' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-arabidopsissthaliana-article-title-check" test="matches($lc,'arabidopsis\p{Zs}?thaliana') and not(italic[contains(.,'Arabidopsis thaliana')])" role="warning" id="arabidopsissthaliana-article-title-check"><name/> contains an organism - 'Arabidopsis thaliana' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-msthermophila-article-title-check" test="matches($lc,'m\.\p{Zs}?thermophila') and not(italic[contains(.,'M. thermophila')])" role="warning" id="msthermophila-article-title-check"><name/> contains an organism - 'M. thermophila' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-myceliophthorasthermophila-article-title-check" test="matches($lc,'myceliophthora\p{Zs}?thermophila') and not(italic[contains(.,'Myceliophthora thermophila')])" role="warning" id="myceliophthorasthermophila-article-title-check"><name/> contains an organism - 'Myceliophthora thermophila' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-dictyostelium-article-title-check" test="matches($lc,'dictyostelium') and not(italic[contains(.,'Dictyostelium')])" role="warning" id="dictyostelium-article-title-check"><name/> contains an organism - 'Dictyostelium' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-psfalciparum-article-title-check" test="matches($lc,'p\.\p{Zs}?falciparum') and not(italic[contains(.,'P. falciparum')])" role="warning" id="psfalciparum-article-title-check"><name/> contains an organism - 'P. falciparum' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-plasmodiumsfalciparum-article-title-check" test="matches($lc,'plasmodium\p{Zs}?falciparum') and not(italic[contains(.,'Plasmodium falciparum')])" role="warning" id="plasmodiumsfalciparum-article-title-check"><name/> contains an organism - 'Plasmodium falciparum' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-ssenterica-article-title-check" test="matches($lc,'s\.\p{Zs}?enterica') and not(italic[contains(.,'S. enterica')])" role="warning" id="ssenterica-article-title-check"><name/> contains an organism - 'S. enterica' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-salmonellasenterica-article-title-check" test="matches($lc,'salmonella\p{Zs}?enterica') and not(italic[contains(.,'Salmonella enterica')])" role="warning" id="salmonellasenterica-article-title-check"><name/> contains an organism - 'Salmonella enterica' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-sspyogenes-article-title-check" test="matches($lc,'s\.\p{Zs}?pyogenes') and not(italic[contains(.,'S. pyogenes')])" role="warning" id="sspyogenes-article-title-check"><name/> contains an organism - 'S. pyogenes' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-streptococcusspyogenes-article-title-check" test="matches($lc,'streptococcus\p{Zs}?pyogenes') and not(italic[contains(.,'Streptococcus pyogenes')])" role="warning" id="streptococcusspyogenes-article-title-check"><name/> contains an organism - 'Streptococcus pyogenes' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-psdumerilii-article-title-check" test="matches($lc,'p\.\p{Zs}?dumerilii') and not(italic[contains(.,'P. dumerilii')])" role="warning" id="psdumerilii-article-title-check"><name/> contains an organism - 'P. dumerilii' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-platynereissdumerilii-article-title-check" test="matches($lc,'platynereis\p{Zs}?dumerilii') and not(italic[contains(.,'Platynereis dumerilii')])" role="warning" id="platynereissdumerilii-article-title-check"><name/> contains an organism - 'Platynereis dumerilii' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-pscynocephalus-article-title-check" test="matches($lc,'p\.\p{Zs}?cynocephalus') and not(italic[contains(.,'P. cynocephalus')])" role="warning" id="pscynocephalus-article-title-check"><name/> contains an organism - 'P. cynocephalus' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-papioscynocephalus-article-title-check" test="matches($lc,'papio\p{Zs}?cynocephalus') and not(italic[contains(.,'Papio cynocephalus')])" role="warning" id="papioscynocephalus-article-title-check"><name/> contains an organism - 'Papio cynocephalus' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-oncopeltussfasciatus-article-title-check" test="matches($lc,'oncopeltus\p{Zs}?fasciatus') and not(italic[contains(.,'Oncopeltus fasciatus')])" role="warning" id="oncopeltussfasciatus-article-title-check"><name/> contains an organism - 'Oncopeltus fasciatus' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-nscrassa-article-title-check" test="matches($lc,'n\.\p{Zs}?crassa') and not(italic[contains(.,'N. crassa')])" role="warning" id="nscrassa-article-title-check"><name/> contains an organism - 'N. crassa' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-neurosporascrassa-article-title-check" test="matches($lc,'neurospora\p{Zs}?crassa') and not(italic[contains(.,'Neurospora crassa')])" role="warning" id="neurosporascrassa-article-title-check"><name/> contains an organism - 'Neurospora crassa' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-cionasintestinalis-article-title-check" test="matches($lc,'ciona\p{Zs}?intestinalis') and not(italic[contains(.,'Ciona intestinalis')])" role="warning" id="cionasintestinalis-article-title-check"><name/> contains an organism - 'Ciona intestinalis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-escuniculi-article-title-check" test="matches($lc,'e\.\p{Zs}?cuniculi') and not(italic[contains(.,'E. cuniculi')])" role="warning" id="escuniculi-article-title-check"><name/> contains an organism - 'E. cuniculi' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-encephalitozoonscuniculi-article-title-check" test="matches($lc,'encephalitozoon\p{Zs}?cuniculi') and not(italic[contains(.,'Encephalitozoon cuniculi')])" role="warning" id="encephalitozoonscuniculi-article-title-check"><name/> contains an organism - 'Encephalitozoon cuniculi' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-hssalinarum-article-title-check" test="matches($lc,'h\.\p{Zs}?salinarum') and not(italic[contains(.,'H. salinarum')])" role="warning" id="hssalinarum-article-title-check"><name/> contains an organism - 'H. salinarum' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-halobacteriumssalinarum-article-title-check" test="matches($lc,'halobacterium\p{Zs}?salinarum') and not(italic[contains(.,'Halobacterium salinarum')])" role="warning" id="halobacteriumssalinarum-article-title-check"><name/> contains an organism - 'Halobacterium salinarum' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-sulfolobusssolfataricus-article-title-check" test="matches($lc,'sulfolobus\p{Zs}?solfataricus') and not(italic[contains(.,'Sulfolobus solfataricus')])" role="warning" id="sulfolobusssolfataricus-article-title-check"><name/> contains an organism - 'Sulfolobus solfataricus' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-ssmediterranea-article-title-check" test="matches($lc,'s\.\p{Zs}?mediterranea') and not(italic[contains(.,'S. mediterranea')])" role="warning" id="ssmediterranea-article-title-check"><name/> contains an organism - 'S. mediterranea' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-schmidteasmediterranea-article-title-check" test="matches($lc,'schmidtea\p{Zs}?mediterranea') and not(italic[contains(.,'Schmidtea mediterranea')])" role="warning" id="schmidteasmediterranea-article-title-check"><name/> contains an organism - 'Schmidtea mediterranea' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-ssrosetta-article-title-check" test="matches($lc,'s\.\p{Zs}?rosetta') and not(italic[contains(.,'S. rosetta')])" role="warning" id="ssrosetta-article-title-check"><name/> contains an organism - 'S. rosetta' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-salpingoecasrosetta-article-title-check" test="matches($lc,'salpingoeca\p{Zs}?rosetta') and not(italic[contains(.,'Salpingoeca rosetta')])" role="warning" id="salpingoecasrosetta-article-title-check"><name/> contains an organism - 'Salpingoeca rosetta' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-nsvectensis-article-title-check" test="matches($lc,'n\.\p{Zs}?vectensis') and not(italic[contains(.,'N. vectensis')])" role="warning" id="nsvectensis-article-title-check"><name/> contains an organism - 'N. vectensis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-nematostellasvectensis-article-title-check" test="matches($lc,'nematostella\p{Zs}?vectensis') and not(italic[contains(.,'Nematostella vectensis')])" role="warning" id="nematostellasvectensis-article-title-check"><name/> contains an organism - 'Nematostella vectensis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-ssaureus-article-title-check" test="matches($lc,'s\.\p{Zs}?aureus') and not(italic[contains(.,'S. aureus')])" role="warning" id="ssaureus-article-title-check"><name/> contains an organism - 'S. aureus' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-staphylococcussaureus-article-title-check" test="matches($lc,'staphylococcus\p{Zs}?aureus') and not(italic[contains(.,'Staphylococcus aureus')])" role="warning" id="staphylococcussaureus-article-title-check"><name/> contains an organism - 'Staphylococcus aureus' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-vscholerae-article-title-check" test="matches($lc,'v\.\p{Zs}?cholerae') and not(italic[contains(.,'V. cholerae')])" role="warning" id="vscholerae-article-title-check"><name/> contains an organism - 'V. cholerae' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-vibrioscholerae-article-title-check" test="matches($lc,'vibrio\p{Zs}?cholerae') and not(italic[contains(.,'Vibrio cholerae')])" role="warning" id="vibrioscholerae-article-title-check"><name/> contains an organism - 'Vibrio cholerae' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-tsthermophila-article-title-check" test="matches($lc,'t\.\p{Zs}?thermophila') and not(italic[contains(.,'T. thermophila')])" role="warning" id="tsthermophila-article-title-check"><name/> contains an organism - 'T. thermophila' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-tetrahymenasthermophila-article-title-check" test="matches($lc,'tetrahymena\p{Zs}?thermophila') and not(italic[contains(.,'Tetrahymena thermophila')])" role="warning" id="tetrahymenasthermophila-article-title-check"><name/> contains an organism - 'Tetrahymena thermophila' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-csreinhardtii-article-title-check" test="matches($lc,'c\.\p{Zs}?reinhardtii') and not(italic[contains(.,'C. reinhardtii')])" role="warning" id="csreinhardtii-article-title-check"><name/> contains an organism - 'C. reinhardtii' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-chlamydomonassreinhardtii-article-title-check" test="matches($lc,'chlamydomonas\p{Zs}?reinhardtii') and not(italic[contains(.,'Chlamydomonas reinhardtii')])" role="warning" id="chlamydomonassreinhardtii-article-title-check"><name/> contains an organism - 'Chlamydomonas reinhardtii' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-nsattenuata-article-title-check" test="matches($lc,'n\.\p{Zs}?attenuata') and not(italic[contains(.,'N. attenuata')])" role="warning" id="nsattenuata-article-title-check"><name/> contains an organism - 'N. attenuata' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-nicotianasattenuata-article-title-check" test="matches($lc,'nicotiana\p{Zs}?attenuata') and not(italic[contains(.,'Nicotiana attenuata')])" role="warning" id="nicotianasattenuata-article-title-check"><name/> contains an organism - 'Nicotiana attenuata' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-escarotovora-article-title-check" test="matches($lc,'e\.\p{Zs}?carotovora') and not(italic[contains(.,'E. carotovora')])" role="warning" id="escarotovora-article-title-check"><name/> contains an organism - 'E. carotovora' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-erwiniascarotovora-article-title-check" test="matches($lc,'erwinia\p{Zs}?carotovora') and not(italic[contains(.,'Erwinia carotovora')])" role="warning" id="erwiniascarotovora-article-title-check"><name/> contains an organism - 'Erwinia carotovora' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-hsapiens-article-title-check" test="matches($lc,'h\.\p{Zs}?sapiens') and not(italic[contains(.,'H. sapiens')])" role="warning" id="hsapiens-article-title-check"><name/> contains an organism - 'H. sapiens' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-homosapiens-article-title-check" test="matches($lc,'homo\p{Zs}?sapiens') and not(italic[contains(.,'Homo sapiens')])" role="warning" id="homosapiens-article-title-check"><name/> contains an organism - 'Homo sapiens' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-ctrachomatis-article-title-check" test="matches($lc,'c\.\p{Zs}?trachomatis') and not(italic[contains(.,'C. trachomatis')])" role="warning" id="ctrachomatis-article-title-check"><name/> contains an organism - 'C. trachomatis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-chlamydiatrachomatis-article-title-check" test="matches($lc,'chlamydia\p{Zs}?trachomatis') and not(italic[contains(.,'Chlamydia trachomatis')])" role="warning" id="chlamydiatrachomatis-article-title-check"><name/> contains an organism - 'Chlamydia trachomatis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-esfaecalis-article-title-check" test="matches($lc,'e\.\p{Zs}?faecalis') and not(italic[contains(.,'E. faecalis')])" role="warning" id="esfaecalis-article-title-check"><name/> contains an organism - 'E. faecalis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-enterococcussfaecalis-article-title-check" test="matches($lc,'enterococcus\p{Zs}?faecalis') and not(italic[contains(.,'Enterococcus faecalis')])" role="warning" id="enterococcussfaecalis-article-title-check"><name/> contains an organism - 'Enterococcus faecalis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-xlaevis-article-title-check" test="matches($lc,'x\.\p{Zs}?laevis') and not(italic[contains(.,'X. laevis')])" role="warning" id="xlaevis-article-title-check"><name/> contains an organism - 'X. laevis' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-xtropicalis-article-title-check" test="matches($lc,'x\.\p{Zs}?tropicalis') and not(italic[contains(.,'X. tropicalis')])" role="warning" id="xtropicalis-article-title-check"><name/> contains an organism - 'X. tropicalis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-xenopustropicalis-article-title-check" test="matches($lc,'xenopus\p{Zs}?tropicalis') and not(italic[contains(.,'Xenopus tropicalis')])" role="warning" id="xenopustropicalis-article-title-check"><name/> contains an organism - 'Xenopus tropicalis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-mmusculus-article-title-check" test="matches($lc,'m\.\p{Zs}?musculus') and not(italic[contains(.,'M. musculus')])" role="warning" id="mmusculus-article-title-check"><name/> contains an organism - 'M. musculus' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-musmusculus-article-title-check" test="matches($lc,'mus\p{Zs}?musculus') and not(italic[contains(.,'Mus musculus')])" role="warning" id="musmusculus-article-title-check"><name/> contains an organism - 'Mus musculus' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-dimmigrans-article-title-check" test="matches($lc,'d\.\p{Zs}?immigrans') and not(italic[contains(.,'D. immigrans')])" role="warning" id="dimmigrans-article-title-check"><name/> contains an organism - 'D. immigrans' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-drosophilaimmigrans-article-title-check" test="matches($lc,'drosophila\p{Zs}?immigrans') and not(italic[contains(.,'Drosophila immigrans')])" role="warning" id="drosophilaimmigrans-article-title-check"><name/> contains an organism - 'Drosophila immigrans' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-dsubobscura-article-title-check" test="matches($lc,'d\.\p{Zs}?subobscura') and not(italic[contains(.,'D. subobscura')])" role="warning" id="dsubobscura-article-title-check"><name/> contains an organism - 'D. subobscura' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-drosophilasubobscura-article-title-check" test="matches($lc,'drosophila\p{Zs}?subobscura') and not(italic[contains(.,'Drosophila subobscura')])" role="warning" id="drosophilasubobscura-article-title-check"><name/> contains an organism - 'Drosophila subobscura' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-daffinis-article-title-check" test="matches($lc,'d\.\p{Zs}?affinis') and not(italic[contains(.,'D. affinis')])" role="warning" id="daffinis-article-title-check"><name/> contains an organism - 'D. affinis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-drosophilaaffinis-article-title-check" test="matches($lc,'drosophila\p{Zs}?affinis') and not(italic[contains(.,'Drosophila affinis')])" role="warning" id="drosophilaaffinis-article-title-check"><name/> contains an organism - 'Drosophila affinis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-dobscura-article-title-check" test="matches($lc,'d\.\p{Zs}?obscura') and not(italic[contains(.,'D. obscura')])" role="warning" id="dobscura-article-title-check"><name/> contains an organism - 'D. obscura' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-drosophilaobscura-article-title-check" test="matches($lc,'drosophila\p{Zs}?obscura') and not(italic[contains(.,'Drosophila obscura')])" role="warning" id="drosophilaobscura-article-title-check"><name/> contains an organism - 'Drosophila obscura' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-ftularensis-article-title-check" test="matches($lc,'f\.\p{Zs}?tularensis') and not(italic[contains(.,'F. tularensis')])" role="warning" id="ftularensis-article-title-check"><name/> contains an organism - 'F. tularensis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-francisellatularensis-article-title-check" test="matches($lc,'francisella\p{Zs}?tularensis') and not(italic[contains(.,'Francisella tularensis')])" role="warning" id="francisellatularensis-article-title-check"><name/> contains an organism - 'Francisella tularensis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-pplantaginis-article-title-check" test="matches($lc,'p\.\p{Zs}?plantaginis') and not(italic[contains(.,'P. plantaginis')])" role="warning" id="pplantaginis-article-title-check"><name/> contains an organism - 'P. plantaginis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-podosphaeraplantaginis-article-title-check" test="matches($lc,'podosphaera\p{Zs}?plantaginis') and not(italic[contains(.,'Podosphaera plantaginis')])" role="warning" id="podosphaeraplantaginis-article-title-check"><name/> contains an organism - 'Podosphaera plantaginis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-planceolata-article-title-check" test="matches($lc,'p\.\p{Zs}?lanceolata') and not(italic[contains(.,'P. lanceolata')])" role="warning" id="planceolata-article-title-check"><name/> contains an organism - 'P. lanceolata' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-plantagolanceolata-article-title-check" test="matches($lc,'plantago\p{Zs}?lanceolata') and not(italic[contains(.,'Plantago lanceolata')])" role="warning" id="plantagolanceolata-article-title-check"><name/> contains an organism - 'Plantago lanceolata' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-mtrossulus-article-title-check" test="matches($lc,'m\.\p{Zs}?trossulus') and not(italic[contains(.,'M. trossulus')])" role="info" id="mtrossulus-article-title-check"><name/> contains an organism - 'M. trossulus' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-medulis-article-title-check" test="matches($lc,'m\.\p{Zs}?edulis') and not(italic[contains(.,'M. edulis')])" role="info" id="medulis-article-title-check"><name/> contains an organism - 'M. edulis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-mytilusedulis-article-title-check" test="matches($lc,'mytilus\p{Zs}?edulis') and not(italic[contains(.,'Mytilus edulis')])" role="info" id="mytilusedulis-article-title-check"><name/> contains an organism - 'Mytilus edulis' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-mytiluschilensis-article-title-check" test="matches($lc,'mytilus\p{Zs}?chilensis') and not(italic[contains(.,'Mytilus chilensis')])" role="info" id="mytiluschilensis-article-title-check"><name/> contains an organism - 'Mytilus chilensis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-umaydis-article-title-check" test="matches($lc,'u\.\p{Zs}?maydis') and not(italic[contains(.,'U. maydis')])" role="info" id="umaydis-article-title-check"><name/> contains an organism - 'U. maydis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-ustilagomaydis-article-title-check" test="matches($lc,'ustilago\p{Zs}?maydis') and not(italic[contains(.,'Ustilago maydis')])" role="info" id="ustilagomaydis-article-title-check"><name/> contains an organism - 'Ustilago maydis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-pknowlesi-article-title-check" test="matches($lc,'p\.\p{Zs}?knowlesi') and not(italic[contains(.,'P. knowlesi')])" role="info" id="pknowlesi-article-title-check"><name/> contains an organism - 'P. knowlesi' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-paeruginosa-article-title-check" test="matches($lc,'p\.\p{Zs}?aeruginosa') and not(italic[contains(.,'P. aeruginosa')])" role="info" id="paeruginosa-article-title-check"><name/> contains an organism - 'P. aeruginosa' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-pseudomonasaeruginosa-article-title-check" test="matches($lc,'pseudomonas\p{Zs}?aeruginosa') and not(italic[contains(.,'Pseudomonas aeruginosa')])" role="info" id="pseudomonasaeruginosa-article-title-check"><name/> contains an organism - 'Pseudomonas aeruginosa' - but there is no italic element with that correct capitalisation or spacing.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-trypanosomabrucei-article-title-check" test="matches($lc,'trypanosoma\p{Zs}?brucei') and not(italic[contains(.,'Trypanosoma brucei')])" role="warning" id="trypanosomabrucei-article-title-check"><name/> contains an organism - 'Trypanosoma brucei' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-tgondii-article-title-check" test="matches($lc,'t\.\p{Zs}?gondii') and not(italic[contains(.,'T. gondii')])" role="warning" id="tgondii-article-title-check"><name/> contains an organism - 'T. gondii' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-toxoplasmagondii-article-title-check" test="matches($lc,'toxoplasma\p{Zs}?gondii') and not(italic[contains(.,'Toxoplasma gondii')])" role="warning" id="toxoplasmagondii-article-title-check"><name/> contains an organism - 'Toxoplasma gondii' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-drerio-article-title-check" test="matches($lc,'d\.\p{Zs}?rerio') and not(italic[contains(.,'D. rerio')])" role="warning" id="drerio-article-title-check"><name/> contains an organism - 'D. rerio' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-daniorerio-article-title-check" test="matches($lc,'danio\p{Zs}?rerio') and not(italic[contains(.,'Danio rerio')])" role="warning" id="daniorerio-article-title-check"><name/> contains an organism - 'Danio rerio' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-yimenosaurus-article-title-check" test="matches($lc,'yimenosaurus') and not(italic[contains(.,'Yimenosaurus')])" role="warning" id="yimenosaurus-article-title-check"><name/> contains an organism - 'Yimenosaurus' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-scelidosaurusharrisonii-article-title-check" test="matches($lc,'scelidosaurus\p{Zs}?harrisonii') and not(italic[contains(.,'Scelidosaurus harrisonii')])" role="warning" id="scelidosaurusharrisonii-article-title-check"><name/> contains an organism - 'Scelidosaurus harrisonii' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-sharrisonii-article-title-check" test="matches($lc,'s\.\p{Zs}?harrisonii') and not(italic[contains(.,'S. harrisonii')])" role="warning" id="sharrisonii-article-title-check"><name/> contains an organism - 'S. harrisonii' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-hayagriva-article-title-check" test="matches($lc,'haya\p{Zs}?griva') and not(italic[contains(.,'Haya griva')])" role="warning" id="hayagriva-article-title-check"><name/> contains an organism - 'Haya griva' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-hgriva-article-title-check" test="matches($lc,'h\.\p{Zs}?griva') and not(italic[contains(.,'H. griva')])" role="warning" id="hgriva-article-title-check"><name/> contains an organism - 'H. griva' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-polacanthusfoxii-article-title-check" test="matches($lc,'polacanthus\p{Zs}?foxii') and not(italic[contains(.,'Polacanthus foxii')])" role="warning" id="polacanthusfoxii-article-title-check"><name/> contains an organism - 'Polacanthus foxii' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-pfoxii-article-title-check" test="matches($lc,'p\.\p{Zs}?foxii') and not(italic[contains(.,'P. foxii')])" role="warning" id="pfoxii-article-title-check"><name/> contains an organism - 'P. foxii' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-scutellosauruslawleri-article-title-check" test="matches($lc,'scutellosaurus\p{Zs}?lawleri') and not(italic[contains(.,'Scutellosaurus lawleri')])" role="warning" id="scutellosauruslawleri-article-title-check"><name/> contains an organism - 'Scutellosaurus lawleri' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-slawleri-article-title-check" test="matches($lc,'s\.\p{Zs}?lawleri') and not(italic[contains(.,'S. lawleri')])" role="warning" id="slawleri-article-title-check"><name/> contains an organism - 'S. lawleri' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-saichaniachulsanensis-article-title-check" test="matches($lc,'saichania\p{Zs}?chulsanensis') and not(italic[contains(.,'Saichania chulsanensis')])" role="warning" id="saichaniachulsanensis-article-title-check"><name/> contains an organism - 'Saichania chulsanensis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-schulsanensis-article-title-check" test="matches($lc,'s\.\p{Zs}?chulsanensis') and not(italic[contains(.,'S. chulsanensis')])" role="warning" id="schulsanensis-article-title-check"><name/> contains an organism - 'S. chulsanensis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-gargoyleosaurusparkpinorum-article-title-check" test="matches($lc,'gargoyleosaurus\p{Zs}?parkpinorum') and not(italic[contains(.,'Gargoyleosaurus parkpinorum')])" role="warning" id="gargoyleosaurusparkpinorum-article-title-check"><name/> contains an organism - 'Gargoyleosaurus parkpinorum' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-gparkpinorum-article-title-check" test="matches($lc,'g\.\p{Zs}?parkpinorum') and not(italic[contains(.,'G. parkpinorum')])" role="warning" id="gparkpinorum-article-title-check"><name/> contains an organism - 'G. parkpinorum' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-europeltacarbonensis-article-title-check" test="matches($lc,'europelta\p{Zs}?carbonensis') and not(italic[contains(.,'Europelta carbonensis')])" role="warning" id="europeltacarbonensis-article-title-check"><name/> contains an organism - 'Europelta carbonensis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-ecarbonensis-article-title-check" test="matches($lc,'e\.\p{Zs}?carbonensis') and not(italic[contains(.,'E. carbonensis')])" role="warning" id="ecarbonensis-article-title-check"><name/> contains an organism - 'E. carbonensis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-stegosaurusstenops-article-title-check" test="matches($lc,'stegosaurus\p{Zs}?stenops') and not(italic[contains(.,'Stegosaurus stenops')])" role="warning" id="stegosaurusstenops-article-title-check"><name/> contains an organism - 'Stegosaurus stenops' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-sstenops-article-title-check" test="matches($lc,'s\.\p{Zs}?stenops') and not(italic[contains(.,'S. stenops')])" role="warning" id="sstenops-article-title-check"><name/> contains an organism - 'S. stenops' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-pinacosaurusgrangeri-article-title-check" test="matches($lc,'pinacosaurus\p{Zs}?grangeri') and not(italic[contains(.,'Pinacosaurus grangeri')])" role="warning" id="pinacosaurusgrangeri-article-title-check"><name/> contains an organism - 'Pinacosaurus grangeri' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-pgrangeri-article-title-check" test="matches($lc,'p\.\p{Zs}?grangeri') and not(italic[contains(.,'P. grangeri')])" role="warning" id="pgrangeri-article-title-check"><name/> contains an organism - 'P. grangeri' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-tatisaurusoehleri-article-title-check" test="matches($lc,'tatisaurus\p{Zs}?oehleri') and not(italic[contains(.,'Tatisaurus oehleri')])" role="warning" id="tatisaurusoehleri-article-title-check"><name/> contains an organism - 'Tatisaurus oehleri' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-toehleri-article-title-check" test="matches($lc,'t\.\p{Zs}?oehleri') and not(italic[contains(.,'T. oehleri')])" role="warning" id="toehleri-article-title-check"><name/> contains an organism - 'T. oehleri' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-hungarosaurustormai-article-title-check" test="matches($lc,'hungarosaurus\p{Zs}?tormai') and not(italic[contains(.,'Hungarosaurus tormai')])" role="warning" id="hungarosaurustormai-article-title-check"><name/> contains an organism - 'Hungarosaurus tormai' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-htormai-article-title-check" test="matches($lc,'h\.\p{Zs}?tormai') and not(italic[contains(.,'H. tormai')])" role="warning" id="htormai-article-title-check"><name/> contains an organism - 'H. tormai' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-lesothosaurusdiagnosticus-article-title-check" test="matches($lc,'lesothosaurus\p{Zs}?diagnosticus') and not(italic[contains(.,'Lesothosaurus diagnosticus')])" role="warning" id="lesothosaurusdiagnosticus-article-title-check"><name/> contains an organism - 'Lesothosaurus diagnosticus' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-ldiagnosticus-article-title-check" test="matches($lc,'l\.\p{Zs}?diagnosticus') and not(italic[contains(.,'L. diagnosticus')])" role="warning" id="ldiagnosticus-article-title-check"><name/> contains an organism - 'L. diagnosticus' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-bienosauruslufengensis-article-title-check" test="matches($lc,'bienosaurus\p{Zs}?lufengensis') and not(italic[contains(.,'Bienosaurus lufengensis')])" role="warning" id="bienosauruslufengensis-article-title-check"><name/> contains an organism - 'Bienosaurus lufengensis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-blufengensis-article-title-check" test="matches($lc,'b\.\p{Zs}?lufengensis') and not(italic[contains(.,'B. lufengensis')])" role="warning" id="blufengensis-article-title-check"><name/> contains an organism - 'B. lufengensis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-fabrosaurusaustralis-article-title-check" test="matches($lc,'fabrosaurus\p{Zs}?australis') and not(italic[contains(.,'Fabrosaurus australis')])" role="warning" id="fabrosaurusaustralis-article-title-check"><name/> contains an organism - 'Fabrosaurus australis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-faustralis-article-title-check" test="matches($lc,'f\.\p{Zs}?australis') and not(italic[contains(.,'F. australis')])" role="warning" id="faustralis-article-title-check"><name/> contains an organism - 'F. australis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-chinshakiangosauruschunghoensis-article-title-check" test="matches($lc,'chinshakiangosaurus\p{Zs}?chunghoensis') and not(italic[contains(.,'Chinshakiangosaurus chunghoensis')])" role="warning" id="chinshakiangosauruschunghoensis-article-title-check"><name/> contains an organism - 'Chinshakiangosaurus chunghoensis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-cchunghoensis-article-title-check" test="matches($lc,'c\.\p{Zs}?chunghoensis') and not(italic[contains(.,'C. chunghoensis')])" role="warning" id="cchunghoensis-article-title-check"><name/> contains an organism - 'C. chunghoensis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-euoplocephalustutus-article-title-check" test="matches($lc,'euoplocephalus\p{Zs}?tutus') and not(italic[contains(.,'Euoplocephalus tutus')])" role="warning" id="euoplocephalustutus-article-title-check"><name/> contains an organism - 'Euoplocephalus tutus' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-etutus-article-title-check" test="matches($lc,'e\.\p{Zs}?tutus') and not(italic[contains(.,'E. tutus')])" role="warning" id="etutus-article-title-check"><name/> contains an organism - 'E. tutus' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'g\.\p{Zs}?beringei') and not(italic[contains(.,'G. beringei')])" role="warning" id="gberingei-article-title-check"><name/> contains an organism - 'G. beringei' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'gorilla\p{Zs}?beringei') and not(italic[contains(.,'Gorilla beringei')])" role="warning" id="gorillaberingei-article-title-check"><name/> contains an organism - 'Gorilla beringei' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'m\.\p{Zs}?assamensis') and not(italic[contains(.,'M. assamensis')])" role="warning" id="massamensis-article-title-check"><name/> contains an organism - 'M. assamensis' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'macaca\p{Zs}?assamensis') and not(italic[contains(.,'Macaca assamensis')])" role="warning" id="macacaassamensis-article-title-check"><name/> contains an organism - 'Macaca assamensis' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'m\.\p{Zs}?fuscata') and not(italic[contains(.,'M. fuscata')])" role="warning" id="mfuscata-article-title-check"><name/> contains an organism - 'M. fuscata' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'macaca\p{Zs}?fuscata') and not(italic[contains(.,'Macaca fuscata')])" role="warning" id="macacafuscata-article-title-check"><name/> contains an organism - 'Macaca fuscata' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'m\.\p{Zs}?mulatta') and not(italic[contains(.,'M. mulatta')])" role="warning" id="mmulatta-article-title-check"><name/> contains an organism - 'M. mulatta' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'macaca\p{Zs}?mulatta') and not(italic[contains(.,'Macaca mulatta')])" role="warning" id="macacamulatta-article-title-check"><name/> contains an organism - 'Macaca mulatta' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'m\.\p{Zs}?nemestrina') and not(italic[contains(.,'M. nemestrina')])" role="warning" id="mnemestrina-article-title-check"><name/> contains an organism - 'M. nemestrina' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'macaca\p{Zs}?nemestrina') and not(italic[contains(.,'Macaca nemestrina')])" role="warning" id="macacanemestrina-article-title-check"><name/> contains an organism - 'Macaca nemestrina' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'m\.\p{Zs}?sphinx') and not(italic[contains(.,'M. sphinx')])" role="warning" id="msphinx-article-title-check"><name/> contains an organism - 'M. sphinx' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'mandrillus\p{Zs}?sphinx') and not(italic[contains(.,'Mandrillus sphinx')])" role="warning" id="mandrillussphinx-article-title-check"><name/> contains an organism - 'Mandrillus sphinx' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'p\.\p{Zs}?anubis') and not(italic[contains(.,'P. anubis')])" role="warning" id="panubis-article-title-check"><name/> contains an organism - 'P. anubis' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'papio\p{Zs}?anubis') and not(italic[contains(.,'Papio anubis')])" role="warning" id="papioanubis-article-title-check"><name/> contains an organism - 'Papio anubis' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'p\.\p{Zs}?hamadryas') and not(italic[contains(.,'P. hamadryas')])" role="warning" id="phamadryas-article-title-check"><name/> contains an organism - 'P. hamadryas' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report test="matches($lc,'papio\p{Zs}?hamadryas') and not(italic[contains(.,'Papio hamadryas')])" role="warning" id="papiohamadryas-article-title-check"><name/> contains an organism - 'Papio hamadryas' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'p\.\p{Zs}?paniscus') and not(italic[contains(.,'P. paniscus')])" role="warning" id="ppaniscus-article-title-check"><name/> contains an organism - 'P. paniscus' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'pan\p{Zs}?paniscus') and not(italic[contains(.,'Pan paniscus')])" role="warning" id="panpaniscus-article-title-check"><name/> contains an organism - 'Pan paniscus' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'p\.\p{Zs}?troglodytes') and not(italic[contains(.,'P. troglodytes')])" role="warning" id="ptroglodytes-article-title-check"><name/> contains an organism - 'P. troglodytes' - but there is no italic element with that correct capitalisation or spacing.</report>
      <report test="matches($lc,'pan\p{Zs}?troglodytes') and not(italic[contains(.,'Pan troglodytes')])" role="warning" id="pantroglodytes-article-title-check"><name/> contains an organism - 'Pan troglodytes' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report test="matches($lc,'m\.\p{Zs}?fortuitum') and not(italic[contains(.,'M. fortuitum')])" role="info" id="mfortuitum-article-title-check"><name/> contains an organism - 'M. fortuitum' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'mycobacterium\p{Zs}?fortuitum') and not(italic[contains(.,'Mycobacterium fortuitum')])" role="info" id="mycobacteriumfortuitum-article-title-check"><name/> contains an organism - 'Mycobacterium fortuitum' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report test="matches($lc,'m\.\p{Zs}?smegmatis') and not(italic[contains(.,'M. smegmatis')])" role="info" id="msmegmatis-article-title-check"><name/> contains an organism - 'M. smegmatis' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'mycobacterium\p{Zs}?smegmatis') and not(italic[contains(.,'Mycobacterium smegmatis')])" role="info" id="mycobacteriumsmegmatis-article-title-check"><name/> contains an organism - 'Mycobacterium smegmatis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report test="matches($lc,'m\.\p{Zs}?tuberculosis') and not(italic[contains(.,'M. tuberculosis')])" role="info" id="mtuberculosis-article-title-check"><name/> contains an organism - 'M. tuberculosis' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'mycobacterium\p{Zs}?tuberculosis') and not(italic[contains(.,'Mycobacterium tuberculosis')])" role="info" id="mycobacteriumtuberculosis-article-title-check"><name/> contains an organism - 'Mycobacterium tuberculosis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report test="matches($lc,'m\.\p{Zs}?bovis') and not(italic[contains(.,'M. bovis')])" role="info" id="mbovis-article-title-check"><name/> contains an organism - 'M. bovis' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'mycobacterium\p{Zs}?bovis') and not(italic[contains(.,'Mycobacterium bovis')])" role="info" id="mycobacteriumbovis-article-title-check"><name/> contains an organism - 'Mycobacterium bovis' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-drosophila-article-title-check" test="matches($lc,'drosophila') and not(italic[contains(text(),'Drosophila')])" role="warning" id="drosophila-article-title-check"><name/> contains an organism - 'Drosophila' - but there is no italic element with that correct capitalisation or spacing.</report>

      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h1ycf-xenopus-article-title-check" test="matches($lc,'xenopus') and not(italic[contains(.,'Xenopus')])" role="warning" id="xenopus-article-title-check"><name/> contains an organism - 'Xenopus' - but there is no italic element with that correct capitalisation or spacing.</report>
      
      <report test="matches($lc,'salmonella') and not(italic[contains(.,'Salmonella')])" role="warning" id="salmonella-article-title-check"><name/> contains an organism - 'Salmonella' - but there is no italic element with that correct capitalisation or spacing.</report>
  ...
  <pattern id="unallowed-symbol-tests-pattern"><rule context="p|td|th|title|xref|bold|italic|sub|sc|named-content|monospace|code|underline|fn|institution|ext-link" id="unallowed-symbol-tests">		
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#h7s38-Inc-presence" test="matches(.,' [Ii]nc\. |[Ii]nc\.\)|[Ii]nc\.,')" role="warning" id="Inc-presence"><name/> element contains 'Inc.' with a full stop. Remove the full stop.</report>
      
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#hkbnb-andand-presence" test="matches(.,' [Aa]nd [Aa]nd ')" role="warning" id="andand-presence"><name/> element contains ' and and ' which is very likely to be incorrect.</report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#figurefigure-presence" test="matches(.,'[Ff]igure [Ff]igure')" role="warning" id="figurefigure-presence"><name/> element contains ' figure figure ' which is very likely to be incorrect.</report>
      
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#hkbnb-plus-minus-presence" test="matches(translate(.,'—– ','-- '),'[\+\-]\p{Zs}+/\p{Zs}?[\+\-]|[\+\-]\p{Zs}?/\p{Zs}+[\+\-]')" role="warning" id="plus-minus-presence"><name/> element contains two plus or minus signs separated by a space and a forward slash (such as '+ /-'). Should the space be removed? - <value-of select="."/></report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supplementalfigure-presence" test="not(ancestor::sub-article) and matches(.,'\p{Zs}?[Ss]upplemental [Ff]igure')" role="warning" id="supplementalfigure-presence"><name/> element contains the phrase ' Supplemental figure ' which almost certainly needs updating. <name/> starts with - <value-of select="substring(.,1,25)"/></report>
      
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supplementalfile-presence" test="not(ancestor::sub-article) and matches(.,'\p{Zs}?[Ss]upplemental [Ff]ile')" role="warning" id="supplementalfile-presence"><name/> element contains the phrase ' Supplemental file ' which almost certainly needs updating. <name/> starts with - <value-of select="substring(.,1,25)"/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#supplement-table-presence" test="not(ancestor::sub-article) and matches(.,'\p{Zs}?[Ss]upplementa(l|ry) [Tt]able')" role="warning" id="supplement-table-presence"><name/> element contains the phrase 'Supplementary table' or 'Supplemental table'. Does it need updating? If it's unclear what should be cited, please query the authors. <name/> starts with - <value-of select="substring(.,1,25)"/></report>
      
      <report test="not(local-name()='code') and not(ancestor::sub-article) and matches(.,' [Rr]ef\. ')" role="error" id="ref-presence"><name/> element contains 'Ref.' which is either incorrect or unnecessary.</report>
      
      <report test="not(local-name()='code') and not(ancestor::sub-article) and matches(.,' [Rr]efs\. ')" role="error" id="refs-presence"><name/> element contains 'Refs.' which is either incorrect or unnecessary.</report>
      
      <report test="contains(.,'�')" role="error" id="replacement-character-presence"><name/> element contains the replacement character '�' which is not allowed.</report>
      
      <report test="contains(.,'')" role="error" id="junk-character-presence"><name/> element contains a junk character '' which should be replaced.</report>
      
      <report test="contains(.,'︎')" role="error" id="junk-character-presence-2"><name/> element contains a junk character '︎' which should be replaced or deleted.</report>
      ...
      <report test="contains(.,'&#x9D;')" role="error" id="operating-system-command-presence"><name/> element contains an operating system command character '&#x9D;' (unicode string: &amp;#x9D;) which should very likely be replaced/removed. - <value-of select="."/></report>

      <report test="matches(lower-case(.),&quot;(^|\s)((i am|i'm) an? ai (language)? model|as an ai (language)? model,? i('m|\s)|(here is|here's) an? (possible|potential)? introduction (to|for) your topic|(here is|here's) an? (abstract|introduction|results|discussion|methods)( section)? for you|certainly(,|!)? (here is|here's)|i'm sorry,?( but)? i (don't|can't)|knowledge (extend|cutoff)|as of my last update|regenerate response)&quot;)" role="warning" id="ai-response-presence-1"><name/> element contains what looks like a response from an AI chatbot after it being provided a prompt. Is that correct? Should the content be adjusted?</report>
    </rule></pattern><pattern id="unallowed-symbol-tests-sup-pattern"><rule context="sup" id="unallowed-symbol-tests-sup">		
      
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#he2sr-copyright-symbol-sup" test="contains(.,'©')" role="error" id="copyright-symbol-sup">'<name/>' element contains the copyright symbol, '©', which is not allowed.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#he2sr-degree-symbol-sup" test="contains(.,'°')" role="error" id="degree-symbol-sup">'<name/>' element contains the degree symbol, '°', which is unnecessary. It does not need to be superscript.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h54ah-united-states-test-1" test="$text = 'United States of America'" role="error" id="united-states-test-1"><value-of select="."/> is not allowed. This should be 'United States'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hvgkz-united-states-test-2" test="$text = 'USA'" role="error" id="united-states-test-2"><value-of select="."/> is not allowed. This should be 'United States'</report>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hko11-united-kingdom-test-2" test="$text = 'UK'" role="error" id="united-kingdom-test-2"><value-of select="."/> is not allowed. This should be 'United Kingdom'</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hiz90-singapore-test-1" test="(. = 'Singapore') and ($city != 'Singapore')" role="error" id="singapore-test-1"><value-of select="ancestor::aff/@id"/> has 'Singapore' as its country but '<value-of select="$city"/>' as its city, which must be incorrect.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#ho7od-final-us-states-test" test="matches($lc,$states-regex)" role="error" id="final-US-states-test">city contains a US state (or an abbreviation for it) - <value-of select="."/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h4yk9-singapore-test-2" test="(. = 'Singapore') and (ancestor::aff/country/text() != 'Singapore')" role="error" id="singapore-test-2"><value-of select="ancestor::aff/@id"/> has 'Singapore' as its city but '<value-of select="ancestor::aff/country/text()"/>' as its country, which must be incorrect.</report>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h54dm-wash-dc-test-1" test="(lower-case(.) = 'washington') and (ancestor::aff/country/text() = 'United States')" role="error" id="wash-dc-test-1"><value-of select="ancestor::aff/@id"/> has 'Washington' as its city. Either it should be changed to 'Washington, DC' or if referring to the US state then changed to the correct city.</report>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hxtjh-city-replacement-character-presence" test="matches(.,'�')" role="error" id="city-replacement-character-presence"><name/> element contains the replacement character '�' which is unallowed.</report>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hbjgo-city-number-presence" test="matches(.,'\d')" role="warning" id="city-number-presence">city contains a number, which is almost certainly incorrect - <value-of select="."/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hra66-city-street-presence" test="matches(lower-case(.),'^rue | rue |^street | street |^building | building |^straße | straße |^stadt | stadt |^platz | platz |^strada | strada |^cedex | cedex |^blvd | blvd |^boulevard| boulevard ')" role="warning" id="city-street-presence">city likely contains a street or building name, which is almost certainly incorrect - <value-of select="."/>.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hc48u-institution-replacement-character-presence" test="matches(.,'�')" role="error" id="institution-replacement-character-presence"><name/> element contains the replacement character '�' which is unallowed.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hd64t-dept-replacement-character-presence" test="contains(.,'�')" role="error" id="dept-replacement-character-presence"><name/> element contains the replacement character '�' which is unallowed.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#RNA" test="($uc = 'RNA') and (. != 'RNA')" role="error" id="RNA">ref '<value-of select="ancestor::ref/@id"/>' contains
        <value-of select="."/>. 'RNA' should be upper-case.</report>
      
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#bmj" test="(matches($uc,'^BMJ$|BMJ[:]? ')) and matches(.,'Bmj|bmj|BMj|BmJ|bMj|bmJ')" role="error" id="bmj">ref '<value-of select="ancestor::ref/@id"/>' contains
      ...
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#ampersand-check" test="matches(.,'[Aa]mp;')" role="warning" id="ampersand-check">ref '<value-of select="ancestor::ref/@id"/>' appears to contain the text 'amp', is this a broken ampersand?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#journal-replacement-character-presence" test="matches(.,'�')" role="error" id="journal-replacement-character-presence"><name/> element contains the replacement character '�' which is unallowed - <value-of select="."/></report>
      
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#journal-off-presence" test="matches(.,'[Oo]fficial [Jj]ournal')" role="warning" id="journal-off-presence">ref '<value-of select="ancestor::ref/@id"/>' has a source title which contains the text 'official journal' - '<value-of select="."/>'. Is this necessary?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#elife-check" test="starts-with($doi,'10.7554/eLife.') and (. != 'eLife')" role="error" id="elife-check">Journal ref '<value-of select="ancestor::ref/@id"/>' has an eLife doi <value-of select="$doi"/>, but the journal name is '<value-of select="."/>', when it should be 'eLife'. Either the journal name needs updating to eLife, or the doi is incorrect.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#article-title-fullstop-check-1" test="(matches($rep,'[A-Za-z][A-Za-z]+\. [A-Za-z]'))" role="info" id="article-title-fullstop-check-1">ref '<value-of select="ancestor::ref/@id"/>' has an article-title with a full stop. Is this correct, or has the journal/source title been included? Or perhaps the full stop should be a colon ':'?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#article-title-fullstop-check-3" test="matches(.,'\.$') and matches(.,'\.\.$')" role="warning" id="article-title-fullstop-check-3">ref '<value-of select="ancestor::ref/@id"/>' has an article-title which ends with some full stops - is this correct? - <value-of select="."/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#article-title-child-1" test="(count(child::*) = 1) and (count(child::text()) = 0)" role="warning" id="article-title-child-1">ref '<value-of select="ancestor::ref/@id"/>' has an article-title with one child <value-of select="*/local-name()"/> element, and no text. This is almost certainly incorrect. - <value-of select="."/></report>
      
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#a-title-replacement-character-presence" test="matches(.,'�')" role="error" id="a-title-replacement-character-presence"><name/> element contains the replacement character '�' which is unallowed - <value-of select="."/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#eloc-page-assert" test="not(fpage) and not(elocation-id) and not(comment)" role="warning" id="eloc-page-assert">ref '<value-of select="ancestor::ref/@id"/>' is a journal, but it doesn't have a page range or e-location. Is this right?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/book-references-x4trb0n2#h6n1g-report-chapter-title-test" test="matches(.,' [Rr]eport |^[Rr]eport | [Rr]eport[\p{Zs}\p{P}]?$')" role="warning" id="report-chapter-title-test">ref '<value-of select="ancestor::ref/@id"/>' is tagged as a book reference, but the chapter title is <value-of select="."/>. Should it be captured as a report type reference instead?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/preprint-references-okxjjp9i#arxiv-test" test="matches($lc,'^arxiv$') and not(. = 'arXiv')" role="error" id="arxiv-test">ref '<value-of select="ancestor::ref/@id"/>' has a source <value-of select="."/>, which is not the correct proprietary capitalisation - 'arXiv'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/preprint-references-okxjjp9i#chemrxiv-test" test="matches($lc,'chemrxiv') and not(. = 'ChemRxiv')" role="error" id="chemrxiv-test">ref '<value-of select="ancestor::ref/@id"/>' has a source <value-of select="."/>, which is not the correct proprietary capitalisation - 'ChemRxiv'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/preprint-references-okxjjp9i#medrxiv-test" test="matches($lc,'medrxiv') and not(. = 'medRxiv')" role="error" id="medrxiv-test">ref '<value-of select="ancestor::ref/@id"/>' has a source <value-of select="."/>, which is not the correct proprietary capitalisation - 'medRxiv'.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/preprint-references-okxjjp9i#paleorxiv-test" test="matches($lc,'paleorxiv') and not(. = 'PaleorXiv')" role="error" id="paleorxiv-test">ref '<value-of select="ancestor::ref/@id"/>' has a source <value-of select="."/>, which is not the correct proprietary capitalisation - 'PaleorXiv'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/preprint-references-okxjjp9i#preprint-replacement-character-presence" test="matches(.,'�')" role="error" id="preprint-replacement-character-presence"><name/> element contains the replacement character '�' which is unallowed - <value-of select="."/>.</report>
      ...
      <report test="matches(.,'�')" role="error" id="webreplacement-character-presence">web citation contains the replacement character '�' which is unallowed - <value-of select="."/></report>
      
      <report test="matches($link,'psyarxiv.org')" role="error" id="psyarxiv-web-test">web ref '<value-of select="ancestor::ref/@id"/>' has a link which points to a preprint server, PsyArXiv, therefore it should be captured as a preprint type ref - <value-of select="ext-link"/></report>
      
      <report test="matches($link,'/arxiv.org')" role="error" id="arxiv-web-test">web ref '<value-of select="ancestor::ref/@id"/>' has a link which points to a preprint server, arXiv, therefore it should be captured as a preprint type ref - <value-of select="ext-link"/></report>
      
      <report test="matches($link,'biorxiv.org')" role="warning" id="biorxiv-web-test">web ref '<value-of select="ancestor::ref/@id"/>' has a link which points to a preprint server, bioRxiv, therefore it should almost certainly be captured as a preprint type ref - <value-of select="ext-link"/></report>
      
      <report test="matches($link,'chemrxiv.org')" role="error" id="chemrxiv-web-test">web ref '<value-of select="ancestor::ref/@id"/>' has a link which points to a preprint server, ChemRxiv, therefore it should be captured as a preprint type ref - <value-of select="ext-link"/></report>
      
      <report test="matches($link,'peerj.com/preprints/')" role="error" id="peerj-preprints-web-test">web ref '<value-of select="ancestor::ref/@id"/>' has a link which points to a preprint server, PeerJ Preprints, therefore it should be captured as a preprint type ref - <value-of select="ext-link"/></report>
      
      <report test="matches($link,'paleorxiv.org')" role="error" id="paleorxiv-web-test">web ref '<value-of select="ancestor::ref/@id"/>' has a link which points to a preprint server, PaleorXiv, therefore it should be captured as a preprint type ref - <value-of select="ext-link"/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#data-pride-test-3" test="contains(pub-id[1]/@xlink:href,'www.ebi.ac.uk/pride') and pub-id[1][@pub-id-type!='accession' or not(@pub-id-type)]" role="warning" id="data-pride-test-3">Data reference with the title '<value-of select="data-title[1]"/>' has a PRIDE 'www.ebi.ac.uk/pride' type link, but is not marked as an accession type link.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#data-figshare-test-2" test="(pub-id or ext-link) and not(starts-with(pub-id[1][@pub-id-type='doi'],'10.6084/m9.figshare')) and (source[1]='figshare')" role="warning" id="data-figshare-test-2">Data reference with the title '<value-of select="data-title[1]"/>' has the database name <value-of select="source[1]"/>, but no doi starting with '10.6084/m9.figshare' - is this correct? Figshare sometimes host for other organisations (example http://doi.org/10.1184/R1/9963566), so this may be fine.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#data-mendeley-test-2" test="(pub-id or ext-link) and not(starts-with(pub-id[1][@pub-id-type='doi'],'10.17632')) and (source[1]='Mendeley Data')" role="warning" id="data-mendeley-test-2">Data reference with the title '<value-of select="data-title[1]"/>' has the database name <value-of select="source[1]"/>, but no doi starting with '10.17632', which is incorrect.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#data-eth-test-2" test="(pub-id or ext-link) and not(starts-with(pub-id[1][@pub-id-type='doi'],'10.3929')) and (source[1]='ETH Library research collection')" role="warning" id="data-eth-test-2">Data reference with the title '<value-of select="data-title[1]"/>' has the database name <value-of select="source[1]"/>, but no doi starting with '10.3929', which is incorrect.</report>
      
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#data-crcns-test-1" test="starts-with(pub-id[1][@pub-id-type='doi'],'10.6080') and (source[1]!='Collaborative Research in Computational Neuroscience')" role="warning" id="data-crcns-test-1">Data reference with the title '<value-of select="data-title[1]"/>' has a doi starting with '10.6080' but the database name is not 'Collaborative Research in Computational Neuroscience' - <value-of select="source[1]"/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#data-crcns-test-2" test="(pub-id or ext-link) and not(starts-with(pub-id[1][@pub-id-type='doi'],'10.6080')) and (source[1]='Collaborative Research in Computational Neuroscience')" role="warning" id="data-crcns-test-2">Data reference with the title '<value-of select="data-title[1]"/>' has the database name <value-of select="source[1]"/>, but no doi starting with '10.6080', which is incorrect.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#data-wwpdb-test-1" test="starts-with(pub-id[1][@pub-id-type='doi'],'10.2210') and (source[1]!='Worldwide Protein Data Bank')" role="warning" id="data-wwpdb-test-1">Data reference with the title '<value-of select="data-title[1]"/>' has a doi starting with '10.2210' but the database name is not 'Worldwide Protein Data Bank' - <value-of select="source[1]"/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#data-wwpdb-test-2" test="(pub-id or ext-link) and not(starts-with(pub-id[1][@pub-id-type='doi'],'10.2210')) and (source[1]='Worldwide Protein Data Bank')" role="warning" id="data-wwpdb-test-2">Data reference with the title '<value-of select="data-title[1]"/>' has the database name <value-of select="source[1]"/>, but no doi starting with '10.2210', which is incorrect.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#data-harvard-dataverse-test-1" test="starts-with(pub-id[1][@pub-id-type='doi'],'10.7910') and (source[1]!='Harvard Dataverse')" role="warning" id="data-harvard-dataverse-test-1">Data reference with the title '<value-of select="data-title[1]"/>' has a doi starting with '10.7910' but the database name is not 'Harvard Dataverse' - <value-of select="source[1]"/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#data-harvard-dataverse-test-2" test="(pub-id or ext-link) and not(starts-with(pub-id[1][@pub-id-type='doi'],'10.7910')) and (source[1]='Harvard Dataverse')" role="warning" id="data-harvard-dataverse-test-2">Data reference with the title '<value-of select="data-title[1]"/>' has the database name <value-of select="source[1]"/>, but no doi starting with '10.7910', which is likely incorrect.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#publisher-name-inc" test="matches(.,'[Ii]nc\.')" role="warning" id="publisher-name-inc">ref '<value-of select="ancestor::ref/@id"/>' has a publisher-name containing the text 'Inc.' Should the fullstop be removed? <value-of select="."/></report>
      
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#pub-name-replacement-character-presence" test="matches(.,'�')" role="error" id="pub-name-replacement-character-presence"><name/> contains the replacement character '�' which is unallowed - <value-of select="."/></report>
      
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#pub-name-newspaper" test="matches(lower-case(.),'guardian|the independent|times|post|news')" role="warning" id="pub-name-newspaper"><name/> contains the text 'guardian', 'independent', 'times' or 'post' - <value-of select="."/> - is it a newspaper reference? If so, it should be captured as a web or a periodical reference.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#author-test-2" test="matches(.,'[Ed]itor')" role="warning" id="author-test-2">name in ref '<value-of select="ancestor::ref/@id"/>' contans the text 'Editor'. Is this correct?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#given-names-count" test="count(given-names) gt 1" role="error" id="given-names-count">ref '<value-of select="ancestor::ref/@id"/>' has an <value-of select="$type"/> with <value-of select="count(given-names)"/> given-names - <value-of select="."/> - which is incorrect.</report>
      ...
      <report test="matches(lower-case(source[1]),'plos|^elife$|^mbio$')" role="error" id="online-journal-w-page"><value-of select="$cite"/> is a <value-of select="source"/> article, but has a page number, which is incorrect.</report>
      ...
      <assert test="e:is-valid-isbn(.)" role="error" id="isbn-conformity-test-2">isbn contains an invalid ISBN - '<value-of select="."/>'. Should it be captured as another type of pub-id?</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#das-doi-conformity-1" test="matches(.,'10\.\d{4,9}/[-._;()/:A-Za-z0-9&lt;&gt;\+#&amp;`~–−]+$') and not(matches(.,'http[s]?://doi.org/'))" role="error" id="das-doi-conformity-1">Data Availability Statement contains a doi, but it does not contain the 'https://doi.org/' proxy. All dois should be updated to include a full 'https://doi.org/...' type link.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/ethics-se0ia1cs#ethics-info-conformity" test="matches(replace(normalize-space(.),'&quot;',''),'\.$|\?$')" role="error" id="ethics-info-conformity">The ethics statement must end with a full stop.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#sec-title-list-check" test="matches(.,'^\p{Zs}?[A-Za-z]{1,3}\)|^\p{Zs}?\([A-Za-z]{1,3}')" role="warning" id="sec-title-list-check">Section title might start with a list indicator - '<value-of select="."/>'. Is this correct?</report>
      
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#sec-title-appendix-check" test="matches(.,'^[Aa]ppendix')" role="warning" id="sec-title-appendix-check">Section title contains the word appendix - '<value-of select="."/>'. Should it be captured as an appendix?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#sec-title-full-stop" test="matches(replace(.,' ',' '),'\.[\p{Zs}]*$')" role="warning" id="sec-title-full-stop">Section title ends with full stop, which is very likely to be incorrect - <value-of select="."/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#sec-title-bold" test="(count(*) = 1) and child::bold and ($free-text='')" role="error" id="sec-title-bold">All section title content is captured in bold. This is incorrect - <value-of select="."/></report>
      
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#sec-title-underline" test="(count(*) = 1) and child::underline and ($free-text='')" role="error" id="sec-title-underline">All section title content is captured in underline. This is incorrect - <value-of select="."/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#sec-title-dna" test="matches(upper-case($no-link-text),'^DNA | DNA | DNA$') and not(matches($no-link-text,'^DNA | DNA | DNA$'))" role="warning" id="sec-title-dna">Section title contains the phrase DNA, but it is not in all caps - <value-of select="."/></report>
      
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#sec-title-rna" test="matches(upper-case($no-link-text),'^RNA | RNA | RNA$') and not(matches($no-link-text,'^RNA | RNA | RNA$'))" role="warning" id="sec-title-rna">Section title contains the phrase RNA, but it is not in all caps - <value-of select="."/></report>
      
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#sec-title-dimension" test="matches($no-link-text,'^[1-4]d | [1-4]d | [1-4]d$')" role="warning" id="sec-title-dimension">Section title contains lowercase abbreviation for dimension, when this should always be uppercase 'D' - <value-of select="."/></report>
      
      <!-- AIDS not included due to other meaning/use -->
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#sec-title-hiv" test="matches(upper-case($no-link-text),'^HIV | HIV | HIV') and not(matches($no-link-text,'^HIV | HIV | HIV'))" role="warning" id="sec-title-hiv">Section title contains the word HIV, but it is not in all caps - <value-of select="."/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#xref-column-test" test="(count(ancestor::*:td/preceding-sibling::td) = 0) or (count(ancestor::*:td/preceding-sibling::td) = 1) or (count(ancestor::*:td/preceding-sibling::td) = 3)" role="warning" id="xref-column-test">'<value-of select="."/>' citation is in a column in the Key Resources Table which usually does not include references. Is it correct?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#colour-check-table" test="descendant::th[@style=$allowed-values] or descendant::td[@style=$allowed-values]" role="warning" id="colour-check-table"><value-of select="if (label) then label else 'table'"/> has colour background. Is this correct and appropriate?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/general-layout-and-formatting-wq0m31at#h3819-colour-styled-content-flag" test="@style = $allowed-values" role="warning" id="colour-styled-content-flag"><value-of select="."/> has colour formatting. Is this correct? Preceding text - <value-of select="substring(preceding-sibling::text()[1],string-length(preceding-sibling::text()[1])-25)"/></report>
      ...
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#mtext-test-1" test="matches(.,'^\p{Zs}*\\')" role="warning" id="mtext-test-1">math (<value-of select="name()"/> element) contains '<value-of select="."/>' which looks suspiciously like LaTeX markup. Is it correct? Or is there missing content or content which has been processed incompletely?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#hvcr0-final-et-al-italic-test" test="matches(.,'et al[\.]?')" role="warning" id="final-et-al-italic-test"><name/> element contains 'et al.' - this should not be in italics (eLife house style).</report>  
      ...
      <report test="matches(@xlink:href,'^http[s]?://www.ncbi.nlm.nih.gov/pubmed/[\d]*')" role="warning" id="pubmed-presence"><value-of select="parent::*/local-name()"/> element contains what looks like a link to a PubMed article - <value-of select="."/> - should this be added a reference instead?</report>
      
      <report test="matches(@xlink:href,'^http[s]?://www.ncbi.nlm.nih.gov/pmc/articles/PMC[\d]*')" role="warning" id="pmc-presence"><value-of select="parent::*/local-name()"/> element contains what looks like a link to a PMC article - <value-of select="."/> - should this be added a reference instead?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/asset-citations-fa3e2yoo#xref-child-test" test="$child = $formatting-elems" role="warning" id="xref-child-test">xref - <value-of select="."/> - has a formatting child element - <value-of select="$child"/> - which is likely not correct.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/archiving-code-zrfi30c5#code-fork-info" test="$test//*:match" role="warning" id="code-fork-info">Article possibly contains code that needs forking. Search - <value-of select="string-join(for $x in $test//*:match return $x,', ')"/>.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/article-structure-5nhfjxj0#section-title-test-1" test="(name()=('italic','bold','underline')) and (ends-with(.,'.') or matches($following-text,'^\p{Zs}?\.|^[\p{P}]?\p{Zs}?[A-Z]|^[\p{P}]?\p{Zs}?\d')) and not((name()='italic') and matches(lower-case(.),$sec-title-regex))" role="warning" id="section-title-test-1"><name/> text begins a paragraph - <value-of select="."/> - Should it be marked up as a section title (Heading level <value-of select="count(ancestor::sec) + 1"/>)?</report>
      
    </rule></pattern><pattern id="strike-tests-pattern"><rule context="strike" id="strike-tests">     
      
      <report test="." role="warning" id="final-strike-flag"><value-of select="parent::*/local-name()"/> element contains text with strikethrough formatting - <value-of select="."/> - Is this correct? Or have the authors added strikethrough formatting as an indication that the content should be removed?</report>
    ...
    <report see="https://elifeproduction.slab.com/posts/general-layout-and-formatting-wq0m31at#hcq0l-title-all-bold-test-1" test="$free-text=''" role="warning" id="title-all-bold-test-1">Title is entirely in <value-of select="child::*[1]/local-name()"/> - '<value-of select="."/>'. Is this correct?</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#hkbnb-italic-org-test-1" test="(substring(.,1,1) = (' ',' ')) or ($pre-token='') or matches($pre-token,'[\p{Zs}\p{P}]')" role="warning" id="italic-org-test-1">There is no space between the organism name '<value-of select="."/>' and its preceding text - '<value-of select="concat(substring($pre-text,string-length($pre-text)-10),.)"/>'. Is this correct or is there a missing space?</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#hkbnb-italic-org-test-2" test="(substring(., string-length(.), 1) = (' ',' ')) or ($post-token='') or matches($post-token,'[\p{Zs}\p{P}]')" role="warning" id="italic-org-test-2">There is no space between the organism name '<value-of select="."/>' and its following text - '<value-of select="concat(.,substring($post-text,1,10))"/>'. Is this correct or is there a missing space?</assert>
    </rule></pattern><pattern id="sub-sup-tests-pattern"><rule context="sub|sup" id="sub-sup-tests">
      
      <report see="https://elifeproduction.slab.com/posts/general-layout-and-formatting-wq0m31at#h6795-double-sup-sub" test="parent::*/name() = name()" role="error" id="double-sup-sub"><name/> is captured as a child of <name/>, which is not permitted.</report>
      ...
      <report test="matches(lower-case(.),'^mmethods[\s\.]|\smmethods[\s\.]')" role="error" id="mmethods-typo-check"><name/> element contains a typo ('mmethods') - <value-of select="."/>.</report>
  ...
  <pattern id="doi-journal-ref-checks-pattern"><rule context="element-citation[(@publication-type='journal') and not(pub-id[@pub-id-type='doi']) and year and source]" id="doi-journal-ref-checks">
      ...
      <assert test="some $x in document($journals)/journals/journal satisfies (($x/@title/string()=$journal) and (number($x/@year) ge $year))" role="warning" id="journal-doi-test-1"><value-of select="$cite"/> is a journal ref without a doi. Should it have one?</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/book-references-x4trb0n2#h5h75-book-doi-test-1" test="some $x in document($publishers)/publishers/publisher satisfies ($x/@title/string()=$publisher)" role="warning" id="book-doi-test-1"><value-of select="$cite"/> is a book ref without a doi, but its publisher (<value-of select="publisher-name[1]"/>) is known to register dois with some books/chapters. Should it have one?</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/software-references-aymhzmlh#software-doi-test-1" test="$host='zenodo' and not(contains(ext-link[1],'10.5281/zenodo'))" role="warning" id="software-doi-test-1"><value-of select="$cite"/> is a software ref with a source (<value-of select="source[1]"/>) known to register dois starting with '10.5281/zenodo'. Should it have a link in the format 'https://doi.org/10.5281/zenodo...'?</report>
      
      <report see="https://elifeproduction.slab.com/posts/software-references-aymhzmlh#software-doi-test-2" test="$host='figshare' and not(contains(ext-link[1],'10.6084/m9.figshare'))" role="warning" id="software-doi-test-2"><value-of select="$cite"/> is a software ref with a source (<value-of select="source[1]"/>) known to register dois starting with '10.6084/m9.figshare'. Should it have a link in the format 'https://doi.org/10.6084/m9.figshare...'?</report>
      ...
      <report test="contains($name,'ieee')" role="warning" id="conf-doi-test-1"><value-of select="e:citation-format1(.)"/> is a conference ref without a doi, but it's a conference which is known to possibly have dois - (<value-of select="conf-name[1]"/>). Should it have one?</report>
  ...
  <pattern id="zenodo-tests-pattern"><rule context="element-citation[(lower-case(source[1])='zenodo') or contains(ext-link[1],'10.5281/zenodo') or contains(pub-id[@pub-id-type='doi'][1],'10.5281/zenodo')]" id="zenodo-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/journal-references-i098980k#zenodo-check" test="@publication-type=('data','software','preprint','report')" role="error" id="zenodo-check"><value-of select="@publication-type"/> type reference <value-of select="if (parent::ref[@id]) then concat('(with id ',parent::ref[1]/@id,')') else ()"/> is a zenodo one, which means that it must be one of the following reference types: data, software, preprint or report.</assert>
  ...
  <pattern id="unicode-tests-pattern"><rule context="sub-article//p[matches(.,'[âÂÅÃËÆ]')]|sub-article//td[matches(.,'[âÂÅÃËÆ]')]|sub-article//th[matches(.,'[âÂÅÃËÆ]')]" id="unicode-tests">
        ...
        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-1" test="contains(.,'â‚¬')" role="warning" id="unicode-test-1"><name/> element contains 'â‚¬' - this should instead be the character '€'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-2" test="contains(.,'Ã€')" role="warning" id="unicode-test-2"><name/> element contains 'Ã€' - this should instead be the character 'À'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-3" test="contains(.,'Ã')" role="warning" id="unicode-test-3"><name/> element contains 'Ã' - this should instead be the character 'Á'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-4" test="contains(.,'â€š')" role="warning" id="unicode-test-4"><name/> element contains 'â€š' - this should instead be the character '‚'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-5" test="contains(.,'Ã‚')" role="warning" id="unicode-test-5"><name/> element contains 'Ã‚' - this should instead be the character 'Â'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-6" test="contains(.,'Æ’')" role="warning" id="unicode-test-6"><name/> element contains 'Æ’' - this should instead be the character 'ƒ'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-7" test="contains(.,'Ãƒ')" role="warning" id="unicode-test-7"><name/> element contains 'Ãƒ' - this should instead be the character 'Ã'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-8" test="contains(.,'â€ž')" role="warning" id="unicode-test-8"><name/> element contains 'â€ž' - this should instead be the character '„'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-9" test="contains(.,'Ã„')" role="warning" id="unicode-test-9"><name/> element contains 'Ã„' - this should instead be the character 'Ä'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-10" test="contains(.,'â€¦')" role="warning" id="unicode-test-10"><name/> element contains 'â€¦' - this should instead be the character '…'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-11" test="contains(.,'Ã…')" role="warning" id="unicode-test-11"><name/> element contains 'Ã…' - this should instead be the character 'Å'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-13" test="contains(.,'Ã†')" role="warning" id="unicode-test-13"><name/> element contains 'Ã†' - this should instead be the character 'Æ'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-14" test="contains(.,'â€¡')" role="warning" id="unicode-test-14"><name/> element contains 'â€¡' - this should instead be the character '‡'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-15" test="contains(.,'Ã‡')" role="warning" id="unicode-test-15"><name/> element contains 'Ã‡' - this should instead be the character 'Ç'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-16" test="contains(.,'Ë†')" role="warning" id="unicode-test-16"><name/> element contains 'Ë†' - this should instead be the character 'ˆ'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-17" test="contains(.,'Ãˆ')" role="warning" id="unicode-test-17"><name/> element contains 'Ãˆ' - this should instead be the character 'È'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-18" test="contains(.,'â€°')" role="warning" id="unicode-test-18"><name/> element contains 'â€°' - this should instead be the character '‰'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-19" test="contains(.,'Ã‰')" role="warning" id="unicode-test-19"><name/> element contains 'Ã‰' - this should instead be the character 'É'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-21" test="contains(.,'ÃŠ')" role="warning" id="unicode-test-21"><name/> element contains 'ÃŠ' - this should instead be the character 'Ê'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-22" test="contains(.,'â€¹')" role="warning" id="unicode-test-22"><name/> element contains 'â€¹' - this should instead be the character '‹'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-23" test="contains(.,'Ã‹')" role="warning" id="unicode-test-23"><name/> element contains 'Ã‹' - this should instead be the character 'Ë'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-24" test="contains(.,'Å’')" role="warning" id="unicode-test-24"><name/> element contains 'Å’' - should this instead be the character 'Œ'? - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-25" test="contains(.,'ÃŒ')" role="warning" id="unicode-test-25"><name/> element contains 'ÃŒ' - this should instead be the character 'Ì'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-26" test="contains(.,'Ã')" role="warning" id="unicode-test-26"><name/> element contains 'Ã' - this should instead be the character 'Í'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-27" test="contains(.,'Å½')" role="warning" id="unicode-test-27"><name/> element contains 'Å½' - this should instead be the character 'Ž'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-28" test="contains(.,'ÃŽ')" role="warning" id="unicode-test-28"><name/> element contains 'ÃŽ' - this should instead be the character 'Î'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-29" test="contains(.,'Ã')" role="warning" id="unicode-test-29"><name/> element contains 'Ã' - this should instead be the character 'Ï'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-30" test="contains(.,'Ã')" role="warning" id="unicode-test-30"><name/> element contains 'Ã' - this should instead be the character 'Ð'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-31" test="contains(.,'â€˜')" role="warning" id="unicode-test-31"><name/> element contains 'â€˜' - this should instead be the character '‘'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-32" test="contains(.,'Ã‘')" role="warning" id="unicode-test-32"><name/> element contains 'Ã‘' - this should instead be the character 'Ñ'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-33" test="contains(.,'â€™')" role="warning" id="unicode-test-33"><name/> element contains 'â€™' - this should instead be the character '’'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-34" test="contains(.,'Ã’')" role="warning" id="unicode-test-34"><name/> element contains 'Ã’' - this should instead be the character 'Ò'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-35" test="contains(.,'â€œ')" role="warning" id="unicode-test-35"><name/> element contains 'â€œ' - this should instead be the character '“'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-36" test="contains(.,'Ã“')" role="warning" id="unicode-test-36"><name/> element contains 'Ã“' - this should instead be the character 'Ó'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-37" test="contains(.,'â€')" role="warning" id="unicode-test-37"><name/> element contains 'â€' - this should instead be the character '”'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-38" test="contains(.,'Ã”')" role="warning" id="unicode-test-38"><name/> element contains 'Ã”' - this should instead be the character 'Ô'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-39" test="contains(.,'Ã•')" role="warning" id="unicode-test-39"><name/> element contains 'Ã•' - this should instead be the character 'Õ'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-40" test="contains(.,'â€“')" role="warning" id="unicode-test-40"><name/> element contains 'â€“' - this should instead be the character '–'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-41" test="contains(.,'Ã–')" role="warning" id="unicode-test-41"><name/> element contains 'Ã–' - this should instead be the character 'Ö'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-42" test="contains(.,'â€”')" role="warning" id="unicode-test-42"><name/> element contains 'â€”' - this should instead be the character '—'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-43" test="contains(.,'Ã—')" role="warning" id="unicode-test-43"><name/> element contains 'Ã—' - this should instead be the character '×'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-44" test="contains(.,'Ëœ')" role="warning" id="unicode-test-44"><name/> element contains 'Ëœ' - this should instead be the character '˜'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-45" test="contains(.,'Ã˜')" role="warning" id="unicode-test-45"><name/> element contains 'Ã˜' - this should instead be the character 'Ø'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-46" test="contains(.,'Ã™')" role="warning" id="unicode-test-46"><name/> element contains 'Ã™' - this should instead be the character 'Ù'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-47" test="contains(.,'Å¡')" role="warning" id="unicode-test-47"><name/> element contains 'Å¡' - this should instead be the character 'š'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-48" test="contains(.,'Ãš')" role="warning" id="unicode-test-48"><name/> element contains 'Ãš' - this should instead be the character 'Ú'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-49" test="contains(.,'â€º')" role="warning" id="unicode-test-49"><name/> element contains 'â€º' - this should instead be the character '›'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-50" test="contains(.,'Ã›')" role="warning" id="unicode-test-50"><name/> element contains 'Ã›' - this should instead be the character 'Û'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-51" test="contains(.,'Å“')" role="warning" id="unicode-test-51"><name/> element contains 'Å“' - this should instead be the character 'œ'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-52" test="contains(.,'Ãœ')" role="warning" id="unicode-test-52"><name/> element contains 'Ãœ' - this should instead be the character 'Ü'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-53" test="contains(.,'Ã')" role="warning" id="unicode-test-53"><name/> element contains 'Ã' - this should instead be the character 'Ý'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-54" test="contains(.,'Å¾')" role="warning" id="unicode-test-54"><name/> element contains 'Å¾' - this should instead be the character 'ž'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-55" test="contains(.,'Ãž')" role="warning" id="unicode-test-55"><name/> element contains 'Ãž' - this should instead be the character 'Þ'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-56" test="contains(.,'Å¸')" role="warning" id="unicode-test-56"><name/> element contains 'Å¸' - this should instead be the character 'Ÿ'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-57" test="contains(.,'ÃŸ')" role="warning" id="unicode-test-57"><name/> element contains 'ÃŸ' - this should instead be the character 'ß'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-58" test="contains(.,'Â¡')" role="warning" id="unicode-test-58"><name/> element contains 'Â¡' - this should instead be the character '¡'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-59" test="contains(.,'Ã¡')" role="warning" id="unicode-test-59"><name/> element contains 'Ã¡' - this should instead be the character 'á'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-60" test="contains(.,'Â¢')" role="warning" id="unicode-test-60"><name/> element contains 'Â¢' - this should instead be the character '¢'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-61" test="contains(.,'Ã¢')" role="warning" id="unicode-test-61"><name/> element contains 'Ã¢' - this should instead be the character 'â'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-62" test="contains(.,'Â£')" role="warning" id="unicode-test-62"><name/> element contains 'Â£' - this should instead be the character '£'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-63" test="contains(.,'Ã£')" role="warning" id="unicode-test-63"><name/> element contains 'Ã£' - this should instead be the character 'ã'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-64" test="contains(.,'Â¤')" role="warning" id="unicode-test-64"><name/> element contains 'Â¤' - this should instead be the character '¤'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-65" test="contains(.,'Ã¤')" role="warning" id="unicode-test-65"><name/> element contains 'Ã¤' - this should instead be the character 'ä'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-66" test="contains(.,'Ã¥')" role="warning" id="unicode-test-66"><name/> element contains 'Ã¥' - this should instead be the character 'å'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-67" test="contains(.,'Â¨')" role="warning" id="unicode-test-67"><name/> element contains 'Â¨' - this should instead be the character '¨'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-68" test="contains(.,'Ã¨')" role="warning" id="unicode-test-68"><name/> element contains 'Ã¨' - this should instead be the character 'è'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-69" test="contains(.,'Âª')" role="warning" id="unicode-test-69"><name/> element contains 'Âª' - this should instead be the character 'ª'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-70" test="contains(.,'Ãª')" role="warning" id="unicode-test-70"><name/> element contains 'Ãª' - this should instead be the character 'ê'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-71" test="contains(.,'Â­')" role="warning" id="unicode-test-71"><name/> element contains 'Â­' - this should instead be the character '­'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-72" test="contains(.,'Ã­')" role="warning" id="unicode-test-72"><name/> element contains 'Ã­' - this should instead be the character 'í'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-73" test="contains(.,'Â¯')" role="warning" id="unicode-test-73"><name/> element contains 'Â¯' - this should instead be the character '¯'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-74" test="contains(.,'Ã¯')" role="warning" id="unicode-test-74"><name/> element contains 'Ã¯' - this should instead be the character 'ï'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-75" test="contains(.,'Â°')" role="warning" id="unicode-test-75"><name/> element contains 'Â°' - this should instead be the character '°'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-76" test="contains(.,'Ã°')" role="warning" id="unicode-test-76"><name/> element contains 'Ã°' - this should instead be the character 'ð'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-77" test="contains(.,'Â±')" role="warning" id="unicode-test-77"><name/> element contains 'Â±' - this should instead be the character '±'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-78" test="contains(.,'Ã±')" role="warning" id="unicode-test-78"><name/> element contains 'Ã±' - this should instead be the character 'ñ'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-79" test="contains(.,'Â´')" role="warning" id="unicode-test-79"><name/> element contains 'Â´' - this should instead be the character '´'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-80" test="contains(.,'Ã´')" role="warning" id="unicode-test-80"><name/> element contains 'Ã´' - this should instead be the character 'ô'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-81" test="contains(.,'Âµ')" role="warning" id="unicode-test-81"><name/> element contains 'Âµ' - this should instead be the character 'µ'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-82" test="contains(.,'Ãµ')" role="warning" id="unicode-test-82"><name/> element contains 'Ãµ' - this should instead be the character 'õ'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-83" test="contains(.,'Â¶')" role="warning" id="unicode-test-83"><name/> element contains 'Â¶' - this should instead be the character '¶'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-84" test="contains(.,'Ã¶')" role="warning" id="unicode-test-84"><name/> element contains 'Ã¶' - this should instead be the character 'ö'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-85" test="contains(.,'Â·')" role="warning" id="unicode-test-85"><name/> element contains 'Â·' - this should instead be the character '·'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-86" test="contains(.,'Ã·')" role="warning" id="unicode-test-86"><name/> element contains 'Ã·' - this should instead be the character '÷'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-87" test="contains(.,'Â¸')" role="warning" id="unicode-test-87"><name/> element contains 'Â¸' - this should instead be the character '¸'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-88" test="contains(.,'Ã¸')" role="warning" id="unicode-test-88"><name/> element contains 'Ã¸' - this should instead be the character 'ø'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-89" test="contains(.,'Ã¹')" role="warning" id="unicode-test-89"><name/> element contains 'Ã¹' - this should instead be the character 'ù'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-90" test="contains(.,'Âº')" role="warning" id="unicode-test-90"><name/> element contains 'Âº' - this should instead be the character 'º'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-91" test="contains(.,'Ãº')" role="warning" id="unicode-test-91"><name/> element contains 'Ãº' - this should instead be the character 'ú'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-92" test="contains(.,'Â¿')" role="warning" id="unicode-test-92"><name/> element contains 'Â¿' - this should instead be the character '¿'. - <value-of select="."/>.</report>

        <report see="https://elifeproduction.slab.com/posts/decision-letters-and-author-responses-rr1pcseo#unicode-test-93" test="contains(.,'Ã¿')" role="warning" id="unicode-test-93"><name/> element contains 'Ã¿' - this should instead be the character 'ÿ'. - <value-of select="."/>.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/general-content-2y3029rs#private-char-test" test="matches(.,'\p{Co}')" role="error" id="private-char-test"><name/> element contains private use character(s). They either need removing or changing to the correct character. Private characters: '<value-of select="string-join(distinct-values(tokenize(.,'\p{Zs}')[matches(.,'\p{Co}')]),' ')"/>'.</report>
  ...
  <pattern id="element-allowlist-pattern"><rule context="article//*[not(ancestor::mml:math)]" id="element-allowlist">
      ...
      <assert test="name()=$allowed-elements" role="error" id="element-conformity"><value-of select="name()"/> element is not allowed.</assert>
  ...
  <pattern id="empty-attribute-test-pattern"><rule context="*[@*/normalize-space(.)='']" id="empty-attribute-test">
      ...
      <report test="." role="error" id="final-empty-attribute-conformance"><value-of select="name()"/> element has attribute(s) with an empty value. &lt;<value-of select="name()"/><value-of select="for $att in ./@*[normalize-space(.)=''] return concat(' ',$att/name(),'=&quot;',$att,'&quot;')"/>&gt;</report>
      ...
      <assert test="name()=$allowed-elements" role="error" id="content-type-value-conformance"><name/> element cannot have a content-type attribute. The only elements that can have that can have a content-type attribtue are named-content, contrib-group, self-uri, institution, fn-group, and disp-quote.</assert>
    </rule></pattern>
  
  <pattern id="final-package-pattern"><rule context="graphic[@xlink:href]|media[@xlink:href]|self-uri[not(parent::event) and @xlink:href]" id="final-package">
      ...
      <assert test="java:file-exists(@xlink:href, $base)" role="error" id="graphic-media-presence"><name/> element points to file <value-of select="@xlink:href"/> - but there is no file with that name in the same folder as the XML file. It should be placed here - <value-of select="$base-path"/></assert>
      
    </rule></pattern><pattern id="final-package-article-xml-pattern"><rule context="article" id="final-package-article-xml">
