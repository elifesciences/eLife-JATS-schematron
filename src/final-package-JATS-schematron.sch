 ...
 <pattern id="article-tests-pattern"><rule context="article" id="article-tests">
  ...
  <pattern id="test-article-metadata-pattern"><rule context="article/front/article-meta" id="test-article-metadata">
   ...
   <let name="template" value="parent::article-meta/custom-meta-group/custom-meta[meta-name='Template']/meta-value[1]"/>
    ...
    <report test="count(contrib[@equal-contrib='yes']) = 1" role="error" id="equal-count-test">There is one contrib with the attribute equal-contrib='yes'. This cannot be correct. Either 2 or more contribs within the same contrib-group should have this attribute, or none. Check <value-of select="contrib[@equal-contrib='yes']/name"/></report>
      ...
      <assert test="child::xref[@ref-type='fn' and matches(@rid,'^con[0-9]{1,3}$')]" role="warning" id="collab-cont-test-1"><value-of select="e:get-surname(.)"/> has no contributions. Please ensure to query this with the authors.</assert>
      ...
      <assert test="role" role="error" id="final-collab-cont-test-1-v2"><value-of select="e:get-surname(.)"/> has no contributions. Please ensure to query this with the authors.</assert>
      
      <report test="role and not(role[@vocab='credit'])" role="warning" id="final-collab-cont-test-2-v2"><value-of select="e:get-surname(.)"/> has no CRediT contributions. Is that correct?</report>
      ...
      <report test="not(ancestor::ref) and not(ancestor::article-meta)" role="error" id="collab-name-test-3"><name/> must only be captured ass a descendant of ref or article-meta. This one is not.</report>
    ...
    <report test="descendant::bold or descendant::sub or descendant::sup or descendant::italic or descendant::sc" role="error" id="surname-test-3">surname must not contain any formatting (bold, or italic emphasis, or smallcaps, superscript or subscript).</report>
	  ...
	  <report test="matches(.,'^\p{Ll}') and not(matches(.,'^(de[lrn]?|van|von|el|te[rn]|d[ai]|dos)\s'))" role="warning" id="surname-test-5">surname doesn't begin with a capital letter - <value-of select="."/>. Is this correct?</report>
  ...
  <pattern id="p-tests-pattern"><rule context="p" id="p-tests">
    ...
    <report test="not(ancestor::fig/permissions[contains(.,'phylopic')]) and matches(@xlink:href,'phylopic\.org')" role="warning" id="phylopic-link-check">This link is to phylopic.org, which is a site where silhouettes/images are typically reproduced from. Please check whether any figures contain reproduced images from this site, and if so whether permissions have been obtained and/or copyright statements are correctly included.</report>
      ...
      <report test="$dtd-version le '1.3' and not(@mimetype=('image','application'))" role="error" id="graphic-test-4"><name/> must have a @mimetype='image' or 'application'.</report>
