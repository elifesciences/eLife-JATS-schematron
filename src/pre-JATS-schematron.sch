 ...
 <pattern id="article-tests-pattern">
  ...
  <pattern id="author-aff-tests-pattern">
    <rule context="article-meta/contrib-group[not(@*)]/aff" id="author-aff-tests">
      ...
      <assert test="addr-line[named-content[@content-type='city']]" role="warning" id="pre-auth-aff-test-2">[pre-auth-aff-test-2] Author affiliations must have a city. This one does not - <value-of select="$display"/>. Pleasde query the authors.</assert>
  ...
  <pattern id="video-test-pattern">
    <rule context="media[child::label]" id="video-test">
      ...
      <assert test="caption/title" role="warning" id="pre-video-title">[pre-video-title] <value-of select="label"/> does not have a title.</assert>
