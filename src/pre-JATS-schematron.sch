 ...
 <pattern id="article-tests-pattern">
  ...
  <pattern id="fig-tests-pattern">
    <rule context="fig[not(ancestor::sub-article)]" id="fig-tests">
      ...
      <report test="if ($article-type = ('correction','retraction')) then ()          else not(caption)" role="warning" id="pre-fig-test-4">[pre-fig-test-4] <value-of select="label"/> has no title or caption (caption element). Ensure this is queried with the author.</report>
      
      
      
      <report test="if ($article-type = ('correction','retraction')) then ()          else not(caption/title)" role="warning" id="pre-fig-test-5">[pre-fig-test-5] <value-of select="label"/> does not have a title.</report>
      ...
      <assert test="graphic" role="warning" id="pre-fig-test-7">[pre-fig-test-7] fig does not have graphic. Ensure author query is added asking for file.</assert>
  ...
  <pattern id="ar-fig-tests-pattern">
    <rule context="fig[ancestor::sub-article[@article-type='reply']]" id="ar-fig-tests">
      ...
      <assert test="graphic" role="warning" id="pre-ar-fig-test-3">[pre-ar-fig-test-3] Author Response fig does not have graphic. Ensure author query is added asking for file.</assert>
      
      
      
      <assert test="$no = string($pos)" role="warning" id="pre-ar-fig-position-test">[pre-ar-fig-position-test] <value-of select="label"/> does not appear in sequence which is likely incorrect. Relative to the other AR images it is placed in position <value-of select="$pos"/>.</assert>
