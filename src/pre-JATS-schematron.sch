 ...
 <pattern id="article-tests-pattern">
  ...
  <pattern id="research-article-pattern">
    <rule context="article[@article-type='research-article']" id="research-article">
	  ...
	  <report test="($disp-channel != 'Scientific Correspondence') and not(sub-article[@article-type='decision-letter'])" role="warning" flag="dl-ar" id="pre-test-r-article-d-letter">[pre-test-r-article-d-letter] A decision letter should be present for research articles.</report>
  ...
  <pattern id="author-aff-tests-pattern">
    <rule context="article-meta/contrib-group[not(@*)]/aff" id="author-aff-tests">
      ...
      <assert test="institution[not(@*)]" role="warning" id="pre-auth-aff-test-3">[pre-auth-aff-test-3] Author affiliations must have a top level institution. This one (with the id <value-of select="@id"/>) does not - <value-of select="$display"/>. Please query the authors.</assert>
  ...
  <pattern id="fig-tests-pattern">
    <rule context="fig[not(ancestor::sub-article)]" id="fig-tests">
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/figures#pre-fig-test-4" test="if ($article-type = $notice-article-types) then ()         else not(caption)" role="warning" id="pre-fig-test-4">[pre-fig-test-4] <value-of select="label"/> has no title or caption (caption element). Ensure this is queried with the author.</report>
      
      
      
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/figures#pre-fig-test-5" test="if ($article-type = $notice-article-types) then ()         else not(caption/title)" role="warning" id="pre-fig-test-5">[pre-fig-test-5] <value-of select="label"/> does not have a title.</report>
  ...
  <pattern id="elem-citation-pattern">
    <rule context="element-citation" id="elem-citation">
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/references#pre-element-cite-year" test="(@publication-type!='periodical') and not(year)" role="warning" id="pre-element-cite-year">[pre-element-cite-year] '<value-of select="@publication-type"/>' type references must have a year. Reference '<value-of select="../@id"/>' does not. If you are unable to determine this, please ensure to add an author query asking for the year of publication.</report>
      
      
      
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/references#pre-element-cite-string-date" test="(@publication-type='periodical') and not(string-date)" role="warning" id="pre-element-cite-string-date">[pre-element-cite-string-date] '<value-of select="@publication-type"/>' type references must have a string-date. Reference '<value-of select="../@id"/>' does not. If you are unable to determine this, please ensure to add an author query asking for the string-date of publication.</report>
 ...
 <pattern id="feature-title-tests-pattern">
  ...
  <pattern id="data-availability-statement-pattern">
    <rule context="sec[@sec-type='data-availability']/p[1]" id="data-availability-statement">
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/data-availability#pre-das-dryad-conformity" test="matches(.,'[Dd]ryad') and not(parent::sec//element-citation[contains(source[1],'Dryad') or pub-id[@pub-id-type='doi' and (contains(.,'10.7272') or contains(.,'10.5061/dryad'))]])" role="warning" id="pre-das-dryad-conformity">[pre-das-dryad-conformity] Data Availability Statement contains the word Dryad, but there is no data citation in the dataset section with a dryad doi and/or database name containing 'Dryad'. If there is a dryad dataset present, ensure that it is captured correctly, otherwise please query the authors for the reference details.</report>
  ...
  <pattern id="rrid-link-pattern">
    <rule context="ext-link[contains(@xlink:href,'scicrunch.org/resolver') and not(ancestor::sub-article)]" id="rrid-link">
      ...
      <report see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/rrids#pre-rrid-spacing" test="ends-with($lc,'rrid: ') or ends-with($lc,'rrid ')" role="error" id="pre-rrid-spacing">[pre-rrid-spacing] RRID (scicrunch) link should be preceded by 'RRID:' with no space but instead it is preceded by '<value-of select="concat(substring($pre-text,string-length($pre-text)-15),.)"/>'.</report>
