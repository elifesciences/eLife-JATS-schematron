<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:ali="http://www.niso.org/schemas/ali/1.0/" xmlns:file="java.io.File" xmlns:java="http://www.java.com/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" queryBinding="xslt2">
 ...
 <pattern id="article-tests-pattern"><rule context="article" id="article-tests">
  ...
  <pattern id="body-video-specific-pattern"><rule context="article[not(@article-type = $notice-article-types)]/body//media[@mimetype='video']" id="body-video-specific">
      ...
      <assert test="$no = string($pos)" role="warning" id="pre-app-video-position-test">[pre-app-video-position-test] <value-of select="label"/> does not appear in sequence which is likely incorrect. Relative to the other appendix videos it is placed in position <value-of select="$pos"/>.</assert>
      ...
      <assert test="$no = string($pos)" role="warning" id="pre-app-fig-video-position-test">[pre-app-fig-video-position-test] <value-of select="label"/> does not appear in sequence which is likely incorrect. Relative to the other appendix videos it is placed in position <value-of select="$pos"/>.</assert>
  ...
  <pattern id="gen-das-tests-pattern"><rule context="sec[@sec-type='data-availability']//element-citation[@publication-type='data']" id="gen-das-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#pre-das-elem-person-1" test="(count(person-group[@person-group-type='author']/name)=0) and (count(person-group[@person-group-type='author']/collab)=0)" role="warning" id="pre-das-elem-person-1">[pre-das-elem-person-1] The reference in position <value-of select="$pos"/> of the data availability section does not have any authors. Please ensure to add them in or query the authors asking for the author list.</report>
