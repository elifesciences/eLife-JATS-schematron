 ...
 <pattern id="article-tests-pattern">
  ...
  <pattern id="elem-citation-pattern">
    <rule context="element-citation" id="elem-citation">
      ...
      <report test="(@publication-type!='periodical') and not(year)" role="warning" id="pre-element-cite-year">[pre-element-cite-year] '<value-of select="@publication-type"/>' type references must have a year. Reference '<value-of select="../@id"/>' does not. If you are unable to determine this, please ensure to add an author query asking for the year of publication.</report>
      
      
      
      <report test="(@publication-type='periodical') and not(string-date)" role="warning" id="pre-element-cite-string-date">[pre-element-cite-string-date] '<value-of select="@publication-type"/>' type references must have a string-date. Reference '<value-of select="../@id"/>' does not. If you are unable to determine this, please ensure to add an author query asking for the string-date of publication.</report>
  ...
  <pattern id="pub-id-tests-pattern">
    <rule context="element-citation/pub-id" id="pub-id-tests">
      ...
      <report test="(@xlink:href) and not(matches(@xlink:href,'^http[s]?://|^s?ftp://'))" role="warning" id="pre-pub-id-test-1">[pre-pub-id-test-1] @xlink:href must start with an http:// or ftp:// protocol. - <value-of select="@xlink:href"/> does not. If this information is missing, please ensure to query it with the authors.</report>
      
      
      
      <report test="(@pub-id-type='doi') and not(matches(.,'^10\.\d{4,9}/[-._;\+()#/:A-Za-z0-9&lt;&gt;\[\]]+$'))" role="warning" id="pre-pub-id-test-2">[pre-pub-id-test-2] pub-id is tagged as a doi, but it is not one - <value-of select="."/>. If this information is missing, please ensure to query it with the authors.</report>
 ...
 <pattern id="feature-title-tests-pattern">
  ...
  <pattern id="ref-link-mandate-pattern">
    <rule context="ref-list/ref" id="ref-link-mandate">
      ...
      <assert test="ancestor::article//xref[@rid = $id]" role="warning" id="pre-ref-link-presence">[pre-ref-link-presence] '<value-of select="$id"/>' has no linked citations. Either the reference should be removed or a citation linking to it needs to be added.</assert>
