 ...
 <pattern id="article-tests-pattern">
  ...
  <pattern id="abstract-word-count-pattern">
    <rule context="front//abstract[not(@abstract-type) and not(sec)]" id="abstract-word-count">
      ...
      <report test="($count gt 280)" role="warning" id="pre-abstract-word-count-restriction">[pre-abstract-word-count-restriction] The abstract contains <value-of select="$count"/> words, when the usual upper limit is 280. Exeter: Please check with the eLife production team who will need to contact the eLife Editorial team.</report>
  ...
  <pattern id="general-video-pattern">
    <rule context="media[@mimetype='video'][matches(@id,'^video[0-9]{1,3}$')]" id="general-video">
      ...
      <report test="($xrefs//*:match) and ($sec-id != $sec1/@id)" role="warning" id="pre-video-placement-1">[pre-video-placement-1] <value-of select="$label"/> does not appear in the same section as where it is first cited (sec with title '<value-of select="$sec1/title"/>'), which is incorrect. If videos are cited out of order, please esnure that this issue is raised with the authors.</report>
