<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:ali="http://www.niso.org/schemas/ali/1.0/" xmlns:file="java.io.File" xmlns:java="http://www.java.com/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" queryBinding="xslt2">
 ...
 <pattern id="article-tests-pattern"><rule context="article" id="article-tests">
  ...
  <pattern id="test-article-metadata-pattern"><rule context="article/front/article-meta" id="test-article-metadata">
   ...
   <let name="template" value="parent::article-meta/custom-meta-group/custom-meta[meta-name='Template']/meta-value[1]"/>
    ...
    <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#equal-count-test" test="count(contrib[@equal-contrib='yes']) = 1" role="error" id="equal-count-test">[equal-count-test] There is one contrib with the attribute equal-contrib='yes'. This cannot be correct. Either 2 or more contribs within the same contrib-group should have this attribute, or none. Check <value-of select="contrib[@equal-contrib='yes']/name"/></report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#pre-collab-cont-test-1-v2" test="role" role="warning" flag="version-2" id="pre-collab-cont-test-1-v2">[pre-collab-cont-test-1-v2] <value-of select="e:get-surname(.)"/> has no contributions. Please ensure to query this with the authors.</assert>
    ...
    <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#given-names-test-9" test="matches(.,'\p{Zs}$')" role="error" id="given-names-test-9">[given-names-test-9] given-names ends with a space, which cannot be correct - '<value-of select="."/>'.</report>
	  ...
	  <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#pre-orcid-test-3" test="count(ancestor::contrib-group//contrib-id[@contrib-id-type='orcid' and .=$text]) = 1" role="warning" id="pre-orcid-test-3">[pre-orcid-test-3] <value-of select="e:get-name(parent::*/name[1])"/>'s ORCiD is the same as another author's - <value-of select="."/>. Duplicated ORCiDs are not allowed. If it is clear who the ORCiD belongs to, remove the duplicate. If it is not clear please add an author query - 'This ORCiD - <value-of select="."/> - is associated with <value-of select="count(ancestor::contrib-group//contrib-id[@contrib-id-type='orcid' and .=$text])"/> authors. Please confirm which author this ORCiD belongs to.'.</assert>
	...
	<assert test="license" role="error" id="permissions-test-5">[permissions-test-5] permissions must contain license.</assert>
	  ...
	  <report see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#pre-abstract-test-5" test="child::sec and not(count(sec) = (5,6))" role="warning" id="pre-abstract-test-5">[pre-abstract-test-5] If an abstract is structured, then it must have 5 or 6 sections depending on whether it is a clinical trial. An article without a clinical trial should have 5 sections, whereas one with a clinical trial should have 6.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#pre-abstract-word-count-restriction" test="($count gt 300)" role="warning" id="pre-abstract-word-count-restriction">[pre-abstract-word-count-restriction] The abstract contains <value-of select="$count"/> words, when the usual upper limit is 300. Exeter: Please check with the eLife production team who will need to contact the eLife Editorial team.</report>
    ...
    <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h5tdf-aff-test-1" test="parent::contrib-group//contrib//xref/@rid = @id" role="error" id="aff-test-1">[aff-test-1] aff elements that are direct children of contrib-group must have an xref in that contrib-group pointing to them.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hdxya-pre-auth-aff-test-1" test="country" role="warning" id="pre-auth-aff-test-1">[pre-auth-aff-test-1] Author affiliations must have a country. This one does not - <value-of select="$display"/>. Please query with the authors.</assert>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h7l30-pre-auth-aff-test-2" test="addr-line[named-content[@content-type='city']]" role="warning" id="pre-auth-aff-test-2">[pre-auth-aff-test-2] Author affiliations must have a city. This one does not - <value-of select="$display"/>. Please query the authors.</assert>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h2v3t-pre-auth-aff-test-3" test="institution[not(@*)] or institution-wrap[institution[not(@*)]]" role="warning" id="pre-auth-aff-test-3">[pre-auth-aff-test-3] Author affiliations (&lt;aff&gt;) must include an &gt;institution&gt; tag. This one (with the id <value-of select="@id"/>) does not - <value-of select="$display"/>. Please query the authors.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#pre-custom-meta-test-6" test="matches(.,'[\.|\?]$')" role="warning" id="pre-custom-meta-test-6">[pre-custom-meta-test-6] Impact statement must end with a full stop or question mark.</assert>
  ...
  <pattern id="p-tests-pattern"><rule context="p" id="p-tests">
    ...
    <report test="not(ancestor::fig/permissions[contains(.,'phylopic')]) and matches(@xlink:href,'phylopic\.org')" role="warning" id="phylopic-link-check">[phylopic-link-check] This link is to phylopic.org, which is a site where silhouettes/images are typically reproduced from. Please check whether any figures contain reproduced images from this site, and if so whether permissions have been obtained and/or copyright statements are correctly included.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#pre-fig-test-4" test="not($article-type = ('discussion',$notice-article-types)) and not(caption)" role="warning" id="pre-fig-test-4">[pre-fig-test-4] <value-of select="label"/> has no title or caption (caption element). Ensure this is queried with the author.</report>
      
      
      
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#pre-fig-test-5" test="not($article-type = ('discussion',$notice-article-types)) and not(caption/title)" role="warning" id="pre-fig-test-5">[pre-fig-test-5] <value-of select="label"/> does not have a title.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#pre-fig-test-7" test="graphic" role="warning" id="pre-fig-test-7">[pre-fig-test-7] fig does not have graphic. Ensure author query is added asking for file.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#pre-ar-fig-test-3" test="graphic" role="warning" flag="dl-ar" id="pre-ar-fig-test-3">[pre-ar-fig-test-3] Author Response fig does not have graphic. Ensure author query is added asking for file.</assert>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#pre-ar-fig-position-test" test="$no = string($pos)" role="warning" flag="dl-ar" id="pre-ar-fig-position-test">[pre-ar-fig-position-test] <value-of select="label"/> does not appear in sequence which is likely incorrect. Relative to the other AR images it is placed in position <value-of select="$pos"/>.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#pre-video-title" test="caption/title" role="warning" id="pre-video-title">[pre-video-title] <value-of select="replace(label,'\.$,','')"/> does not have a title. Please query the authors for one.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#pre-supplementary-material-test-3" test="if (caption) then not(caption/title)         else ()" role="warning" id="pre-supplementary-material-test-3">[pre-supplementary-material-test-3] <value-of select="label"/> does not have a title.</report>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#pre-supplementary-material-test-5" test="media" role="warning" id="pre-supplementary-material-test-5">[pre-supplementary-material-test-5] <value-of select="label"/> is missing a file (supplementary-material missing a media element) - please ensure that this is queried with the author.</assert>		
      ...
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#pre-table-title-test-1" test="caption/title" role="warning" id="pre-table-title-test-1">[pre-table-title-test-1] <value-of select="replace(label[1],'\.$','')"/> does not have a title. Please ensure to query the authors for one.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#pre-video-cite" test="$xrefs//*:match" role="warning" id="pre-video-cite">[pre-video-cite] There is no citation to <value-of select="$label"/>. Ensure to query the author asking for a citation.</assert>
      
      
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#pre-video-placement-1" test="($xrefs//*:match) and ($sec-id != $sec1/@id)" role="warning" id="pre-video-placement-1">[pre-video-placement-1] <value-of select="$label"/> does not appear in the same section as where it is first cited (sec with title '<value-of select="$sec1/title"/>'), which is incorrect. If videos are cited out of order, please ensure that this issue is raised with the authors.</report>
  ...
  <pattern id="body-table-pos-conformance-pattern"><rule context="article[not(@article-type=$notice-article-types)]/body//table-wrap[matches(@id,'^table[\d]+$')]" id="body-table-pos-conformance">
      ...
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#pre-body-table-report" test="($no = string($pos))" role="warning" id="pre-body-table-report">[pre-body-table-report] <value-of select="label"/> does not appear in sequence. Relative to the other numbered tables it is placed in position <value-of select="$pos"/>.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#pre-app-table-report" test="($no = string($pos))" role="warning" id="pre-app-table-report">[pre-app-table-report] <value-of select="label"/> does not appear in sequence. Relative to the other numbered tables in the same appendix it is placed in position <value-of select="$pos"/>.</assert>
  ...
  <pattern id="fig-specific-tests-pattern"><rule context="article/body//fig[not(@specific-use='child-fig')][not(ancestor::boxed-text)]" id="fig-specific-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/licensing-and-copyright-rqdavyty?shr=rqdavyty#hjmtk-pre-fig-biorender-test-1" test="not(descendant::ext-link[matches(lower-case(@xlink:href),'biorender.com/[a-z\d]')]) and (descendant::ext-link[matches(lower-case(@xlink:href),'biorender.com')] or matches(lower-case(.),'biorender.com'))" role="warning" id="pre-fig-biorender-test-1">[pre-fig-biorender-test-1] Caption or attrib for <value-of select="$label"/> contains a BioRender link, but it does not look like a BioRender 'unique figure citation URL'. Exeter: If one has not been provided already, please query this with the eLife team.</report>
  ...
  <pattern id="ref-list-ordering-pattern"><rule context="ref[preceding-sibling::ref]" id="ref-list-ordering">
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#pre-element-cite-year" test="not(year)" role="warning" id="pre-element-cite-year">[pre-element-cite-year] '<value-of select="@publication-type"/>' type references must have a year. Reference '<value-of select="../@id"/>' does not. If you are unable to determine this, please ensure to add an author query asking for the year of publication.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#pre-empty-elem-cit-des" test="normalize-space(.)=''" role="warning" id="pre-empty-elem-cit-des">[pre-empty-elem-cit-des] <name/> element is empty - this is not allowed. It must contain content. If the details are missing and cannot be determined, please query the authors.</report>
  ...
  <pattern id="elem-citation-journal-pattern"><rule context="element-citation[@publication-type='journal']" id="elem-citation-journal">
      ...
      <assert see="https://elifeproduction.slab.com/posts/journal-references-i098980k#pre-err-elem-cit-journal-2-1" test="count(person-group)=1" role="warning" id="pre-err-elem-cit-journal-2-1">[pre-err-elem-cit-journal-2-1] Each  &lt;element-citation&gt; of type 'journal' must contain one and only one &lt;person-group&gt; element. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(person-group)"/> &lt;person-group&gt; elements. If this information is missing, please query the authors for it.</assert>
  ...
  <pattern id="elem-citation-book-pattern"><rule context="element-citation[@publication-type='book']" id="elem-citation-book">
      ...
      <assert see="https://elifeproduction.slab.com/posts/book-references-x4trb0n2#hq3u9-pre-err-elem-cit-book-2-3" test="count(person-group)=1 or (count(person-group[@person-group-type='author'])=1 and count(person-group[@person-group-type='editor'])=1)" role="warning" id="pre-err-elem-cit-book-2-3">[pre-err-elem-cit-book-2-3] In a book reference, there should be a single person-group element (either author or editor) or one person-group with @person-group-type="author" and one person-group with @person-group-type=editor. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(person-group)"/> &lt;person-group&gt; elements. If this finromation is missing, please query it with the authors.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/book-references-x4trb0n2#hg5ky-pre-err-elem-cit-book-13-1" test="count(publisher-name)=1" role="warning" id="pre-err-elem-cit-book-13-1">[pre-err-elem-cit-book-13-1] One and only one &lt;publisher-name&gt; is required in a book reference. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(publisher-name)"/> &lt;publisher-name&gt; elements. If this information is missing, please query it with the authors.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/book-references-x4trb0n2#h7i77-pre-err-elem-cit-book-22" test="count(../person-group[@person-group-type='author'])=1" role="warning" id="pre-err-elem-cit-book-22">[pre-err-elem-cit-book-22] If there is a &lt;chapter-title&gt; element there must be one and only one &lt;person-group person-group-type="author"&gt;. Reference '<value-of select="ancestor::ref/@id"/>' does not meet this requirement. If this information is missing, please query the authors for it.</assert>
  ...
  <pattern id="elem-citation-data-pattern"><rule context="ref/element-citation[@publication-type='data']" id="elem-citation-data">
      ...
      <report see="https://elifeproduction.slab.com/posts/data-references-4jxukxzy#pre-err-elem-cit-data-3-2" test="count(person-group) lt 1" role="warning" id="pre-err-elem-cit-data-3-2">[pre-err-elem-cit-data-3-2] Data references must have one and only one &lt;person-group person-group-type='author'&gt;. Reference '<value-of select="ancestor::ref/@id"/>' has 0. If this information is missing, please query the authors asking for it.</report>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/data-references-4jxukxzy#pre-err-elem-cit-data-10" test="count(data-title)=1" role="warning" id="pre-err-elem-cit-data-10">[pre-err-elem-cit-data-10] Data reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(data-title)"/> data-title elements, when it should contain one. If this information is missing, please query it with the authors.</assert>
  ...
  <pattern id="elem-citation-patent-pattern"><rule context="element-citation[@publication-type='patent']" id="elem-citation-patent">
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#pre-err-elem-cit-patent-2-1" test="count(person-group[@person-group-type='inventor'])=1" role="warning" id="pre-err-elem-cit-patent-2-1">[pre-err-elem-cit-patent-2-1] There must be one person-group with @person-group-type="inventor". Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(person-group[@person-group-type='inventor'])"/> &lt;person-group&gt; elements of type 'inventor'. If this information is missing, please query the authors for it.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#pre-err-elem-cit-patent-8-1" test="count(article-title)=1" role="warning" id="pre-err-elem-cit-patent-8-1">[pre-err-elem-cit-patent-8-1] Each  &lt;element-citation&gt; of type 'patent' must contain one and only one &lt;article-title&gt; element. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(article-title)"/> &lt;article-title&gt; elements.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#pre-err-elem-cit-patent-10-1-1" test="patent" role="warning" id="pre-err-elem-cit-patent-10-1-1">[pre-err-elem-cit-patent-10-1-1] The  &lt;patent&gt; element is required. Reference '<value-of select="ancestor::ref/@id"/>' has no &lt;patent&gt; elements. If you are unable to determine this yourself, please add an author query asking for this.</assert>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#pre-err-elem-cit-patent-11-1" test="ext-link" role="warning" id="pre-err-elem-cit-patent-11-1">[pre-err-elem-cit-patent-11-1] The &lt;ext-link&gt; element is required in a patent reference. Reference '<value-of select="ancestor::ref/@id"/>' has no &lt;ext-link&gt; elements. If you are unable to determine this yourself, please add an author query asking for this.</assert>
  ...
  <pattern id="elem-citation-preprint-pattern"><rule context="element-citation[@publication-type='preprint']" id="elem-citation-preprint">
      ...
      <assert test="count(pub-id)=1 or count(ext-link)=1" role="warning" id="pre-err-elem-cit-preprint-10-3">[pre-err-elem-cit-preprint-10-3] Either one &lt;pub-id&gt; or one &lt;ext-link&gt; element is required in a preprint reference. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(pub-id)"/> &lt;pub-id&gt; elements and <value-of select="count(ext-link)"/> &lt;ext-link&gt; elements.</assert>
  ...
  <pattern id="elem-citation-web-pattern"><rule context="element-citation[@publication-type='web']" id="elem-citation-web">
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#pre-err-elem-cit-web-8-1" test="count(article-title)=1" role="warning" id="pre-err-elem-cit-web-8-1">[pre-err-elem-cit-web-8-1] Each  &lt;element-citation&gt; of type 'web' must contain one and only one &lt;article-title&gt; element. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(article-title)"/> &lt;article-title&gt; elements. If you are unable to determine this yourself, please query the authors for this information.</assert>
  ...
  <pattern id="gen-das-tests-pattern"><rule context="sec[@sec-type='data-availability']//element-citation[@publication-type='data']" id="gen-das-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#pre-das-elem-person-group-1" test="count(person-group[@person-group-type='author'])=1" role="warning" id="pre-das-elem-person-group-1">[pre-das-elem-person-group-1] The reference in position <value-of select="$pos"/> of the data availability section does not have any authors (no person-group[@person-group-type='author']). Please ensure to add them in or query the authors asking for the author list.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#pre-das-elem-person-1" test="(count(person-group[@person-group-type='author']/name)=0) and (count(person-group[@person-group-type='author']/collab)=0) and (count(person-group[@person-group-type='author']/collab-name)=0)" role="warning" id="pre-das-elem-person-1">[pre-das-elem-person-1] The reference in position <value-of select="$pos"/> of the data availability section does not have any authors. Please ensure to add them in or query the authors asking for the author list.</report>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#pre-das-elem-data-title-1" test="count(data-title)=1" role="warning" id="pre-das-elem-data-title-1">[pre-das-elem-data-title-1] The reference in position <value-of select="$pos"/> of the data availability section does not have a title (no data-title). Please ensure to add it in or query the authors asking for it.</assert>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#pre-das-elem-source-1" test="count(source)=1" role="warning" id="pre-das-elem-source-1">[pre-das-elem-source-1] The reference in position <value-of select="$pos"/> of the data availability section does not have a database name (no source). Please ensure to add it in or query the authors asking for it.</assert>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#pre-das-elem-pub-id-1" test="count(pub-id)=1" role="warning" id="pre-das-elem-pub-id-1">[pre-das-elem-pub-id-1] The reference in position <value-of select="$pos"/> of the data availability section does not have an identifier (no pub-id). Please ensure to add it in or query the authors asking for it.</assert>
      
      
      
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#pre-das-elem-pub-id-2" test="normalize-space(pub-id)=''" role="warning" id="pre-das-elem-pub-id-2">[pre-das-elem-pub-id-2] The reference in position <value-of select="$pos"/> of the data availability section does not have an id (pub-id is empty). Please ensure to add it in or query the authors asking for it.</report>
      
      
      
      <assert see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#pre-das-elem-year-1" test="count(year)=1" role="warning" id="pre-das-elem-year-1">[pre-das-elem-year-1] The reference in position <value-of select="$pos"/> of the data availability section does not have a year. Please ensure to add it in or query the authors asking for it.</assert>
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#pre-das-pub-id-1" test="normalize-space(.)!='' and not(@pub-id-type=('accession', 'doi'))" role="warning" id="pre-das-pub-id-1">[pre-das-pub-id-1] Each pub-id element must have an @pub-id-type which is either accession or doi. If this identifier is missing, then ensure to query the author for this information.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#pre-das-elem-citation-year-2" test="normalize-space(.)=''" role="warning" id="pre-das-elem-citation-year-2">[pre-das-elem-citation-year-2] Reference with id <value-of select="parent::*/@id"/> has an empty year. Please ensure to add it in or query the authors asking for it.</report>
  ...
  <pattern id="pub-id-tests-pattern"><rule context="element-citation/pub-id" id="pub-id-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#pre-pub-id-test-2" test="(@pub-id-type='doi') and not(matches(.,'^10\.\d{4,9}/[-._;\+()#/:A-Za-z0-9%&lt;&gt;\[\]]+$'))" role="warning" id="pre-pub-id-test-2">[pre-pub-id-test-2] pub-id is tagged as a doi, but it is not one - <value-of select="."/>. If this information is missing, please ensure to query it with the authors.</report>
 ...
 <pattern id="feature-title-tests-pattern"><rule context="article-meta[descendant::subj-group[@subj-group-type='display-channel']/subject = $features-subj]//title-group/article-title" id="feature-title-tests">
     ...
     <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#pre-feature-templates-author-cont-2" test="$template = ('3','4') and not(back/fn-group[@content-type='author-contribution'])" role="warning" id="pre-feature-templates-author-cont-2">[pre-feature-templates-author-cont-2] <value-of select="$type"/>s should have Author contributions. This one does not. Exeter please check with the Production team who will check with the Features team.</report>
  ...
  <pattern id="unallowed-symbol-tests-pattern"><rule context="p|td|th|title|xref|bold|italic|sub|sc|named-content|monospace|code|underline|fn|institution|ext-link" id="unallowed-symbol-tests">		
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hh80v-pre-us-states-test" test="matches($lc,$states-regex)" role="warning" id="pre-US-states-test">[pre-US-states-test] city contains a US state (or an abbreviation for it) - <value-of select="."/>. Please raise with eLife production staff.</report>
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#pre-das-dryad-conformity" test="matches(.,'[Dd]ryad') and          not(parent::sec//element-citation[contains(source[1],'Dryad') or pub-id[@pub-id-type='doi' and (contains(.,'10.7272') or contains(.,'10.5061/dryad'))]])         and not(ancestor::back//ref-list/ref[element-citation[@publication-type='data' and (contains(source[1],'Dryad') or pub-id[@pub-id-type='doi' and (contains(.,'10.7272') or contains(.,'10.5061/dryad'))])]]/@id = xref[@ref-type='bibr']/@rid)" role="warning" id="pre-das-dryad-conformity">[pre-das-dryad-conformity] Data Availability Statement contains the word Dryad, but there is no data citation in the dataset section with a dryad doi and/or database name containing 'Dryad'. If there is a dryad dataset present, ensure that it is captured correctly, otherwise please query the authors for the reference details.</report>
      ...
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#pre-colour-check-table-2" test="@style=($allowed-values)" role="warning" id="pre-colour-check-table-2">[pre-colour-check-table-2] <name/> element containing '<value-of select="."/>' has an @style with an unallowed value - '<value-of select="@style"/>'. The only supported values are <value-of select="string-join($allowed-values,', ')"/> for blue, green orange, yellow, purple, red, pink and grey respectively. Please ensure one of these is used. If unsupported colours have been used, please query the Production team who will check with the authors.</assert>
      ...
      <assert see="https://elifeproduction.slab.com/posts/general-layout-and-formatting-wq0m31at#hm00e-pre-styled-content-style-check" test="@style = $allowed-values" role="warning" id="pre-styled-content-style-check">[pre-styled-content-style-check] <value-of select="."/> - text in <value-of select="parent::*/name()"/> element is captured in a &lt;styled-content style="<value-of select="@style"/>"&gt;. The only allowed values for the @style are <value-of select="string-join($allowed-values,', ')"/> as only blue, purple, and red text is permitted (respectively). If this is coloured text and it is not one of the allowed colours, please query the authors - 'eLife only supports the following colours for text - red, blue and purple. Please confirm how you would like the colour(s) here captured given this information.'</assert>
  ...
  <pattern id="empty-attribute-test-pattern"><rule context="*[@*/normalize-space(.)='']" id="empty-attribute-test">
      ...
      <report test="." role="warning" id="pre-empty-attribute-conformance">[pre-empty-attribute-conformance] <value-of select="name()"/> element has attribute(s) with an empty value. &lt;<value-of select="name()"/><value-of select="for $att in ./@*[normalize-space(.)=''] return concat(' ',$att/name(),'=&quot;',$att,'&quot;')"/>&gt;. If this cannot be filled out yet (due to missing or incomplete information), please ensure that the authors are queried, as appropriate.</report>
