 ...
 <pattern id="article-tests-pattern">
  ...
  <pattern id="research-article-pattern">
    <rule context="article[@article-type='research-article']" id="research-article">
	  ...
	  <report test="($disp-channel != 'Scientific Correspondence') and not(sub-article[@article-type='decision-letter'])" role="warning" id="pre-test-r-article-d-letter">[pre-test-r-article-d-letter] A decision letter should be present for research articles.</report>
  ...
  <pattern id="ar-fig-tests-pattern">
    <rule context="fig[ancestor::sub-article[@article-type='reply']]" id="ar-fig-tests">
      ...
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/figures#pre-ar-fig-test-3" test="graphic" role="warning" id="pre-ar-fig-test-3">[pre-ar-fig-test-3] Author Response fig does not have graphic. Ensure author query is added asking for file.</assert>
      
      
      
      <assert see="https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/allowed-assets/figures#pre-ar-fig-position-test" test="$no = string($pos)" role="warning" id="pre-ar-fig-position-test">[pre-ar-fig-position-test] <value-of select="label"/> does not appear in sequence which is likely incorrect. Relative to the other AR images it is placed in position <value-of select="$pos"/>.</assert>
  ...
  <pattern id="ar-video-specific-pattern">
    <rule context="sub-article/body//media[@mimetype='video']" id="ar-video-specific">
      ...
      <assert test="$no = string($pos)" role="warning" id="pre-ar-video-position-test">[pre-ar-video-position-test] <value-of select="label"/> does not appear in sequence which is likely incorrect. Relative to the other AR videos it is placed in position <value-of select="$pos"/>.</assert>
 ...
 <pattern id="feature-title-tests-pattern">
  ...
  <pattern id="italic-house-style-pattern">
    <rule context="italic[not(ancestor::ref) and not(ancestor::sub-article)]" id="italic-house-style">  
      ...
      <report test="matches(.,'[Ss]ensu')" role="error" id="pre-sensu-italic-test">[pre-sensu-italic-test] <name/> element contains 'sensu' - this should not be in italics (eLife house style).</report>
