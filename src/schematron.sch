<schema
        ...
        <xsl:choose>
          <xsl:when test="lower-case($token1)='elife'">
            <xsl:value-of select="concat('eLife',
              ' ',
              string-join(for $x in tokenize(substring-after($token2,' '),'\p{Zs}') return e:titleCaseToken($x),' ')
              )"/>
          </xsl:when>
  ...
  <xsl:function name="e:ref-list-string" as="xs:string">
    ...
    <xsl:choose>
      <xsl:when test="$ref/element-citation[1]/person-group[1]/* and $ref/element-citation[1]/year">
        <xsl:value-of select="concat(
          replace(e:get-collab-or-surname($ref/element-citation[1]/person-group[1]/*[1]),'\p{Zs}',''),
          ' ',
          $ref/element-citation[1]/year[1],
          ' ',
          string-join(for $x in $ref/element-citation[1]/person-group[1]/*[position()=(2,3)]
          return replace(e:get-collab-or-surname($x),'\p{Zs}',''),' ')
          )"/>
      </xsl:when>
  ...
  <xsl:function name="e:tex-arrays-without-spacing" as="xs:string*">
    ...
    <xsl:variable name="marked"
      select="replace(
      replace($text, '\\begin[{]array[}][^}]*[}]', '|__BEGIN__|'),
      '\\end[{]array[}]', '|__END__|'
      )"/>
  ...
  <xsl:function name="e:walk-tex-arrays" as="xs:string*">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$tokens[1] = '__BEGIN__'">
        <xsl:sequence select="e:walk-tex-arrays(
          tail($tokens),
          $depth + 1,
          if ($depth gt 0) then concat($current, $tokens[1]) else ''
        )"/>
      </xsl:when>
      ...
      <xsl:otherwise>
        <xsl:sequence select="e:walk-tex-arrays(
          tail($tokens),
          $depth,
          if ($depth gt 0) then concat($current, $tokens[1]) else $current
        )"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:function>
  ...
  <pattern id="covid-prologue-pattern">
    <rule context="article[front/article-meta//article-title[matches(lower-case(.),'sars-cov-2|covid-19|coronavirus')]]" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/article-content-house-style-k617122o#h1vp1-covid-processing-instruction"
        test="preceding::processing-instruction('covid-19-tdm')" 
        role="warning" 
 ...
 <pattern id="article">
    ...
    <report test="(@article-type = ('article-commentary','discussion','editorial','research-article','review-article')) and count(back) != 1" 
      ...
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#h3q4a-retraction-info"
        test="@article-type='retraction'" 
        role="info" 
      ...
      <report test="not(@article-type='research-article') and not(@article-type='review-article' and e:is-prc(.)) and sub-article"
        role="error"
	...
	<rule context="article[@article-type='research-article']" id="research-article">
	  ...
	  <report test="if ($is-prc) then ($disp-channel != 'Scientific Correspondence') and not(sub-article[@article-type='referee-report'])
        ...
        role="warning"
        flag="dl-ar"
        id="test-r-article-d-letter"><value-of select="if ($is-prc) then 'Public reviews and recomendations for the authors' else 'A decision letter'"/> should almost always be present for research articles. This one doesn't have one. Check that this is correct.</report>
	  
	  <report see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#final-test-r-article-d-letter-feat"
	    test="$disp-channel = 'Feature Article' and not(sub-article[@article-type='decision-letter'])" 
        role="warning" 
        flag="dl-ar"
	  ...
	  <report test="$disp-channel != 'Scientific Correspondence' and not(sub-article[@article-type=('reply','author-comment')])" 
        role="warning"
        flag="dl-ar"
  ...
  <rule context="article/front/article-meta" id="test-article-metadata">
    ...
    <report test="not($article-type = $notice-article-types) and not(self-uri)" 
        role="error" 
        id="test-self-uri-presence">There must be a child self-uri in article-meta.</report>
		
    <report test="not($article-type = $notice-article-types) and not(self-uri[@content-type='pdf'])" 
        role="error" 
        id="test-self-uri-att">self-uri must have an @content-type="pdf"</report>
		
    <report test="not($article-type = $notice-article-types) and not(self-uri[starts-with(@xlink:href,concat('elife-', $article-id))])" 
        role="error" 
    ...
    <report test="not($article-type = ($notice-article-types,'article-commentary','editorial')) and ($dtd le '1.3') and count(history) != 1" 
        role="error" 
    ...
    <report test="not($article-type = ($notice-article-types,'article-commentary','editorial')) and ($dtd ge '1.4') and count(history) != 0" 
        role="error" 
        id="test-history-presence-dtd">The history element is deprecated in JATS version <value-of select="$dtd"/>. Remove it.</report>
   ...
   <let name="template" value="parent::article-meta/custom-meta-group/custom-meta[meta-name='Template']/meta-value[1]"/>
    ...
    <let name="article-type" value="ancestor::article/@article-type"/> 
      ...
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#h0zbc-disp-subj-value-test-5"
        test="($article-type = 'correction') and not(.='Correction')" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#h7pnv-disp-subj-value-test-8"
        test="($article-type = 'retraction') and not(.='Retraction')" 
        role="error" 
      ...
      <report test="($article-type = 'expression-of-concern') and not(.='Expression of Concern')" 
        role="error" 
        id="disp-subj-value-test-9">Article is an @article-type="<value-of select="$article-type"/>" but the display channel is <value-of select="."/>. It should be 'Expression of Concern' according to the article-type.</report>
  </rule>
    ...
    <report test="article-title//bold" 
	  ...
	  <report test="not($subj-type = ($notice-display-types,'Scientific Correspondence','Replication Study')) and matches($tokens,'[A-Za-z]')" 
        role="warning" 
    ...
    <rule context="article//article-meta/contrib-group[1][contrib[@contrib-type='author']/*[name()=('collab','collab-wrap')]/contrib-group]" 
      ...
      <let name="members" value="for $member in contrib[@contrib-type='author']/*[name()=('collab','collab-wrap')]/contrib-group/contrib[@contrib-type='author']/name[1]
    ...
    <rule context="article-meta//contrib[@contrib-type='author']/xref" id="author-xref-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hzwb3-author-xref-test-1" 
        test="(@ref-type='aff') and preceding-sibling::xref[not(@ref-type='aff')]" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hzwb3-author-xref-test-3" 
        test="(@ref-type='fn') and contains(@rid,'pa') and following-sibling::xref[@ref-type='aff' or contains(@rid,'equal')]" 
        role="error" 
	...
	<rule context="article-meta//contrib" id="contrib-tests">
	  ...
	  <let name="name" value="if (child::name[1]) then e:get-name(child::name[1]) else if (collab or collab-wrap) then e:get-surname(*[name()=('collab','collab-wrap')][1]) else ()"/>
    	...
    	<report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hjuk3-contrib-test-1" 
    	  test="if ($subj-type = $notice-display-types) then ()
    	  else if (collab or collab-wrap or ancestor::*[name()=('collab','collab-wrap')]) then ()
	  ...
	  <report see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#contrib-test-5"
        ...
        role="warning" 
        id="contrib-test-5">Group author members should likely have an affiliation. <value-of select="$name"/> does not. Is this OK?</report>
	  
	  <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hjuk3-contrib-test-2" 
    	  test="($type = 'senior_editor') and (count(xref[@ref-type='aff']) + count(aff) = 0)" 
        role="warning" 
        id="contrib-test-2">The <value-of select="role[1]"/> doesn't have an affiliation - <value-of select="$name"/> - is this correct? Exeter: If it is not present in the eJP output, please check with eLife production. Production: Please check eJP or ask Editorial for the correct affiliation.</report>
	  
	  <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hjuk3-contrib-test-4" 
    	  test="($type = 'editor') and (count(xref[@ref-type='aff']) + count(aff) = 0)" 
        role="error" 
    ...
    <rule context="article[e:get-version(.)!='1']//article-meta//contrib[@contrib-type='author']/*" id="author-children-tests-v2">
      ...
      <assert see="https://elifeproduction.slab.com/posts/article-contributors-m3y6vxlc#author-children-test-v2"
        ...
        role="error" 
        flag="version-2"
        id="author-children-test-v2"><value-of select="self::*/local-name()"/> is not allowed as a child of author.</assert>
	...
	<rule context="article[(not(@article-type) or @article-type=('research-article','review-article','discussion','')) and not(e:is-prc(.))]//article-meta/history" id="history-tests">
	   <let name="dtd-version" value="ancestor::article/@dtd-version"/>
        ...
        role="error" 
        id="history-date-test-1">history in JATS version <value-of select="$dtd-version"/> must contain date[@date-type='received']</report>
		
    	<report test="($dtd-version le '1.3') and not(date[@date-type='accepted'])" 
        role="error" 
        id="history-date-test-2">history in JATS version <value-of select="$dtd-version"/> must contain date[@date-type='accepted']</report>
	  
	  <report test="($dtd-version ge '1.4')" 
        role="error" 
        id="history-1.4-test-1">history in article-meta is deprecated in JATS version <value-of select="$dtd-version"/>. Please move the items to pub-history and remove the history element.</report>
    ...
    <rule context="article[(not(@article-type) or @article-type=('research-article','review-article','discussion','')) and e:is-prc(.)]//article-meta/history" id="prc-history-tests">
      ...
      <report test="($dtd-version le '1.3') and not(date[@date-type='sent-for-review'])" 
        role="error" 
        id="prc-history-date-test-1">history must contain date[@date-type='sent-for-review'] in PRC articles tagged in JATS version <value-of select="$dtd-version"/>.</report>
      
      <report test="($dtd-version le '1.3') and date[@date-type!='sent-for-review' or not(@date-type)]" 
        role="error" 
        id="prc-history-date-test-2">PRC articles can only have sent-for-review dates in their history. This one has a <value-of select="if (date[@date-type!='sent-for-review']) then date[@date-type!='sent-for-review']/@date-type else 'undefined'"/> date.</report>
      
      <report test="($dtd-version ge '1.4')" 
        role="error" 
        id="history-1.4-test-2">history in article-meta is deprecated in JATS version <value-of select="$dtd-version"/>. Please move the items to pub-history and remove the history element.</report>
    ...
    <rule context="pub-date" id="pub-date-tests">
      ...
      <assert test="parent::article-meta" 
        role="error" 
        id="pub-date-test-1">pub-date can only be a child of article-meta. This one is a child of <value-of select="parent::*/name()"/></assert>

      <report test="@*[not(name()=$allowed-attributes)]" 
        role="error" 
        id="pub-date-test-2">Only the following attributes are permitted on pub-date. This one has the following unallowed attributes: <value-of select="string-join(@*[not(name()=$allowed-attributes)]/name(),'; ')"/>.</report>

      <report test="@publication-format and @publication-format!='electronic'" 
        role="error" 
        id="pub-date-test-3">pub-date has the attribute publication-format, but the value is "<value-of select="@publication-format"/>". The only permitted value is "electronic".</report>

      <report test="@date-type and @date-type!='publication'" 
        role="error" 
        id="pub-date-test-4">pub-date has the attribute date-type, but the value is "<value-of select="@date-type"/>". The only permitted value is "publication".</report>

      <report test="preceding-sibling::pub-date" 
        role="error" 
        id="pub-date-test-5">There can only be one pub-date in article-meta. But this pub-date has a preceding one.</report>

      <assert test="@publication-format" 
        role="error" 
        id="pub-date-test-6">pub-date must have the attribute publication-format.</assert>
    ...
    <assert test="name()=$allowed-children" 
        role="error" 
        id="pub-date-child-test-1"><value-of select="name()"/> is not allowed as a child of pub-date. The only permitted elements are <value-of select="string-join($allowed-children,'; ')"/>.</assert>
    ...
    <rule context="pub-date[not(@pub-type='collection') and day and month and year][concat(year[1],'-',month[1],'-',day[1]) gt format-date(current-date(), '[Y0001]-[M01]-[D01]')]" id="press-pub-date">
      ...
      <report test="e:get-weekday($date) != 2" 
        role="warning" 
        id="press-pub-date-check">The publication date for this article is in the future (<value-of select="$date"/>), but the day of publication is not a Tuesday (for Press). Is that correct?</report>
    ...
    <rule context="event-desc" id="event-desc-tests">
      ...
      <report test="matches(parent::event/self-uri[1]/@xlink:href,'elifesciences\.org|10.7554/e[lL]ife') and not(matches(.,'^Reviewed preprint v\d$') or .=$template-descriptions)" 
        role="error" 
        id="event-desc-content-2"><name/> that's a child of an event with an eLife DOI must contain the text 'Reviewed preprint v0' (or in older content one of the folllowing template descriptions: '<value-of select="string-join($template-descriptions,'; ')"/>'). This one has '<value-of select="."/>'.</report>
    ...
    <rule context="abstract[not(@abstract-type)]/sec" id="abstract-sec-titles">
      ...
      <report see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#pre-clintrial-conformance-9"
      ...
      <report see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#final-clintrial-conformance-9"
        test="starts-with(lower-case(title),'clinical trial number') and not(descendant::related-object[@document-id-type='clinical-trial-number'])" 
        role="error" 
    ...
    <rule context="abstract[not(@abstract-type)]/sec/title" id="abstract-sec-title-content">
      ...
      <report see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#struct-abs-title-1" 
        test="*"
        role="error" 
    ...
    <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h5tdf-aff-test-1" 
    ...
    <rule context="article-meta/contrib-group[not(@*)]//aff" id="author-aff-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hdxya-pre-auth-aff-test-1" 
    	  test="country" 
        role="warning" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hdxya-final-auth-aff-test-1" 
    	  test="country" 
        role="error" 
        id="final-auth-aff-test-1">Author affiliations must have a country. This one does not - <value-of select="$display"/>.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h7l30-pre-auth-aff-test-2"
        test="addr-line[named-content[@content-type='city']]" 
        role="warning" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hzqn1-final-auth-aff-test-2"
        test="addr-line[named-content[@content-type='city']]" 
        role="error" 
        id="final-auth-aff-test-2">Author affiliations must have a city. This one does not - <value-of select="$display"/></assert>
      
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h2v3t-pre-auth-aff-test-3"
        test="institution[not(@*)] or institution-wrap[institution[not(@*)]]" 
        role="warning" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h60a2-final-auth-aff-test-3"
        test="institution[not(@*)] or institution-wrap[institution[not(@*)]]" 
        role="error" 
    ...
    <rule context="aff//institution-wrap" id="aff-institution-wrap-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hx5tv-aff-institution-wrap-test-1"
        test="institution-id and institution[not(@*)]" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h07qo-aff-institution-wrap-test-2"
        test="parent::aff" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h07qo-aff-institution-wrap-test-3"
        test="text()" 
        role="error" 
        id="aff-institution-wrap-test-3">institution-wrap cannot contain text. It can only contain elements.</report>
      
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hjf05-aff-institution-wrap-test-4"
        test="count(institution-id) = 1" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hfnxv-aff-institution-wrap-test-5"
        test="count(institution[not(@*)]) = 1" 
        role="error" 
    ...
    <rule context="aff//institution-id" id="aff-institution-id-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h9fxi-aff-institution-id-test-1"
        test="@institution-id-type='ror'" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h6qq5-aff-institution-id-test-3"
        test="*" 
        role="error" 
    ...
    <rule context="aff" id="gen-aff-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hxnmu-gen-aff-test-1"
        test="count(institution[not(@*)]) + count(institution-wrap/institution[not(@*)]) gt 1" 
        role="error" 
        id="gen-aff-test-1">Affiliations cannot have more than 1 top level institutions. <value-of select="$display"/> has <value-of select="count(institution[not(@*)]) + count(institution-wrap/institution[not(@*)])"/>.</report>
    
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hg0km-gen-aff-test-2" 
        test="count(institution[@content-type='dept']) + count(institution-wrap/institution[@content-type='dept']) ge 1" 
        role="warning" 
        id="gen-aff-test-2">Affiliation has <value-of select="count(institution[@content-type='dept']) + count(institution-wrap/institution[@content-type='dept'])"/> department field(s) - <value-of select="$display"/>. Is this correct?</report>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hnan2-gen-aff-test-3"
        test="count(label) gt 1" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h2vgk-gen-aff-test-4" 
        test="count(addr-line) gt 1" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hvgj9-gen-aff-test-5" 
        test="count(country) gt 1" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hf1l4-gen-aff-test-6"
        test="text()" 
        role="error" 
    ...
    <rule context="aff/*" id="aff-child-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hrsm6-aff-child-conformity"
    ...
    <rule context="aff[institution-wrap/institution-id[@institution-id-type='ror']]" id="aff-ror-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hentg-aff-ror"
        test="exists($matching-ror)"
        role="warning" 
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#htfjl-aff-ror-name"
        test="exists($matching-ror) and not(contains(institution-wrap[1]/institution[1],$matching-ror/*:name))"
        role="warning" 
      ...
      <report test="$ror='https://ror.org/05t99sp05' and matches(institution-wrap[1]/institution[1]/lower-case(.),'univ(\.|ersity) of california')"
        role="error" 
        id="aff-ror-cal-coast">Affiliation has the ROR id for California Coast University (https://ror.org/05t99sp05), but the institution name contains 'University of California' (or similar) - <value-of select="institution-wrap[1]/institution[1]"/>. The ROR id is incorrect.</report>
    </rule>
    
    <rule context="addr-line" id="addr-line-parent-test">
      
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hjjnh-addr-line-parent"
        test="parent::aff" 
        role="error" 
    ...
    <rule context="addr-line/*" id="addr-line-child-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h1gbm-addr-line-child-1"
        test="name()='named-content'" 
        role="error" 
        id="addr-line-child-1"><value-of select="name()"/> is not allowed as a child of &lt;addr-line&gt;.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hoyaj-addr-line-child-2"
        test="(name()='named-content') and not(@content-type='city')" 
        role="error" 
    ...
    <rule context="article-meta/funding-group[descendant::institution[matches(lower-case(.),'wellcome') and not(matches(lower-case(.),'burroughs'))]]/funding-statement" id="wellcome-fund-statement-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/funding-3sv64358#wellcome-fund-statement"
        test="matches(lower-case(.),'for the purpose of open access, the authors have applied a cc by public copyright license to any author accepted manuscript version arising from this submission\.')" 
        role="warning" 
    ...
    <rule context="article-meta/funding-group/funding-statement[not(contains(lower-case(.),'open access funding provided by max planck society'))]" id="max-planck-fund-statement-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/funding-3sv64358#max-planck-fund-statement"
        test="some $aff in ($nested-affs,$group-affs) satisfies matches(lower-case($aff),'^max[\p{Zs}-]+plan[ck]+|\p{Zs}max[\p{Zs}-]+plan[ck]+')"
        role="warning" 
        id="max-planck-fund-statement">This article has a corresponding author that is affiliated with a Max Planck Institute, but the funding statement does not contain the text 'Open access funding provided by Max Planck Society.' Should it? The funding statement currently reads: <value-of select="."/>.</report>
	...
	<rule context="funding-group/award-group" id="award-group-tests">
	  ...
	  <let name="institution" value="funding-source[1]/institution-wrap[1]/institution[1]"/>
		...
		<assert see="https://elifeproduction.slab.com/posts/funding-3sv64358#award-group-test-3" 
        test="principal-award-recipient" 
        role="warning" 
        id="award-group-test-3">award-group should almost always contain a principal-award-recipient. If it is not clear which author(s) are associated with this funding, please query with the authors, and only leave it without an author if appropriate.</assert>
    ...
    <rule context="article//award-group//institution-wrap/institution-id" id="institution-id-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/funding-3sv64358#institution-id-test-5" 
        test="@institution-id-type='FundRef' and (normalize-space(.) != '') and not(matches(.,'^http[s]?://d?x?\.?doi.org/10.13039/\d*$'))" 
        role="error" 
        id="institution-id-test-5"><name/> element in funding entry for <value-of select="parent::institution-wrap/institution"/> has the institution-id-type 'fundref' and contains the following text - <value-of select="."/> - which is not a fundref doi.</report>
    ...
    <rule context="article//award-group//institution-wrap/institution-id[@institution-id-type='doi']" id="institution-id-doi-tests">
      ...
      <assert test="@vocab='open-funder-registry'" 
        role="error" 
        id="institution-id-test-6"><name/> in funding must have a vocab="open-funder-registry" attribute. This one does not.</assert>
      
      <assert test="@vocab-identifier='10.13039/open-funder-registry'" 
        role="error" 
        id="institution-id-test-7"><name/> in funding must have a vocab-identifier="10.13039/open-funder-registry" attribute. This one does not.</assert>
    ...
    <rule context="funding-group//principal-award-recipient" id="par-tests">
      <let name="authors" value="for $x in ancestor::article//article-meta/contrib-group[1]/contrib[@contrib-type='author']
        return if ($x/name) then e:get-name($x/name[1])
        else if ($x/collab or $x/collab-name) then e:get-collab($x/*[name()=('collab','collab-name')][1])
        else ''"/>
      <let name="par-text" value="if (name) then e:get-name(name[1])
        ...
        else e:get-collab(*[name()=('collab','collab-name')][1])"/>
      ...
      <assert test="$par-text = $authors"
        role="error" 
        id="par-test-2">Author name in funding section (<value-of select="$par-text"/>) does not match any of the author names in the author list: <value-of select="string-join($authors,', ')"/>.</assert>
    ...
    <rule context="funding-group//principal-award-recipient[count(name) gt 1]" id="multi-par-tests">
      ...
      <assert test="empty($indistinct-names)"
        role="error" 
        id="multi-par-test-1">Funding entry from <value-of select="ancestor::award-group[1]/funding-source[1]/descendant::institution[1]"/> has duplicate author names associated - <value-of select="string-join($indistinct-names,';')"/>.</assert>
    ...
    <rule context="article-meta/custom-meta-group" id="custom-meta-group-tests">
      ...
      <report test="($type = $research-subj) and not(count(custom-meta[@specific-use='meta-only'])=(1,2))" 
        role="error" 
    ...
    <rule context="article-meta/custom-meta-group/custom-meta" id="custom-meta-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/abstracts-digests-and-impact-statements-tiau2k6x#custom-meta-test-2" 
        test="($type = ($research-subj,$notice-article-types)) and not(meta-name = ('Author impact statement','publishing-route'))" 
        role="error" 
        id="custom-meta-test-2">The value of meta-name can only be 'Author impact statement' or 'publishing-route'. Currently it is <value-of select="meta-name"/>.</report>
      
      <report test="($type = ($research-subj,$notice-article-types)) and ($pos=2) and  (meta-name != 'publishing-route')" 
        role="error" 
        id="custom-meta-test-17">The value of the 2nd meta-name can only be 'publishing-route'. Currently it is <value-of select="meta-name"/>.</report>
      ...
      <report test="($type = $features-subj) and ($pos=3) and  (meta-name != 'publishing-route')" 
        role="error" 
        id="custom-meta-test-18">The value of the 3rd meta-name can only be 'publishing-route'. Currently it is <value-of select="meta-name"/>.</report>
    ...
    <rule context="article-meta/custom-meta-group/custom-meta[meta-name='schema-version']/meta-value" id="schema-value-tests">
      ...
      <assert test=".='2.0.0'" 
        role="error" 
        id="schema-custom-meta-test-1">The meta-value element for schema-version must have a value of '2.0.0'.</assert>
  ...
  <pattern id="content-containers">
    ...
    <rule context="xref" id="xref-target-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hkftz-aff-xref-target-test"
        test="(@ref-type='aff') and ($target/local-name() != 'aff')" 
        role="error" 
    ...
    <report test="not(ancestor::fig/permissions[contains(.,'phylopic')]) and matches(@xlink:href,'phylopic\.org')" 
      ...
      <report test="matches(@xlink:href,'^mailto:')" 
        role="error" 
        id="ext-link-mailto-check">Embedded URL within text starts with the mailto prefix - <value-of select="if (not(. = (@xlink:href,replace(@xlink:href,'^mailto:','')))) then concat(@xlink:href,' embedded in ',.) else ."/>. This will fail ingestion at PMC. Remove this link (the text can remain).</report>
    </rule>
    ...
    <rule context="media" id="media-tests">
      ...
      <report test="if ($file='octet-stream') then ()
        ...
        else if ($file = 'gif') then not(matches(@xlink:href,'\.mp4$|\.gif$'))
    ...
    <rule context="graphic[@xlink:href]|media[@xlink:href]" id="file-extension-tests">
      ...
      <assert test="matches(@xlink:href,'\.[a-z0-9]+$')" 
        role="error"
        id="file-extension-conformance">The file extenstion for a file must be in lower case. This <name/> element has an xlink:href which does not end with a lowercase file extension (<value-of select="tokenize(@xlink:href,'\.')[last()]"/> in <value-of select="@xlink:href"/>).</assert>
      
    </rule>
    
    <rule context="media[child::label]" id="video-test">
      
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#pre-video-title"
        test="caption/title" 
        role="warning" 
        id="pre-video-title"><value-of select="replace(label,'\.$,','')"/> does not have a title. Please query the authors for one.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-video-title"
        test="not(ancestor::sub-article) and not(caption/title)" 
        role="error" 
        id="final-video-title"><value-of select="replace(label,'\.$,','')"/> does not have a title, which is incorrect.</report>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-video-title-sa"
        test="ancestor::sub-article and not(caption/title)" 
        role="warning" 
    ...
    <rule context="supplementary-material" id="supplementary-material-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/additional-files-60jpvalx#supplementary-material-test-8"
        test="matches(label[1],'^Inclusion in global research form$|^MDAR checklist$|^Transparent reporting form$|^Supplementary file \d{1,4}\.$|^Source data \d{1,4}\.$|^Source code \d{1,4}\.$|^Reporting standard \d{1,4}\.$') and not(ancestor::sec[@sec-type='supplementary-material'])" 
        role="error" 
    ...
    <rule context="disp-formula" id="disp-formula-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#disp-formula-test-4"
        ...
        role="warning" 
        id="disp-formula-test-4">disp-formula should very rarely be placed as the first child of a p element with no content before it (ie. &lt;p>&lt;disp-formula ...). Either capture it at the end of the previous paragraph or capture it as a child of <value-of select="parent::p/parent::*/local-name()"/></report>
    </rule>
    ...
    <rule context="mml:math" id="math-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#math-test-19"
        ...
        role="warning" 
        id="math-test-19">Maths containing '<value-of select="."/>' has what looks like words or terms which need separating with a space. With it's current markup the space will not be preserved on the eLife website. Please add in the space(s) using the latex '\;' in the appropriate place(s), so that the space is preserved in the HTML.</report>
      
      <report test="matches(.,'\p{Zs}\p{Zs}\p{Zs}\p{Zs}+$') and not(matches(.,'\s\s\s\s+$'))" 
        role="error" 
        id="math-test-20"><value-of select="ancestor::*[name()=('inline-formula','disp-formula')][1]/name()"/> ends with 4 or more spaces. These types of spaces may cause the equation to break over numerous lines in the HTML or shift the equation to the left. Please ensure they are removed.</report>
      
      <report test="matches(.,'^\p{Zs}\p{Zs}\p{Zs}\p{Zs}+') and not(matches(.,'^\s\s\s\s+'))" 
        role="error" 
        id="math-test-21"><value-of select="ancestor::*[name()=('inline-formula','disp-formula')][1]/name()"/> starts with 4 or more spaces. These types of spaces may cause the equation to break over numerous lines in the HTML or shift the equation to the right. Please ensure they are removed.</report>
    ...
    <rule context="tex-math" id="tex-math-tests">
      ...
      <report test="ancestor::disp-formula and not(contains($formula-text,'\displaystyle'))" 
        role="warning" 
        id="tex-math-test-6"><name/> element in a disp-formula does not contain the \displaystyle command. Is that correct? <value-of select="$formula-text"/></report>
      ...
      <report test="exists($arrays-without-spaces)" 
        role="warning" 
        id="tex-math-test-8"><name/> contains array(s) without horizontal or vertical spacing - <value-of select="string-join($arrays-without-spaces,' ---- ')"/></report>
    </rule>
    ...
    <rule context="table-wrap[contains(@id,'keyresource')]" id="kr-table-tests">
      ...
      <report test="following::table-wrap[contains(@id,'keyresource') or contains(lower-case(label[1]),'key resources table')]" 
        role="warning" 
        id="pre-duplicate-kr-table-1">There appears to be more than one key resources table. Exeter please check with Production on how to proceed.</report>
      
      <report test="following::table-wrap[contains(@id,'keyresource') or contains(lower-case(label[1]),'key resources table')]" 
        role="error" 
        id="final-duplicate-kr-table-1">There is more than one key resources table, which is not permitted.</report>
      
      <assert test="table/thead" 
        role="error" 
        id="kr-table-head-presence">Key resources table must have a header (thead). This one does not have a header.</assert>
    ...
    <rule context="list-item" id="list-item-tests">
      ...
      <report test="($type='order') and matches(.,'^\p{Zs}?\d+')" 
        role="warning" 
    ...
    <rule context="media[starts-with(@mimetype,'video')][matches(@id,'^video[0-9]{1,3}$')]" id="general-video">
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#pre-video-cite"
        test="$xrefs//*:match" 
        role="warning" 
        id="pre-video-cite">There is no citation to <value-of select="$label"/>. Ensure to query the author asking for a citation.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-video-cite"
        test="$xrefs//*:match" 
        role="warning" 
        id="final-video-cite">There is no citation to <value-of select="$label"/>. Ensure this is added.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#pre-video-placement-1"
        test="($xrefs//*:match) and ($sec-id != $sec1/@id)" 
        role="warning" 
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-video-placement-1"
        test="($xrefs//*:match) and ($sec-id != $sec1/@id)" 
        role="error"
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-placement-2"
    ...
    <rule context="article[not(@article-type) or @article-type!='correction']//disp-formula/label" id="equation-label-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/maths-0gfptlyl#equation-label-conformance-1" 
        ...
        role="warning" 
        id="equation-label-conformance-1">Duplicated display formula labels - <value-of select="."/> is present more than once in the same appendix. Is that correct?</report>
    ...
    <rule context="aff/label" id="aff-label-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hw7yy-aff-label-conformance-1"
        test="some $x in preceding::aff/label satisfies (replace($x,'\p{P}','') = $label-2)" 
        role="error" 
    ...
    <rule context="disp-quote" id="disp-quote-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/general-layout-and-formatting-wq0m31at#hq129-disp-quote-test-2" 
        test="not(ancestor::sub-article) and ($subj=($research-subj,$notice-article-types))" 
        role="error"
    ...
    <rule context="attrib" id="attrib-tests">
      ...
      <assert test="$parent=$allowed-parents" 
        role="error" 
        id="attrib-parent">attrib is a child of <value-of select="$parent"/>, which is not allowed. It can only be a child of the following elements: <value-of select="string-join($allowed-parents,'; ')"/></assert>
      
      <report test="preceding-sibling::attrib" 
        role="warning" 
        id="attrib-sibling">attrib has a preceding sibling. Does the <value-of select="$parent"/> really need more than one attrib?</report>
      
      <assert test="* or normalize-space(.)!=''" 
        role="error" 
        id="attrib-content">attrib cannot be empty.</assert>
    ...
    <rule context="attrib/*" id="attrib-child-tests">
      ...
      <assert test="name()=$allowed-children" 
        role="error" 
        id="attrib-child"><value-of select="name()"/> is not permitted as a child of attrib. Only the following elements are: <value-of select="string-join($allowed-children,'; ')"/></assert>
  ...
  <pattern id="video-tests">
    ...
    <rule context="article[not(@article-type = $notice-article-types)]/body//media[starts-with(@mimetype,'video')]" id="body-video-specific">
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-body-video-position-test-1"
        test="not(ancestor::fig-group) and (matches(label[1],'[Vv]ideo')) and ($no != string($pos))" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#fig-video-label-test"
        test="starts-with(label[1],$fig-label)" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#fig-video-position-test"
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#fig-video-check-1"
    ...
    <rule context="app//media[starts-with(@mimetype,'video') and not(ancestor::fig-group)]" id="app-video-specific">
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-app-video-position-test"
        test="$no = string($pos)" 
        role="error" 
    ...
    <rule context="app//fig-group//media[starts-with(@mimetype,'video')]" id="app-fig-video-specific">
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-app-fig-video-position-test"
        test="$no = string($pos)" 
        role="error" 
    ...
    <rule context="fig-group/media[starts-with(@mimetype,'video')]" id="fig-video-specific">
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#fig-video-position-test-2"
        test="not($is-explainer) and following-sibling::fig" 
        role="error" 
    ...
    <rule context="sub-article[@article-type=('decision-letter','referee-report')]/body//media[starts-with(@mimetype,'video')]" id="dl-video-specific">
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#"
        ...
        role="warning" 
        flag="dl-ar"
        id="pre-dl-video-position-test"><value-of select="label"/> does not appear in sequence which is likely incorrect. Relative to the other DL videos it is placed in position <value-of select="$pos"/>.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-dl-video-position-test"
        test="$no = string($pos)" 
        role="error" 
    ...
    <rule context="sub-article[@article-type=('reply','author-comment')]/body//media[starts-with(@mimetype,'video')]" id="ar-video-specific">
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#final-ar-video-position-test"
        test="$no = string($pos)" 
        role="error" 
    ...
    <rule context="media/label[matches(lower-case(.),'^video \d+\.$')]" id="video-labels">
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-label-1"
        test="$number != 1 and (not(preceding::media[matches(lower-case(*:label[1]),'^video \d+\.$')]) or (number(preceding::media[matches(lower-case(*:label[1]),'^video \d+\.$')][1]/label/replace(.,'[^\d]','')) != ($number - 1)))"
        role="error" 
        id="video-label-1">Video has the label '<value-of select="."/>', but there is no preceding video with the label number <value-of select="$number - 1"/>. Either they are not correctly ordered, or the label numbering is incorrect.</report>
    ...
    <rule context="media/label[matches(lower-case(.),'^figure \d+—video \d+\.$')]" id="fig-video-labels">
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#fig-video-label-1"
        test="$number != 1 and (not(parent::media/preceding-sibling::media/label[contains(.,concat($figure-string,'—video'))]) or (number(parent::media/preceding-sibling::media[label[contains(.,concat($figure-string,'—video'))]][1]/label/replace(substring-after(.,'—'),'[^\d]','')) != ($number - 1)))"
        role="error" 
        id="fig-video-label-1">Video has the label '<value-of select="."/>', but there is no preceding video with the label number <value-of select="concat($figure-string,'—video ',string($number - 1))"/>. Either they are not correctly ordered, or the label numbering is incorrect.</report>
    ...
    <rule context="media/label[matches(lower-case(.),'^animation \d+\.$')]" id="animation-labels">
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#animation-label-1"
        test="$number != 1 and (not(preceding::media[matches(lower-case(*:label[1]),'^animation \d+\.$')]) or (number(preceding::media[matches(lower-case(*:label[1]),'^animation \d+\.$')][1]/label/replace(.,'[^\d]','')) != ($number - 1)))"
        role="error" 
        id="animation-label-1">Animation has the label '<value-of select="."/>', but there is no preceding animation with the label number <value-of select="$number - 1"/>. Either they are not correctly ordered, or the label numbering is incorrect.</report>
    ...
    <rule context="media/label[matches(lower-case(.),'^figure \d+—animation \d+\.$')]" id="fig-animation-labels">
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#fig-animation-label-1"
        test="$number != 1 and (not(parent::media/preceding-sibling::media/label[contains(.,concat($figure-string,'—animation'))]) or (number(parent::media/preceding-sibling::media[label[contains(.,concat($figure-string,'—animation'))]][1]/label/replace(substring-after(.,'—'),'[^\d]','')) != ($number - 1)))"
        role="error" 
        id="fig-animation-label-1">Animation has the label '<value-of select="."/>', but there is no preceding animation with the label number <value-of select="concat($figure-string,'—animation ',string($number - 1))"/>. Either they are not correctly ordered, or the label numbering is incorrect.</report>
  ...
  <pattern id="further-fig-tests">
    ...
    <rule context="article/body//fig[not(@specific-use='child-fig')][not(ancestor::boxed-text)]" id="fig-specific-tests">
      ...
      <report test="not($article-type = $notice-article-types) and ancestor::article//xref[(ancestor::caption or ancestor::table-wrap) and @rid = $id] and not(ancestor::article//xref[(@rid = $id) and not(ancestor::caption) and not(ancestor::table-wrap)])" 
        role="warning"
    ...
    <rule context="article//app//fig[not(@specific-use='child-fig')]" id="app-fig-pos-tests"> 
      ...
      <report test="if ($count = 0) then ()
        else if (not(matches($id,'^app[0-9]{1,3}fig[0-9]{1,3}$'))) then ()
        else $no != string($pos)" 
        role="warning" 
        id="pre-app-fig-pos-test"><value-of select="replace(label[1],'\.','')"/> does not appear in sequence. Relative to the other figures in the same appendix it is placed in position <value-of select="$pos"/>. Please query this with the author.</report>
      
      <report test="if ($count = 0) then ()
        else if (not(matches($id,'^app[0-9]{1,3}fig[0-9]{1,3}$'))) then ()
        else $no != string($pos)" 
        role="error" 
        id="final-app-fig-pos-test"><value-of select="replace(label[1],'\.','')"/> does not appear in sequence which is incorrect. Relative to the other figures in the same appendix it is placed in position <value-of select="$pos"/>.</report>
    ...
    <rule context="permissions[not(parent::article-meta)]//ali:license_ref" id="permissions-3a">
      ...
      <report test="replace(.,'^https?|/$','')=replace($article-license,'^https?|/$','')" 
        role="error" 
        id="block-permish-ali-license">ali:license_ref in permissions for <value-of select="$label"/> is the same as the license link for the article - <value-of select="."/> - which is incorrect.</report>
      
      <assert test="contains(.,'creativecommons.org')" 
        role="error" 
        id="block-permish-ali-license-2">ali:license_ref should only contain creativecommons.org licenses. This one in permissions for <value-of select="$label"/> is not a creative commons link - <value-of select="."/> - which is incorrect.</assert>
    ...
    <rule context="permissions[not(parent::article-meta)]//license-p//ext-link" id="permissions-3b">
      ...
      <report test=".=$article-license or @xlink:href=$article-license" 
        role="error" 
        id="block-permish-license-p-link">ext-link in license text in permissions for <value-of select="$label"/> is the same as the license link for the article - <value-of select="$article-license"/> - which is incorrect.</report>
    ...
    <rule context="permissions[not(parent::article-meta)]//license" id="permissions-3c">
      ...
      <report test="@xlink:href=$article-license" 
        role="error" 
        id="block-permish-license-link">license link (the xlink:href attribute value on the license element) in permissions for <value-of select="$label"/> is the same as the license link for the article - <value-of select="$article-license"/> - which is incorrect.</report>
    ...
    <rule context="fig/caption/p[not(child::supplementary-material)]" id="fig-caption-tests">
      ...
      <let name="text-tokens" value="for $x in tokenize($no-panels,'\. ') return
        ...
        else if (matches($x,'^\p{Zs}{1,3}?[a-z]')) then $x
        else ()"/>
      ...
      <report see="https://elifeproduction.slab.com/posts/figures-and-figure-supplements-8gb4whlr#h7hrp-fig-caption-test-3" 
        test="$panel-list//*:item" 
        role="warning" 
  ...
  <pattern id="body">
    ...
    <rule context="article[@article-type='research-article' and descendant::article-meta[not(//subj-group[@subj-group-type='display-channel']/subject[lower-case(.)='feature article']) and //subj-group[@subj-group-type='heading']/subject[.=('Medicine','Epidemiology and Global Health')] and history/date[@date-type='received']/@iso-8601-date gt '2021-04-05']]/body/sec" 
  ...
  <pattern id="title-conformance">
    ...
    <rule context="media/caption/title" id="video-title-tests"> 
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-1"
        test="matches(.,'^\([A-Za-z]|^[A-Za-z]\)')" 
        role="warning" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-2"
        test="matches(.,'\.$|\?$')" 
        role="error" 
        id="video-title-test-2">title for <value-of select="$label"/> must end with a full stop.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-3"
        test="matches(.,' vs\.$')" 
        role="warning" 
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-4"
        test="matches(.,'^\p{Zs}')" 
        role="error" 
        id="video-title-test-4">title for <value-of select="$label"/> begins with a space, which is not allowed.</report>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-7"
        test="string-length(.) gt 250" 
        role="warning" 
        id="video-title-test-7">title for <value-of select="$label"/> is longer than 250 characters. Is it a caption instead?</report>
      
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-8"
        test="$sentence-count gt 1" 
        role="warning" 
      ...
      <report see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-title-test-9"
        test="matches(.,'\p{Zs}$')" 
        role="error" 
    ...
    <rule context="ack" id="ack-title-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/funding-3sv64358#ack-funding"
        test="p[* or not(normalize-space(.)='')]" 
        role="warning" 
    ...
    <rule context="fn-group" id="fn-group-tests">
      ...
      <assert test="@content-type" 
        role="error" 
        id="fn-content-type-1">fn-group that is descendant of back must have a content-type attribute.</assert>
      
      <report test="@content-type and not(@content-type=$allowed-content-types)" 
        role="error" 
        id="fn-content-type-2">fn-group with content-type '<value-of select="@content-type"/>' is not permitted. The only permitted fn-group types are <value-of select="string-join($allowed-content-types,'; ')"/>.</report>
      
      <assert test="parent::back or parent::sec[@sec-type='additional-information' and parent::back]" 
        role="error" 
        id="fn-parent">fn-group is only allowed as a child of back or as a child of sec[@sec-type='additional-information'] (which in turn is in back). This one is placed as a child of <value-of select="parent::*/name()"/></assert>
    ...
    <rule context="sub-article[@article-type='decision-letter']/front-stub/title-group" id="dec-letter-title-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-title-test" 
    ...
    <rule context="sub-article[@article-type=('reply','author-comment')]/front-stub/title-group" id="reply-title-tests">
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#reply-title-test" 
  ...
  <pattern id="id-conformance">
    ...
    <rule context="funding-group/award-group" id="award-group-ids">
      ...
      <assert see="https://elifeproduction.slab.com/posts/funding-3sv64358#award-group-test-1"
    ...
    <rule context="article/body//fig-group/media[starts-with(@mimetype,'video') and not(ancestor::boxed-text)]" id="video-sup-ids">
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-sup-id-test-1"
        test="matches(@id,'^fig[0-9]{1,3}video[0-9]{1,3}$')" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-sup-id-test-2"
        test="starts-with(@id,$id-prefix)" 
        role="error" 
    ...
    <rule context="article/back//app//media[starts-with(@mimetype,'video') and not(parent::fig-group)]" id="app-video-ids">
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#app-video-id-test-1"
        test="matches(@id,'^app[0-9]{1,3}video[0-9]{1,3}$')" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#app-video-id-test-2"
        test="starts-with(@id,concat('app',$id-prefix))" 
        role="error" 
    ...
    <rule context="article/back//app//media[starts-with(@mimetype,'video') and (parent::fig-group)]" id="app-video-sup-ids">
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#app-video-sup-id-test-1"
        test="matches(@id,'^app[0-9]{1,3}fig[0-9]{1,3}video[0-9]{1,3}$')" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#app-video-sup-id-test-2"
        test="starts-with(@id,concat('app',$id-prefix-1))" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#app-video-sup-id-test-3"
        test="starts-with(@id,$id-prefix-2)" 
        role="error" 
    ...
    <rule context="article/body//boxed-text//media[starts-with(@mimetype,'video')]" id="box-vid-ids">
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#box-vid-id-1"
        test="matches(@id,'^box[0-9]{1,3}video[0-9]{1,3}$')" 
        role="error" 
        id="box-vid-id-1">video must have @id in the format box0video0.  <value-of select="@id"/> does not conform to this.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#box-vid-id-2"
        test="starts-with(@id,$box-id)" 
        role="error" 
    ...
    <rule context="aff[not(parent::contrib)]" id="aff-ids">
      ...
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h2lcw-aff-id-test"
        test="if (label) then @id = concat('aff',label[1])
    ...
    <rule context="sub-article//table-wrap" id="resp-table-wrap-ids">
      ...
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#resp-table-wrap-id-test" 
        ...
        role="warning" 
        flag="dl-ar"
        id="resp-table-wrap-id-test">table-wrap @id in a sub-article must be in the format 'resptable0' or 'sa0table0' if it has a label, or in the format 'respinlinetable0' or 'sa0inlinetable0' if it does not.</assert>
    </rule>
    ...
    <rule context="supplementary-material[matches(label[1],'^Inclusion in global research form$')]" 
      id="igrf-ids">
      
      <assert test="matches(@id,'^igrf$')" 
        role="error" 
        id="igrf-id">The id (<value-of select="@id"/>) for <value-of select="replace(label,'\.$','')"/> is not in the correct format. It must be 'igrf'.</assert>
  ...
  <pattern id="back">
    ...
    <rule context="back" id="back-tests">
      ...
      <report test="if ($version='1') then ($article-type = 'research-article') and (count(sec[@sec-type='additional-information']/fn-group[@content-type='author-contribution']) != 1)
        else ()" 
        role="error" 
    ...
    <rule context="sec[@sec-type='supplementary-material']" id="additional-files-tests">
      ...
      <report test="descendant::supplementary-material[matches(lower-case(label[1]),'transparent reporting form')] and descendant::supplementary-material[matches(lower-case(label[1]),'mdar checklist')]" 
        role="error" 
        id="add-files-4">This article has both a transparent reporting form and an MDAR checklist - there should only be one. Please check with the eLife team who will decide which should be retained.</report>
      
    </rule>
    
    <rule context="article[@article-type='research-article']" id="trf-presence">
      
      <assert test="descendant::supplementary-material[matches(lower-case(label[1]),'transparent reporting form|mdar checklist')]" 
        role="warning" 
        id="add-files-2">This article does not have a transparent reporting form or MDAR checklist. Is that correct?</assert>
  ...
  <pattern id="dec-letter-auth-response">
    ...
    <rule context="article/sub-article" id="dec-letter-reply-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-reply-test-1" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-reply-test-2" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-reply-test-3" 
        test="count(front-stub) = 1" 
        role="error" 
        flag="dl-ar"
        id="dec-letter-reply-test-3">sub-article must contain one and only one front-stub.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-reply-test-4" 
        test="count(body) = 1" 
        role="error" 
        flag="dl-ar"
    ...
    <rule context="article/sub-article//p" id="dec-letter-reply-content-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-reply-test-5" 
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-reply-test-6" 
    ...
    <rule context="sub-article[@article-type='editor-report']/front-stub" id="ed-eval-front-tests">
      ...
      <report test="e:is-prc(.) and $article-type='review-article' and kwd-group[@kwd-group-type='evidence-strength']" 
        role="error" 
        flag="dl-ar"
      ...
      <report test="e:is-prc(.) and $article-type='review-article' and kwd-group[@kwd-group-type='claim-importance']" 
        role="error" 
        flag="dl-ar"
        id="ed-eval-front-test-5b">eLife Assessment front-stub contains a significance term keyword group, but this is a PRC Review article (which don't use the terms), so this must be incorrect.</report>
    </rule>
    ...
    <rule context="sub-article[@article-type='decision-letter']/front-stub" id="dec-letter-front-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-front-test-1" 
        test="count(article-id[@pub-id-type='doi']) = 1" 
        role="error" 
        flag="dl-ar"
        id="dec-letter-front-test-1">sub-article front-stub must contain article-id[@pub-id-type='doi'].</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-front-test-2" 
        test="$count gt 0" 
        role="error" 
        flag="dl-ar"
        id="dec-letter-front-test-2">decision letter front-stub must contain at least 1 contrib-group element.</assert>
      
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-front-test-3" 
        test="$count gt 2" 
        role="error" 
        flag="dl-ar"
        id="dec-letter-front-test-3">decision letter front-stub contains more than 2 contrib-group elements.</report>
      
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-front-test-4" 
    ...
    <rule context="sub-article[@article-type='decision-letter']/front-stub/contrib-group[1]" id="dec-letter-editor-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-editor-test-1" 
        test="count(contrib[@contrib-type='editor']) = 1" 
        role="warning" 
        flag="dl-ar"
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-editor-test-2" 
    ...
    <rule context="sub-article[@article-type='decision-letter']/front-stub/contrib-group[1]/contrib[@contrib-type='editor']" id="dec-letter-editor-tests-2">
      ...
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-editor-test-3" 
    ...
    <rule context="sub-article[@article-type='decision-letter']/front-stub/contrib-group[2]" id="dec-letter-reviewer-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-reviewer-test-1" 
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-reviewer-test-2" 
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-reviewer-test-6" 
    ...
    <rule context="sub-article[@article-type='decision-letter']/front-stub/contrib-group[2]/contrib[@contrib-type='reviewer']" id="dec-letter-reviewer-tests-2">
      ...
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-reviewer-test-3" 
        test="role='Reviewer'" 
        role="error" 
        flag="dl-ar"
    ...
    <rule context="sub-article[@article-type='decision-letter']/body" id="dec-letter-body-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-body-test-1" 
    ...
    <rule context="sub-article[@article-type=('decision-letter','referee-report')]/body//p" id="dec-letter-body-p-tests">  
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-body-test-2" 
    ...
    <rule context="sub-article[@article-type='decision-letter']/body/boxed-text[1]" id="dec-letter-box-tests">  
      ...
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-box-test-1" 
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-box-test-2" 
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#dec-letter-box-test-3" 
        test="matches(.,concat($permitted-text-1,'|',$permitted-text-2)) and not(descendant::ext-link[.='the preprint'])" 
        role="error" 
        flag="dl-ar"
        id="dec-letter-box-test-3">At the top of the decision letter, the text 'the preprint' must contain an embedded link to this article's preprint.</report>
    </rule>
    
    <rule context="sub-article[@article-type='decision-letter']" 
      id="decision-missing-table-tests">
      
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#decision-missing-table-test" 
    ...
    <rule context="sub-article[@article-type=('reply','author-comment')]/front-stub" id="reply-front-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#reply-front-test-1" 
        test="count(article-id[@pub-id-type='doi']) = 1" 
        role="error" 
        flag="dl-ar"
    ...
    <rule context="sub-article[@article-type=('reply','author-comment')]/body" id="reply-body-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#reply-body-test-1" 
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#reply-body-test-2" 
        test="count(p) = 0" 
        role="error" 
        flag="dl-ar"
    ...
    <rule context="sub-article[@article-type=('reply','author-comment')]/body//disp-quote" id="reply-disp-quote-tests">
      ...
      <assert see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#reply-disp-quote-test-1" 
    ...
    <rule context="sub-article[@article-type=('reply','author-comment')]/body//p[not(ancestor::disp-quote)]" id="reply-missing-disp-quote-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#reply-missing-disp-quote-test-1" 
    ...
    <rule context="sub-article[@article-type=('reply','author-comment')]//italic[not(ancestor::disp-quote)]" id="reply-missing-disp-quote-tests-2">
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#reply-missing-disp-quote-test-2" 
    ...
    <rule context="sub-article[@article-type=('reply','author-comment')]" 
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#reply-missing-table-test" 
    ...
    <rule context="sub-article//ext-link" 
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#paper-pile-test" 
    ...
    <rule context="sub-article[@article-type='decision-letter']/body//bold" 
      id="sub-article-dl-image-tests">
      
      <report test="matches(.,'[Dd]ecision letter image \d')"
        role="error" 
        flag="dl-ar"
    ...
    <rule context="sub-article[@article-type='referee-report']/body//bold" 
      id="sub-article-pr-image-tests">
      
      <report test="matches(.,'[Rr]eview image \d')"
        role="error" 
        flag="dl-ar"
    ...
    <rule context="sub-article[@article-type=('reply','author-comment')]/body//bold" 
      id="sub-article-ar-image-tests">
      
      <report test="matches(.,'[Aa]uthor response image \d')"
        role="error" 
        flag="dl-ar"
  ...
  <pattern id="sub-article-version-2">
    ...
    <rule context="sub-article[@article-type=('editor-report','referee-report','author-comment')]/front-stub/contrib-group/contrib" 
      ...
      <assert test="name or anonymous or collab or collab-wrap" 
        role="error" 
        flag="dl-ar"
        id="sub-article-contrib-test-2">sub-article contrib must have either a child name or a child anonymous element.</assert>
      
      <report test="(name and anonymous) or ((collab or collab-wrap) and anonymous) or (name and (collab or collab-wrap))" 
        role="error" 
        flag="dl-ar"
    ...
    <rule context="sub-article[@article-type='referee-report']/front-stub[lower-case(title-group[1]/article-title[1])='recommendations for authors']" id="ref-report-editor-tests">
      ...
      <assert test="count(descendant::contrib[role[@specific-use='referee']]) > 0" 
        role="error" 
        flag="dl-ar"
        id="ref-report-reviewer-1">The Recommendations for authors must contain 1 or more reviewers (a contrib with a role[@specific-use='referee']). This one has 0.</assert>
    </rule>
    ...
    <rule context="anonymous" 
      ...
      <report test="* or normalize-space(.)!=''"
        role="error" 
        id="anonymous-test-2">anonymous element cannot contain any elements on text.</report>
      
      <report test="@*"
        role="error" 
        id="anonymous-test-3">anonymous element cannot have any attributes.</report>
    </rule>
    ...
    <rule context="sub-article[e:is-prc(.)]//contrib[role[@specific-use='referee']]" 
      ...
      <report see="https://elifeproduction.slab.com/posts/review-materials-r9uiav3j#prc-reviewer-test-1"
        test="name or collab or collab-wrap"
        role="error" 
        id="prc-reviewer-test-1">A reviewer contrib in a PRC article cannot have a child <value-of select="*[name()=('name','collab','collab-wrap')]/name()"/> element, since all reviewers are captured as anonymous. They must have an anonymous element instead.</report>
  ...
  <pattern id="related-articles">
    ...
    <rule context="article[@article-type='correction']//article-meta" id="correction-test">
      ...
      <assert see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hzu2u-related-articles-test-8"
        test="count(related-article[@related-article-type='corrected-article']) gt 0" 
        role="error" 
    ...
    <rule context="article[@article-type='retraction']//article-meta" id="retraction-test">
      ...
      <assert see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hl977-related-articles-test-9"
        test="count(related-article[@related-article-type='retracted-article']) gt 0" 
        role="error" 
    ...
    <rule context="article[@article-type='expression-of-concern']//article-meta" id="eoc-test">
      ...
      <assert test="count(related-article[@related-article-type='object-of-concern']) gt 0" 
        role="error" 
        id="related-articles-test-13">Expressions of Concern must contain at least 1 related-article link with the attribute related-article-type='object-of-concern'.</assert>
    ...
    <rule context="related-article" id="related-articles-conformance">
      ...
      <assert test="@related-article-type = $allowed-values" 
        role="error" 
        id="related-articles-test-4">@related-article-type must be equal to one of the allowed values, ('article-reference', 'commentary', 'commentary-article', 'corrected-article', 'retracted-article', and 'object-of-concern').</assert>
  ...
  <pattern id="parent-tests">
    ...
    <rule context="media[starts-with(@mimetype,'video')]" id="video-parent-conformance">
      ...
      <assert see="https://elifeproduction.slab.com/posts/videos-m0p9ve8m#video-parent-test"
        test="$parent = ('sec','fig-group','body','boxed-text','app')" 
        role="error" 
  ...
  <pattern id="element-citation-general-tests">
    ...
    <rule context="element-citation/person-group" id="elem-citation-gen-name-3-1">
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#err-elem-cit-gen-name-3-1"
        test=".[not (name or collab or collab-name)]" 
        role="error" 
    ...
    <rule context="ref/element-citation/year" id="elem-citation-year">
      ...
      <assert see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#err-elem-cit-gen-date-1-6"
        ...
        or ancestor::element-citation/person-group[1]/collab-name[1] = $y/ancestor::element-citation/person-group[1]/collab-name[1]
        )))" 
        role="error" 
        id="err-elem-cit-gen-date-1-6">If the &lt;year&gt; element contains the letter 'a' after the digits, there must be another reference with the same first author surname (or collab-name) with a letter "b" after the year. Reference '<value-of select="ancestor::ref/@id"/>' does not fulfill this requirement.</assert>
    ...
    <rule context="ref/element-citation//collab | ref/element-citation//collab-name" id="collab-content">
      ...
      <report test="matches(.,'[\[\]\(\)]')" 
        role="warning" 
        id="collab-brackets"><name/> in reference '<value-of select="ancestor::ref/@id"/>' contains brackets - <value-of select="."/>. Are the brackets necessary?</report>
  ...
  <pattern id="element-citation-high-tests">
    ...
    <rule context="ref[preceding-sibling::ref]" id="ref-list-ordering">
      ...
      <assert see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#err-elem-cit-high-2-2"
        test="($order-value gt $preceding-ref-order-value)" 
        role="error" 
  ...
  <pattern id="element-citation-data-tests">
    ...
    <rule context="article[e:get-version(.)!='1']//element-citation[@publication-type='data']" id="elem-citation-data-v2">
      ...
      <assert  test="@specific-use=('generated','analyzed')" 
        role="error" 
        flag="version-2"
        id="err-elem-cit-data-1-v2">element-citation[@publication-type='data'] must have a specific-use attribute is either 'generated' or 'analyzed'. Reference '<value-of select="ancestor::ref/@id"/>' does not have this attribute or the value is incorrect.</assert>
  ...
  <pattern id="element-citation-patent-tests">
    <rule context="element-citation[@publication-type='patent']" id="elem-citation-patent">
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#pre-err-elem-cit-patent-2-1"
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#final-err-elem-cit-patent-2-1"
        test="count(person-group[@person-group-type='inventor'])=1" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-2-3"
        test="every $type in person-group/@person-group-type
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-2A"
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#pre-err-elem-cit-patent-8-1"
        test="count(article-title)=1" 
        role="warning" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#final-err-elem-cit-patent-8-1"
        test="count(article-title)=1" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#pre-err-elem-cit-patent-9-1"
        test="count(source)=1" 
        role="warning" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#final-err-elem-cit-patent-9-1"
        test="count(source)=1" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#pre-err-elem-cit-patent-10-1-1"
        test="patent" 
        role="warning" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#final-err-elem-cit-patent-10-1-1"
        test="patent" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#pre-err-elem-cit-patent-11-1"
        test="ext-link" 
        role="warning" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#final-err-elem-cit-patent-11-1"
        test="ext-link" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-18"
    ...
    <rule context="element-citation[@publication-type='patent']/article-title" id="elem-citation-patent-article-title"> 
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-8-2-1"
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-8-2-2"
        test="count(*)=count(italic | sub | sup)" 
        role="error" 
    ...
    <rule context="element-citation[@publication-type='patent']/source" id="elem-citation-patent-source"> 
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-9-2-2"
        test="count(*)=count(italic | sub | sup)" 
        role="error" 
    ...
    <rule context="element-citation[@publication-type='patent']/patent" id="elem-citation-patent-patent"> 
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-10-1-2"
        test="count(*)=0" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/patent-references-qcd31iv5#err-elem-cit-patent-10-2"
        test="some $x in document($countries)/countries/country/@country satisfies ($x=@country)" 
        role="error" 
        id="err-elem-cit-patent-10-2">The &lt;patent&gt; element must have a country attribute, the value of which must be a 2 letter ISO 3166-1 country code. Reference '<value-of select="ancestor::ref/@id"/>' has a patent/@country attribute with the value '<value-of select="@country"/>', which is not in the list.</assert>
  ...
  <pattern id="element-citation-preprint-tests">
    <rule context="element-citation[@publication-type='preprint']" id="elem-citation-preprint">
      ...
      <assert test="count(pub-id)=1 or count(ext-link)=1" 
        role="warning" 
        id="pre-err-elem-cit-preprint-10-3">Either one &lt;pub-id&gt; or one &lt;ext-link&gt; element is required in a preprint reference. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(pub-id)"/> &lt;pub-id&gt; elements and <value-of select="count(ext-link)"/> &lt;ext-link&gt; elements.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/preprint-references-okxjjp9i#err-elem-cit-preprint-10-3" 
        test="count(pub-id)=1 or count(ext-link)=1" 
        role="error" 
        id="final-err-elem-cit-preprint-10-3">Either one &lt;pub-id&gt; or one &lt;ext-link&gt; element is required in a preprint reference. Reference '<value-of select="ancestor::ref/@id"/>' has <value-of select="count(pub-id)"/> &lt;pub-id&gt; elements and <value-of select="count(ext-link)"/> &lt;ext-link&gt; elements.</assert>
  ...
  <pattern id="element-citation-web-tests">
    <rule context="element-citation[@publication-type='web']" id="elem-citation-web">
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-2-1"
        test="count(person-group)=1" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#pre-err-elem-cit-web-8-1"
        test="count(article-title)=1" 
        role="warning" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#final-err-elem-cit-web-8-1"
        test="count(article-title)=1" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-9-1"
        test="count(source) &gt; 1" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-10-1"
        test="count(ext-link)=1" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#pre-err-elem-cit-web-11-1"
        test="count(date-in-citation) lt 1" 
        role="warning" 
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#final-err-elem-cit-web-11-1"
        test="count(date-in-citation) lt 1" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-11-1-1"
        test="count(date-in-citation) gt 1" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-12"
    ...
    <rule context="element-citation[@publication-type='web']/person-group" id="elem-citation-web-person-group"> 
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-2-2"
        test="@person-group-type='author'" 
        role="error" 
    ...
    <rule context="element-citation[@publication-type='web']/article-title" id="elem-citation-web-article-title"> 
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-8-2-1"
        test="./string-length() + sum(*/string-length()) ge 2" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-8-2-2"
        test="count(*)=count(italic | sub | sup)" 
        role="error" 
    ...
    <rule context="element-citation[@publication-type='web']/source" id="elem-citation-web-source"> 
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-9-2-2"
        test="count(*)=count(italic | sub | sup)" 
        role="error" 
    ...
    <rule context="element-citation[@publication-type='web']/date-in-citation" id="elem-citation-web-date-in-citation"> 
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-11-2-1"
        test="./@iso-8601-date" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-11-2-2"
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-11-3"
      ...
      <assert see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-11-5"
        test="(matches(@iso-8601-date,replace($date-regex,'\p{Zs}','')))" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#err-elem-cit-web-11-4"
        test="if (matches(@iso-8601-date,replace($date-regex,'\p{Zs}',''))) then format-date(xs:date(@iso-8601-date), '[MNn] [D], [Y]')!=.
        else ()" 
        role="error" 
  ...
  <pattern id="element-citation-confproc-tests">
    <rule context="element-citation[@publication-type='confproc']" id="elem-citation-confproc"> 
      ...
      <assert see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#hxnwn-err-elem-cit-confproc-2-1" 
        test="count(person-group)=1" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#h2qee-err-elem-cit-confproc-8-1" 
        test="count(article-title)=1" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#hlxi8-err-elem-cit-confproc-10-1" 
        test="count(conf-name)=1" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#hgdyn-err-elem-cit-confproc-12-1" 
      ...
      <report see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#h5jtw-err-elem-cit-confproc-12-2" 
      ...
      <report see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#hjoa6-err-elem-cit-confproc-12-3" 
        test="(lpage and fpage) and (number(replace(fpage[1],'[^\d]','')) ge number(replace(lpage[1],'[^\d]','')))" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#hyk2a-err-elem-cit-confproc-12-4" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#hm9rv-err-elem-cit-confproc-16-1" 
        test="count(pub-id) le 1" 
        role="error" 
      ...
      <assert see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#h08e2-err-elem-cit-confproc-17" 
        test="count(*) = count(person-group | article-title | year | conf-loc | conf-name | lpage |
    ...
    <rule context="element-citation[@publication-type='confproc']/person-group" id="elem-citation-confproc-preson-group">
      <assert see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#hslvg-err-elem-cit-confproc-2-2" 
        test="@person-group-type='author'" 
        role="error" 
    ...
    <rule context="element-citation[@publication-type='confproc']/article-title" id="elem-citation-confproc-article-title">
      ...
      <assert see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#hubhw-err-elem-cit-confproc-8-2" 
        test="count(*) = count(sub|sup|italic)" 
        role="error" 
    ...
    <rule context="element-citation[@publication-type='confproc']/conf-name" id="elem-citation-confproc-conf-name">
      ...
      <assert see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#hwknk-err-elem-cit-confproc-10-2" 
        test="count(*)=0" 
        role="error" 
    ...
    <rule context="element-citation[@publication-type='confproc']/conf-loc" id="elem-citation-confproc-conf-loc">
      ...
      <assert see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#hiuzp-err-elem-cit-confproc-11-2" 
        test="count(*)=0" 
        role="error" 
    ...
    <rule context="element-citation[@publication-type='confproc']/fpage" id="elem-citation-confproc-fpage">
      ...
      <assert see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#hxcbf-err-elem-cit-confproc-12-5" 
        test="matches(normalize-space(.),'^\d.*') or (substring(normalize-space(../lpage[1]),1,1) = substring(normalize-space(.),1,1))" 
        role="error" 
    ...
    <rule context="element-citation[@publication-type='confproc']/pub-id" id="elem-citation-confproc-pub-id">
      ...
      <assert see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#hzrg2-err-elem-cit-confproc-16-2" 
  ...
  <pattern id="das-element-citation-tests">
    <rule context="sec[@sec-type='data-availability']//element-citation[@publication-type='data']" id="gen-das-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/data-availability-qi8vg0qp#pre-das-elem-person-1" 
        test="(count(person-group[@person-group-type='author']/name)=0) and (count(person-group[@person-group-type='author']/collab)=0) and (count(person-group[@person-group-type='author']/collab-name)=0)" 
        role="warning" 
  ...
  <pattern id="pub-id-pattern">
    ...
    <rule context="pub-id[@xlink:href]" 
      id="pub-id-xlink-href-tests">
      
      <assert test="matches(@xlink:href,'^https?:..(www\.)?[-a-zA-Z0-9@:%.,_\+~#=!]{2,256}\.[a-z]{2,6}([-a-zA-Z0-9@:;%,_\\(\)+.~#?!&amp;//=]*)$|^ftp://.')" 
        role="warning" 
        id="pub-id-url-conformance-test">@xlink:href doesn't look like a URL - '<value-of select="@xlink:href"/>'. Is this correct?</assert>
      
      <report test="matches(@xlink:href,'^(ftp|sftp)://\S+:\S+@')" 
        role="warning" 
        id="ftp-credentials-flag-2">@xlink:href contains what looks like a link to an FTP site which contains credentials (username and password) - '<value-of select="@xlink:href"/>'. If the link without credentials works (<value-of select="concat(substring-before(@xlink:href,'://'),'://',substring-after(@xlink:href,'@'))"/>), then please replace it with that and notify the authors that you have done so. If the link without credentials does not work, please query with the authors in order to obtain a link without credentials.</report>
      
      <report test="matches(@xlink:href,'\.$')" 
        role="error" 
        id="pub-id-url-fullstop-report">'<value-of select="@xlink:href"/>' - Link ends in a full stop which is incorrect.</report>
      
      <report test="matches(@xlink:href,'[\p{Zs}]')" 
        role="error" 
        id="pub-id-url-space-report">'<value-of select="@xlink:href"/>' - Link contains a space which is incorrect.</report>
 ...
 <pattern id="features">
   ...
   <rule context="article//article-meta[article-categories//subj-group[@subj-group-type='display-channel']/subject=$features-subj]//contrib[@contrib-type='author']" id="feature-author-tests">
     ...
     <assert see="https://elifeproduction.slab.com/posts/feature-content-alikl8qp#feature-author-test-1"
       test="bio or collab or ancestor::collab or collab-wrap or ancestor::collab-wrap" 
        role="error" 
  ...
  <pattern id="correction-retraction">
    ...
    <rule context="article[@article-type = 'correction']" id="correction-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hy53l-corr-aff-presence" 
        test="descendant::article-meta//aff" 
        role="error" 
        id="corr-aff-presence">Correction notices should not contain affiliations.</report>
      
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hohvo-corr-coi-presence"
      ...
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hrxuv-corr-self-uri-presence"
        test="descendant::self-uri" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hg8b8-corr-back-sec"
      ...
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hnc5o-corr-impact-statement"
      ...
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hs0ll-corr-se-bre"
    ...
    <rule context="article[@article-type = ('retraction','expression-of-concern')]" id="retraction-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#h577w-retr-aff-presence"
        test="descendant::article-meta//aff" 
        role="error" 
        id="retr-aff-presence"><value-of select="$display-subject"/> notices should not contain affiliations.</report>
      
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#heu8l-retr-coi-presence"
        test="descendant::fn-group[@content-type='competing-interest']" 
        role="error" 
        id="retr-COI-presence"><value-of select="$display-subject"/> notices should not contain competing interests.</report>
      
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hmmm1-retr-self-uri-presence"
        test="descendant::self-uri" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hwi7f-retr-back"
        test="back/*" 
        role="error" 
        id="retr-back"><value-of select="$display-subject"/> notices should not contain any content in the back.</report>
      
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hgn3c-retr-impact-statement"
        test="descendant::meta-name[text() = 'Author impact statement']" 
        role="error" 
        id="retr-impact-statement"><value-of select="$display-subject"/> notices should not contain an impact statement.</report>
      
      <report see="https://elifeproduction.slab.com/posts/versioning-li6miptl#hqq70-retr-se-bre"
        test="descendant::contrib-group[@content-type='section']" 
        role="error" 
        id="retr-SE-BRE"><value-of select="$display-subject"/> notices must not contain any Senior or Reviewing Editors.</report>
    ...
    <rule context="article[@article-type = ('retraction','expression-of-concern','correction')]/body" id="notice-body-tests">
      ...
      <assert test="*[1]/name()='boxed-text'" 
        role="error" 
        id="notice-body-test-1">The first child element of body in a <value-of select="$display-subject"/> must be boxed-text. This one has <value-of select="*[1]/name()"/>.</assert>
    ...
    <rule context="article[@article-type = ('retraction','expression-of-concern','correction')]/body/boxed-text[1]" id="notice-box-tests">
      ...
      <assert test="count(p)=2" 
        role="error" 
        id="notice-box-1">The boxed-text at the start of a <value-of select="$display-subject"/> notice should contain 2 paragraphs. The first should contain the citation details for the related article. The second should state the fulld ate on which it was published. This boxed-text has <value-of select="count(p)"/> p elements.</assert>
      
      <report test="*/name()!='p'" 
        role="error" 
        id="notice-box-2"><name/> in boxed-text at the start of a <value-of select="$display-subject"/> notice is not allowed. Only p is permitted.</report>

      <assert test="contains(p[1],$obj-title)" 
        role="error" 
        id="notice-box-3">The first paragraph in boxed text at the start of this <value-of select="$display-subject"/> does not contain the title of the related article as written in the title of the notice. Related article title (from notice title) = '<value-of select="$obj-title"/>'. First boxed-text para = '<value-of select="p[1]"/>'</assert>
      
      <assert test="contains(p[1],'eLife')" 
        role="error" 
        id="notice-box-4">The first paragraph in boxed text at the start of a <value-of select="$display-subject"/> should contain the citation details for the related article, but this one does not contain the word 'eLife'.</assert>
      
      <assert test="matches(lower-case(p[1]),'10.7554/elife.\d{5,6}')" 
        role="error" 
        id="notice-box-5">The first paragraph in boxed text at the start of a <value-of select="$display-subject"/> should contain the citation details for the related article, but this one does not contain an eLife DOI.</assert>
      
      <assert test="matches(p[2],'^Published ([1-9]|[1-2][0-9]|3[0-1]) (January|February|March|April|May|June|July|August|September|October|November|December) 20([1][2-9]|[2][0-9])')" 
        role="error" 
        id="notice-box-6">The second paragraph in boxed text at the start of a <value-of select="$display-subject"/> should contain the full publication for the related article in the format 'Published 13 January 2022', but this one does not.</assert>
    </rule>
  ...
  <pattern id="rrid-org-pattern">
    ...
    <rule context="p|td|th" id="rrid-org-code">
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#hxls5-diabetes-1-test" 
        test="matches(.,'[Tt]ype\p{Zs}?[Oo]ne\p{Zs}?[Dd]iabetes') and not(descendant::p[matches(.,'[Tt]ype\p{Zs}?[Oo]ne\p{Zs}?[Dd]iabetes')]) and not(descendant::td[matches(.,'[Tt]ype\p{Zs}?[Oo]ne\p{Zs}?[Dd]iabetes')]) and not(descendant::th[matches(.,'[Tt]ype\p{Zs}?[Oo]ne\p{Zs}?[Dd]iabetes')])" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#hxls5-diabetes-2-test" 
       test="matches(.,'[Tt]ype\p{Zs}?[Tt]wo\p{Zs}?[Dd]iabetes') and not(descendant::p[matches(.,'[Tt]ype\p{Zs}?[Tt]wo\p{Zs}?[Dd]iabetes')]) and not(descendant::td[matches(.,'[Tt]ype\p{Zs}?[Tt]wo\p{Zs}?[Dd]iabetes')]) and not(descendant::th[matches(.,'[Tt]ype\p{Zs}?[Tt]wo\p{Zs}?[Dd]iabetes')])" 
        role="error" 
      ...
      <report test="matches(.,'\p{Zs}[1-2][0-9][0-9]0\p{Zs}s[\p{Zs}\.]') and not(descendant::p[matches(.,'\p{Zs}[1-2][0-9][0-9]0\p{Zs}s[\p{Zs}\.]')]) and not(descendant::td) and not(descendant::th)" 
        role="warning" 
  ...
  <pattern id="org-pattern">
    ...
    <rule context="element-citation/article-title|
      element-citation/chapter-title|
      element-citation/source|
      element-citation/data-title" id="org-ref-article-book-title">
      
      <let name="organisms" value="if (matches(lower-case(.),$org-regex)) then (e:org-conform(.)) else ()"/>
      
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#hbbhc-ref-article-title-organism-check" 
        test="$organisms//*:organism" 
        role="info" 
        id="ref-article-title-organism-check">ref <value-of select="ancestor::ref/@id"/> has a <name/> element containing an organism - <value-of select="string-join($organisms//*:organism,'; ')"/> - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <rule context="article//article-meta/title-group/article-title | article/body//sec/title | article//article-meta//kwd" 
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#hrmnk-article-title-organism-check" 
        test="$organisms//*:organism" 
        role="info" 
        id="article-title-organism-check"><name/> contains an organism - <value-of select="string-join($organisms//*:organism,'; ')"/> - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <rule context="italic[matches(lower-case(.),$genus-regex)]" id="italic-genus">
      ...
      <report see="https://elifeproduction.slab.com/posts/house-style-yi0641ob#hxrsi-italic-genus-oocytes"
        test="matches(lower-case(.),concat($regex-prefix,'\p{Zs}*oocytes'))" 
        role="error" 
  ...
  <pattern id="house-style">
    ...
    <rule context="p[not(descendant::mml:math)]|td[not(descendant::mml:math)]|th[not(descendant::mml:math)]|monospace|code" id="latex-tests">
      ...
      <report test="matches(.,'\p{Zs}*\\[a-z]*\p{Ps}')" 
        role="warning" 
    ...
    <rule context="front//aff/country" id="country-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h54ah-united-states-test-1"
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hvgkz-united-states-test-2"
        test="$text = 'USA'" 
        role="error" 
        id="united-states-test-2"><value-of select="."/> is not allowed. This should be 'United States'</report>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hko11-united-kingdom-test-2"
        test="$text = 'UK'" 
        role="error" 
        id="united-kingdom-test-2"><value-of select="."/> is not allowed. This should be 'United Kingdom'</report>
      
      <assert see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hulu7-gen-country-test"
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hiz90-singapore-test-1"
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hyh7x-taiwan-test"
        test="(. != 'Taiwan') and  (matches(lower-case($city),'ta[i]?pei|tai\p{Zs}?chung|kaohsiung|taoyuan|tainan|hsinchu|keelung|chiayi|changhua|jhongli|tao-yuan|hualien'))" 
        role="warning" 
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h44n1-s-korea-test"
        test="(. != 'Republic of Korea') and  (matches(lower-case($city),'chuncheon|gyeongsan|daejeon|seoul|daegu|gwangju|ansan|goyang|suwon|gwanju|ochang|wonju|jeonnam|cheongju|ulsan|inharo|chonnam|miryang|pohang|deagu|gwangjin-gu|gyeonggi-do|incheon|gimhae|gyungnam|muan-gun|chungbuk|chungnam|ansung|cheongju-si'))" 
        role="warning" 
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h2i0t-n-korea-test"
        test="replace(.,'\p{P}','') = 'Democratic Peoples Republic of Korea'" 
        role="warning" 
    ...
    <rule context="front//aff//named-content[@content-type='city']" id="city-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hh80v-pre-us-states-test"
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#ho7od-final-us-states-test"
        test="matches($lc,$states-regex)" 
        role="error" 
        id="final-US-states-test">city contains a US state (or an abbreviation for it) - <value-of select="."/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h4yk9-singapore-test-2"
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h54dm-wash-dc-test-1"
        test="(lower-case(.) = 'washington') and (ancestor::aff/country/text() = 'United States')" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hxtjh-city-replacement-character-presence"
        test="matches(.,'�')" 
        role="error" 
        id="city-replacement-character-presence"><name/> element contains the replacement character '�' which is unallowed.</report>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hbjgo-city-number-presence"
        test="matches(.,'\d')" 
        role="warning" 
        id="city-number-presence">city contains a number, which is almost certainly incorrect - <value-of select="."/>.</report>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hra66-city-street-presence"
        test="matches(lower-case(.),'^rue | rue |^street | street |^building | building |^straße | straße |^stadt | stadt |^platz | platz |^strada | strada |^cedex | cedex |^blvd | blvd |^boulevard| boulevard ')" 
        role="warning" 
    ...
    <rule context="aff/institution[not(@*)]" id="institution-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hfc9g-uc-no-test-1"
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hgke7-uc-no-test-2"
        test="matches(normalize-space(.),'[Uu]niversity of [Cc]alifornia.') and not(contains(.,'San Diego')) and ($city !='') and not(contains(.,$city))" 
        role="warning" 
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hw55n-uc-no-test-3"
        test="matches(normalize-space(.),'[Uu]niversity of [Cc]alifornia.') and not(contains(.,'San Diego')) and ($city='La Jolla')" 
        role="warning" 
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hc48u-institution-replacement-character-presence"
        test="matches(.,'�')" 
        role="error" 
        id="institution-replacement-character-presence"><name/> element contains the replacement character '�' which is unallowed.</report>
      
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#h69nr-institution-street-presence"
        test="matches(lower-case(.),'^rue | rue |^street | street |^building | building |^straße | straße |^stadt | stadt |^platz | platz |^strada | strada |^cedex | cedex |^blvd | blvd |^boulevard| boulevard ')" 
        role="warning" 
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hs0dc-institution-ampersand-presence"
        test="matches(replace(lower-case(.),'(texas a\s*&amp;\s*m|hygiene &amp; tropical|r\s*&amp;\s*d)',''),'&amp;')" 
        role="warning" 
    ...
    <rule context="aff/institution[@content-type='dept']" id="department-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/affiliations-js7opgq6#hd64t-dept-replacement-character-presence"
        test="contains(.,'�')" 
        role="error" 
    ...
    <rule context="element-citation[@publication-type='journal']/source" id="journal-title-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#journal-bracket-check" 
        test="matches(.,'\[|\(|\)|\]')" 
        role="warning" 
        id="journal-bracket-check">Journal ref '<value-of select="ancestor::ref/@id"/>' has a journal name which contains brackets '<value-of select="."/>'. It is very unlikely that the content in the brackets is required.</report>
    </rule>
    ...
    <rule context="element-citation[@publication-type='journal']/article-title" id="ref-article-title-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/journal-references-i098980k#article-title-case" 
        test="($word-count gt 4) and ($title-word-count gt ($word-count div 2))" 
        role="warning" 
        id="article-title-case">Journal ref has <name/> in mostly title case. Is that correct? eLife style is to use sentence case. "<value-of select="."/>"</report>
    ...
    <rule context="element-citation[@publication-type='web']" id="website-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#webreplacement-character-presence"
        test="matches(.,'�')" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#psyarxiv-web-test"
        test="matches($link,'psyarxiv.org')" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#arxiv-web-test"
        test="matches($link,'/arxiv.org')" 
        role="error" 
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#biorxiv-web-test"
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#chemrxiv-web-test"
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#peerj-preprints-web-test"
      ...
      <report see="https://elifeproduction.slab.com/posts/website-references-ua2hs8qo#paleorxiv-web-test"
    ...
    <rule context="element-citation[@publication-type='software']" id="software-ref-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/software-references-aymhzmlh#r-test-1" 
        test="matches($lc,'r: a language and environment for statistical computing') and not(matches(person-group[@person-group-type='author']/*[name()=('collab','collab-name')][1],'^R Development Core Team$'))" 
        role="error" 
        id="R-test-1">software ref '<value-of select="ancestor::ref/@id"/>' has a data-title '<value-of select="data-title[1]"/>' but it does not have one collab-name element containing 'R Development Core Team'.</report>
      
      <report see="https://elifeproduction.slab.com/posts/software-references-aymhzmlh#r-test-2" 
        test="matches($lc,'r: a language and environment for statistical computing') and (count(person-group[@person-group-type='author']/*[name()=('collab','collab-name')]) != 1)" 
        role="error" 
        id="R-test-2">software ref '<value-of select="ancestor::ref/@id"/>' has a data-title '<value-of select="data-title[1]"/>' but it has <value-of select="count(person-group[@person-group-type='author']/*[name()=('collab','collab-name')])"/> collab-name element(s).</report>
    ...
    <rule context="element-citation//name" id="ref-name-tests">
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#all-caps-surname"
        ...
        role="warning" 
        id="all-caps-surname">surname in ref '<value-of select="ancestor::ref/@id"/>' is composed of only capitalised letters - <value-of select="surname[1]"/>. Should this be captured as a collab-name? If not, Should it be - <value-of select="concat(substring(surname[1],1,1),lower-case(substring(surname[1],2)))"/>?</report>
      
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#surname-number-check"
        test="matches(.,'[0-9]')" 
        role="warning" 
        id="surname-number-check">name in ref '<value-of select="ancestor::ref/@id"/>' contains numbers - <value-of select="."/>. Should this be captured as a collab-name?</report>
    ...
    <rule context="article[e:get-version(.)!='1']//sec[@sec-type='data-availability']" id="data-availability-version-2">
      ...
      <report test="descendant::element-citation" 
        role="error" 
        id="das-no-refs">Version 2 xml cannot have references in the data availability section. Data references must instead be cited from the data availability section.</report>
      
      <assert test="count(p) = 1" 
        role="error" 
        id="das-p-count">Version 2 xml cannot have more than one p element in the data availability section.</assert>
      
    </rule>
    
    <rule context="article[e:get-version(.)!='1']//sec[@sec-type='data-availability']/*" id="data-availability-child-version-2">
      
      <assert test="name()=('p','title')" 
        role="error" 
        id="das-child-conformance">Only p and title elements are permitted within the data availability section. In version 2 XML <value-of select="name()"/> is not permitted.</assert>
    ...
    <rule context="article//sec[@sec-type='data-availability']/p[position() gt 1 and not(element-citation) and following-sibling::p[element-citation[@specific-use=('isSupplementedBy','generated')]]]" id="data-availability-generated-p">
      ...
      <report test="$ref-count = 1 and not(matches(.,'^The following dataset was generated:\s?$'))" 
        role="error" 
        id="das-generated-p-1">p element before generated datasets in data availability sections that contain 1 generated dataset should contain 'The following dataset was generated:', but this one contains '<value-of select="."/>'.</report>
      
      <report test="($ref-count gt 1) and not(matches(.,'^The following datasets were generated:\s?$'))" 
        role="error" 
        id="das-generated-p-2">p element before generated datasets in data availability sections that contain more than 1 generated dataset should contain 'The following datasets were generated:', but this one contains '<value-of select="."/>'.</report>
    ...
    <rule context="article//sec[@sec-type='data-availability']/p[position() gt 1 and not(element-citation) and not(following-sibling::p[element-citation[@specific-use=('isSupplementedBy','generated')]]) and following-sibling::p[element-citation[@specific-use=('references','analyzed')]]]" id="data-availability-used-p">
      ...
      <report test="$ref-count = 1 and not(matches(.,'^The following previously published dataset was used:\s?$'))" 
        role="error" 
        id="das-used-p-1">p element before used datasets in data availability sections that contain 1 used dataset should contain 'The following previously published dataset was used:', but this one contains '<value-of select="."/>'.</report>
      
      <report test="($ref-count gt 1) and not(matches(.,'^The following previously published datasets were used:\s?$'))" 
        role="error" 
        id="das-used-p-2">p element before used datasets in data availability sections that contain more than 1 used dataset should contain 'The following previously published datasets were used:', but this one contains '<value-of select="."/>'.</report>
      
    </rule>
    
    <rule context="sec[@sec-type='data-availability' and not(descendant::element-citation)]/p" id="data-availability-extra-p">
      
      <report test="preceding-sibling::p" 
        role="warning" 
        id="das-extra-p">Is this extra p element in the data availability section required? There are no dataset references within the section.</report>
    ...
    <rule context="table-wrap" id="colour-table">
      ...
      <report see="https://elifeproduction.slab.com/posts/tables-3nehcouh#colour-check-table" 
        ...
        role="warning" 
        id="colour-check-table"><value-of select="if (label) then label else 'table'"/> has colour background. Is this correct and appropriate?</report>
    </rule>
    ...
    <rule context="th[@style]|td[@style]" id="colour-table-2">
      ...
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#pre-colour-check-table-2" 
        ...
        role="warning" 
        id="pre-colour-check-table-2"><name/> element containing '<value-of select="."/>' has an @style with an unallowed value - '<value-of select="@style"/>'. The only supported values are <value-of select="string-join($allowed-values,', ')"/> for blue, green orange, yellow, purple, red, pink and grey respectively. Please ensure one of these is used. If unsupported colours have been used, please query the Production team who will check with the authors.</assert>
      
      <assert see="https://elifeproduction.slab.com/posts/tables-3nehcouh#final-colour-check-table-2" 
        test="@style=($allowed-values)" 
        role="error" 
        id="final-colour-check-table-2"><name/> element contanining '<value-of select="."/>' has an @style with an unallowed value - '<value-of select="@style"/>'. The only allowed values are <value-of select="string-join($allowed-values,', ')"/> for blue, green orange, yellow, purple, red, pink and grey respectively.</assert>
    </rule>
    ...
    <rule context="article[not(@article-type=($notice-article-types,'article-commentary'))]/body//p[not(parent::list-item) and not(descendant::*[last()]/ancestor::disp-formula) and not(table-wrap)]|
    ...
    <rule context="ext-link[not(ancestor::sub-article or ancestor::element-citation or ancestor::sec[@sec-type='data-availability']) and contains(lower-case(@xlink:href),'github.com') and not(contains(@xlink:href,'archive.softwareheritage.org'))]" 
      id="flag-github">
    ...
    <rule context="ext-link[not(ancestor::sub-article or ancestor::element-citation or ancestor::sec[@sec-type='data-availability']) and contains(lower-case(@xlink:href),'gitlab.com') and not(contains(@xlink:href,'archive.softwareheritage.org'))]" 
      id="flag-gitlab">
      <let name="l" value="lower-case(@xlink:href)"/>
    ...
    <rule context="p|td|th|title" 
      id="data-request-checks">
      ...
      <report test="matches(lower-case(.),'^mmethods[\s\.]|\smmethods[\s\.]')" 
        role="error" 
        id="mmethods-typo-check"><name/> element contains a typo ('mmethods') - <value-of select="."/>.</report>
  ...
  <pattern id="doi-ref-checks">
    ...
    <rule context="element-citation[(@publication-type='confproc') and not(pub-id[@pub-id-type='doi']) and year and conf-name]" id="doi-conf-ref-checks">
      ...
      <report see="https://elifeproduction.slab.com/posts/conference-references-d51f08lj?shr=d51f08lj#h5t13-conf-doi-test-1" 
        test="contains($name,'ieee')" 
        role="warning" 
  ...
  <pattern id="links-in-ref-tests">
    ...
    <rule context="element-citation/source | element-citation/article-title | element-citation/chapter-title | element-citation/data-title"
      ...
      <let name="t-count" value="if (count($t) lt 1) then 1
                                 else count($t)"/>
      ...
      <report see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#hg303-duplicated-content"
        test="lower-case(.)!='cell stem cell' and ($d-count div $t-count) lt 0.75" 
        role="warning" 
        id="duplicated-content">Does <value-of select="name(.)"/> in <value-of select="e:citation-format1(parent::element-citation)"/> have duplicated content? <value-of select="."/></report>
  ...
  <pattern id="element-allowlist-pattern">
    ...
    <rule context="article//*[not(ancestor::mml:math)]" id="element-allowlist">
      <let name="allowed-elements" value="('abstract',
        'ack',
        'addr-line',
        'aff',
        'ali:free_to_read',
