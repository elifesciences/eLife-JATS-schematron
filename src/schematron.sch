 ...
 <pattern id="article">
  ...
  <pattern id="dec-letter-auth-response">
    ...
    <rule context="sub-article[@article-type='editor-report']/front-stub/related-object" id="ed-eval-rel-obj-tests">
      ...
      <assert test="@xlink:href = concat('https://sciety.org/articles/activity/',@object-id)" 
        role="error" 
        flag="dl-ar"
      ...
      <assert test="@xlink:href = concat('https://sciety.org/articles/activity/',$event-preprint-doi)" 
        role="error" 
        flag="dl-ar"
 ...
 <pattern id="features">
  ...
  <pattern id="figure-xref-pattern">
    ...
    <rule context="xref[@ref-type='fig' and @rid]" id="fig-xref-conformance">
      ...
      <report test="matches(lower-case(.),'author\s+response') and not(matches(.,'^Author response image'))" 
        role="warning" 
        id="fig-xref-test-19">Figure citation - '<value-of select="."/>' - is pointing to an image in the author response but it does not contain the text 'Author response image'. Is that correct?</report>
      
      <report test="matches(lower-case(.),'review') and not(matches(.,'^Review image'))" 
        role="warning" 
        id="fig-xref-test-20">Figure citation - '<value-of select="."/>' - is pointing to an image in the peer reviews but it does not contain the text 'Review image'. Is that correct?</report>
    </rule>
  </pattern>
  ...
  <pattern id="links-in-ref-tests">
    ...
    <rule context="element-citation/source | element-citation/article-title | element-citation/chapter-title | element-citation/data-title"
      ...
      <assert see="https://elifeproduction.slab.com/posts/references-ghxfa7uy#hg303-duplicated-content"
        test="($d-count div $t-count) gt 0.75" 
        role="warning" 
        id="duplicated-content">Does <value-of select="name(.)"/> in <value-of select="e:citation-format1(parent::element-citation)"/> have duplicated content? <value-of select="."/></assert>
