  ...
  <pattern
    ...
    <rule context="fn-group[@content-type='author-contribution']"
      ...
      <assert test="$author-count = count(fn)"
        role="warning"
        id="auth-cont-test-1">There are <value-of select="count(fn)"/> contribution footnotes, but <value-of select="$author-count"/> authors. Each author should have their own contribution footnote.</assert>
    </rule>
