   ...
   <let name="template" value="parent::article-meta/custom-meta-group/custom-meta[meta-name='Template']/meta-value[1]"/>
    ...
    <rule context="aff" 
     ...
     <report test="count(institution[@content-type='dept']) ge 1"
      role="warning"
      id="gen-aff-test-2">Affiliations has <value-of select="count(institution[@content-type='dept'])"/> depatement field(s) - <value-of select="$display"/>. Is this correct?</report>
  ...
  <pattern
    ...
    <rule context="table-wrap" 
      ...
      <report test="($id = 'keyresource') and ($lab != 'Key resources table')"
        role="error"
        id="kr-table-wrap-test-1">table-wrap has an id <value-of select="$id"/> but its label is not 'Key resources table', which is incorrect.</report>
 ...
 <pattern
  ...
  <pattern
    ...
    <rule context="aff/institution[not(@*)]" 
      ...
      <report test="matches(normalize-space(.),'[Uu]niversity of [Cc]alifornia.') and ($city !='') and not(contains(.,$city))"
        role="error" 
