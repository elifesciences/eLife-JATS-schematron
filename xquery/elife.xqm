...
return delete node $x,
        ...
        for $x in $copy1//*:let[@name=("countries","publisher-locations","journals","publishers","funders","registries","credit-roles","rors","languages","res-org-doc")]
  ...
  modify(
        for $t in $copy2
         let $r := 
             <pattern id="root-pattern">
                <rule context="root" id="root-rule">{
                for $z in $t//sch:rule[not(@id=("missing-ref-cited","strike-tests","colour-styled-content","empty-attribute-test","strike-checks","preformat-checks","code-checks","uri-checks","sc-checks","funding-group-presence-tests"))]
...
return delete node $x,
  ...
  modify(
    ...
    return 
    if ($id=('code-flag','preformat-flag','strike-warning','uri-flag', 'sc-check-1','funding-group-presence')) then ()
    else insert node 
        <pattern id="root-pattern">
        <rule context="root" id="root-rule">
        {$q}
