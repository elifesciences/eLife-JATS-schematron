...
return delete node $x,
        ...
        for $x in $copy1//*:let[@name=("countries","publisher-locations","journals","publishers")]
  ...
  modify(
        for $t in $copy2
         let $r := 
             <pattern id="root-pattern">
                <rule context="root" id="root-rule">{
                for $z in $t//sch:rule[not(@id="missing-ref-cited") and not(@id="colour-styled-content")]
