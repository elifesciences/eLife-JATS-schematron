  ...
  <rule context="article/front/article-meta" id="test-article-metadata">
   ...
   <rule context="article-meta/article-categories" id="test-article-categories">
    ...
    <report test="article-title//bold"
	  ...
	  <report test="matches($lc,$org-regex) and not(article-title/italic[text() = e:org-conform($lc)])"
	    role="error" 
	    id="article-title-test-7">article title contains an organism - <value-of select="e:org-conform($lc)"/> - but there is no italic element with that correct capitalisation or spacing.</report>
 ...
 <pattern
  ...
  <pattern
    ...
    <rule context="element-citation[@publication-type='journal']/article-title" id="ref-article-title-tests">
      ...
      <report test="matches($lc,$org-regex) and not(italic[text() = e:org-conform($lc)])"
        role="error" 
        id="ref-article-title-org-check">ref '<value-of select="ancestor::ref/@id"/>' references an organism - <value-of select="e:org-conform($lc)"/> - but there is no italic element with that correct capitalisation or spacing.</report>
    ...
    <rule context="th|td" 
      ...
      <report test="starts-with(@style,'author-callout')"
        role="warning" 
        id="colour-check-table"><value-of select="local-name()"/> element has colour background. Is this correct? It contains <value-of select="."/></report>
    </rule>
