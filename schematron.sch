...
<let name="MSAs" value="('Biochemistry and Chemical Biology', 'Cancer Biology', 'Cell Biology', 'Chromosomes and Gene Expression', 'Computational and Systems Biology', 'Developmental Biology', 'Ecology', 'Epidemiology and Global Health', 'Evolutionary Biology', 'Genetics and Genomics', 'Human Biology and Medicine', 'Immunology and Inflammation', 'Microbiology and Infectious Disease', 'Neuroscience', 'Physics of Living Systems', 'Plant Biology', 'Stem Cells and Regenerative Medicine', 'Structural Biology and Molecular Biophysics')"/>
 ...
 <pattern
    ...
    <rule context="article" id="article-tests">
	  ...
	  <assert test="count(back) = 1"
        role="error" 
        id="test-article-back">Article must have one child back. Currently there are <value-of select="count(back)"/></assert>
	...
	<rule context="article[@article-type='research-article']" id="research-article">
	  ...
	  <assert test="sub-article[@article-type='decision-letter']"
        role="error" 
        id="test-r-article-d-letter">sub-article[@article-type="decision-letter"] must be present for article[@article-type="research-article"]</assert>
  ...
  <rule context="article/front/article-meta" id="test-article-metadata">
    ...
    <assert test="self-uri"
        role="error" 
        id="test-self-uri-presence">There must be a child self-uri in article-meta.</assert>
		
    <assert test="self-uri[@content-type='pdf']"
        role="error" 
        id="test-self-uri-att">self-uri must have an @content-type="pdf"</assert>
		
    <assert test="self-uri[@xlink:href = concat('elife-', $article-id, '.pdf')]"
        role="error" 
        id="test-self-uri-pdf">self-uri must have attribute xlink:href="elife-xxxxx.pdf" where xxxxx = the article-id. Currently it is <value-of select="self-uri/@xlink:href"/>. It should be elife-<value-of select="$article-id"/>.pdf</assert>
		
    <assert test="count(history) = 1"
          role="error" 
          id="test-history-presence">There must be one and only one history element in the article-meta. Currently there are <value-of select="count(history)"/></assert>
    ...
    <assert test="count(permissions) = 1"
     ...
     <assert test="count(abstract[not(@abstract-type='executive-summary')]) = 1 or (count(abstract[not(@abstract-type='executive-summary')]) = 1 and count(abstract[@abstract-type='executive-summary']) = 1)"
      role="error" 
    ...
    <assert test="count(kwd-group[@kwd-group-type='author-keywords']) = 1"
      role="error" 
      id="test-auth-kwd-group-presence">One author keyword group must be present in article-meta .</assert>
   ...
   <rule context="article-meta/article-categories" id="test-article-categories">
     ...
     <report test="if ($article-type = 'editorial') then ()
                   else count(subj-group[@subj-group-type='heading']) lt 1"
       role="error" 
	...
	<rule context="article-meta/contrib-group//name" 
	  ...
	  <assert test="matches(surname,'^[\p{L}\s-]*$')"
      	role="warning" 
	  ...
	  <assert test="matches(given-names,'^[\p{L}\s-]*$')"
      	role="error" 
      	id="given-names-test-5">given-names may only contain letters, spaces or hyphens.</assert>
    ...
    <rule context="pub-date[not(@pub-type='collection')]" 
      ...
      <assert test="matches(day,'^[0-9]{2}$')"
        role="error" 
        id="pub-date-test-1">date must contain day in the format 00. Currently it is '<value-of select="day"/>'.</assert>
      
      <assert test="matches(month,'^[0-9]{2}$')"
        role="error" 
        id="pub-date-test-2">date must contain month in the format 00. Currently it is '<value-of select="month"/>'.</assert>
      
      <assert test="matches(year,'^[0-9]{4}$')"
        role="error" 
        id="pub-date-test-3">date must contain year in the format 0000. Currently it is '<value-of select="year"/>'.</assert>
 ...
 <pattern
	...
	<let name="article-id" value="ancestor::article/front//article-id[@pub-id-type='publisher-id']"/>
  	...
  	role="error" 
  	id="object-doi-test-1">object-doi must start with the elife doi prefix, '10.7554/eLife.' and the article id <value-of select="$article-id"/>.</assert>
	
	  <assert test="matches(.,'^10.7554/eLife\.[\d]{5}\.[0-9]{3}$')"
  	role="error" 
  	id="object-doi-test-2">object-doi must follow this convention - '10.7554/eLife.00000.000'.</assert>
	  
	  <report test=". = preceding::object-id[@pub-id-type='doi'] or . = following::object-id[@pub-id-type='doi']"
	    role="error" 
	    id="object-doi-test-3">object-dois must always be distinct. <value-of select="."/> is not distinct.</report>
  ...
  <pattern
    ...
    <rule context="fig[not(ancestor::sub-article[@article-type='reply'])]" 
      ...
      <report test="if (ancestor::article/@article-type = $features-article-types) then () 
                    else count(object-id[@pub-id-type='doi']) != 1" 
        role="error"
        id="fig-test-1">fig must have one and only one object-id[@pub-id-type='doi'].</report>
      ...
      <assert test="label" 
        role="error"
        id="fig-test-3">fig must have a label.</assert>
      ...
      <assert test="graphic"
        role="error"
        id="fig-test-7">fig must have a graphic.</assert>
    </rule>
    ...
    <rule context="table-wrap" 
      ...
      <assert test="table"
        role="error"
        id="table-wrap-test-1">table-wrap must have at least one table.</assert>
  ...
  <pattern
    ...
    <rule context="sub-article[@article-type='reply']//fig" 
      ...
      <assert test="object-id[@pub-id-type='doi']" 
        role="error"
  ...
  <pattern
    ...
    <rule context="ref-list" 
      ...
      <assert test="title = 'References'"
        role="error"
        id="ref-list-title-test">ref-list must have a title that contains 'References'. Currently it is '<value-of select="title"/>'.</assert>
  ...
  <pattern
    ...
    id="ack-child-tests">
      ...
      <assert test="local-name() = ('p','sec','title')"
        role="error"
        id="ack-child-test-1">Only p, sec or title can be children of ack. <value-of select="local-name()"/> is not allowed.</assert>
    </rule>
    ...
    <rule context="back//app"
      ...
      <assert test="count(descendant::boxed-text) = 1"
        role="error"
        id="app-test-3">app must have one and only one boxed-text.</assert>
    </rule>
    
    <!-- This assumes features content (except for template 5) will not have an appendix -->
    <rule context="back//app/boxed-text"
      id="app-boxed-text-tests">
      
      <assert test="count(object-id[@pub-id-type='doi']) = 1"
        role="error"
        id="app-test-4">boxed-text must have an object-id.</assert>
      
    </rule>
    
    <rule context="back//app//*[not(local-name() = ('sec','title','boxed-text'))]"
      id="app-content-tests">
      
      <assert test="ancestor::boxed-text"
        role="error"
        id="app-content-test-1">app content must be captured within boxed-text.</assert>
    </rule>
    ...
    <rule context="fn-group[@content-type='author-contribution']"
      ...
      <assert test="$author-count = count(fn)"
        role="error"
        id="auth-cont-test-1">fn-group must contain one fn for each author. Currently there are <value-of select="$author-count"/> authors but <value-of select="count(fn)"/> footnotes.</assert>
    </rule>
  ...
  <pattern
    ...
    <rule context="sub-article[@article-type='reply']/body//p"
      id='reply-p-tests'>
      
      <report test="not(parent::disp-quote) and matches(.,'^[0-9]')"
        role="warning"
        id="reply-p-test-1">paragraph in author reply begins with number but isn't captured in a parent disp-quote. Is this correct?</report>
    </rule>
  ...
  <pattern
    ...
    <rule context="related-article" 
      ...
      <assert test="@related-article-type = $allowed-values"
        role="error"
        id="related-articles-test-4">@related-article-type must be equal to one of the allowed values, ('article-reference', 'commentary', 'commentary-article', and 'corrected-article').</assert>
 ...
 <pattern
   ...
   <rule context="subj-group[@subj-group-type='sub-display-channel']/subject"
     ...
     <assert test=". = concat(concat(upper-case(substring($token1, 1, 1)), lower-case(substring($token1, 2))),
       ' ',
       string-join(for $x in tokenize(substring-after($token2,' '),'\s') return e:titleCase($x),' ')
       )"
       role="error"
       id="feature-subj-test-2">The content of the sub-display-channel should be in title case - <value-of select="concat(concat(upper-case(substring($token1, 1, 1)), lower-case(substring($token1, 2))),' ',string-join(for $x in tokenize(substring-after($token2,' '),'\s') return e:titleCase($x),' '))"/></assert>
