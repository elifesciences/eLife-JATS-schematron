 ...
 <pattern
  ...
  <pattern
    ...
    <rule context="p" 
      ...
      <report test="(descendant::*[1]/local-name() = 'bold') and not(ancestor::caption) and not(descendant::*[1]/preceding-sibling::text())"
        role="warning" 
 ...
 <pattern
  ...
  <pattern id="video-xref-pattern">
    ...
    <rule context="xref[@ref-type='video']" id="vid-xref-conformance">
      ...
      <report test="(. != $t-label)"
        role="warning" 
        id="vid-xref-conformity-2">video citation does not matches the label of the video that it links to <value-of select="."/> -> <value-of select="$t-label"/>. Is that correct?</report>
      ...
      <report test="(matches($post-text,'^ in $|^ from $')) and (following-sibling::*[1]/@ref-type='bibr')"
        role="error"
