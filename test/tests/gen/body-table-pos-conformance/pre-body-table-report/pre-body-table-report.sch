  ...
  <pattern id="table-pos-tests">
    <rule context="article[(@article-type!='correction') and (@article-type!='retraction')]/body//table-wrap[matches(@id,'^table[\d]+$')]" id="body-table-pos-conformance">
      ...
      <assert test="($no = string($pos))" role="warning" id="pre-body-table-report">
        <value-of select="label"/> does not appear in sequence. Relative to the other numbered tables it is placed in position <value-of select="$pos"/>. More information here - https://elifesciences.gitbook.io/productionhowto/-M1eY9ikxECYR-0OcnGt/article-details/content/tables#pre-body-table-report</assert>
    </rule>
  </pattern>
