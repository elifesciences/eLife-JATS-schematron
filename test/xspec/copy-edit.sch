<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ali="http://www.niso.org/schemas/ali/1.0/" xmlns:mml="http://www.w3.org/1998/Math/MathML" queryBinding="xslt2">
    ...
    <pattern>
        <rule context="article" id="copyedit-test">
            ...
            <report test="($us-count &gt; $uk-count)" role="info" id="us-confirm">US terms are prevalent, and corresponding author affiliaiton countries are --- <value-of select="$countries"/>
      </report>
            
            <report test="($uk-count = $us-count)" role="warning" id="uk-us-equal">Same number of UK (<value-of select="$uk-count"/>) and US (<value-of select="$us-count"/>) terms. Corresponding author affiliaiton countries are --- <value-of select="$countries"/>
      </report>
            
            <report test="($us-count &gt; $uk-count) and ($uk-text != '')" role="warning" id="us-conformity">There are more US terms than UK terms (<value-of select="$us-count"/> to <value-of select="$uk-count"/> respectively), but UK words are present in the article. Search --- <value-of select="$uk-text"/>. Correct US variants are --- <value-of select="string-join(distinct-values(for $x in $uk-tokens return if ($x = 'dr.') then 'dr' else ($list//item[@uk/string() = replace(lower-case($x),'[^\p{L}]','')]/@us/string())),'; ')"/>.</report>
            
            <report test="($uk-count &gt; $us-count) and ($us-text != '')" role="warning" id="uk-conformity">There are more UK terms than US terms (<value-of select="$uk-count"/> to <value-of select="$us-count"/> respectively), but US words are present in the article. Search --- <value-of select="$us-text"/>. Correct UK variants are --- <value-of select="string-join(distinct-values(for $x in $us-tokens return if ($x = 'dr.') then 'dr' else ($list//item[@us/string() = replace(lower-case($x),'[^\p{L}]','')]/@uk/string())),'; ')"/>.</report>
        </rule>
    </pattern>
