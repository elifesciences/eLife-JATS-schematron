<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:java="http://www.java.com/" xmlns:file="java.io.File" xmlns:ali="http://www.niso.org/schemas/ali/1.0/" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:meca="http://manuscriptexchange.org" queryBinding="xslt2">
  ...
  <pattern id="aff-ror-tests-pattern">
    <rule context="aff[count(institution-wrap/institution-id[@institution-id-type='ror'])=1]" id="aff-ror-tests">
      ...
      <report test="(city or ancestor::contrib[@contrib-type='author' and not(ancestor::sub-article)]) and exists($matching-ror) and not(contains(city[1],$matching-ror/*:city))" role="warning" id="aff-ror-city">Affiliation has a ROR id, but its city is not the same one as in the ROR data. Is that OK? ROR has '<value-of select="$matching-ror/*:city"/>', but the affiliation city is <value-of select="city[1]"/>.</report>
  ...
  <pattern id="ref-name-space-checks-pattern">
    ...
    <rule context="ref//pub-id" id="ref-pub-id-checks">
        ...
        <report test="(@pub-id-type='pmcid') and not(matches(.,'^PMC[0-9]{7,15}$'))" role="error" id="ref-pmcid-conformance">pub-id is tagged as a pmcid, but it is not a valid PMCID ('PMC' followed by 7+ digits) - <value-of select="."/>. The id must be either incorrect or have the wrong pub-id-type.</report>
  ...
  <pattern id="isbn-conformity-pattern">
    <rule context="ref//pub-id[@pub-id-type='isbn']|isbn" id="isbn-conformity">
        ...
        <assert test="$sum = 0" role="error" id="isbn-conformity-test">
