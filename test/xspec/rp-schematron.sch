<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:java="http://www.java.com/" xmlns:file="java.io.File" xmlns:ali="http://www.niso.org/schemas/ali/1.0/" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:meca="http://manuscriptexchange.org" xmlns:sqf="http://www.schematron-quickfix.com/validator/process" queryBinding="xslt2">
  ...
  <pattern id="sub-article-p-checks-pattern">
    <rule context="sub-article/body//p" id="sub-article-p-checks">
        ...
        <report test="not(matches(.,'\$?\$.*?\$\$?')) and matches(.,'\\[a-z]+\p{Ps}')" role="warning" id="sub-article-tex-2">sub-article contains what looks like potential latex: <value-of select="string-join(distinct-values(e:analyze-string(.,'\\[a-z]+\p{Ps}')//*:match),'; ')"/>. If this is maths it should either be represented in plain unicode or as an image.</report>
      </rule>
  </pattern>
  ...
  <pattern id="refinery-comment-checks-pattern">
    <rule context="ref//comment()" id="refinery-comment-checks">
      ...
      <report test="matches(lower-case(.),'refinery:') and not(matches(lower-case(.),'refinery:.*?(doi|pmid).*?suggested'))" role="warning" sqf:fix="dismiss-refinery" id="refinery-unknown-suggestion">Ref (with id <value-of select="ancestor::ref/@id"/>) has a suggested change '<value-of select="normalize-space(.)"/>'.</report>
      
      <assert test="matches(lower-case(.),'refinery:')" role="error" sqf:fix="delete-node" id="ref-comment-2">Ref (with id <value-of select="ancestor::ref/@id"/>) has comment node wit the content '<value-of select="."/>'. Comments should be removed.</assert>
